...
def main():
    ...
    print("Enabling %d columns with each having %d HDIFF cores" % (arraycols,broadcast_cores))
...
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception\n\n\n""")
    ...
    def gagan_gen_ddr(col):
            ...
            for b_tile in range (startrow,startrow+ broadcast_cores+1):
                f.write("  %%ext_buffer_out_%d_%d = AIE.external_buffer  {sym_name = \"ddr_buffer_out_%d_%d\"}: memref<%d x i32>\n" %(col, b_tile,col,b_tile,  dram_bufsize_out))
            f.write("\n")
