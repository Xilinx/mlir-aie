...
def parse_args():
    ...
    parser.add_argument('--mlir', 
            help='mlir source file', required=True)
    parser.add_argument('--colshift',
            help='column shift adjustment to source mlir', required=False)
...
def core_trace_and_mem_trace_de_interleave(word_stream):
    ...
    for i in range(len(word_stream)):
        ...
        if((i % 8) == 0):
            ...
            if curr_vld: # ignores first 8 chunks of data is pkt hdr was invalid. TODO Is this right?
                         # or shoudl we require valid header in first chunk of data
                toks_list[curr_pkt_type][curr_loc].append(word_stream[i]) # TODO assuem curr_pkt_type is valid
                # for tt in range(NumTraceTypes):
                #     if curr_pkt_type == tt:
            ...
            try:
                while(True):
                    if ((byte_stream[cursor] & 0b11111011) == 0b11110000):
                        ...
                        if not zero:
                            for i in range(7):
                                com["timer_value"] += (byte_stream[cursor+i+1]) * (256 ** (6-i))
                    ...
                    if ((byte_stream[cursor] & 0b11110000) == 0b11000000):
                        ...
                        for i in range(0,8):
                            e = (events >> i) & 0b1
                            if e:
                                com["event"+str(i)] = i # TODO is this how event# is stored in IR?
                        commands[t][key].append(com)
                        cursor = cursor + 2
...
def deactivate(multiples, active_events, timer, cycles, pid, trace_type, loc, pid_events):
    for k in active_events.keys(): # an active event
        ...
        # if not k in multiples: # active event it not in multiples list
            if active_events[k] > 0:
                ...
                trace_event = {'name':lookup_event_name_by_type(trace_type, pid_events[trace_type][loc][k])} # TODO remove
...
def convert_commands_to_json(trace_events, commands, pid_events):
    ...
    for tt in range(len(commands)): # byte_stream_dict for each trace type. TODO how to get index of bsd?
        ...
        for loc, command in byte_stream_dict.items(): # row,col with list of commands
            ...
            #for c in flat_commands:
                ...
                if 'Single' in t:
                    ...
                    deactivate(multiple_list, active_events, timer, cycles, pid, tt, loc, pid_events)
...
# def thread_name_metadata(trace_events, trace_type, pid, tid):
    ...
    trace_event['args']['name'] = lookup_event_name_by_type(trace_type, pid_events[trace_type][loc][tid])
    trace_events.append(trace_event)
        ...
        if result: # match found
            ...
            if(address == 0x340E0): # 213216, match ignoring case
                if pid_events[0].get(key) == None:
                    pid_events[0][key] = [0,0,0,0,0,0,0,0] # TODO no better way to init this?
            ...
            # mem event 0
                ...
                if pid_events[1].get(key) == None:
                    pid_events[1][key] = [0,0,0,0,0,0,0,0] # TODO no better way to init this?
                # print("Trace event 0 configured to be ",hex(value))
    ...
    for (i, line) in enumerate(lines):
        ...
        if check_time:
            ...
            if result: # match found
                ...
                errors += 1
                if DEBUG:
                    print("WARNING: Invalid time in line #"+str(i)+" expecting time (" + line + ")")
        # checking signal
        else:
            ...
            if result: # match found
                ...
                error += 1
                if DEBUG:
                    print("WARNING: Invalid signal in line #"+str(i)+" expecting time (" + line + ")")
    # if DEBUG:
