...
void matvecVectorized(T_in *__restrict a, T_in *__restrict b,
  ...
  for(int col = 0; col < k; col += 8) { 
      ...
      for(int row = 0; row  < m; row += r) chess_loop_range(m/r,) { 
          ...
          auto c_acc_out = aie::accumulate<r>(c_acc_in, b_vec, 0, 
                                              a_vec_0_0, a_vec_0_1, a_vec_1_0, a_vec_1_1,
...
#define matvec_scalar_c_func(ctype_in, mlir_type_in, ctype_out, mlir_type_out, ctype_acc) \
    void matvec_scalar_##mlir_type_in##_##mlir_type_out(ctype_in *a_in, ctype_in *b_in, ctype_out *c_out) \
    { \
        matvecScalar<ctype_in, ctype_out, 64, 32>(a_in, b_in, c_out); \
    }

#define matvec_vectorized_c_func(ctype_in, mlir_type_in, ctype_out, mlir_type_out, ctype_acc) \
    void matvec_vectorized_##mlir_type_in##_##mlir_type_out(ctype_in *a_in, ctype_in *b_in, ctype_out *c_out) \
    { \
        matvecVectorized<ctype_in, ctype_out, ctype_acc, 64, 32, 16, 8>(a_in, b_in, c_out); \
    }

#define zero_vectorized_c_func(ctype_in, mlir_type_in, ctype_out, mlir_type_out, ctype_acc) \
    void zero_vectorized_##mlir_type_out(ctype_out *c_out) { \
        zeroVectorized<ctype_out, 64, 1, 32>(c_out); \
    }

#define zero_scalar_c_func(ctype_in, mlir_type_in, ctype_out, mlir_type_out, ctype_acc) \
