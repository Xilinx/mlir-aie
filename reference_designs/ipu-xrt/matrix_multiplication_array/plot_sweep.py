...
def get_args():
    ...
    if args.output is None:
        args.output = "{0}.{1}".format(os.path.basename(args.input.name), args.outputfmt)
    if not args.ynames:
        header = args.input.readline()
        args.input.seek(0)
        iteration_ys = ["It{}".format(m.group(1)) for m in re.finditer(r'It(\d+)', header)]
        if args.ytrans == 'mean':
...
def main():
    args = get_args()
    xs, ys = get_plot_values(args.input, args.xnames, args.ynames, args.xtrans, args.ytrans, args.filter)
    fig, ax = plt.subplots()
