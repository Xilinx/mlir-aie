...
aie.device(ipu) {
 ...
 aie.core(%tile13) {
  ...
  func.func @sequence(%in0 : memref<16384xi32>, %wts0 : memref<35840xi32>, %out : memref<65536xi32>) {
      ...
      aiex.ipu.rtp_write(1, 5, 0,  9) { buffer_sym_name = "rtp15" }  // scale 11 || 6
      aiex.ipu.rtp_write(1, 4, 0,  9) { buffer_sym_name = "rtp14" }  // scale 11 || 8
      aiex.ipu.rtp_write(1, 2, 0,  9) { buffer_sym_name = "rtp12" }  // scale 11 || 8
      aiex.ipu.rtp_write(1, 3, 0,  12)  { buffer_sym_name = "rtp13" }  // scale: 10 || 8 conv1x1 with the same scale as the input so we match the scaling factor of output after conv1x1 and the initial input
