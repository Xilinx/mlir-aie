...
class DataShaper:
    ...
    def _reorder_decode(self, shape, order, defOrder=None):
        ...
        for z in reversed(order):
            if z.isdigit():
            ...
            elif z == '(': group = False; off=0; val=''; val_gi=''
                ...
                d[idx]-=1; p-=1
        if self.print_info:
            self.log_msg.append('[INFO]: reorder s={:<15} o={:<15} -> pi={:<15} s={:<30} p={:<30} pe={:<30}, b={:<30}, a={:<30}'.format(*map(str, (shape, order, pad_im, size, perm, pad_ex, brdcst, align))))
    ...
    def get_dim_steps(self, shape, order, defOrder=None, bits=8, ebs=None, sparse_ratio=1):
            ...
            if sz >= shape[d]+pad_im[d]:
                ...
                if p+1 < sp and p+1 in perm:
                    ...
                    if pi0+1 == pi1:
                        # Found XX coupling
                        dim[d] = pi1
                        self.log_msg.append('INFO: Found XX coupling (order={}, size={}, perm={}, p={})'.format(order, size, perm, p))
                    elif len(perm) > pi0+1:
                        pb = perm[pi0+1]
                        if pi0+2 == pi1 and size[pb]==1:
                            # Found XNX sequence with N=1, simplify
                            dim[d] = pi1
                            self.log_msg.append('INFO: Found XNX sequency with N=1, simplify (order={}, size={}, perm={}, p={})'.format(order, size, perm, p))
                sz = 1
                d -= 1
                sp = p
        ...
        if ebs or sparse_ratio: assert size_inv[idx] >= 8, "Sparse/exponent block is too small. Data (order) unexpected or update to script is required"
        size_inv[idx] = int(size_inv[idx]*sparse_ratio*(bits-(8 if ebs else 0))/8) + (size_inv[idx]//ebs if ebs else 0)
        step = [0]*(len(shape)+1)
        cur = 1
