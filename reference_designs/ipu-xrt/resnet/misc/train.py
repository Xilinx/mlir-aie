...
net=resnet_projected.Complete_SingleBottleneckModel3x_projected_NOQUANT(num_classes).to(device)
...
if dataset_name=="CIFAR":
    ...
    train_dataset = torchvision.datasets.CIFAR10(root='./data_torchvision', train=True, download=True, transform=transform_train)
    test_dataset = torchvision.datasets.CIFAR10(root='./data_torchvision', train=False, download=True, transform=transform_test)
...
def test(epoch):
    ...
    with torch.no_grad():
            ...
            print('\nEpoch : %d Test Acc : %.3f' % (epoch, 100.*correct/total))
            print('--------------------------------------------------------------')
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        progress_bar = tqdm(
                total=len(trainloader), desc='[0/{}]'.format(len(trainloader.sampler)),
        ...
        with progress_bar:
            if epoch%50 == 0:
                    test(epoch)
                    torch.save(net.state_dict(), os.path.join(workdir,dataset_name+"_"+project+"_weight_%s.tar" % epoch))
            for i, inp in enumerate(trainloader):
                ...
                progress_bar.set_description("[{}/{}]".format(chunks, len(trainloader.sampler)))
                progress_bar.update()
