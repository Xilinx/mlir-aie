...
class QuantBottleneck_projected_OFFLOAD_FUSED(nn.Module):
    expansion = 4
    def __init__(self, in_planes, planes, stride=1, option='A',padding_mode ='replicate'):
        ...
        if(padding_mode=="replicate"):
            self.conv2 = QuantConv2d(planes, planes, kernel_size=3,bit_width=8,weight_bit_width=8, bias=False,padding=1,padding_mode =padding_mode, weight_quant=Int8WeightPerTensorFixedPoint, return_quant_tensor=True)
        else:
            self.conv2 = QuantConv2d(planes, planes, kernel_size=3,bit_width=8,weight_bit_width=8, bias=False,padding=1, weight_quant=Int8WeightPerTensorFixedPoint, return_quant_tensor=True)
        ...
        self.relu1 = QuantReLU(act_quant=Int8ActPerTensorFixedPoint ,bit_width=8, return_quant_tensor=True)
        self.relu2 = QuantReLU(act_quant=Int8ActPerTensorFixedPoint ,bit_width=8, return_quant_tensor=True)
        self.relu3 = QuantReLU(act_quant=Int8ActPerTensorFixedPoint ,bit_width=8, return_quant_tensor=True)
...
class QuantBottleneck_projected(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A',padding_mode ='zeros'):
        ...
        self.quant_id_3 = QuantIdentity(act_quant=Int8ActPerTensorFixedPoint, bit_width=8, return_quant_tensor=True)
        self.quant_id_4 = QuantIdentity(act_quant=Int8ActPerTensorFixedPoint, bit_width=8, return_quant_tensor=True) 
        self.conv1 = QuantConv2d(in_planes, planes, kernel_size=1,bit_width=8,weight_bit_width=8, bias=False, weight_quant=Int8WeightPerTensorFixedPoint, return_quant_tensor=True)
        if(padding_mode=="replicate"):
            self.conv2 = QuantConv2d(planes, planes, kernel_size=3,bit_width=8,weight_bit_width=8, bias=False,padding=1,padding_mode =padding_mode, weight_quant=Int8WeightPerTensorFixedPoint, return_quant_tensor=True)
        else:
        ...
        if stride != 1 or in_planes != self.expansion*planes:
                ...
                self.shortcut = nn.Sequential(
                    qnn.QuantConv2d(in_planes, self.expansion*planes,
                            kernel_size=1,bias_quant=None,
                return_quant_tensor=False, stride=stride, bias=False),
                    nn.BatchNorm2d(self.expansion*planes)
                )
...
class Bottleneck_projected_OFFLOAD(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A',padding_mode ='replicate'):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        if (padding_mode=="replicate"):
            self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,padding_mode ="replicate", 
...
class QuantResNet_projected(nn.Module):
    def __init__(self, block, num_blocks, num_classes):
        ...
        self.conv1 = qnn.QuantConv2d(3, 64, kernel_size=3,
                               stride=1, padding=1, bias=False, weight_bit_width=8)
        self.bn1 = nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.linear = qnn.QuantLinear(512*block.expansion, num_classes, weight_bit_width=8,bias_quant=None,
                return_quant_tensor=False,bias=False)
...
class QuantNonAIELayerPost(nn.Module):
    def __init__(self,  num_classes):
        ...
        expansion = 4
        self.linear = qnn.QuantLinear(64*expansion, num_classes, weight_bit_width=8,bias_quant=None,
                return_quant_tensor=False,bias=False)
...
class Bottleneck_plain_without_bn(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, padding=1, padding_mode ='zeros', bias=False)
...
class double_conv_without_bn(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        self.conv2 = nn.Conv2d(planes, 32, kernel_size=3, padding=1, padding_mode ='zeros', bias=False)
...
class QuantSingleBottleneck(nn.Module):
    def __init__(self, block, num_blocks, num_classes):
        ...
        self.in_planes = 64
        self.quant_inp = qnn.QuantIdentity(bit_width=8, return_quant_tensor=True)
        self.conv1 = qnn.QuantConv2d(3, 64, kernel_size=3,bias_quant=None,
                                return_quant_tensor=False,
                               stride=1, padding=1, bias=False, weight_bit_width=8)
        self.bn1 = nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.linear = qnn.QuantLinear(64*block.expansion, num_classes, weight_bit_width=8,bias_quant=None,
                return_quant_tensor=False,bias=False)
        self.relu1 = qnn.QuantReLU(bit_width=8)
    def _make_layer(self, block, planes, num_blocks, stride):
        strides = [stride] + [1]*(num_blocks-1)
...
def SingleBottleneckModel_projected(num_classes):
    return SingleBottleneck(Bottleneck_projected, [1,],num_classes)



def QuantSingleBottleneckModel2x_projected(num_classes):
    return QuantSingleBottleneck(QuantBottleneck_projected, [2,],num_classes)
...
def DoubleBottleneckModel_projected(num_classes):
    return DoubleBottleneck(Bottleneck_projected, [2, 2, ],num_classes)
...
def QuantBottleneck_conv_Layer():
    ...
    return NonOffloadBottleneck(QuantBottleneck_projected, [1,])

def QuantAIELayer():
    # bottleneck with identiy in skip path
    return AIELayerOffload(QuantBottleneck_projected_OFFLOAD, [1,])

def QuantAIELayerFUSED():
    # bottleneck with identiy in skip path
    return AIELayerOffload(QuantBottleneck_projected_OFFLOAD_FUSED, [1,])
...
def Bottleneck_conv_Layer():
    # bottleneck with 1x1 conv in skip path
    return NonOffloadBottleneck(Bottleneck_projected, [1,])
def AIELayer():
    # bottleneck with identiy in skip path
    return AIELayerOffload(Bottleneck_projected_OFFLOAD, [1,])
...
class Bottleneck_projected_OFFLOAD_BREAK(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A',padding_mode ='replicate'):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        if (padding_mode=="replicate"):
            self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,padding_mode =padding_mode, 
                                stride=stride, padding=1, bias=False)
        else:
            self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                                stride=stride, padding=1, bias=False)
...
def Complete_SingleBottleneckModel3x_projected_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(Bottleneck_projected, [3,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              NonAIELayerPost(num_classes))
...
def Complete_DoubleConvModel1x_projected_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(double_conv_without_bn, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              DualConvNonAIELayerPost(num_classes))

def Complete_DoubleConvModel_with_add1x_projected_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(double_conv_withskip_without_bn, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              SingleConvNonAIELayerPost(num_classes))
    
def Complete_TripleConvModel1x_projected_with_skip_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(triple_conv_withskip_without_bn, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              SingleConvNonAIELayerPost(num_classes))
    
def Complete_TripleConvModel1x_projected_without_skip_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(triple_conv_without_bn, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              SingleConvNonAIELayerPost(num_classes))

def Complete_SingleBottleneckModel1x_projected_NOQUANT_with_bn(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(Bottleneck_projected, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              NonAIELayerPost(num_classes))
...
def Complete_TripleConvModel1x_proper_projected_without_skip_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(triple_conv_proper_without_bn, [1,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              NonAIELayerPost(num_classes))

def Resnet50_2(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload(triple_conv_proper_without_bn, [2,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
...
def Complete_TripleConvModel2x_proper_projected_without_skip_NOQUANT(num_classes):
    return CombinedModel(NonAIELayerInitial(),AIELayerOffload2(triple_conv_proper_without_bn, [2,]), \
                        #  AIELayerOffload(Bottleneck_projected_OFFLOAD, [2,]),\
                              NonAIELayerPost(num_classes))
...
def SingleBottleneck_x1_plain(num_classes):
    return CombinedModel_MIX(NonAIELayerInitial(),NonOffloadBottleneck(Bottleneck_projected, [1,]), \
                         AIELayerOffloadPlain(Bottleneck_plain_without_bn, [1,]),\
                              NonAIELayerPost(num_classes))

def SingleBottleneck_x1_plain_with_bn(num_classes):
    return CombinedModel_MIX(NonAIELayerInitial(),NonOffloadBottleneck(Bottleneck_projected, [1,]), \
                         AIELayerOffloadPlain(Bottleneck_plain_with_bn, [1,]),\
                              NonAIELayerPost(num_classes))
