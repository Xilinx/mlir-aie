...
aie.device(ipu) {
 ...
 // 3x3 conv
  ...
  func.func @sequence(%in0 : memref<65536xi32>, %wts0 : memref<17408xi32>, %out : memref<65536xi32>) {
    ...
    aiex.ipu.rtp_write(0, 3, 0,  9) { buffer_sym_name = "rtp3" }  // scale 11 || 8
    aiex.ipu.rtp_write(0, 5, 0,  9) { buffer_sym_name = "rtp4" }  // scale 11 || 8
    aiex.ipu.rtp_write(0, 4, 0,  12)  { buffer_sym_name = "rtp5" }  // scale: 10 || 8 conv1x1 with the same scale as the input so we match the scaling factor of output after conv1x1 and the initial input
