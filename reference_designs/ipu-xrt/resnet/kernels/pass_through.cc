...
void pass_through_scalar(uint8_t *input0, uint8_t *input1, uint8_t *output,
    ...
    if (oc >= output_channels / 16) {
      for (oc8b = 0; oc8b < 8; oc8b++) {
        for (x = 0; x < input_width; x++) { // col of output image
          int val2 = input1[((oc - (output_channels / 16)) * input_width * 8) +
                            (x * 8) + oc8b];
...
// #ifdef SCALAR
void pass_through(uint8_t *input0, uint8_t *input1, uint8_t *output,
                  const int32_t input_width, const int32_t input_channels,
                  const int32_t output_channels) {
  pass_through_scalar(input0, input1, output, input_width, input_channels,
                      output_channels);
}
