...
void conv2dk1_skip_init_i8_vector(uint8_t *input0,uint8_t *input1,  int8_t *kernels, uint8_t *output, int8_t *skip, const int32_t  input_width, const int32_t  input_channels,const int32_t  output_channels,const int32_t  input_channels_skip,const int scale,const int skip_scale ,const int scale_skip_conv )                    
    ...
    for(int oc=0; oc<(output_channels/8); oc++) {
        for(int x=0; x<iw_32; x++) {
            ...
            for(int ic=0; ic<(input_channels_skip/8); ic++) {
                // For oc > 8, we would load the next 64 weights after all the ic weights {OC}{IC}{IC8}{OC8}
                aie::vector<int8, 64> in_b = aie::load_v<64>(kernels_skip); kernels_skip+=64; // wts ic0..7(oc0..7)

                for(int x8=0; x8<NUM_ACC; x8++) 
                {                                                      
                    aie::vector<int8, 32> in_a      = aie::load_v<32>(skip+input_offset3); input_offset3 += 32; // act oc0..3(ic0..7)
                    acci_tmp[x8].mac(in_a, in_b);
                }
                input_offset3 += (iw*8)-256; // Move to next ic/8 position. 256 = 32 input * 8 ic
            }
            for(int x8=0; x8<NUM_ACC; x8++) {
            ...
            input_offset3 -= ((input_channels_skip/8)*iw*8) - 256; // reset to next input_width/32 block. 256 = 32 input * 8 ic
            kernels       -= (input_channels/8)*64; // reset kernel back to beginning of ic/8
            kernels_skip  -= (input_channels_skip/8)*64; // reset kernel back to beginning of ic/8
        } // for(int x=0; x<iw_32; x++) {
        ...
        kernels_skip  += (input_channels_skip/8) * 64;  // move to next oc/8 weights
...
void conv2dk1_skip_init_ui8(uint8_t *input0, uint8_t *input1,  int8_t *kernels, uint8_t *output, uint8_t *skip, const int32_t  input_width, const int32_t  input_channels,const int32_t  output_channels,const int32_t  input_channels_skip,const int scale,const int skip_scale ,const int scale_skip_conv )                    
{
    // conv2dk1_skip_init_ui8_scalar(input0, input1, kernels, output, skip, input_width, input_channels, output_channels, input_channels_skip, scale, skip_scale, scale_skip_conv);
}
...
void conv2dk1_skip_init_i8(uint8_t *input0, uint8_t *input1,  int8_t *kernels, uint8_t *output, int8_t *skip, const int32_t  input_width, const int32_t  input_channels,const int32_t  output_channels,const int32_t  input_channels_skip,const int scale,const int skip_scale ,const int scale_skip_conv )                    
{
    conv2dk1_skip_init_i8_vector(input0, input1, kernels, output, skip, input_width, input_channels, output_channels, input_channels_skip, scale, skip_scale, scale_skip_conv);
}

void conv2dk1_skip_init_ui8(uint8_t *input0, uint8_t *input1,  int8_t *kernels, uint8_t *output, uint8_t *skip, const int32_t  input_width, const int32_t  input_channels,const int32_t  output_channels,const int32_t  input_channels_skip,const int scale,const int skip_scale ,const int scale_skip_conv )                    
{
    // conv2dk1_skip_init_ui8_vector(input0, input1, kernels, output, skip, input_width, input_channels, output_channels, input_channels_skip, scale, skip_scale, scale_skip_conv);
}
