...
int TRACE_SIZE = (8192 * sizeof(uint32_t));
...
void convolution2D(int8_t *input, int8_t *kernels, int8_t *output,
        ...
        for (int cin = 0; cin < input_channels; cin++) {
          for (int ky = 0; ky < kernel_height; ky++) {
            for (int kx = 0; kx < kernel_height; kx++) {
              ...
              sum += input[cin * (X + 2 * PADDING) * (Y + 2 * PADDING) +
                           input_y * (X + 2 * PADDING) + input_x] *
                     kernels[cin * output_channels * kernel_height *
                                 kernel_height +
                             cout * kernel_height * kernel_height +
                             ky * kernel_height + kx];
            }
          }
        }
...
void conv2d_simple(int8_t *input, int8_t *kernels, int8_t *output,
                   int input_width, int input_height, int input_channels,
                   int output_channels, int kernel_width, int kernel_height) {
  int y, x, ky, kx, ic, oc;
        ...
        for (ic = 0; ic < input_channels; ic++) {
          for (ky = 0; ky < kernel_height; ky++) {
            for (kx = 0; kx < kernel_width; kx++) {
              ...
              int8_t val = input[ic * input_width * input_height +
                                 in_y * input_width + in_x];
              int8_t k = kernels[(oc * input_channels + ic) * kernel_width *
                                     kernel_height +
                                 ky * kernel_width + kx];
              sum += val * k;
            }
...
int main(int argc, const char *argv[]) {
  ...
  int8_t wts_1[WTS_SIZE_1] = {-7, -7, 4, -3, -4, 5,  -3, 6,  -4, 4, 3,
                              -5, -2, 4, 0,  -3, -1, 3,  2,  5,  1, -1,
                              6,  3,  5, 5,  3,  -7, 3,  -5, -4, -3};
  ...
  auto bo_inA = xrt::bo(device, IN_SIZE_1 * sizeof(int8_t),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(2));
  auto bo_inB =
      xrt::bo(device, (WTS_SIZE_1 + WTS_SIZE_2 + WTS_SIZE_3) * sizeof(int8_t),
              XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
  auto bo_out = xrt::bo(device, OUT_SIZE_3 * sizeof(int8_t) + TRACE_SIZE,
  ...
  memset(out_1_padded, 0,
         sizeof(out_1_padded)); // Initialize the padded input to zeros
  ...
  for (int cin = 0; cin < Co_1; cin++) {
    for (int y = 0; y < Y_out; y++) {
      for (int x = 0; x < X; x++) {
        out_1_padded[cin * (X + 2 * PADDING) * (Y_out + 2 * PADDING) +
                     (y + PADDING) * (X + 2 * PADDING) + (x + PADDING)] =
  ...
  for (unsigned yy = 0; yy < Y_out; yy++) {
    ...
    for (unsigned cc = 0; cc < Co_3; cc++) {
      ...
      for (unsigned xx = 0; xx < X; xx++) {
        // printf("\n****** OFM %i ******\n",cc);
        printf("%i\t", (int)gold[xx + cc * X + Co_3 * X * yy] +
                           input[xx + cc * X + Co_3 * X * yy]);
      }
    }
    printf("\n");
  }

  std::ofstream of("trace.txt", std::ofstream::out);
  of << std::hex;
  for (uint32_t i = 0; i < TRACE_SIZE / 4; i++) {
    of << std::setfill('0') << std::setw(8)
       << ((uint32_t *)&(bufOut[OUT_SIZE_3]))[i] << "\n";
  }
  of.close();
