...
module {
  ...
  aie.objectfifo @inOF_act_L3L2(%tile00, {%tile01}, 2: i32): !aie.objectfifo<memref<32x1x256xi8>> // link the skip buffer in memtile to conv1_skip in tile4
  aie.objectfifo @act_L2_02(%tile01, {%tile02}, 2: i32): !aie.objectfifo<memref<32x1x256xi8>> // link the skip buffer in memtile to conv1_skip in tile4
  aie.objectfifo.link[@inOF_act_L3L2]-> [@act_L2_02] ()

   //wts
  aie.objectfifo @inOF_wts_0_L3L2(%tile00, {%tile02,%tile03,%tile04,%tile05}, 1 : i32) : !aie.objectfifo<memref<16384xi8>> // total buffer for weights


  // Final output OF
  aie.objectfifo @out_02_L2(%tile02, {%tile01}, 2 : i32) : !aie.objectfifo<memref<32x1x64xui8>> //32x1x32
  aie.objectfifo @outOFL2L3(%tile01, {%tile00}, 2 : i32) : !aie.objectfifo<memref<32x1x64xui8>> //2048
  ...
  aie.core(%tile02) {
    ...
    %wts_itr = arith.constant 2 : index
    ...
    scf.for %arg3 = %c0 to %intmax step %c1 {
            // acquire wts once
        scf.for %n = %c0 to %wts_itr step %c1 {
            %subviewWts = aie.objectfifo.acquire @inOF_wts_0_L3L2(Consume, 1) : !aie.objectfifosubview<memref<16384xi8>>
            ...
            scf.for %n = %c0 to %y_dim step %c1 {
              %subviewIn = aie.objectfifo.acquire @act_L2_02(Consume, 1) : !aie.objectfifosubview<memref<32x1x256xi8>>
              %elemIn = aie.objectfifo.subview.access %subviewIn[0] : !aie.objectfifosubview<memref<32x1x256xi8>> -> memref<32x1x256xi8>      

              %subviewOut = aie.objectfifo.acquire @out_02_L2(Produce, 1) : !aie.objectfifosubview<memref<32x1x64xui8>>
  ...
  func.func @sequence(%in0 : memref<65536xi32>, %wts0 : memref<8192xi32>, %out : memref<32768xi32>) {
    aiex.ipu.rtp_write(0, 2, 0,  10) { buffer_sym_name = "rtp2" }  // scale 11 || 6
      ...
      %act_out= arith.constant  32768 : i64 
      %total_wts = arith.constant  8192 : i64 
      ...
      aiex.ipu.dma_memcpy_nd(0, 0, %out[0, 0, 0, 0][1, 1, 1, %act_out][0, 0, 0]) {id = 2 : i64, metadata = @outOFL2L3} : memref<32768xi32>
      aiex.ipu.dma_memcpy_nd(0, 0, %wts0[0, 0, 0, 0][1, 1, 1, %total_wts][0, 0, 0]) {id = 1 : i64, metadata = @inOF_wts_0_L3L2} : memref<8192xi32>
