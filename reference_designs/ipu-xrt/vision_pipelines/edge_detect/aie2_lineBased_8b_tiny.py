...
def edge_detect():
    ...
    def deviceBody():
        ...
        def coreBody():
            ...
            def loopBody():
                elemIn = Acquire("inOF_L2L1", ObjectFifoPort.Consume, 1, memRef_256_ty).acquiredElem()
                elemOut = Acquire("OF_2to3", ObjectFifoPort.Produce, 1, memRef_64_ty).acquiredElem()
        ...
        def coreBody():  
            ...
            # Preamble : Top Border
            elemsInPre = Acquire("OF_2to3", ObjectFifoPort.Consume, 2, memRef_64_ty).acquiredElem()
            elemPreOut = Acquire("OF_3to4", ObjectFifoPort.Produce, 1, memRef_64_ty).acquiredElem()
            ...
            def loopBody():
                elemsIn = Acquire("OF_2to3", ObjectFifoPort.Consume, 3, memRef_64_ty).acquiredElem()
                elemOut = Acquire("OF_3to4", ObjectFifoPort.Produce, 1, memRef_64_ty).acquiredElem()
            ...
            elemsInPost = Acquire("OF_2to3", ObjectFifoPort.Consume, 2, memRef_64_ty).acquiredElem()
            elemPostOut = Acquire("OF_3to4", ObjectFifoPort.Produce, 1, memRef_64_ty).acquiredElem()
        ...
        def coreBody():  
            ...
            @forLoop(lowerBound = 0, upperBound = 36, step = 1)
            def loopBody():
                elemIn = Acquire("OF_3to4", ObjectFifoPort.Consume, 1, memRef_64_ty).acquiredElem()
                elemOut = Acquire("OF_4to5", ObjectFifoPort.Produce, 1, memRef_64_ty).acquiredElem()
        ...
        def coreBody():
            @forLoop(lowerBound = 0, upperBound = 36, step = 1)
            def loopBody():
                elemIn = Acquire("OF_4to5", ObjectFifoPort.Consume, 1, memRef_64_ty).acquiredElem()
                elemOut = Acquire("OF_5to5", ObjectFifoPort.Produce, 1, memRef_256_ty).acquiredElem()
                ...
                elemIn1 = Acquire("OF_5to5", ObjectFifoPort.Consume, 1, memRef_256_ty).acquiredElem()
                elemIn2 = Acquire("inOF_L2L1", ObjectFifoPort.Consume, 1, memRef_256_ty).acquiredElem()
                elemOut2 = Acquire("outOF_L1L2", ObjectFifoPort.Produce, 1, memRef_256_ty).acquiredElem()
