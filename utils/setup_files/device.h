...
public:
  ...
  explicit device(std::unique_ptr<hal::device> &&hal)
  ...
  std::shared_ptr<xrt_core::device> get_core_device() const {
    return m_hal->get_core_device();
  }

  void add_close_callback(callback_function_type fcn) {
  ...
  std::string getDriverLibraryName() const {
    return m_hal->getDriverLibraryName();
  }

  std::string getName() const { return m_hal->getName(); }

  unsigned int getBankCount() const { return m_hal->getBankCount(); }

  size_t getDdrSize() const { return m_hal->getDdrSize(); }

  size_t getAlignment() const { return m_hal->getAlignment(); }

  /**
   * @return
   *   List of clock frequency from device info
   */
  range<const unsigned short *> getClockFrequencies() const {
    return m_hal->getClockFrequencies();
  }

  std::ostream &printDeviceInfo(std::ostream &) const;

  size_t get_cdma_count() const { return m_hal->get_cdma_count(); }
   ...
   */
  bool open() { return m_hal->open(); }

  XRT_EXPORT
  void close();

  xclDeviceHandle get_xcl_handle() const { return m_hal->get_xcl_handle(); }

  xrt::device get_xrt_device() const { return m_hal->get_xrt_device(); }

  void acquire_cu_context(const uuid &uuid, size_t cuidx, bool shared) {
    m_hal->acquire_cu_context(uuid, cuidx, shared);
  }

  void release_cu_context(const uuid &uuid, size_t cuidx) {
    m_hal->release_cu_context(uuid, cuidx);
  }

  void acquire_cu_context(size_t cuidx, bool shared) {
    acquire_cu_context(m_uuid, cuidx, shared);
  }

  void release_cu_context(size_t cuidx) { release_cu_context(m_uuid, cuidx); }

  execbuffer_object_handle allocExecBuffer(size_t sz) {
    return m_hal->allocExecBuffer(sz);
  }

  buffer_object_handle alloc(size_t sz, memoryDomain domain,
                             uint64_t memoryIndex, void *user_ptr) {
   ...
   */
  buffer_object_handle alloc(const buffer_object_handle &bo, size_t sz,
                             size_t offset) {
    return m_hal->alloc(bo, sz, offset);
  }

  void *alloc_svm(size_t sz) { return m_hal->alloc_svm(sz); }

  void free_svm(void *svm_ptr) { m_hal->free_svm(svm_ptr); }
  ...
  event // ssize_t
  write(const buffer_object_handle &bo, const void *buffer, size_t sz,
        size_t offset, bool async = false) {
   ...
   */
  event // ssize_t
  read(const buffer_object_handle &bo, void *buffer, size_t sz, size_t offset,
       bool async = false) {
   ...
   */
  event // ssize_t
  sync(const buffer_object_handle &bo, size_t sz, size_t offset, direction dir,
       bool async = true) {
   ...
   */
  event // ssize_t
  copy(const buffer_object_handle &dst_bo, const buffer_object_handle &src_bo,
       size_t sz, size_t dst_offset, size_t src_offset) {
   ...
   */
  void *map(const buffer_object_handle &bo) {
    void *p = m_hal->map(bo);
    retain(bo);
    return p;
  }
   ...
   *   Handle to buffer object to map
   */
  void unmap(const buffer_object_handle &bo) {
  ...
  void *map(const execbuffer_object_handle &bo) { return m_hal->map(bo); }

  void unmap(const execbuffer_object_handle &bo) { m_hal->unmap(bo); }
   ...
   */
  int exec_buf(const execbuffer_object_handle &bo) {
    return m_hal->exec_buf(bo);
  }

  int exec_wait(int timeout_ms) const { return m_hal->exec_wait(timeout_ms); }
...
public:
   ...
   *   false otherwise
   */
  virtual bool is_imported(const buffer_object_handle &boh) const {
   ...
   */
  uint64_t getDeviceAddr(const buffer_object_handle &boh) {
   ...
   */
  buffer_object_handle getBufferFromFd(const int fd, size_t &size,
                                       unsigned flags) {
    // TODO: check for the device match, Success/Fail
...
private:
  ...
  void release(const buffer_object_handle &bo) {
    ...
    auto itr = std::find_if(
        ...
        [&bo](const buffer_object_handle &boh) { return bo == boh; });
...
public:
   ...
   */
  hal::operations_result<int> loadXclBin(const axlf *xclbin) {
    m_uuid = xclbin->m_header.uuid;
    return m_hal->loadXclBin(xclbin);
  }

  bool hasBankAlloc() const { return m_hal->hasBankAlloc(); }
   ...
   *   return value is implementation dependent.
   */
  hal::operations_result<ssize_t> readKernelCtrl(uint64_t offset, void *hbuf,
                                                 size_t size) {
   ...
   *   return value is implementation dependent.
   */
  hal::operations_result<int> reClock(unsigned int freqMHz) {
   ...
   *   return value is implementation dependent.
   */
  hal::operations_result<int> reClock2(unsigned short numClocks,
                                       unsigned short *freqMHz) {
    return m_hal->reClock2(numClocks, freqMHz);
  }

  // Following functions are undocumented profiling functions
  hal::operations_result<size_t> clockTraining(xdp::MonitorType type) {
    return m_hal->clockTraining(type);
  }

  hal::operations_result<uint32_t> countTrace(xdp::MonitorType type) {
    return m_hal->countTrace(type);
  }

  hal::operations_result<double> getDeviceClock() {
    return m_hal->getDeviceClock();
  }

  hal::operations_result<size_t> getDeviceTime() {
    return m_hal->getDeviceTime();
  }

  hal::operations_result<double> getHostMaxRead() {
    return m_hal->getHostMaxRead();
  }

  hal::operations_result<double> getHostMaxWrite() {
    return m_hal->getHostMaxWrite();
  }

  hal::operations_result<double> getKernelMaxRead() {
    return m_hal->getKernelMaxRead();
  }

  hal::operations_result<double> getKernelMaxWrite() {
    return m_hal->getKernelMaxWrite();
  }

  hal::operations_result<size_t> readCounters(xdp::MonitorType type,
                                              xdp::CounterResults &result) {
    return m_hal->readCounters(type, result);
  }

  hal::operations_result<size_t> debugReadIPStatus(xclDebugReadType type,
                                                   void *result) {
    return m_hal->debugReadIPStatus(type, result);
  }

  hal::operations_result<size_t> readTrace(xdp::MonitorType type,
                                           xdp::TraceEventsVector &vec) {
    return m_hal->readTrace(type, vec);
  }

  hal::operations_result<void> xclRead(xclAddressSpace space, uint64_t offset,
                                       void *hostBuf, size_t size) {
    return m_hal->xclRead(space, offset, hostBuf, size);
  }

  hal::operations_result<void> xclWrite(xclAddressSpace space, uint64_t offset,
                                        const void *hostBuf, size_t size) {
    return m_hal->xclWrite(space, offset, hostBuf, size);
  }

  hal::operations_result<ssize_t> xclUnmgdPread(unsigned flags, void *buf,
                                                size_t count, uint64_t offset) {
    return m_hal->xclUnmgdPread(flags, buf, count, offset);
  }

  hal::operations_result<void> setProfilingSlots(xdp::MonitorType type,
                                                 uint32_t slots) {
    return m_hal->setProfilingSlots(type, slots);
  }

  hal::operations_result<uint32_t> getProfilingSlots(xdp::MonitorType type) {
    return m_hal->getProfilingSlots(type);
  }

  hal::operations_result<void> getProfilingSlotName(xdp::MonitorType type,
                                                    uint32_t slotnum,
                                                    char *slotName,
                                                    uint32_t length) {
  ...
  hal::operations_result<void> configureDataflow(xdp::MonitorType type,
                                                 unsigned *ip_config) {
    return m_hal->configureDataflow(type, ip_config);
  }

  hal::operations_result<size_t> startCounters(xdp::MonitorType type) {
    return m_hal->startCounters(type);
  }

  hal::operations_result<size_t> startTrace(xdp::MonitorType type,
                                            uint32_t options) {
    return m_hal->startTrace(type, options);
  }

  hal::operations_result<size_t> stopCounters(xdp::MonitorType type) {
    return m_hal->stopCounters(type);
  }

  hal::operations_result<size_t> stopTrace(xdp::MonitorType type) {
    return m_hal->stopTrace(type);
  }

  hal::operations_result<uint32_t> getNumLiveProcesses() {
    return m_hal->getNumLiveProcesses();
  }

  hal::operations_result<std::string> getSysfsPath(const std::string &subdev,
                                                   const std::string &entry) {
    return m_hal->getSysfsPath(subdev, entry);
  }

  hal::operations_result<std::string> getSubdevPath(const std::string &subdev,
                                                    uint32_t idx) {
    return m_hal->getSubdevPath(subdev, idx);
  }

  hal::operations_result<std::string> getDebugIPlayoutPath() {
    return m_hal->getDebugIPlayoutPath();
  }

  hal::operations_result<int> getTraceBufferInfo(uint32_t nSamples,
                                                 uint32_t &traceSamples,
                                                 uint32_t &traceBufSz) {
    return m_hal->getTraceBufferInfo(nSamples, traceSamples, traceBufSz);
  }

  hal::operations_result<int> readTraceData(void *traceBuf, uint32_t traceBufSz,
                                            uint32_t numSamples,
                                            uint64_t ipBaseAddress,
                                            uint32_t &wordsPerSample) {
    return m_hal->readTraceData(traceBuf, traceBufSz, numSamples, ipBaseAddress,
                                wordsPerSample);
  }

  hal::operations_result<void> getDebugIpLayout(char *buffer, size_t size,
                                                size_t *size_ret) {
  ...
  event schedule(F &&f, queue_type qt, Args &&...args) {
  ...
  event scheduleM(F &&f, C &c, queue_type qt, Args &&...args) {
    // Ensure that the worker threads are running
    if (!m_setup_done)
      setup();
...
std::vector<device> loadDevices(UnaryPredicate pred) {
 ...
 *   Rvalue container of xrt_xocl::device objects
 */
inline std::vector<device> loadDevices() {
