// RUN: aie-opt %s --convert-linalg-to-affine-loops --affine-loop-unroll="unroll-full unroll-full-threshold=3" --canonicalize --aie-affine-vectorize="virtual-vector-size=8" -unaligned-loads-check=false -split-input-file | FileCheck %s
