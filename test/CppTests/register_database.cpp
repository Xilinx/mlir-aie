...
void test_lookup_pl_events() {
  ...
  if (!dma_s2mm_0_start) {
    throw std::runtime_error("Failed to find DMA_S2MM_0_START_TASK in pl module");
  }
  std::cout << "  ✓ DMA_S2MM_0_START_TASK = " << *dma_s2mm_0_start << "\n";
  
  auto dma_mm2s_0_start = db->lookupEvent("DMA_MM2S_0_START_TASK", "pl");
  if (!dma_mm2s_0_start) {
    throw std::runtime_error("Failed to find DMA_MM2S_0_START_TASK in pl module");
  }
  ...
  if (!dma_s2mm_0_finished) {
    throw std::runtime_error("Failed to find DMA_S2MM_0_FINISHED_TASK in pl module");
  }
  std::cout << "  ✓ DMA_S2MM_0_FINISHED_TASK = " << *dma_s2mm_0_finished << "\n";
  
  auto dma_starvation = db->lookupEvent("DMA_S2MM_0_STREAM_STARVATION", "pl");
  if (!dma_starvation) {
    throw std::runtime_error("Failed to find DMA_S2MM_0_STREAM_STARVATION in pl module");
  }
...
void test_wrong_module() {
  ...
  if (result.has_value()) {
    throw std::runtime_error("Expected nullopt when looking up core event in pl module");
  }
...
void test_trace_event_attr() {
  ...
  // Get the name - this should return const std::string&
  const std::string& name = eventAttr.getName();
  ...
  std::cout << "  ✓ Successfully looked up event: " << name << " = " << *eventNum << "\n";
}
