...
// CHECK: XAie_StrmPktSwMstrPortEnable(&(ctx->DevInst), XAie_TileLoc(x,y), CORE, 1, {{.*}} XAIE_SS_PKT_DONOT_DROP_HEADER, {{.*}} 0, {{.*}} 0x3);
...
// CHECK: XAie_StrmPktSwSlaveSlotEnable(&(ctx->DevInst), XAie_TileLoc(x,y), WEST, 1, {{.*}} 0, {{.*}} XAie_PacketInit(1,0), {{.*}} 0x1F, {{.*}} 1, {{.*}} 0);
 ...
 AIE.device(xcvc1902) {
  ...
  AIE.switchbox(%t11) {
    ...
    AIE.packetrules(West : 0) {
      AIE.rule(0x1F, 0x0, %a0_0) // multi-cast
    }
