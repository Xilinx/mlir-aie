...
// CHECK: XAie_DmaSetNextBd(&([[bd0]]), {{.*}}0, {{.*}}0);
...
// CHECK: XAie_DmaSetLock(&([[bd1]]), XAie_LockInit(1,1),XAie_LockInit(1,0));
// CHECK: XAie_DmaSetAddrLen(&([[bd1]]), {{.*}}0x1400, {{.*}}256 * 4);
// CHECK: XAie_DmaSetNextBd(&([[bd1]]), {{.*}}0, {{.*}}0);
...
// CHECK: XAie_DmaChannelEnable(&(ctx->DevInst), XAie_TileLoc(3,3), {{.*}}0, {{.*}}DMA_MM2S);
// CHECK: XAie_DmaChannelPushBdToQueue(&(ctx->DevInst), XAie_TileLoc(3,3), {{.*}}0, {{.*}}DMA_S2MM, {{.*}}1);
