...
module @aie_module  {
 aie.device(xcve2802) {
  ...
  %m01 = aie.memtile_dma(%t01) {
    ...
    ^bd0:
      aie.dma_bd(%buf_w : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%lock_w, "Release", 1)
      aie.next_bd ^bd1
    ^bd1:
      aie.dma_bd(%buf_l : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%lock_l, "Release", 1)
      aie.next_bd ^bd2
    ^bd2:
      aie.dma_bd(%buf_e : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
