...
// CHECK:           %[[VAL_1:.*]] = aie.lock(%[[VAL_0]]) {init = 1 : i32, sym_name = "of_in_prod_lock_0"}
// CHECK:           %[[VAL_2:.*]] = aie.lock(%[[VAL_0]]) {init = 0 : i32, sym_name = "of_in_cons_lock_0"}
// CHECK:           %[[VAL_3:.*]] = aie.tile(2, 2)
// CHECK:           %[[VAL_4:.*]] = aie.buffer(%[[VAL_3]]) {sym_name = "of_in_0_cons_buff_0"} : memref<16xi32>
// CHECK:           %[[VAL_5:.*]] = aie.buffer(%[[VAL_3]]) {sym_name = "of_in_0_cons_buff_1"} : memref<16xi32>
// CHECK:           %[[VAL_6:.*]] = aie.lock(%[[VAL_3]]) {init = 2 : i32, sym_name = "of_in_0_cons_prod_lock_0"}
// CHECK:           %[[VAL_7:.*]] = aie.lock(%[[VAL_3]]) {init = 0 : i32, sym_name = "of_in_0_cons_cons_lock_0"}
// CHECK:           %[[VAL_8:.*]] = aie.tile(2, 3)
// CHECK:           %[[VAL_9:.*]] = aie.buffer(%[[VAL_8]]) {sym_name = "of_in_1_cons_buff_0"} : memref<16xi32>
// CHECK:           %[[VAL_10:.*]] = aie.buffer(%[[VAL_8]]) {sym_name = "of_in_1_cons_buff_1"} : memref<16xi32>
// CHECK:           %[[VAL_11:.*]] = aie.lock(%[[VAL_8]]) {init = 2 : i32, sym_name = "of_in_1_cons_prod_lock_0"}
// CHECK:           %[[VAL_12:.*]] = aie.lock(%[[VAL_8]]) {init = 0 : i32, sym_name = "of_in_1_cons_cons_lock_0"}
// CHECK:           %[[VAL_13:.*]] = aie.tile(3, 3)
// CHECK:           %[[VAL_14:.*]] = aie.buffer(%[[VAL_13]]) {sym_name = "of_in_2_cons_buff_0"} : memref<16xi32>
// CHECK:           %[[VAL_15:.*]] = aie.buffer(%[[VAL_13]]) {sym_name = "of_in_2_cons_buff_1"} : memref<16xi32>
// CHECK:           %[[VAL_16:.*]] = aie.lock(%[[VAL_13]]) {init = 2 : i32, sym_name = "of_in_2_cons_prod_lock_0"}
// CHECK:           %[[VAL_17:.*]] = aie.lock(%[[VAL_13]]) {init = 0 : i32, sym_name = "of_in_2_cons_cons_lock_0"}
...
// CHECK:             %[[VAL_21:.*]] = aie.dma_start(MM2S, 0, ^bb1, ^bb2)
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_2]], AcquireGreaterEqual, %[[VAL_20]])
// CHECK:             aie.dma_bd(%[[VAL_18]] : memref<64xi32> offset = 0 len = 64)
// CHECK:             aie.use_lock(%[[VAL_1]], Release, %[[VAL_20]])
