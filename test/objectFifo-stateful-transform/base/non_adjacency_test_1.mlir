...
// CHECK:           %[[VAL_8:.*]] = aie.lock(%[[VAL_5]]) {init = 0 : i32, sym_name = "objfifo_cons_lock_0"}
// CHECK:           %[[VAL_9:.*]] = aie.lock(%[[VAL_5]]) {init = 0 : i32, sym_name = "objfifo_cons_lock_1"}
...
// CHECK:               aie.use_lock(%[[VAL_3]], Acquire, %[[VAL_15]])
// CHECK:               func.call @some_work(%[[VAL_1]]) : (memref<16xi32>) -> ()
// CHECK:               aie.use_lock(%[[VAL_3]], Release, %[[VAL_16]])
// CHECK:               aie.use_lock(%[[VAL_4]], Acquire, %[[VAL_15]])
// CHECK:               func.call @some_work(%[[VAL_2]]) : (memref<16xi32>) -> ()
// CHECK:               aie.use_lock(%[[VAL_4]], Release, %[[VAL_16]])
// CHECK:             }
// CHECK:             aie.end
// CHECK:           }
...
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_3]], Acquire, %[[VAL_26]])
// CHECK:             aie.dma_bd(%[[VAL_1]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_3]], Release, %[[VAL_27]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_4]], Acquire, %[[VAL_26]])
// CHECK:             aie.dma_bd(%[[VAL_2]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_4]], Release, %[[VAL_27]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_29:.*]] = aie.mem(%[[VAL_5]]) {
// CHECK:             %[[VAL_30:.*]] = arith.constant 0 : i32
// CHECK:             %[[VAL_31:.*]] = arith.constant 1 : i32
// CHECK:             %[[VAL_32:.*]] = aie.dma_start(S2MM, 0, ^bb1, ^bb3)
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_8]], Acquire, %[[VAL_30]])
// CHECK:             aie.dma_bd(%[[VAL_6]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_8]], Release, %[[VAL_31]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_9]], Acquire, %[[VAL_30]])
// CHECK:             aie.dma_bd(%[[VAL_7]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_9]], Release, %[[VAL_31]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:
// CHECK:             aie.end
// CHECK:           }
