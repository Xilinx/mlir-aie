...
// CHECK:           %[[VAL_2:.*]] = aie.lock(%[[VAL_0]], 0) {init = 0 : i32, sym_name = "outC_cons_prod_lock_0"}
// CHECK:           %[[VAL_3:.*]] = aie.lock(%[[VAL_0]], 1) {init = 0 : i32, sym_name = "outC_cons_cons_lock_0"}
...
// CHECK:           %[[VAL_6:.*]] = aie.lock(%[[VAL_1]], 0) {init = 2 : i32, sym_name = "outC_prod_lock_0"}
// CHECK:           %[[VAL_7:.*]] = aie.lock(%[[VAL_1]], 1) {init = 0 : i32, sym_name = "outC_cons_lock_0"}
...
// CHECK:           %[[VAL_12:.*]] = aie.lock(%[[VAL_0]], 2) {init = 0 : i32, sym_name = "inB_prod_lock_0"}
// CHECK:           %[[VAL_13:.*]] = aie.lock(%[[VAL_0]], 3) {init = 0 : i32, sym_name = "inB_cons_lock_0"}
...
// CHECK:           %[[VAL_16:.*]] = aie.lock(%[[VAL_1]], 4) {init = 2 : i32, sym_name = "inA_cons_prod_lock_0"}
// CHECK:           %[[VAL_17:.*]] = aie.lock(%[[VAL_1]], 5) {init = 0 : i32, sym_name = "inA_cons_cons_lock_0"}
// CHECK:           %[[VAL_18:.*]] = aie.lock(%[[VAL_0]], 4) {init = 0 : i32, sym_name = "inA_prod_lock_0"}
// CHECK:           %[[VAL_19:.*]] = aie.lock(%[[VAL_0]], 5) {init = 0 : i32, sym_name = "inA_cons_lock_0"}
...
// CHECK:             %[[VAL_25:.*]] = arith.constant 4294967295 : index
// CHECK:             %[[VAL_26:.*]] = arith.constant 4 : index
// CHECK:             %[[VAL_27:.*]] = arith.constant 1 : index
// CHECK:             %[[VAL_28:.*]] = arith.constant 0 : index
// CHECK:             %[[VAL_29:.*]] = arith.constant 2 : index
// CHECK:             %[[VAL_30:.*]] = arith.constant 1 : i32
// CHECK:             scf.for %[[VAL_31:.*]] = %[[VAL_28]] to %[[VAL_25]] step %[[VAL_27]] {
// CHECK:               scf.for %[[VAL_32:.*]] = %[[VAL_28]] to %[[VAL_26]] step %[[VAL_29]] {
// CHECK:                 aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_30]])
// CHECK:                 func.call @zero_scalar_i16(%[[VAL_4]]) : (memref<16x16xi16>) -> ()
// CHECK:                 scf.for %[[VAL_33:.*]] = %[[VAL_28]] to %[[VAL_26]] step %[[VAL_29]] {
// CHECK:                   aie.use_lock(%[[VAL_17]], AcquireGreaterEqual, %[[VAL_30]])
...
// CHECK:                   aie.use_lock(%[[VAL_16]], Release, %[[VAL_30]])
...
// CHECK:                   aie.use_lock(%[[VAL_10]], Release, %[[VAL_30]])
// CHECK:                 }
// CHECK:                 aie.use_lock(%[[VAL_7]], Release, %[[VAL_30]])
// CHECK:                 aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_30]])
// CHECK:                 func.call @zero_scalar_i16(%[[VAL_5]]) : (memref<16x16xi16>) -> ()
// CHECK:                 scf.for %[[VAL_34:.*]] = %[[VAL_28]] to %[[VAL_26]] step %[[VAL_29]] {
// CHECK:                   aie.use_lock(%[[VAL_17]], AcquireGreaterEqual, %[[VAL_30]])
// CHECK:                   aie.use_lock(%[[VAL_11]], AcquireGreaterEqual, %[[VAL_30]])
...
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, %[[VAL_36]])
// CHECK:             aie.dma_bd(%[[VAL_14]] : memref<16x8xi16> offset = 0 len = 128)
// CHECK:             aie.use_lock(%[[VAL_17]], Release, %[[VAL_36]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, %[[VAL_36]])
// CHECK:             aie.dma_bd(%[[VAL_15]] : memref<16x8xi16> offset = 0 len = 128)
// CHECK:             aie.use_lock(%[[VAL_17]], Release, %[[VAL_36]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:
// CHECK:             %[[VAL_38:.*]] = aie.dma_start(S2MM, 1, ^bb4, ^bb6)
// CHECK:           ^bb4:
// CHECK:             aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, %[[VAL_36]])
// CHECK:             aie.dma_bd(%[[VAL_8]] : memref<8x16xi16> offset = 0 len = 128)
// CHECK:             aie.use_lock(%[[VAL_11]], Release, %[[VAL_36]])
// CHECK:             aie.next_bd ^bb5
// CHECK:           ^bb5:
// CHECK:             aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, %[[VAL_36]])
// CHECK:             aie.dma_bd(%[[VAL_9]] : memref<8x16xi16> offset = 0 len = 128)
// CHECK:             aie.use_lock(%[[VAL_11]], Release, %[[VAL_36]])
...
// CHECK:             aie.dma_bd(%[[VAL_4]] : memref<16x16xi16> offset = 0 len = 256)
// CHECK:             aie.use_lock(%[[VAL_6]], Release, %[[VAL_36]])
// CHECK:             aie.next_bd ^bb8
// CHECK:           ^bb8:
// CHECK:             aie.use_lock(%[[VAL_7]], AcquireGreaterEqual, %[[VAL_36]])
// CHECK:             aie.dma_bd(%[[VAL_5]] : memref<16x16xi16> offset = 0 len = 256)
// CHECK:             aie.use_lock(%[[VAL_6]], Release, %[[VAL_36]])
