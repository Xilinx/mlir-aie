...
// RUN: aie-opt --aie-objectFifo-stateful-transform="dynamic-objFifos=false" --aie-assign-lock-ids %s
...
// CHECK:       aie.dma_bd(%[[VAL_8]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
...
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_9]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_11]], Release, %{{.*}})
...
// CHECK:       aie.dma_bd(%[[VAL_4]] : memref<128xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_7]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_5]] : memref<128xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_7]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[VAL_0]] : memref<128xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_3]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_2]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_1]] : memref<128xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_3]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
