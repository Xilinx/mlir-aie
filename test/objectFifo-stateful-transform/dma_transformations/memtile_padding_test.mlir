...
// RUN: aie-opt --aie-objectFifo-stateful-transform="dynamic-objFifos=false" %s | FileCheck %s
    ...
    // CHECK:         aie.dma_bd(%[[IN1_BUFF_0]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[IN1_CONS_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb2
    // CHECK:       ^bb2:
    // CHECK:         aie.use_lock(%[[IN1_PROD_LOCK]], AcquireGreaterEqual, 1)
    // CHECK:         aie.dma_bd(%[[IN1_BUFF_1]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[IN1_CONS_LOCK]], Release, 1)
    ...
    // CHECK:         aie.dma_bd(%[[OUT1_CONS_BUFF_0]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[OUT1_CONS_CONS_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb8
    // CHECK:       ^bb8:
    // CHECK:         aie.use_lock(%[[OUT1_CONS_PROD_LOCK]], AcquireGreaterEqual, 1)
    // CHECK:         aie.dma_bd(%[[OUT1_CONS_BUFF_1]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[OUT1_CONS_CONS_LOCK]], Release, 1)
    ...
    // CHECK:         aie.dma_bd(%[[IN1_CONS_BUFF_0]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[IN1_CONS_CONS_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb2
    // CHECK:       ^bb2:
    // CHECK:         aie.use_lock(%[[IN1_CONS_PROD_LOCK]], AcquireGreaterEqual, 1)
    // CHECK:         aie.dma_bd(%[[IN1_CONS_BUFF_1]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[IN1_CONS_CONS_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb1
    // CHECK:       ^bb3:
    // CHECK:         %1 = aie.dma_start(MM2S, 0, ^bb4, ^bb6)
    ...
    // CHECK:         aie.dma_bd(%[[OUT1_BUFF_0]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[OUT1_PROD_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb5
    // CHECK:       ^bb5:
    // CHECK:         aie.use_lock(%[[OUT1_CONS_LOCK]], AcquireGreaterEqual, 1)
    // CHECK:         aie.dma_bd(%[[OUT1_BUFF_1]] : memref<64x64xi8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
    // CHECK:         aie.use_lock(%[[OUT1_PROD_LOCK]], Release, 1)
    // CHECK:         aie.next_bd ^bb4
    // CHECK:       ^bb6:
    // CHECK:         aie.end
