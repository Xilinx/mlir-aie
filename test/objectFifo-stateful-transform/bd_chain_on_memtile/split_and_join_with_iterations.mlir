...
// CHECK:       aie.dma_bd(%in_cons_buff_0 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
...
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%in_cons_prod_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%in_cons_buff_1 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%in_cons_cons_lock_0, Release, 1)
...
// CHECK:       aie.dma_bd(%out_buff_0 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%out_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb15
// CHECK:     ^bb15:  // pred: ^bb14
// CHECK:       aie.use_lock(%out_prod_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%out_buff_1 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%out_cons_lock_0, Release, 1)
...
// CHECK:       aie.dma_bd(%split_0_cons_buff_0 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%split_0_cons_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%split_0_cons_prod_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%split_0_cons_buff_1 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%split_0_cons_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
...
// CHECK:       aie.dma_bd(%join_0_buff_0 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%join_0_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb5
// CHECK:     ^bb5:  // pred: ^bb4
// CHECK:       aie.use_lock(%join_0_cons_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%join_0_buff_1 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%join_0_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb4
// CHECK:     ^bb6:  // pred: ^bb3
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%split_1_cons_buff_0 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%split_1_cons_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%split_1_cons_prod_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%split_1_cons_buff_1 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%split_1_cons_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       %1 = aie.dma_start(MM2S, 0, ^bb4, ^bb6)
...
// CHECK:       aie.dma_bd(%join_1_buff_0 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%join_1_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb5
// CHECK:     ^bb5:  // pred: ^bb4
// CHECK:       aie.use_lock(%join_1_cons_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%join_1_buff_1 : memref<512xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%join_1_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb4
// CHECK:     ^bb6:  // pred: ^bb3
// CHECK:       aie.end
