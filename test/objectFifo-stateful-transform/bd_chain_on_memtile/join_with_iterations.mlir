...
// CHECK:       aie.dma_bd(%input_small_buff_0 : memref<256xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_small_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%input_small_cons_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%input_small_buff_1 : memref<256xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_small_prod_lock_0, Release, 1)
...
// CHECK:       aie.dma_bd(%combined_output_buff_0 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%combined_output_cons_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%combined_output_prod_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%combined_output_buff_1 : memref<1024xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%combined_output_cons_lock_0, Release, 1)
...
// CHECK:       aie.dma_bd(%input_medium_buff_0 : memref<384xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_medium_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%input_medium_cons_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%input_medium_buff_1 : memref<384xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_medium_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%input_large_buff_0 : memref<384xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_large_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%input_large_cons_lock_0, AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%input_large_buff_1 : memref<384xui8> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%input_large_prod_lock_0, Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
