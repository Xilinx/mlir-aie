...
// CHECK:             scf.for %[[VAL_23:.*]] = %[[VAL_20]] to %[[VAL_19]] step %[[VAL_18]] {
// CHECK:               aie.use_lock(%[[VAL_3]], Acquire, %[[VAL_21]])
// CHECK:               func.call @some_work(%[[VAL_1]]) : (memref<16xi32>) -> ()
// CHECK:               aie.use_lock(%[[VAL_3]], Release, %[[VAL_22]])
// CHECK:               aie.use_lock(%[[VAL_4]], Acquire, %[[VAL_21]])
// CHECK:               func.call @some_work(%[[VAL_2]]) : (memref<16xi32>) -> ()
// CHECK:               aie.use_lock(%[[VAL_4]], Release, %[[VAL_22]])
// CHECK:             }
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_24:.*]] = aie.mem(%[[VAL_0]]) {
// CHECK:             %[[VAL_25:.*]] = arith.constant 1 : i32
// CHECK:             %[[VAL_26:.*]] = arith.constant 0 : i32
// CHECK:             %[[VAL_27:.*]] = aie.dma_start(MM2S, 1, ^bb1, ^bb3)
// CHECK:           ^bb1:
...
// CHECK:           ^bb3:
// CHECK:             %[[VAL_28:.*]] = aie.dma_start(S2MM, 0, ^bb4, ^bb5)
// CHECK:           ^bb4:
// CHECK:             aie.use_lock(%[[VAL_15]], Acquire, %[[VAL_26]])
// CHECK:             aie.dma_bd(%[[VAL_14]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_15]], Release, %[[VAL_25]])
// CHECK:             aie.next_bd ^bb4
// CHECK:           ^bb5:
// CHECK:             %[[VAL_29:.*]] = aie.dma_start(MM2S, 0, ^bb6, ^bb8)
// CHECK:           ^bb6:
// CHECK:             aie.use_lock(%[[VAL_3]], Acquire, %[[VAL_25]])
// CHECK:             aie.dma_bd(%[[VAL_1]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_3]], Release, %[[VAL_26]])
// CHECK:             aie.next_bd ^bb7
// CHECK:           ^bb7:
// CHECK:             aie.use_lock(%[[VAL_4]], Acquire, %[[VAL_25]])
...
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_9]], Acquire, %[[VAL_31]])
// CHECK:             aie.dma_bd(%[[VAL_7]] : memref<16xi32> offset = 0 len = 16)
// CHECK:             aie.use_lock(%[[VAL_9]], Release, %[[VAL_32]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:
// CHECK:             aie.end
// CHECK:           }
