...
// CHECK:             aie.use_lock(%lock_2_0, Acquire, %{{.*}})
// CHECK:             aie.dma_bd(%0 : memref<16xi32> offset = {{.*}} len = {{.*}})
// CHECK:             aie.use_lock(%lock_2_0, Release, %{{.*}})
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%lock_2_0_0, Acquire, %{{.*}})
// CHECK:             aie.dma_bd(%1 : memref<16xi32> offset = {{.*}} len = {{.*}})
// CHECK:             aie.use_lock(%lock_2_0_0, Release, %{{.*}})
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:  // pred: ^bb0
// CHECK:             %4 = aie.dma_start(S2MM, 0, ^bb4, ^bb5)
// CHECK:           ^bb4:  // 2 preds: ^bb3, ^bb4
// CHECK:             aie.use_lock(%lock_2_0_1, Acquire, %{{.*}})
// CHECK:             aie.dma_bd(%2 : memref<16xi32> offset = {{.*}} len = {{.*}})
// CHECK:             aie.use_lock(%lock_2_0_1, Release, %{{.*}})
// CHECK:             aie.next_bd ^bb4
// CHECK:           ^bb5:  // pred: ^bb3
// CHECK:             %5 = aie.dma_start(MM2S, 0, ^bb6, ^bb7)
// CHECK:           ^bb6:  // 2 preds: ^bb5, ^bb6
// CHECK:             aie.use_lock(%[[VAL_5]], AcquireGreaterEqual, %{{.*}})
// CHECK:             aie.dma_bd(%ext_buffer_in : memref<16xi32> offset = {{.*}} len = {{.*}})
