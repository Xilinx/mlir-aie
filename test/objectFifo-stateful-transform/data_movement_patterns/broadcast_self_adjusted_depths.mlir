...
// CHECK:             aie.dma_bd(%[[VAL_29]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_31]], Release, 0)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_32]], Acquire, 1)
// CHECK:             aie.dma_bd(%[[VAL_30]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
...
// CHECK:             aie.dma_bd(%[[VAL_5]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_7]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_8]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_6]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
...
// CHECK:             aie.dma_bd(%[[VAL_9]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_12]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_13]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_10]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_13]], Release, 1)
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:  // pred: ^bb2
// CHECK:             aie.use_lock(%[[VAL_14]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_11]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
...
// CHECK:             aie.dma_bd(%[[VAL_15]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_19]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_20]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_16]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_20]], Release, 1)
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:  // pred: ^bb2
// CHECK:             aie.use_lock(%[[VAL_21]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_17]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_21]], Release, 1)
// CHECK:             aie.next_bd ^bb4
// CHECK:           ^bb4:  // pred: ^bb3
// CHECK:             aie.use_lock(%[[VAL_22]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_18]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
...
// CHECK:             aie.dma_bd(%[[VAL_23]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_26]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_27]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_24]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_27]], Release, 1)
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:  // pred: ^bb2
// CHECK:             aie.use_lock(%[[VAL_28]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_25]] : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
