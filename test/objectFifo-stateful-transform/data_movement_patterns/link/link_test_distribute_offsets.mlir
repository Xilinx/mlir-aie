...
// CHECK:       aie.dma_bd(%[[VAL_12]] : memref<48xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_15]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_12]] : memref<48xi32> offset = 16 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_17]], Release, 1)
// CHECK:       aie.next_bd ^bb3
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%[[VAL_18]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_12]] : memref<48xi32> offset = 36 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_19]], Release, 1)
// CHECK:       aie.next_bd ^bb4
// CHECK:     ^bb4:  // pred: ^bb3
// CHECK:       aie.use_lock(%[[VAL_14]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_13]] : memref<48xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_15]], Release, 1)
// CHECK:       aie.next_bd ^bb5
// CHECK:     ^bb5:  // pred: ^bb4
// CHECK:       aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_13]] : memref<48xi32> offset = 16 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_17]], Release, 1)
// CHECK:       aie.next_bd ^bb6
// CHECK:     ^bb6:  // pred: ^bb5
// CHECK:       aie.use_lock(%[[VAL_18]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_13]] : memref<48xi32> offset = 36 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_19]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[VAL_8]] : memref<4x4xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_11]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_9]] : memref<4x4xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_11]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[VAL_4]] : memref<20xi32> offset = 0 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_7]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_5]] : memref<20xi32> offset = 0 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_7]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[VAL_0]] : memref<12xi32> offset = 0 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_3]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_2]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_1]] : memref<12xi32> offset = 0 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_3]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
