...
// CHECK:             aie.dma_bd(%[[VAL_10]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_13]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_11]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_13]], Release, 1)
...
// CHECK:             aie.dma_bd(%[[VAL_14]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_15]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:  // pred: ^bb2
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_16]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb4
// CHECK:           ^bb4:  // pred: ^bb3
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_17]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb5
// CHECK:           ^bb5:  // pred: ^bb4
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_18]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb6
// CHECK:           ^bb6:  // pred: ^bb5
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_19]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb7
// CHECK:           ^bb7:  // pred: ^bb6
// CHECK:             aie.use_lock(%[[VAL_21]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_20]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_22]], Release, 1)
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb8:  // pred: ^bb0
...
// CHECK:             aie.dma_bd(%[[VAL_4]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_9]], Release, 1)
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:  // pred: ^bb1
// CHECK:             aie.use_lock(%[[VAL_8]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_5]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_9]], Release, 1)
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:  // pred: ^bb2
// CHECK:             aie.use_lock(%[[VAL_8]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_6]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_9]], Release, 1)
// CHECK:             aie.next_bd ^bb4
// CHECK:           ^bb4:  // pred: ^bb3
// CHECK:             aie.use_lock(%[[VAL_8]], AcquireGreaterEqual, 1)
// CHECK:             aie.dma_bd(%[[VAL_7]] : memref<3000xi32> offset = 0 len = 3000 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_9]], Release, 1)
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb5:  // pred: ^bb0
// CHECK:             aie.end
