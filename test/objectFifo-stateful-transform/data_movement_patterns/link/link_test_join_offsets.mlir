...
// CHECK:       aie.dma_bd(%[[LINK1_BUFF_0]] : memref<4x4xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK1_PROD_LOCK]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:
// CHECK:       aie.use_lock(%[[LINK1_CONS_LOCK]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK1_BUFF_1]] : memref<4x4xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK1_PROD_LOCK]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_0]] : memref<48xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_0]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:
// CHECK:       aie.use_lock(%[[LINK4_PROD_LOCK_0]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_1]] : memref<48xi32> offset = 0 len = 16 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_0]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:
...
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_0]] : memref<48xi32> offset = 16 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_1]], Release, 1)
// CHECK:       aie.next_bd ^bb5
// CHECK:     ^bb5:
// CHECK:       aie.use_lock(%[[LINK4_PROD_LOCK_1]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_1]] : memref<48xi32> offset = 16 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_1]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_0]] : memref<48xi32> offset = 36 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_2]], Release, 1)
// CHECK:       aie.next_bd ^bb8
// CHECK:     ^bb8:
// CHECK:       aie.use_lock(%[[LINK4_PROD_LOCK_2]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK4_BUFF_1]] : memref<48xi32> offset = 36 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK4_CONS_LOCK_2]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[LINK2_BUFF_0]] : memref<20xi32> offset = 0 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK2_PROD_LOCK]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:
// CHECK:       aie.use_lock(%[[LINK2_CONS_LOCK]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK2_BUFF_1]] : memref<20xi32> offset = 0 len = 20 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK2_PROD_LOCK]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[LINK3_BUFF_0]] : memref<12xi32> offset = 0 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK3_PROD_LOCK]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:
// CHECK:       aie.use_lock(%[[LINK3_CONS_LOCK]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[LINK3_BUFF_1]] : memref<12xi32> offset = 0 len = 12 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[LINK3_PROD_LOCK]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:
// CHECK:       aie.end
