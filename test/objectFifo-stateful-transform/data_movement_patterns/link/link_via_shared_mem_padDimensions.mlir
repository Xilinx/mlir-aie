...
// CHECK:   aie.dma_bd(%[[MT_BUF0]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:   aie.use_lock(%[[MT_CONS]], Release, 1)
// CHECK:   aie.next_bd ^bb2
// CHECK: ^bb2:
// CHECK:   aie.use_lock(%[[MT_PROD]], AcquireGreaterEqual, 1)
// CHECK:   aie.dma_bd(%[[MT_BUF1]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:   aie.use_lock(%[[MT_CONS]], Release, 1)
...
// CHECK:   aie.dma_bd(%[[OUT_BUF0]] : memref<512xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:   aie.use_lock(%[[OUT_CONS]], Release, 1)
// CHECK:   aie.next_bd ^bb2
// CHECK: ^bb2:
// CHECK:   aie.use_lock(%[[OUT_PROD]], AcquireGreaterEqual, 1)
// CHECK:   aie.dma_bd(%[[OUT_BUF1]] : memref<512xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:   aie.use_lock(%[[OUT_CONS]], Release, 1)
// CHECK:   aie.next_bd ^bb1
// CHECK: ^bb3:
// CHECK:   aie.end
