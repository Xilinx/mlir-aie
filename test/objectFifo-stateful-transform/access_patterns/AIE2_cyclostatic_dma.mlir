...
// CHECK:       aie.dma_bd(%[[buf0_0]] : memref<i32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[PL]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[CL]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[buf0_1:.*]] : memref<i32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[PL]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[buf1_0:.*]] : memref<i32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[C_CL]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[C_PL]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[buf1_1]] : memref<i32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[C_CL]], Release, 1)
// CHECK:       aie.next_bd ^bb3
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%[[C_PL]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[buf1_2]] : memref<i32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.use_lock(%[[C_CL]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb4:  // pred: ^bb0
// CHECK:       aie.end
