...
// CHECK-DAG:     %[[OUT1_PROD_LOCK:.*]] = aie.lock(%[[TILE_0_2]], 0) {init = 2 : i32, sym_name = "out1_prod_lock_0"}
// CHECK-DAG:     %[[OUT1_CONS_LOCK:.*]] = aie.lock(%[[TILE_0_2]], 1) {init = 0 : i32, sym_name = "out1_cons_lock_0"}
...
// CHECK-DAG:     %[[OUT0_PROD_LOCK:.*]] = aie.lock(%[[MEM_TILE_0_1]], 0) {init = 2 : i32, sym_name = "out0_prod_lock_0"}
// CHECK-DAG:     %[[OUT0_CONS_LOCK:.*]] = aie.lock(%[[MEM_TILE_0_1]], 1) {init = 0 : i32, sym_name = "out0_cons_lock_0"}
...
// CHECK-DAG:     %[[IN1_CONS_PROD_LOCK:.*]] = aie.lock(%[[TILE_0_2]], 2) {init = 2 : i32, sym_name = "in1_cons_prod_lock_0"}
// CHECK-DAG:     %[[IN1_CONS_CONS_LOCK:.*]] = aie.lock(%[[TILE_0_2]], 3) {init = 0 : i32, sym_name = "in1_cons_cons_lock_0"}
...
// CHECK-DAG:     %[[IN0_CONS_PROD_LOCK:.*]] = aie.lock(%[[MEM_TILE_0_1]], 2) {init = 2 : i32, sym_name = "in0_cons_prod_lock_0"}
// CHECK-DAG:     %[[IN0_CONS_CONS_LOCK:.*]] = aie.lock(%[[MEM_TILE_0_1]], 3) {init = 0 : i32, sym_name = "in0_cons_cons_lock_0"}
...
// CHECK:       aie.dma_bd(%[[IN0_CONS_BUFF_0]] : memref<64000xi32> offset = {{.*}} len = {{.*}})
...
// CHECK:     ^bb7:
// CHECK:       aie.use_lock(%[[OUT0_CONS_LOCK]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[OUT0_BUFF_0]] : memref<64000xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[OUT0_PROD_LOCK]], Release, %{{.*}})
// CHECK:     ^bb9:
// CHECK:       aie.dma_start(S2MM, 0, ^bb10, ^bb12)
// CHECK:     ^bb10:
// CHECK:       aie.use_lock(%[[OUT0_PROD_LOCK]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[OUT0_BUFF_0]] : memref<64000xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[OUT0_CONS_LOCK]], Release, %{{.*}})
// CHECK:     }
