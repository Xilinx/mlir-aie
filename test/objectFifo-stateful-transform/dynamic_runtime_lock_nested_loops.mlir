//===- dynamic_runtime_lock_nested_loops.mlir -----------------*- MLIR -*-===//
//
// Copyright (C) 2026 Advanced Micro Devices, Inc.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
//===----------------------------------------------------------------------===//

// Acquires/releases at both the outer and inner loop levels on two fifos.
// The runtime held counters are shared across the loop nest, so the inner
// loop's per-iteration acquire delta and the outer loop's acquire delta are
// each computed from the current held value regardless of loop depth.

// RUN: aie-opt --aie-objectFifo-stateful-transform --aie-objectFifo-unroll="dynamic-objFifos=true" --canonicalize %s | FileCheck %s

// NOTE: Assertions have been autogenerated by utils/generate-test-checks.py

// The script is designed to make adding checks to
// a test case fast, it is *not* designed to be authoritative
// about what constitutes a good test! The CHECK should be
// minimized and named to reflect the test intent.


// CHECK-LABEL:   aie.device(npu2) {
// CHECK:           %[[VAL_0:.*]] = aie.tile(0, 1)
// CHECK:           %[[VAL_1:.*]] = aie.tile(0, 2)
// CHECK:           %[[VAL_2:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_X_cons_buff_0"} : memref<8xi8>
// CHECK:           %[[VAL_3:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_X_cons_buff_1"} : memref<8xi8>
// CHECK:           %[[VAL_4:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_X_cons_buff_2"} : memref<8xi8>
// CHECK:           %[[VAL_5:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_X_cons_buff_3"} : memref<8xi8>
// CHECK:           %[[VAL_6:.*]] = aie.lock(%[[VAL_1]], 2) {init = 4 : i32, sym_name = "inOF_X_cons_prod_lock_0"}
// CHECK:           %[[VAL_7:.*]] = aie.lock(%[[VAL_1]], 3) {init = 0 : i32, sym_name = "inOF_X_cons_cons_lock_0"}
// CHECK:           %[[VAL_8:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_X_buff_0"} : memref<8xi8>
// CHECK:           %[[VAL_9:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_X_buff_1"} : memref<8xi8>
// CHECK:           %[[VAL_10:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_X_buff_2"} : memref<8xi8>
// CHECK:           %[[VAL_11:.*]] = aie.lock(%[[VAL_0]], 2) {init = 3 : i32, sym_name = "inOF_X_prod_lock_0"}
// CHECK:           %[[VAL_12:.*]] = aie.lock(%[[VAL_0]], 3) {init = 0 : i32, sym_name = "inOF_X_cons_lock_0"}
// CHECK:           %[[VAL_13:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_W_cons_buff_0"} : memref<8xi8>
// CHECK:           %[[VAL_14:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_W_cons_buff_1"} : memref<8xi8>
// CHECK:           %[[VAL_15:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "inOF_W_cons_buff_2"} : memref<8xi8>
// CHECK:           %[[VAL_16:.*]] = aie.lock(%[[VAL_1]], 0) {init = 3 : i32, sym_name = "inOF_W_cons_prod_lock_0"}
// CHECK:           %[[VAL_17:.*]] = aie.lock(%[[VAL_1]], 1) {init = 0 : i32, sym_name = "inOF_W_cons_cons_lock_0"}
// CHECK:           %[[VAL_18:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_W_buff_0"} : memref<8xi8>
// CHECK:           %[[VAL_19:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_W_buff_1"} : memref<8xi8>
// CHECK:           %[[VAL_20:.*]] = aie.buffer(%[[VAL_0]]) {sym_name = "inOF_W_buff_2"} : memref<8xi8>
// CHECK:           %[[VAL_21:.*]] = aie.lock(%[[VAL_0]], 0) {init = 3 : i32, sym_name = "inOF_W_prod_lock_0"}
// CHECK:           %[[VAL_22:.*]] = aie.lock(%[[VAL_0]], 1) {init = 0 : i32, sym_name = "inOF_W_cons_lock_0"}
// CHECK:           aie.flow(%[[VAL_0]], DMA : 0, %[[VAL_1]], DMA : 0)
// CHECK:           aie.flow(%[[VAL_0]], DMA : 1, %[[VAL_1]], DMA : 1)
// CHECK:           %[[VAL_23:.*]] = aie.core(%[[VAL_1]]) {
// CHECK:             %[[VAL_24:.*]] = arith.constant -2 : i32
// CHECK:             %[[VAL_25:.*]] = arith.constant -3 : i32
// CHECK:             %[[VAL_26:.*]] = arith.constant 4 : i32
// CHECK:             %[[VAL_27:.*]] = arith.constant 1 : i32
// CHECK:             %[[VAL_28:.*]] = arith.constant 3 : i32
// CHECK:             %[[VAL_29:.*]] = arith.constant 2 : i32
// CHECK:             %[[VAL_30:.*]] = arith.constant 0 : i32
// CHECK:             %[[VAL_31:.*]] = arith.constant 0 : index
// CHECK:             %[[VAL_32:.*]] = arith.constant 1 : index
// CHECK:             %[[VAL_33:.*]] = arith.constant 14 : index
// CHECK:             %[[VAL_34:.*]]:4 = scf.for %[[VAL_35:.*]] = %[[VAL_31]] to %[[VAL_33]] step %[[VAL_32]] iter_args(%[[VAL_36:.*]] = %[[VAL_30]], %[[VAL_37:.*]] = %[[VAL_30]], %[[VAL_38:.*]] = %[[VAL_30]], %[[VAL_39:.*]] = %[[VAL_30]]) -> (i32, i32, i32, i32) {
// CHECK:               %[[VAL_40:.*]] = arith.subi %[[VAL_29]], %[[VAL_38]] : i32
// CHECK:               %[[VAL_41:.*]] = arith.maxsi %[[VAL_40]], %[[VAL_30]] : i32
// CHECK:               aie.use_lock(%[[VAL_17]], AcquireGreaterEqual, %[[VAL_41]])
// CHECK:               %[[VAL_42:.*]] = arith.addi %[[VAL_38]], %[[VAL_41]] : i32
// CHECK:               %[[VAL_43:.*]]:2 = scf.for %[[VAL_44:.*]] = %[[VAL_31]] to %[[VAL_33]] step %[[VAL_32]] iter_args(%[[VAL_45:.*]] = %[[VAL_37]], %[[VAL_46:.*]] = %[[VAL_39]]) -> (i32, i32) {
// CHECK:                 %[[VAL_47:.*]] = arith.subi %[[VAL_28]], %[[VAL_46]] : i32
// CHECK:                 %[[VAL_48:.*]] = arith.maxsi %[[VAL_47]], %[[VAL_30]] : i32
// CHECK:                 aie.use_lock(%[[VAL_7]], AcquireGreaterEqual, %[[VAL_48]])
// CHECK:                 %[[VAL_49:.*]] = arith.addi %[[VAL_46]], %[[VAL_48]] : i32
// CHECK:                 aie.use_lock(%[[VAL_6]], Release, %[[VAL_27]])
// CHECK:                 %[[VAL_50:.*]] = arith.subi %[[VAL_49]], %[[VAL_27]] : i32
// CHECK:                 %[[VAL_51:.*]] = arith.addi %[[VAL_45]], %[[VAL_27]] : i32
// CHECK:                 %[[VAL_52:.*]] = arith.cmpi sge, %[[VAL_51]], %[[VAL_26]] : i32
// CHECK:                 %[[VAL_53:.*]] = arith.addi %[[VAL_45]], %[[VAL_25]] : i32
// CHECK:                 %[[VAL_54:.*]] = arith.select %[[VAL_52]], %[[VAL_53]], %[[VAL_51]] : i32
// CHECK:                 scf.yield %[[VAL_54]], %[[VAL_50]] : i32, i32
// CHECK:               }
// CHECK:               aie.use_lock(%[[VAL_6]], Release, %[[VAL_29]])
// CHECK:               %[[VAL_55:.*]] = arith.subi %[[VAL_56:.*]]#1, %[[VAL_29]] : i32
// CHECK:               %[[VAL_57:.*]] = arith.addi %[[VAL_56]]#0, %[[VAL_29]] : i32
// CHECK:               %[[VAL_58:.*]] = arith.cmpi sge, %[[VAL_57]], %[[VAL_26]] : i32
// CHECK:               %[[VAL_59:.*]] = arith.addi %[[VAL_56]]#0, %[[VAL_24]] : i32
// CHECK:               %[[VAL_60:.*]] = arith.select %[[VAL_58]], %[[VAL_59]], %[[VAL_57]] : i32
// CHECK:               aie.use_lock(%[[VAL_16]], Release, %[[VAL_27]])
// CHECK:               %[[VAL_61:.*]] = arith.subi %[[VAL_42]], %[[VAL_27]] : i32
// CHECK:               %[[VAL_62:.*]] = arith.addi %[[VAL_36]], %[[VAL_27]] : i32
// CHECK:               %[[VAL_63:.*]] = arith.cmpi sge, %[[VAL_62]], %[[VAL_28]] : i32
// CHECK:               %[[VAL_64:.*]] = arith.addi %[[VAL_36]], %[[VAL_24]] : i32
// CHECK:               %[[VAL_65:.*]] = arith.select %[[VAL_63]], %[[VAL_64]], %[[VAL_62]] : i32
// CHECK:               scf.yield %[[VAL_65]], %[[VAL_60]], %[[VAL_61]], %[[VAL_55]] : i32, i32, i32, i32
// CHECK:             }
// CHECK:             aie.use_lock(%[[VAL_16]], Release, %[[VAL_27]])
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_66:.*]] = aie.memtile_dma(%[[VAL_0]]) {
// CHECK:             %[[VAL_67:.*]] = arith.constant 1 : i32
// CHECK:             %[[VAL_68:.*]] = aie.dma_start(MM2S, 0, ^bb1, ^bb4)
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_22]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_18]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_21]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_22]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_19]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_21]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:
// CHECK:             aie.use_lock(%[[VAL_22]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_20]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_21]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb4:
// CHECK:             %[[VAL_69:.*]] = aie.dma_start(MM2S, 1, ^bb5, ^bb8)
// CHECK:           ^bb5:
// CHECK:             aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_8]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_11]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb6
// CHECK:           ^bb6:
// CHECK:             aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_9]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_11]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb7
// CHECK:           ^bb7:
// CHECK:             aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, %[[VAL_67]])
// CHECK:             aie.dma_bd(%[[VAL_10]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_11]], Release, %[[VAL_67]])
// CHECK:             aie.next_bd ^bb5
// CHECK:           ^bb8:
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_70:.*]] = aie.mem(%[[VAL_1]]) {
// CHECK:             %[[VAL_71:.*]] = arith.constant 1 : i32
// CHECK:             %[[VAL_72:.*]] = aie.dma_start(S2MM, 0, ^bb1, ^bb4)
// CHECK:           ^bb1:
// CHECK:             aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_13]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_17]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_14]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_17]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb3
// CHECK:           ^bb3:
// CHECK:             aie.use_lock(%[[VAL_16]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_15]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_17]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb4:
// CHECK:             %[[VAL_73:.*]] = aie.dma_start(S2MM, 1, ^bb5, ^bb9)
// CHECK:           ^bb5:
// CHECK:             aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_2]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb6
// CHECK:           ^bb6:
// CHECK:             aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_3]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb7
// CHECK:           ^bb7:
// CHECK:             aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_4]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb8
// CHECK:           ^bb8:
// CHECK:             aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_71]])
// CHECK:             aie.dma_bd(%[[VAL_5]] : memref<8xi8> offset = 0 len = 8)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_71]])
// CHECK:             aie.next_bd ^bb5
// CHECK:           ^bb9:
// CHECK:             aie.end
// CHECK:           }
// CHECK:         }


module {
  aie.device(npu2) {
    %tile_0_1 = aie.tile(0, 1)
    %tile_0_2 = aie.tile(0, 2)

    aie.objectfifo @inOF_W(%tile_0_1, {%tile_0_2}, 3 : i32) : !aie.objectfifo<memref<8xi8>>
    aie.objectfifo @inOF_X(%tile_0_1, {%tile_0_2}, 3 : i32) : !aie.objectfifo<memref<8xi8>>

    %core_0_2 = aie.core(%tile_0_2) {
      %c0 = arith.constant 0 : index
      %c1 = arith.constant 1 : index
      %c14 = arith.constant 14 : index
      scf.for %arg0 = %c0 to %c14 step %c1 {
        %w = aie.objectfifo.acquire @inOF_W(Consume, 2) : !aie.objectfifosubview<memref<8xi8>>
        scf.for %arg1 = %c0 to %c14 step %c1 {
          %x = aie.objectfifo.acquire @inOF_X(Consume, 3) : !aie.objectfifosubview<memref<8xi8>>
          aie.objectfifo.release @inOF_X(Consume, 1)
        }
        aie.objectfifo.release @inOF_X(Consume, 2)
        aie.objectfifo.release @inOF_W(Consume, 1)
      }
      aie.objectfifo.release @inOF_W(Consume, 1)
      aie.end
    }
  }
}
