...
// CHECK:       aie.dma_bd(%[[LINK1_BUFF]] : memref<4x4xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
...
// CHECK:     ^bb1:
// CHECK:       aie.dma_bd(%[[LINK3_BUFF]] : memref<36xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb2:
...
// CHECK:       aie.dma_bd(%[[LINK2_BUFF]] : memref<20xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
