...
// CHECK:           %[[VAL_4:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "link3_buff_0"} : memref<36xi32>
// CHECK:           %[[VAL_5:.*]] = aie.buffer(%[[VAL_3]]) {sym_name = "link2_buff_0"} : memref<20xi32>
// CHECK:           %[[VAL_6:.*]] = aie.lock(%[[VAL_3]]) {init = 1 : i32, sym_name = "link2_prod_lock_0"}
// CHECK:           %[[VAL_7:.*]] = aie.lock(%[[VAL_3]]) {init = 0 : i32, sym_name = "link2_cons_lock_0"}
// CHECK:           %[[VAL_8:.*]] = aie.buffer(%[[VAL_2]]) {sym_name = "link1_buff_0"} : memref<4x4xi32>
// CHECK:           %[[VAL_9:.*]] = aie.lock(%[[VAL_2]]) {init = 1 : i32, sym_name = "link1_prod_lock_0"}
// CHECK:           %[[VAL_10:.*]] = aie.lock(%[[VAL_2]]) {init = 0 : i32, sym_name = "link1_cons_lock_0"}
...
// CHECK:             aie.dma_bd(%[[VAL_8]] : memref<4x4xi32> offset = 0 len = 16)
...
// CHECK:           ^bb1:
// CHECK:             aie.dma_bd(%[[VAL_4]] : memref<36xi32> offset = 0 len = 16)
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb2:
// CHECK:             %[[VAL_16:.*]] = aie.dma_start(S2MM, 1, ^bb3, ^bb4)
// CHECK:           ^bb3:
// CHECK:             aie.dma_bd(%[[VAL_4]] : memref<36xi32> offset = 16 len = 20)
...
// CHECK:             aie.dma_bd(%[[VAL_5]] : memref<20xi32> offset = 0 len = 20)
