...
// CHECK-DAG:     %[[VAL_0:.*]] = aie.buffer(%{{.*}}tile_1_3) {sym_name = "of0_cons_buff_0"} : memref<16xi32>
// CHECK-DAG:     %[[VAL_1:.*]] = aie.buffer(%{{.*}}tile_1_3) {sym_name = "of0_cons_buff_1"} : memref<16xi32>
// CHECK-DAG:     aie.flow(%{{.*}}tile_2_0, DMA : 0, %{{.*}}tile_1_3, DMA : 0)
// CHECK-DAG:     %ext_buffer_in = aie.external_buffer {sym_name = "ext_buffer_in"} : memref<64xi32>
// CHECK-DAG:     aie.shim_dma_allocation @of0_shim_alloc(%shim_noc_tile_2_0, MM2S, 0)
// CHECK:     %shim_dma_2_0 = aie.shim_dma(%{{.*}}tile_2_0) {
// CHECK:       %0 = aie.dma_start(MM2S, 0, ^bb1, ^bb2)
// CHECK:     ^bb1:  // 2 preds: ^bb0, ^bb1
// CHECK:       aie.dma_bd(%ext_buffer_in : memref<64xi32> offset = {{.*}} len = {{.*}})
