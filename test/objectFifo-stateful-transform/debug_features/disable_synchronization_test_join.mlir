...
// CHECK:     %link3_cons_prod_lock = aie.lock(%{{.*}}tile_2_3, 0) {init = 1 : i32, sym_name = "link3_cons_prod_lock_0"}
// CHECK:     %link3_cons_cons_lock = aie.lock(%{{.*}}tile_2_3, 1) {init = 0 : i32, sym_name = "link3_cons_cons_lock_0"}
// CHECK:     %link2_cons_buff_0 = aie.buffer(%{{.*}}tile_2_2) {sym_name = "link2_cons_buff_0"} : memref<4x4xi32> 
// CHECK:     %link2_cons_prod_lock = aie.lock(%{{.*}}tile_2_2, 0) {init = 1 : i32, sym_name = "link2_cons_prod_lock_0"}
// CHECK:     %link2_cons_cons_lock = aie.lock(%{{.*}}tile_2_2, 1) {init = 0 : i32, sym_name = "link2_cons_cons_lock_0"}
// CHECK:     %link1_cons_buff_0 = aie.buffer(%{{.*}}tile_2_1) {sym_name = "link1_cons_buff_0"} : memref<36xi32> 
...
// CHECK:       aie.dma_bd(%link1_cons_buff_0 : memref<36xi32>, 0, 36)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb2:  // pred: ^bb0
// CHECK:       %1 = aie.dma_start(MM2S, 0, ^bb3, ^bb4)
// CHECK:     ^bb3:  // 2 preds: ^bb2, ^bb3
// CHECK:       aie.dma_bd(%link1_cons_buff_0 : memref<36xi32>, 0, 16)
// CHECK:       aie.next_bd ^bb3
// CHECK:     ^bb4:  // pred: ^bb2
// CHECK:       %2 = aie.dma_start(MM2S, 1, ^bb5, ^bb6)
// CHECK:     ^bb5:  // 2 preds: ^bb4, ^bb5
// CHECK:       aie.dma_bd(%link1_cons_buff_0 : memref<36xi32>, 16, 20)
