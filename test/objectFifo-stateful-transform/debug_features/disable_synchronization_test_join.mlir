...
// CHECK-DAG:     %[[VAL_0:.*]] = aie.buffer(%{{.*}}tile_2_3) {sym_name = "link3_cons_buff_0"} : memref<20xi32>
// CHECK-DAG:     %[[VAL_1:.*]] = aie.lock(%{{.*}}tile_2_3) {init = 1 : i32, sym_name = "link3_cons_prod_lock_0"}
// CHECK-DAG:     %[[VAL_2:.*]] = aie.lock(%{{.*}}tile_2_3) {init = 0 : i32, sym_name = "link3_cons_cons_lock_0"}
// CHECK-DAG:     %[[VAL_3:.*]] = aie.buffer(%{{.*}}tile_2_2) {sym_name = "link2_cons_buff_0"} : memref<4x4xi32>
// CHECK-DAG:     %[[VAL_4:.*]] = aie.lock(%{{.*}}tile_2_2) {init = 1 : i32, sym_name = "link2_cons_prod_lock_0"}
// CHECK-DAG:     %[[VAL_5:.*]] = aie.lock(%{{.*}}tile_2_2) {init = 0 : i32, sym_name = "link2_cons_cons_lock_0"}
// CHECK-DAG:     %[[VAL_6:.*]] = aie.buffer(%{{.*}}tile_2_1) {sym_name = "link1_cons_buff_0"} : memref<36xi32>
...
// CHECK:       aie.dma_bd(%[[VAL_6]] : memref<36xi32> offset = {{.*}} len = {{.*}})
...
// CHECK:       aie.dma_bd(%[[VAL_3]] : memref<4x4xi32> offset = {{.*}} len = {{.*}})
...
// CHECK:       aie.dma_bd(%[[VAL_0]] : memref<20xi32> offset = {{.*}} len = {{.*}})
