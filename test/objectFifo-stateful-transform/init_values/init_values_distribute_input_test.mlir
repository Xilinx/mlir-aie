...
// CHECK-DAG:     %[[VAL_0:.*]] = aie.buffer(%{{.*}}tile_2_3) {sym_name = "of2_cons_buff_0"} : memref<2xi32>
// CHECK-DAG:     %[[VAL_1:.*]] = aie.buffer(%{{.*}}tile_2_3) {sym_name = "of2_cons_buff_1"} : memref<2xi32>
...
// CHECK-DAG:     %[[VAL_4:.*]] = aie.buffer(%{{.*}}tile_1_2) {sym_name = "of1_cons_buff_0"} : memref<2xi32>
// CHECK-DAG:     %[[VAL_5:.*]] = aie.buffer(%{{.*}}tile_1_2) {sym_name = "of1_cons_buff_1"} : memref<2xi32>
...
// CHECK-DAG:     %[[VAL_8:.*]] = aie.buffer(%{{.*}}tile_1_1) {sym_name = "of0_cons_buff_0"} : memref<4xi32>
// CHECK-DAG:     %[[VAL_9:.*]] = aie.buffer(%{{.*}}tile_1_1) {sym_name = "of0_cons_buff_1"} : memref<4xi32>
...
// CHECK-DAG:     %[[VAL_14:.*]] = aie.buffer(%{{.*}}tile_1_3) {sym_name = "of0_buff_0"} : memref<4xi32> = dense<[0, 1, 2, 3]>
// CHECK-DAG:     %[[VAL_15:.*]] = aie.buffer(%{{.*}}tile_1_3) {sym_name = "of0_buff_1"} : memref<4xi32> = dense<[4, 5, 6, 7]>
// CHECK-DAG:     %[[VAL_16:.*]] = aie.lock(%{{.*}}tile_1_3) {init = 0 : i32, sym_name = "of0_prod_lock_0"}
// CHECK-DAG:     %[[VAL_17:.*]] = aie.lock(%{{.*}}tile_1_3) {init = 2 : i32, sym_name = "of0_cons_lock_0"}
// CHECK-DAG:     aie.flow(%{{.*}}tile_1_3, DMA : 0, %{{.*}}tile_1_1, DMA : 0)
...
// CHECK:       aie.dma_bd(%[[VAL_14]] : memref<4xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_16]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_17]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_15]] : memref<4xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_16]], Release, %{{.*}})
...
// CHECK:       aie.dma_bd(%[[VAL_8]] : memref<4xi32> offset = {{.*}} len = {{.*}})
...
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_9]] : memref<4xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_11]], Release, %{{.*}})
...
// CHECK:       aie.dma_bd(%[[VAL_4]] : memref<2xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_7]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_5]] : memref<2xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_7]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[VAL_0]] : memref<2xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_3]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_2]], AcquireGreaterEqual, %{{.*}})
// CHECK:       aie.dma_bd(%[[VAL_1]] : memref<2xi32> offset = {{.*}} len = {{.*}})
// CHECK:       aie.use_lock(%[[VAL_3]], Release, %{{.*}})
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
