...
// CHECK:       aie.dma_bd(%[[VAL_14]] : memref<4xi32> offset = 0 len = 4 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_16]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_17]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_15]] : memref<4xi32> offset = 0 len = 4 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_16]], Release, 1)
...
// CHECK:       aie.dma_bd(%[[VAL_8]] : memref<4xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_11]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_8]] : memref<4xi32> offset = 2 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_13]], Release, 1)
// CHECK:       aie.next_bd ^bb3
// CHECK:     ^bb3:  // pred: ^bb2
// CHECK:       aie.use_lock(%[[VAL_10]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_9]] : memref<4xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_11]], Release, 1)
// CHECK:       aie.next_bd ^bb4
// CHECK:     ^bb4:  // pred: ^bb3
// CHECK:       aie.use_lock(%[[VAL_12]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_9]] : memref<4xi32> offset = 2 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_13]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb5:  // pred: ^bb0
...
// CHECK:       aie.dma_bd(%[[VAL_4]] : memref<2xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_7]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_5]] : memref<2xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_7]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
...
// CHECK:       aie.dma_bd(%[[VAL_0]] : memref<2xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_3]], Release, 1)
// CHECK:       aie.next_bd ^bb2
// CHECK:     ^bb2:  // pred: ^bb1
// CHECK:       aie.use_lock(%[[VAL_2]], AcquireGreaterEqual, 1)
// CHECK:       aie.dma_bd(%[[VAL_1]] : memref<2xi32> offset = 0 len = 2 sizes = [] strides = [])
// CHECK:       aie.use_lock(%[[VAL_3]], Release, 1)
// CHECK:       aie.next_bd ^bb1
// CHECK:     ^bb3:  // pred: ^bb0
// CHECK:       aie.end
