...
// CHECK:           %[[VAL_2:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "of2_buff_0"} : memref<4xi32> = dense<[0, 1, 2, 3]>
// CHECK:           %[[VAL_3:.*]] = aie.buffer(%[[VAL_1]]) {sym_name = "of2_buff_1"} : memref<4xi32> = dense<[4, 5, 6, 7]>
// CHECK:           %[[VAL_4:.*]] = aie.lock(%[[VAL_1]]) {init = 0 : i32, sym_name = "of2_prod_lock_0"}
// CHECK:           %[[VAL_5:.*]] = aie.lock(%[[VAL_1]]) {init = 2 : i32, sym_name = "of2_cons_lock_0"}
// CHECK:           %[[VAL_6:.*]] = aie.lock(%[[VAL_1]]) {init = 0 : i32, sym_name = "of2_prod_lock_1"}
// CHECK:           %[[VAL_7:.*]] = aie.lock(%[[VAL_1]]) {init = 2 : i32, sym_name = "of2_cons_lock_1"}
// CHECK:           %[[VAL_8:.*]] = aie.tile(1, 2)
// CHECK:           %[[VAL_9:.*]] = aie.buffer(%[[VAL_8]]) {sym_name = "of0_buff_0"} : memref<2xi32>
// CHECK:           %[[VAL_10:.*]] = aie.buffer(%[[VAL_8]]) {sym_name = "of0_buff_1"} : memref<2xi32>
// CHECK:           %[[VAL_11:.*]] = aie.lock(%[[VAL_8]]) {init = 2 : i32, sym_name = "of0_prod_lock_0"}
// CHECK:           %[[VAL_12:.*]] = aie.lock(%[[VAL_8]]) {init = 0 : i32, sym_name = "of0_cons_lock_0"}
// CHECK:           %[[VAL_13:.*]] = aie.tile(2, 3)
// CHECK:           %[[VAL_14:.*]] = aie.buffer(%[[VAL_13]]) {sym_name = "of1_buff_0"} : memref<2xi32>
// CHECK:           %[[VAL_15:.*]] = aie.buffer(%[[VAL_13]]) {sym_name = "of1_buff_1"} : memref<2xi32>
// CHECK:           %[[VAL_16:.*]] = aie.lock(%[[VAL_13]]) {init = 2 : i32, sym_name = "of1_prod_lock_0"}
// CHECK:           %[[VAL_17:.*]] = aie.lock(%[[VAL_13]]) {init = 0 : i32, sym_name = "of1_cons_lock_0"}
...
// CHECK:             aie.dma_bd(%[[VAL_2]] : memref<4xi32> offset = 0 len = 2)
// CHECK:             aie.use_lock(%[[VAL_5]], Release, %[[VAL_22]])
// CHECK:             aie.next_bd ^bb2
// CHECK:           ^bb2:
// CHECK:             aie.use_lock(%[[VAL_4]], AcquireGreaterEqual, %[[VAL_22]])
// CHECK:             aie.dma_bd(%[[VAL_3]] : memref<4xi32> offset = 0 len = 2)
// CHECK:             aie.use_lock(%[[VAL_5]], Release, %[[VAL_22]])
// CHECK:             aie.next_bd ^bb1
// CHECK:           ^bb3:
...
// CHECK:             aie.dma_bd(%[[VAL_2]] : memref<4xi32> offset = 2 len = 2)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_22]])
// CHECK:             aie.next_bd ^bb5
// CHECK:           ^bb5:
// CHECK:             aie.use_lock(%[[VAL_6]], AcquireGreaterEqual, %[[VAL_22]])
// CHECK:             aie.dma_bd(%[[VAL_3]] : memref<4xi32> offset = 2 len = 2)
// CHECK:             aie.use_lock(%[[VAL_7]], Release, %[[VAL_22]])
