...
// CHECK-LABEL: func @vecaddi_i8_i32_2(
// CHECK-SAME: %[[LHS:.*]]: vector<32xi8>,
// CHECK-SAME: %[[RHS:.*]]: vector<32xi32>)
func.func @vecaddi_i8_i32_2(%arg0 : vector<32xi8>, %arg1 : vector<32xi32>) -> vector<32xi32> {
  // CHECK:  %[[LUPS:.*]] = aievec.ups %[[LHS]] {shift = 0 : i8} : vector<32xi8>, vector<32xi32>
  // CHECK:  %[[RCAST:.*]] = aievec.cast %[[RHS]] {isResAcc = true} : vector<32xi32>, vector<32xi32>
  // CHECK:  %[[ADD:.*]] = aievec.add_elem %[[LUPS]], %[[RCAST]] : vector<32xi32>
  // CHECK:  %[[CAST:.*]] = aievec.cast %[[ADD]] {isResAcc = false} : vector<32xi32>, vector<32xi32>
  %1 = arith.extsi %arg0 : vector<32xi8> to vector<32xi32>
  %2 = arith.addi %1, %arg1 : vector<32xi32>
  // CHECK: return %[[CAST]] : vector<32xi32>
  return %2 : vector<32xi32>
}
...
// CHECK-LABEL: func @vecaddf_bf16_float(
// CHECK-SAME: %[[LHS:.*]]: vector<16xbf16>,
// CHECK-SAME: %[[RHS:.*]]: vector<16xbf16>)
func.func @vecaddf_bf16_float(%arg0: vector<16xbf16>, %arg1: vector<16xbf16>) -> vector<16xf32> {
  // CHECK:  %[[LUPS:.*]] = aievec.ups %[[LHS]] {shift = 0 : i8} : vector<16xbf16>, vector<16xf32>
  // CHECK:  %[[RUPS:.*]] = aievec.ups %[[RHS]] {shift = 0 : i8} : vector<16xbf16>, vector<16xf32>
  // CHECK:  %[[ADD:.*]] = aievec.add_elem %[[LUPS]], %[[RUPS]] : vector<16xf32>
  // CHECK:  %[[CAST:.*]] = aievec.cast %[[ADD]] {isResAcc = false} : vector<16xf32>, vector<16xf32>
  %0 = arith.extf %arg0 : vector<16xbf16> to vector<16xf32>
  %1 = arith.extf %arg1 : vector<16xbf16> to vector<16xf32>
  %2 = arith.addf %0, %1 : vector<16xf32>
  // CHECK: return %[[CAST]] : vector<16xf32>
  return %2 : vector<16xf32>
}

// CHECK-LABEL: func @vecaddf_bf16_float_2(
// CHECK-SAME: %[[LHS:.*]]: vector<16xbf16>,
// CHECK-SAME: %[[RHS:.*]]: vector<16xf32>)
func.func @vecaddf_bf16_float_2(%arg0: vector<16xbf16>, %arg1: vector<16xf32>) -> vector<16xf32> {
  // CHECK:  %[[LUPS:.*]] = aievec.ups %[[LHS]] {shift = 0 : i8} : vector<16xbf16>, vector<16xf32>
  // CHECK:  %[[RCAST:.*]] = aievec.cast %[[RHS]] {isResAcc = true} : vector<16xf32>, vector<16xf32>
  // CHECK:  %[[ADD:.*]]  = aievec.add_elem %[[LUPS]], %[[RCAST]] : vector<16xf32>
  // CHECK:  %[[CAST:.*]] = aievec.cast %[[ADD]] {isResAcc = false} : vector<16xf32>, vector<16xf32>
  %0 = arith.extf %arg0 : vector<16xbf16> to vector<16xf32>
  %1 = arith.addf %0, %arg1 : vector<16xf32>
  // CHECK: return %[[CAST]] : vector<16xf32>
...
// CHECK-LABEL: func @vecsubf_bf16_float(
// CHECK-SAME: %[[LHS:.*]]: vector<16xbf16>,
// CHECK-SAME: %[[RHS:.*]]: vector<16xbf16>)
func.func @vecsubf_bf16_float(%arg0: vector<16xbf16>, %arg1: vector<16xbf16>) -> vector<16xf32> {
  // CHECK:  %[[LUPS:.*]] = aievec.ups %[[LHS]] {shift = 0 : i8} : vector<16xbf16>, vector<16xf32>
  // CHECK:  %[[RUPS:.*]] = aievec.ups %[[RHS]] {shift = 0 : i8} : vector<16xbf16>, vector<16xf32>
  // CHECK:  %[[SUB:.*]] = aievec.sub_elem %[[LUPS]], %[[RUPS]] : vector<16xf32>
  // CHECK:  %[[CAST:.*]] = aievec.cast %[[ADD]] {isResAcc = false} : vector<16xf32>, vector<16xf32>
  %0 = arith.extf %arg0 : vector<16xbf16> to vector<16xf32>
  %1 = arith.extf %arg1 : vector<16xbf16> to vector<16xf32>
  %2 = arith.subf %0, %1 : vector<16xf32>
  // CHECK: return %[[CAST]] : vector<16xf32>
  return %2 : vector<16xf32>
}
