...
template<typename T>
struct Format;
template<>struct Format<double> { static constexpr const char *value = "%la";};
template<>struct Format<float> { static constexpr const char *value = "%a";};
template<>struct Format<uint64_t> { static constexpr const char *value = "%llu";};
template<>struct Format<int64_t> { static constexpr const char *value = "%lld";};
template<>struct Format<uint32_t> { static constexpr const char *value = "%u";};
template<>struct Format<int32_t> { static constexpr const char *value = "%d";};
template<>struct Format<uint16_t> { static constexpr const char *value = "%hu";};
template<>struct Format<int16_t> { static constexpr const char *value = "%hd";};
template<>struct Format<uint8_t> { static constexpr const char *value = "%hhu";};
template<>struct Format<int8_t> { static constexpr const char *value = "%hhd";};
...
template<typename T>
void
writeData(T const *data, unsigned numWords, std::string const & filename)
{
...
template<typename T>
void
readData(T *data, unsigned numWords, std::string const &filename)
{
...
template<typename T>
bool checkData(T const *data, T const *expected, unsigned num, unsigned absTol = 0, float relTol = 0.0, float absTolF = 0.0)
{
  return 0 == CheckData<T>(absTol,relTol,absTolF).check(data, expected, num);
}

template<>
bool checkData<cint16>(cint16 const *data, cint16 const *expected, unsigned num, unsigned absTol, float relTol, float absTolF)
{
  return checkData<int16_t>((int16_t const *)data, (int16_t const *)expected, 2*num, absTol, relTol, absTolF);
}

template<>
bool checkData<cint32>(cint32 const *data, cint32 const *expected, unsigned num, unsigned absTol, float relTol, float absTolF)
{
  return checkData<int32_t>((int32_t const *)data, (int32_t const *)expected, 2*num, absTol, relTol, absTolF);
}
...
template<typename T>
T
random_integer(unsigned int bits = 8 * sizeof(T))
{
    ...
    using TT = typename std::conditional<std::is_signed<T>::value, int64_t, uint64_t>::type;
    TT sval = (TT) val;
    if (bits < 64) {
        sval <<= (64 - bits);
        sval >>= (64 - bits);
...
float
random_float(int minExp = -126, int maxExp = 127, unsigned mantBits = 23)
{
  unsigned expRange;
...
bfloat16
random_bfloat16(int minExp = -126, int maxExp = 127, unsigned mantBits = 7)
{
  unsigned expRange;
  if (minExp >= -126 && maxExp <= 127 && minExp <= maxExp) {
    expRange = 1 + maxExp - minExp ;
