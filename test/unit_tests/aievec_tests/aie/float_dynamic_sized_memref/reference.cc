...
void reference(float *restrict img_in, float *restrict kernel_coeff,
  ...
  v8float *restrict ptr1 = ptr_img_buffer + 1 * stride / PARALLEL_FACTOR_FLOAT;
  v8float *restrict ptr2 = ptr_img_buffer + 2 * stride / PARALLEL_FACTOR_FLOAT;
  ...
  for (int i = 0; i < image_height; i++) {
    ...
    ptr0 = ptr_img_buffer + (i + 1) * stride / PARALLEL_FACTOR_FLOAT;
    ptr1 = ptr_img_buffer + (i + 2) * stride / PARALLEL_FACTOR_FLOAT;
    ptr2 = ptr_img_buffer + (i + 3) * stride / PARALLEL_FACTOR_FLOAT;
  }
}
