...
#define VEC PARALLEL_FACTOR_16b // Vectorization factor

int16_t in_data[MAX_IMAGE_SIZE];
int16_t out_data[MAX_IMAGE_SIZE];
int16_t kernelCoeff[MAX_KERNEL_SIZE];
...
int main() {
  ...
  conv2d(in_data, imageHeight + kernel_height - 1, stride, kernelCoeff,
  ...
  for (int i = 0; i < imageSize; i++) {
    ...
    if (out_data[i] != (int16_t)ref_output) {
