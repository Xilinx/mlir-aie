...
void reference(int16_t *restrict img_in, int16_t *restrict kernel_coeff,
  ...
  v16int16 *restrict ptr1 = ptr_img_buffer + 18;
  v16int16 *restrict ptr2 = ptr_img_buffer + 36;
  ...
  for (int i = 0; i < 16; i++)
    chess_prepare_for_pipelining chess_loop_range(16, 16) {
      for (int j = 0; j < 256; j += PARALLEL_FACTOR_16b) // 16x samples per loop
        chess_prepare_for_pipelining chess_loop_range(16, 16) {
          // 1st row
          data_buf = upd_w(data_buf, 0, *(ptr0++)); // r1:00++15|_________
          data_buf = upd_w(data_buf, 1, *(ptr0));   // r1:00..15|r1:16++31
          acc = mul16(data_buf, 0, 0x03020100, 0x07060504, 0x2110, kernel_vec0,
                      0, 0, 0, 1); // kernel 0, 1 (r1:00..16)
      ...
      ptr0 = ptr_img_buffer + (i + 1) * 18;
      ptr1 = ptr_img_buffer + (i + 2) * 18;
      ptr2 = ptr_img_buffer + (i + 3) * 18;
    }
}
