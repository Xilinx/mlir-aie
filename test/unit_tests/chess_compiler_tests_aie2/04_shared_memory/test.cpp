...
int main(int argc, char *argv[]) {
  ...
  if (mlir_aie_acquire_input_write_lock(_xaie, -1, LOCK_TIMEOUT)) {
    errors++;
  ...
  mlir_aie_release_input_read_lock(_xaie, 1, 0);
  printf("Waiting to acquire output lock for read ...\n");
  sleep(1);
  if (mlir_aie_acquire_output_read_lock(_xaie, -1, LOCK_TIMEOUT)) {
