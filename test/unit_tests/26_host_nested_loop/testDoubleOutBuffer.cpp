...
// Set virtual pointer used to configure
  ...
  for (int i = 1; i < 7; i++) {
    ...
    if (mlir_aie_acquire_of_3_lock_1(_xaie, 1, 10000) == XAIE_OK)
      printf("Acquired objFifo 3 lock 1 for read\n");
    else
      printf("ERROR: timed out on objFifo 3 lock 1 for read\n");
    ...
    if (mlir_aie_release_of_3_lock_1(_xaie, 0, 10000) == XAIE_OK)
      printf("Released objFifo 3 lock 1 for write\n");
    else
      printf("ERROR: timed out on objFifo 3 lock 1 for write\n");
