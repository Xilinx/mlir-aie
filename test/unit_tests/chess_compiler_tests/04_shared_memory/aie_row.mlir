...
// RUN: aiecc.py --aiesim --xchesscc --xbridge %VitisSysrootFlag% --host-target=%aieHostTargetTriplet% %s -I%host_runtime_lib%/test_lib/include %extraAieCcFlags% %S/test.cpp -o test.elf -L%host_runtime_lib%/test_lib/lib -ltest_lib
...
module @test4_row_shared_memory {
  ...
  %lock13_5 = AIE.lock(%tile13, 5) { sym_name = "inter_lock" } // interbuffer lock
