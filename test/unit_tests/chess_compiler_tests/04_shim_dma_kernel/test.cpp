    ...
    */
    ...
    for (int bd=0;bd<16;bd++) {
        ...
        for(int ofst=0;ofst<0x14;ofst+=0x4){
            u32 rb = mlir_aie_read32(_xaie, mlir_aie_get_tile_addr(_xaie, 7, 0) + 0x0001D000+(bd*0x14)+ofst);
            if (rb != 0){
                printf("Before : bd%d_%x control is %08X\n", bd, ofst, rb);
            }
            //mlir_aie_write32(TileInst[7][0].TileAddr + 0x0001D000+(bd*0x14)+ofst, 0x0);
        }
    }

    for (int dma=0;dma<4;dma++) {
        for(int ofst=0;ofst<0x8;ofst+=0x4){
            u32 rb = mlir_aie_read32(_xaie, mlir_aie_get_tile_addr(_xaie, 7, 0) + 0x0001D140+(dma*0x8)+ofst);
            if (rb != 0){
                printf("Before : dma%d_%x control is %08X\n", dma, ofst, rb);
            }
            //mlir_aie_write32(TileInst[7][0].TileAddr + 0x0001D140+(dma*0x8)+ofst, 0x0);
        }
    }
...
*/
    ...
    mlir_aie_check("After", mlir_aie_read_buffer_a_pong(_xaie, 3), 256+4, errors);
    mlir_aie_check("After", mlir_aie_read_buffer_b_ping(_xaie, 5), 20, errors);
    mlir_aie_check("After", mlir_aie_read_buffer_b_pong(_xaie, 5), (256+4)*5, errors);    
...
*/
    ...
    */
    for (int bd=0;bd<16;bd++) {
        ...
        // Overwrites the DMA_BDX_Control registers
        for(int ofst=0;ofst<0x14;ofst+=0x4){
            //u32 rb = mlir_aie_read32(TileInst[7][0].TileAddr + 0x0001D000+(bd*0x14)+ofst);
            //printf("Before : bd%d_%x control is %08X\n", bd, ofst, rb);
            mlir_aie_write32(_xaie, mlir_aie_get_tile_addr(_xaie, 7, 0) + 0x0001D000+(bd*0x14)+ofst, 0x0);
        }
    }

    for (int dma=0;dma<4;dma++) {
        for(int ofst=0;ofst<0x8;ofst+=0x4){
            //u32 rb = mlir_aie_read32(TileInst[7][0].TileAddr + 0x0001D140+(dma*0x8)+ofst);
            //printf("Before : dma%d_%x control is %08X\n", dma, ofst, rb);
            mlir_aie_write32(_xaie, mlir_aie_get_tile_addr(_xaie, 7, 0) + 0x0001D140+(dma*0x8)+ofst, 0x0);
        }
    }
