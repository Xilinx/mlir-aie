...
#define DMA_COUNT 512
  ...
  */
  ...
  if (mlir_aie_acquire_input_lock(_xaie, /*r/w*/ 0, 1000) == XAIE_OK)
    printf("Acquired input_lock (0) in tile (7,0). Done.\n");
  else {
    errors++;
    printf("Timed out while trying to acquire input_lock.\n");
  }
  if (mlir_aie_acquire_output_lock(_xaie, /*r/w*/ 0, 1000) == XAIE_OK)
