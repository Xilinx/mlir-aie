...
void write_stream(aie_libxaie_ctx_t *_xaie, ext_mem_model_t &ping_buf,
  ...
  for (int i = 0; i < BUF_SIZE; i++) {
    if (i % 2 == 0) {
      assert(XAIE_OK == mlir_aie_acquire_fifo0_prod_lock(_xaie, -1, 1000));
      ping[0] = i;
      ping[1] = 1000 + i;
      ping[2] = 2000 + i;
...
void read_all_into(aie_libxaie_ctx_t *_xaie, int *buf,
                   int (*read_buf_func)(aie_libxaie_ctx_t *, int)) {
  for (int i = 0; i < BUF_SIZE; i++) {
...
int main(int argc, char *argv[]) {
  ...
  ping = mlir_aie_mem_alloc(buf0, 3);
  pong = mlir_aie_mem_alloc(buf1, 3);
#ifdef __AIESIM__
  ...
  for (int i = 0; i < BUF_SIZE; i++) {
    printf("%04d, %04d, %04d  ", seen33[i], seen34[i], seen35[i]);
    if ((i + 1) % 3 == 0) {
      printf("\n");
    }
    if (seen33[i] != i || seen34[i] != 1000 + i || seen35[i] != 2000 + i) {
      printf("\nFAIL!\n");
      return 1;
    }
  }
