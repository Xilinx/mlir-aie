...
// RUN: %aiecc --no-xchesscc --no-xbridge %VitisSysrootFlag% --host-target=%aieHostTargetTriplet% %link_against_hsa% %s %test_lib_flags %extraAieCcFlags% -o test.elf -- %S/test.cpp
...
module @test04_shared_memory {
  aie.device(xcve2802) {
    ...
    %core23 = aie.core(%tile33) {
      ...
      aie.use_lock(%lock23_7, AcquireGreaterEqual, %c1_ul5) // acquire output for write
      ...
      aie.use_lock(%lock23_8, Release, %c1_ul7) // release output for read
      aie.end
    }
  }
}
