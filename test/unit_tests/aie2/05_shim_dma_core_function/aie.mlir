  ...
  aie.device(xcve2802) {
    ...
    %m73 = aie.mem(%t73) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%buf_a_ping : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
        aie.use_lock(%lock_a_read, Release, 1)
        aie.next_bd ^bd1
      ^bd1:
        aie.use_lock(%lock_a_write, AcquireGreaterEqual, 1)
        aie.dma_bd(%buf_a_pong : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
        aie.use_lock(%lock_a_read, Release, 1)
        aie.next_bd ^bd0
      ^bd2:
        aie.use_lock(%lock_b_read, AcquireGreaterEqual, 1)
        aie.dma_bd(%buf_b_ping : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
        aie.use_lock(%lock_b_write, Release, 1)
        aie.next_bd ^bd3
      ^bd3:
        aie.use_lock(%lock_b_read, AcquireGreaterEqual, 1)
        aie.dma_bd(%buf_b_pong : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
        aie.use_lock(%lock_b_write, Release, 1)
        aie.next_bd ^bd2
      ^end:
        aie.end
    ...
    %dma = aie.shim_dma(%t70) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%buffer_in : memref<32 x i32> offset = 0 len = 32 sizes = [] strides = [])
        aie.use_lock(%lock1_write, Release, 1)
        aie.next_bd ^bd0
      ^bd1:
        aie.use_lock(%lock2_write, AcquireGreaterEqual, 1)
        aie.dma_bd(%buffer_out : memref<32 x i32> offset = 0 len = 32 sizes = [] strides = [])
