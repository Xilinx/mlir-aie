  ...
  aie.device(xcve2802) {
    ...
    %m73 = aie.mem(%t73) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%buf_a_ping : memref<64xi32> offset = 0 len = 64 sizes = [] strides = [])
        aie.use_lock(%lock_a_ping, "Release", 1)
        aie.next_bd ^bd1
      ^bd1:
        aie.use_lock(%lock_a_pong, "Acquire", 0)
        aie.dma_bd(%buf_a_pong : memref<64xi32> offset = 0 len = 64 sizes = [] strides = [])
        aie.use_lock(%lock_a_pong, "Release", 1)
        aie.next_bd ^bd0
      ^bd2:
        aie.use_lock(%lock_b_ping, "Acquire", 1)
        aie.dma_bd(%buf_b_ping : memref<64xi32> offset = 0 len = 64 sizes = [] strides = [])
        aie.use_lock(%lock_b_ping, "Release", 0)
        aie.next_bd ^bd3
      ^bd3:
        aie.use_lock(%lock_b_pong, "Acquire", 1)
        aie.dma_bd(%buf_b_pong : memref<64xi32> offset = 0 len = 64 sizes = [] strides = [])
    ...
    %dma = aie.shim_dma(%t70) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%buffer_in : memref<512 x i32> offset = 0 len = 512 sizes = [] strides = [])
        aie.use_lock(%lock1, Release, 0)
        aie.next_bd ^bd0
      ^bd1:
        aie.use_lock(%lock2, Acquire, 1)
        aie.dma_bd(%buffer_out : memref<512 x i32> offset = 0 len = 512 sizes = [] strides = [])
