...
// Unified mode lowers each device once, then carves that module into one
// module per core -- so the object stage stays as wide as the core count. Pin
// both halves: the lowering happens, and each core still gets its own object.
// Merging the cores into one shared object would serialize a stage that is
// otherwise fully parallel.
// CHECK: ({{[0-9]+}}/{{[0-9]+}}) lowered_{{.*}}.mlir
