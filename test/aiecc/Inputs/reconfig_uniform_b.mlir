...
//
// Sibling of reconfig_uniform_a.mlir: the SAME default `sequence` entry name,
// a distinct config device (@cfg_b). The union must rename the colliding host
// sequences to configs_1/configs_2 (block order = design order).
module {
  aie.device(npu2) @main {
    aie.runtime_sequence @sequence(%argout_b : memref<4xi32>) {
