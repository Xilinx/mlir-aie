...
// RUN: aiecc --no-xchesscc --no-xbridge --get-ctrlpkt --keep-loc --dump-intermediates --tmpdir=%t_ctrlpkt_tmp --verbose --ctrlpkt-name=%t_ctrlpkt.bin --ctrlpkt-dma-seq-name=%t_dma_seq.bin %t_overlay.mlir 2>&1 | FileCheck %s
...
// CHECK: ({{[0-9]+}}/{{[0-9]+}}) ctrlpkt_lowered_{{.*}}.mlir
// CHECK-DAG: wrote edge '{{.*}}ctrlpkt.bin'
// CHECK-DAG: wrote edge '{{.*}}dma_seq.bin'
