//===- cpp_unified_parallel_link.mlir --------------------------*- MLIR -*-===//
//
...
// Test that --unified -j2 parallelizes the per-core link step and produces
// byte-identical ELFs to the serial (-j1) path.

// RUN: aiecc --no-xchesscc --no-xbridge --verbose --unified -j 2 %s 2>&1 | FileCheck %s

// CHECK: Linking {{[0-9]+}} core(s) in parallel (2 threads)
// CHECK-DAG: Generated ELF: {{.*}}main_core_0_2{{.*}}
// CHECK-DAG: Generated ELF: {{.*}}main_core_1_2{{.*}}

// Byte-identical check: parallel link must produce the same ELFs as serial.
// RUN: aiecc --no-xchesscc --no-xbridge --unified -j 1 --tmpdir=%t.ser %s
// RUN: aiecc --no-xchesscc --no-xbridge --unified -j 2 --tmpdir=%t.par %s
// RUN: diff %t.ser/main_core_0_2.elf %t.par/main_core_0_2.elf
// RUN: diff %t.ser/main_core_1_2.elf %t.par/main_core_1_2.elf
