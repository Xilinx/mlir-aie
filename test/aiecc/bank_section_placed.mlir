...
//===----------------------------------------------------------------------===//
//
// A static pinned to a bank lands in that bank.
//
// The linker script gives each bank its own region and routes .aie.bank<N>
// there, so the addresses below fall in bank 1 (0x74000) and bank 2 (0x78000)
// of this tile's local memory at 0x70000.
...
// CHECK-DAG: 00074{{[0-9a-f]+}} {{.*}} tbl_ab
// CHECK-DAG: 00078{{[0-9a-f]+}} {{.*}} tbl_cd
module {
  aie.device(npu2) {
