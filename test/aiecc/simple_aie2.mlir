...
// RUN: %aiecc --get-input-with-symbols -nv %VitisSysrootFlag% --host-target=%aieHostTargetTriplet% %s -I%aie_runtime_lib%/test_lib/include %extraAieCcFlags% -L%aie_runtime_lib%/test_lib/lib -ltest_lib -o simple_aie2.elf -- %S/test.cpp 2>&1 | FileCheck %s --check-prefix=NOCOMPILE
...
// RUN: %aiecc --no-unified --get-input-with-symbols -nv %VitisSysrootFlag% --host-target=%aieHostTargetTriplet% %s -I%aie_runtime_lib%/test_lib/include %extraAieCcFlags% -L%aie_runtime_lib%/test_lib/lib -ltest_lib -o simple_aie2.elf -- %S/test.cpp 2>&1 | FileCheck %s --check-prefix=NOCOMPILE
