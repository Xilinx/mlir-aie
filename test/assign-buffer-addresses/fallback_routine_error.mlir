...
// CHECK:   error: Failed to allocate buffer: "e" with size: 32 bytes.
// CHECK:      %b1 = aie.buffer(%tile12) { sym_name = "e" } : memref<16xi16> // CHECK:    32 bytes
// CHECK:            ^
...
// CHECK:                    d       : 0x4000-0x5FFF         (8192 bytes)
// CHECK:            bank : 3        0x6000-0x7FFF
// CHECK:                    a       : 0x6000-0x6FFF         (4096 bytes)
...
// CHECK:            act_3_4_buff_2  : 0x7800-0x7BFF         (1024 bytes)
// CHECK:            act_3_4_buff_3  : 0x7C00-0x7FFF         (1024 bytes)
// CHECK:            e       : 0x8000-0x801F         (32 bytes)
...
module @test {
 aie.device(xcvc1902) {
  ...
  %4 = aie.buffer(%tile12) { sym_name = "d" } : memref<2048xi32>  //8192 bytes
  %b1 = aie.buffer(%tile12) { sym_name = "e" } : memref<16xi16>   //32 bytes
  %tile13 = aie.tile(1, 3)
