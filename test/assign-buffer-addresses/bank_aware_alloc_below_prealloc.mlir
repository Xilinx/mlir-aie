...
// Free space *below* a buffer with an explicit address must be usable. The
// allocator used to keep a single "next free address" watermark per bank,
// which a fixed-address buffer pushed past itself, permanently stranding
// everything underneath it.
//
// Note the hardware imposes no natural-size or bank alignment requirement on a
// buffer, so nothing here is alignment-driven: buffers only round up to the
...
// Same defect on a memtile, reached through the mem_bank path instead: bank 0
