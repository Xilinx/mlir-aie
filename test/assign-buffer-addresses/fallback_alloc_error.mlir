...
module @test4 {
  aie.device(npu1) {
    ...
    %tile34 = aie.tile(3, 4)
    // expected-error @below {{'aie.buffer' op address 0x4000 lies in bank 1, but mem_bank requests bank 2}}
    %buf0 = aie.buffer(%tile34) { sym_name = "a", address = 16384 : i32, mem_bank = 2 : i32 } : memref<16xi32>
