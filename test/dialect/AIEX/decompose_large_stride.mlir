...
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[0, 0, 0, 0][1, 1, 64, 512][2, 0, 8192, 1])
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[2097152, 0, 0, 0][1, 1, 64, 512][2, 0, 8192, 1])
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[4194304, 0, 0, 0][1, 1, 64, 512][2, 0, 8192, 1])
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[6291456, 0, 0, 0][1, 1, 64, 512][2, 0, 8192, 1])
...
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[0, 0, 0, 0][64, 1, 1, 2][3, 0, 0, 2])
// CHECK: aiex.npu.dma_memcpy_nd(%{{.*}}[64, 0, 0, 0][1, 1, 1, 2][3, 0, 0, 2])
// CHECK-NOT: aiex.npu.dma_memcpy_nd
module {
  aie.device(npu2_1col) {
    %t = aie.tile(0, 0)
    ...
    aie.runtime_sequence @singleton_wrap_tail(%B: memref<256xi32>) {
      aiex.npu.dma_memcpy_nd(%B[0, 0, 0, 0][65, 1, 1, 2][3, 0, 0, 2]) {id = 0 : i64, metadata = @b} : memref<256xi32>
    }
  }
}
