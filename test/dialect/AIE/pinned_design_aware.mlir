...
// RUN: aie-opt %s --aie-pin-control-overlay --aie-create-pathfinder-flows | FileCheck %s --check-prefix=BLIND
// RUN: aie-opt %s --aie-pin-control-overlay="design-aware=true" --aie-create-pathfinder-flows | FileCheck %s --check-prefix=AWARE

// Design-aware control pinning (Layer 1, eager avoidance). By default control
// pins BLIND (routed on the data-free @ctrl_pkt_overlay with no knowledge of
// config data). With design-aware=true the pinning first routes each config's
