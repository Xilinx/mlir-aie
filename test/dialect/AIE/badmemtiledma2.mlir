    ...
    %t12 = aie.tile(1, 2)
      ...
      %m12 = aie.memtile_dma(%t12) {
        ...
        ^bd1:
          ...
          aie.next_bd ^bd1
        ^bd2:
          aie.dma_bd(%buf_2 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
          aie.use_lock(%lock_2, Release, 1)
          aie.next_bd ^bd2
        ^bd3:
          aie.dma_bd(%buf_3 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
          aie.use_lock(%lock_3, Release, 1)
          aie.next_bd ^bd3
        ^bd4:
          aie.dma_bd(%buf_4 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
          aie.use_lock(%lock_4, Release, 1)
          aie.next_bd ^bd4
        ^bd5:
          aie.dma_bd(%buf_5 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
          aie.use_lock(%lock_5, Release, 1)
          aie.next_bd ^bd5
        ^bd6:
          aie.dma_bd(%buf_6 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
