...
module @test {
    ...
    %m73 = aie.mem(%t73) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%buf_e : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
        aie.use_lock(%lock_e, Release, 1)
        aie.next_bd ^bd1
      ^bd1:
        aie.dma_bd(%buf_l : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
        aie.use_lock(%lock_l, Release, 1)
        aie.next_bd ^end
      ^bd2:
        aie.dma_bd(%buf_n : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
        aie.use_lock(%lock_n, Release, 1)
        aie.next_bd ^bd3
      ^bd3:
        aie.dma_bd(%buf_s : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
