...
module @test {
  aie.device(npu1) {
    ...
    %m00 = aie.shim_dma(%t00) {
      ...
      ^bd0:
        aie.dma_bd(%buf_e : memref<256xi32> offset = 1 len = 256 sizes = [] strides = [])
        aie.use_lock(%lock_e, Release, 1)
        aie.next_bd ^bd0
      ^bd1:
        aie.dma_bd(%buf_l : memref<256xi32> offset = 1 len = 256 sizes = [] strides = [])
        aie.use_lock(%lock_l, Release, 1)
        aie.next_bd ^bd1
      ^bd2:
        aie.dma_bd(%buf_n : memref<256xi32> offset = 1 len = 256 sizes = [] strides = [])
