...
// CHECK:             aie.dma_bd(%[[VAL_7]] : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_6]], Release, 0)
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_13:.*]] = aie.mem(%[[VAL_3]]) {
// CHECK:             aie.use_lock(%[[VAL_4]], Acquire, 1)
// CHECK:             aie.dma_bd(%[[VAL_8]] : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_4]], Release, 0)
// CHECK:             aie.end
// CHECK:           }
// CHECK:           %[[VAL_15:.*]] = aie.mem(%[[VAL_0]]) {
// CHECK:             aie.use_lock(%[[VAL_2]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_9]] : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
// CHECK:             aie.use_lock(%[[VAL_2]], Release, 1)
// CHECK:             aie.use_lock(%[[VAL_1]], Acquire, 0)
// CHECK:             aie.dma_bd(%[[VAL_10]] : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
 ...
 aie.device(xcvc1902) {
  ...
  %m33 = aie.mem(%t33) {
    ...
    ^bd0:
      ...
      aie.dma_bd(%buf33 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
  ...
  %m44 = aie.mem(%t44) {
    ...
    ^bd0:
      ...
      aie.dma_bd(%buf44 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
  ...
  %m55 = aie.mem(%t55) {
    ...
    ^bd0:
      aiex.useToken @token0(Acquire, 1)
      aie.dma_bd(%buf55_0 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aiex.useToken @token0(Release, 2)
      aie.next_bd ^end
    ^bd1:
      aiex.useToken @token1(Acquire, 1)
      aie.dma_bd(%buf55_1 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aiex.useToken @token1(Release, 2)
      aie.next_bd ^end
    ^end:
      aie.end
