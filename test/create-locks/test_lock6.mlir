...
// CHECK:             aie.dma_bd(%[[VAL_7]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
...
// CHECK:             aie.dma_bd(%[[VAL_10]] : memref<256xi32> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
...
module @test_lock6 {
 aie.device(xcvc1902) {
  ...
  %m55 = aie.mem(%t55) {
    ...
    ^bd0:
      aiex.useToken @token0(Acquire, 1)
      aie.dma_bd(%buf55_0 : memref<256xi32> offset = %c0_i32 len = %c256_i32 sizes = [] strides = [])
      aiex.useToken @token0(Release, 2)
      aie.next_bd ^end
    ^bd1:
      aiex.useToken @token1(Acquire, 1)
      aie.dma_bd(%buf55_1 : memref<256xi32> offset = %c0_i32 len = %c256_i32 sizes = [] strides = [])
      aiex.useToken @token1(Release, 2)
      aie.next_bd ^end
    ^end:
      aie.end
