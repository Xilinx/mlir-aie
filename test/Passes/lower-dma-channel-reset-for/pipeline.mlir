...
// RUN:   --aie-lower-set-lock --aie-lower-dma-channel-reset %s | FileCheck %s

// Consumer core tile (0,2), S2MM channel 0. Reset pulse on CTRL 0x1DE00 =
// 122368; START_QUEUE re-push on 0x1DE04 = 122372. Only the two CORE consumer
// locks are re-armed; the shim producer's locks are left to the host.
...
// CHECK: aiex.npu.write32(%{{.*}}, %{{.*}}) {column = 0 : i32, row = 2 : i32}
// CHECK: aiex.npu.write32(%{{.*}}, %{{.*}}) {column = 0 : i32, row = 2 : i32}
// No high-level re-arm op and no binding survive to the end.
