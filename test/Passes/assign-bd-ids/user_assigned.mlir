  ...
  aie.device(npu1) {
    ...
    %mem_0_2 = aie.mem(%tile_0_2) {
      ...
      %player_a = aie.dma(S2MM, 0) {sym_name = "player_a"} [{
        ...
        aie.dma_bd(%double_buffer : memref<32xi32> offset = %c0_i32 sizes = [] strides = []) {bd_id = 0 : i32}
        aie.use_lock(%lock_Y, Release, 0)
      }, {
        aie.use_lock(%lock_X, Acquire, 1)
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 1 : i32}
      ...
      %player_b = aie.dma(S2MM, 1) {sym_name = "player_b"} [{
        ...
        aie.use_lock(%lock_X, Acquire, 1)
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 4 : i32}
        aie.use_lock(%lock_X, Release, -1)
      }, {
        aie.use_lock(%lock_Y, Acquire) {acq_en = false}
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = [])
    ...
    %memtile_dma_0_1 = aie.memtile_dma(%tile_0_1) {
      ...
      %2 = aie.dma(S2MM, 1) {loop = false, repeat_count = 10 : i32} [{
        ...
        aie.dma_bd(%buffer_0_1 : memref<32xi32> sizes = [] strides = []) {bd_id = 24 : i32}
  ...
  aie.device(xcve2302) {
    ...
    %m01 = aie.memtile_dma(%t01) {
      ...
      ^bd1:
        ...
        aie.dma_bd(%buf01_0 : memref<16xi32> offset = %c0_i32 len = %c16_i32 sizes = [] strides = []) {bd_id = 24 : i32}
      ...
      ^bd3:
        ...
        aie.dma_bd(%buf01_1 : memref<16xi32> offset = %c0_i32 len = %c16_i32 sizes = [] strides = []) {bd_id = 1 : i32}
...
module {
  aie.device(npu1) {
    ...
    %mem_0_2 = aie.mem(%tile_0_2) {
      ...
      %player_a = aie.dma(S2MM, 0) {sym_name = "player_a"} [{
        aie.use_lock(%lock_Y, Acquire, 0)
        aie.dma_bd(%double_buffer : memref<32xi32> offset = %c0_i32 sizes = [] strides = []) {bd_id = 5 : i32}
        aie.use_lock(%lock_Y, Release, 0)
      }, {
        aie.use_lock(%lock_X, Acquire, 1)
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 4 : i32}
        ...
        aie.use_lock(%lock_Y, Acquire) {acq_en = false}
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 3 : i32}
        aie.use_lock(%lock_Y, Release, 1)
      }]
      %player_b = aie.dma(S2MM, 1) {sym_name = "player_b"} [{
        aie.use_lock(%lock_Y, Acquire, 1)
        aie.dma_bd(%double_buffer : memref<32xi32> offset = %c0_i32 sizes = [] strides = []) {bd_id = 2 : i32}
        aie.use_lock(%lock_Y, Release, 0)
      }, {
        aie.use_lock(%lock_X, Acquire, 1)
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 1 : i32}
        ...
        aie.use_lock(%lock_Y, Acquire) {acq_en = false}
        aie.dma_bd(%double_buffer : memref<32xi32> sizes = [] strides = []) {bd_id = 0 : i32}
        aie.use_lock(%lock_Y, Release, -1)
      }]
      aie.end
    }
