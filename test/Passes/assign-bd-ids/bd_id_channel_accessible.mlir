...
// Even channel with a bd_id from the odd range (dma_start block form).
module {
  aie.device(npu2) {
    %t = aie.tile(0, 1)
    ...
    aie.memtile_dma(%t) {
      ...
      ^bd0:
        ...
        aie.dma_bd(%b : memref<8xi32> offset = 0 len = 4) { bd_id = 24 : i32 }
        aie.next_bd ^end
      ^end:
        aie.end
    }
...
// Odd channel with a bd_id from the even range (aie.dma block form).
module {
  aie.device(npu2) {
    %t = aie.tile(0, 1)
    %b = aie.buffer(%t) : memref<8xi32>
    ...
    aie.memtile_dma(%t) {
      %0 = aie.dma(S2MM, 1) [{
        ...
        aie.dma_bd(%b : memref<8xi32>) { bd_id = 0 : i32 }
