...
// CHECK:   aie.objectfifo.pool @in_cons_pool(%[[T21]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 16>, #aie.objectfifo_segment<offset = 16, size = 32>]} : memref<48xi32>
// CHECK:   aie.objectfifo.dma_endpoint @in_prod_dma(%[[T20]]) {fifoName = "in"}
...
// CHECK:   aie.objectfifo.pool @out0_cons_pool(%[[T22]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 16>]} : memref<16xi32>
...
// CHECK:   aie.objectfifo.pool @out1_cons_pool(%[[T23]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 32>]} : memref<32xi32>
