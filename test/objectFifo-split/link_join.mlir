...
// CHECK:   aie.objectfifo.pool @in0_pool(%[[T22]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 16>]} : memref<16xi32>
...
// CHECK:   aie.objectfifo.pool @in1_pool(%[[T23]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 32>]} : memref<32xi32>
...
// CHECK:   aie.objectfifo.pool @out_pool(%[[T21]]) {depth = 2 : i32, segments = [#aie.objectfifo_segment<offset = 0, size = 16>, #aie.objectfifo_segment<offset = 16, size = 32>]} : memref<48xi32>
// CHECK:   aie.objectfifo.dma_endpoint @out_prod_dma(%[[T21]]) drains @out_pool {fifoName = "out"}
// CHECK:   aie.objectfifo.dma_endpoint @out_cons_dma(%[[T20]]) {fifoName = "out"}
