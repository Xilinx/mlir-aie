...
// CHECK:   aie.objectfifo.dangling_endpoint @in_prod_dma(%[[T00]]) MM2S DMA {fifoName = "in"}
...
module @join_on_core {
  aie.device(npu1) {
    ...
    %core04 = aie.core(%tile04) {
      %e = aie.objectfifo.acquire @in0(Produce) : memref<16xi32>
      aie.objectfifo.release @in0(Produce) [1]
      aie.end
    }
    %core05 = aie.core(%tile05) {
      %e = aie.objectfifo.acquire @in1(Produce) : memref<16xi32>
...
// CHECK:   aie.objectfifo.dangling_endpoint @out_cons_dma(%[[U00]]) S2MM DMA {fifoName = "out"}
// CHECK-NOT: aie.objectfifo.link
