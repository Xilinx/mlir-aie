...
main(int argc, char *argv[])
{
    ...
    mlir_aie_check_float("Before release lock:", mlir_aie_read_buffer_debuf(_xaie, 5), 0.0f,
                   errors);
...
//        printf("Waiting to acquire lock ...\n");
    ...
    for(int i = 1; i < framerows; i++) {
        ...
        computetime += (double)(computedone[i].tv_nsec - copydone[i-1].tv_nsec)/1000000000.0f;
        copytime += (copydone[i].tv_sec-computedone[i].tv_sec);
        copytime += (double)(copydone[i].tv_nsec - computedone[i].tv_nsec)/1000000000.0f;
    }
