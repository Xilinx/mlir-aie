//===- arbiter_gemma_join_collision.mlir -----------------------*- MLIR -*-===//
//
...
// Reduced gemma3 decode hang. Cores c0..c5 each stream packets (ids 0..2) into
// their own memtile (0,1) S2MM channel; memtile MM2S 0 joins all six channels,
// taking one buffer from each in turn, and forwards them to core (0,3). That
// is seven packet master ports at (0,1) for six arbiters, so two of them share
// one: when one flow runs ahead and its S2MM fills, its packet holds the
// arbiter until tlast while the join waits on the other flow, which can never
// pass.
//
// The router circuit-switches the hop out to core (0,3), which alone uses
// its ports, leaving six masters on six arbiters. Without that, every pairing
// is a hazard whatever the routing, and the router fails, saying what it
// could not know: how much each core sends.

// RUN: aie-opt --aie-create-pathfinder-flows %s 2>&1 | FileCheck %s
// RUN: not aie-opt --aie-create-pathfinder-flows="circuit-switch-hops=false" %s 2>&1 | FileCheck %s --check-prefix=ROUTER

// CHECK-NOT:     warning
// CHECK-LABEL: aie.switchbox(%mem_tile_0_1)
// CHECK-NEXT:    aie.connect<DMA : 0, North : 1>
...
// ROUTER: error: Unable to find a legal routing: at tile (0, 1), no two of
// ROUTER-SAME: packet flow (0, 1) DMA:0 -> (0, 3) DMA:1 (id 4) can share an arbiter, and each takes one there whatever the routing, but the switchbox has 6 free. For example,
// ROUTER-SAME: The volume packet flow {{.*}} carries is unknown, so it is assumed to overrun its receiver.
...
module {
  aie.device(npu2) {
    ...
    aie.packet_flow(0) { aie.packet_source<%t5, DMA : 0> aie.packet_dest<%m, DMA : 5> }
    aie.packet_flow(1) { aie.packet_source<%t5, DMA : 0> aie.packet_dest<%m, DMA : 5> }
    aie.packet_flow(2) { aie.packet_source<%t5, DMA : 0> aie.packet_dest<%m, DMA : 5> }
    ...
    %p5 = aie.lock(%t5, 0) {init = 1 : i32, sym_name = "p5"}
    ...
    ^s0:
      ...
      aie.dma_bd(%b5 : memref<256xi32> offset = 0 len = 256) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 0>}
      aie.use_lock(%p5, Release, %one)
      aie.next_bd ^s1
    ^s1:
      aie.use_lock(%q5, AcquireGreaterEqual, %one)
      aie.dma_bd(%b5 : memref<256xi32> offset = 0 len = 256) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 1>}
      aie.use_lock(%p5, Release, %one)
      aie.next_bd ^s2
    ^s2:
      aie.use_lock(%q5, AcquireGreaterEqual, %one)
      aie.dma_bd(%b5 : memref<256xi32> offset = 0 len = 256) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 2>}
      aie.use_lock(%p5, Release, %one)
      aie.next_bd ^s0
    ^end:
      aie.end
    ...
    ^j4:
      ...
      aie.next_bd ^j5
    ^j5:
      aie.use_lock(%mq5, AcquireGreaterEqual, %one)
      aie.dma_bd(%mb5 : memref<256xi32> offset = 0 len = 256) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 6>}
      aie.use_lock(%mp5, Release, %one)
      aie.next_bd ^j0
    ^end:
      aie.end
    }
