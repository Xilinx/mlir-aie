...
//
// Six same-id packet sources merge into a single mem_tile S2MM DMA channel.
// Same-id flows cannot share a switch channel, so each source needs its own path
// to the destination, and this mem_tile's switchbox has only five free incoming
// channels to offer. The congestion-aware pathfinder cannot find a complete
// assignment for this placement. The design used to compile "successfully" while
// silently dropping one source's connection on device. The pass must instead
// fail loudly.

module {
  // expected-error @+1 {{Unable to find a legal routing}}
  aie.device(npu2) {
