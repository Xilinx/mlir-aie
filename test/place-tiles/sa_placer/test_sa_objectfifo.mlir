...
// Broadcast with 5 consumers: triggers isMulticast (>4 consumers) for 2x HPWL
// CHECK-LABEL: @fifo_broadcast_multicast
module @fifo_broadcast_multicast {
  aie.device(npu2) {
    %mem = aie.logical_tile<MemTile>(?, ?)
    %c0 = aie.logical_tile<CoreTile>(?, ?)
    %c1 = aie.logical_tile<CoreTile>(?, ?)
    ...
    // 5 consumers triggers isMulticast = true (>4)
    // CHECK: aie.objectfifo @multicast(
    aie.objectfifo @multicast(%mem, {%c0, %c1, %c2, %c3, %c4}, 2 : i32) : !aie.objectfifo<memref<256xi32>>
