...
// Cascade pair deterministic: same seed, same placement
// RUN: aie-opt --aie-place-tiles='placer=sa_placer sa-seed=99' %s > %t.cascade1.mlir 2>/dev/null
// RUN: aie-opt --aie-place-tiles='placer=sa_placer sa-seed=99' %s > %t.cascade2.mlir 2>/dev/null

// CHECK-LABEL: @cascade_deterministic
module @cascade_deterministic {
  aie.device(npu2) {
    %src = aie.logical_tile<CoreTile>(?, ?)
    %dst = aie.logical_tile<CoreTile>(?, ?)
    ...
    aie.objectfifo @p1(%c1, {%src}, 2 : i32) : !aie.objectfifo<memref<256xi32>>
    aie.objectfifo @p2(%dst, {%c2}, 2 : i32) : !aie.objectfifo<memref<256xi32>>
