...
module {
  aie.device(NPUDEVICE) {
    ...
    ^bb1:  // 2 preds: ^bb0, ^bb2
      ...
      aie.dma_bd(%in1_cons_buff_0 : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%in1_cons_cons_lock, Release, 1)
      aie.next_bd ^bb2
    ^bb2:  // pred: ^bb1
      aie.use_lock(%in1_cons_prod_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%in1_cons_buff_1 : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%in1_cons_cons_lock, Release, 1)
      aie.next_bd ^bb1
    ^bb3:  // pred: ^bb0
    ...
    ^bb7:  // 2 preds: ^bb6, ^bb8
      ...
      aie.dma_bd(%out_buff_0 : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%out_prod_lock, Release, 1)
      aie.next_bd ^bb8
    ^bb8:  // pred: ^bb7
      aie.use_lock(%out_cons_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%out_buff_1 : memref<16xi32> offset = 0 len = 16 sizes = [] strides = [])
      aie.use_lock(%out_prod_lock, Release, 1)
      aie.next_bd ^bb7
    ^bb9:  // pred: ^bb6
      aie.end
