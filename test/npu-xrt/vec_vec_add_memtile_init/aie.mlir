...
module {
  aie.device(NPUDEVICE) {
    ...
    ^bb4:  // 2 preds: ^bb3, ^bb5
      ...
      aie.dma_bd(%in2_mem_cons_buff_0 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aie.use_lock(%in2_mem_cons_cons_lock, Release, 1)
      aie.next_bd ^bb5
    ^bb5:  // pred: ^bb4
      aie.use_lock(%in2_mem_cons_prod_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%in2_mem_cons_buff_1 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aie.use_lock(%in2_mem_cons_cons_lock, Release, 1)
      aie.next_bd ^bb4
    ^bb6:  // pred: ^bb3
    ...
    ^bb1:
      ...
      aie.dma_bd(%in2_mem_buff_0 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aie.use_lock(%in2_mem_prod_lock, Release, 1)
      aie.next_bd ^bb2
    ^bb2:
      aie.use_lock(%in2_mem_cons_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%in2_mem_buff_1 : memref<256xi32> offset = 0 len = 256 sizes = [] strides = [])
      aie.use_lock(%in2_mem_prod_lock, Release, 1)
      aie.next_bd ^bb1
    ^bb3:
      aie.end
