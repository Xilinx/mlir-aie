  ...
  aie.device(NPUDEVICE) {
    ...
    %mem_0_2 = aie.mem(%tile_0_2) {
      %0 = aie.dma(S2MM, 0) [{
        ...
        aie.dma_bd(%objFifo_core02_cons_buff_0 : memref<64x64xi8> sizes = [] strides = [])
        aie.use_lock(%objFifo_core02_cons_cons_lock, Release, 1)
      }, {
        aie.use_lock(%objFifo_core02_cons_prod_lock, AcquireGreaterEqual, 1)
        aie.dma_bd(%objFifo_core02_cons_buff_1 : memref<64x64xi8> sizes = [] strides = [])
        aie.use_lock(%objFifo_core02_cons_cons_lock, Release, 1)
      }]
      %1 = aie.dma(MM2S, 0) [{
        aie.use_lock(%objFifo_core02_cons_lock, AcquireGreaterEqual, 1)
        aie.dma_bd(%objFifo_core02_buff_0 : memref<64x64xi8> sizes = [] strides = []) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 1>}
