...
module {
  aie.device(npu1_1col) {
    ...
    ^bb1:  // 2 preds: ^bb0, ^bb2
      ...
      aie.dma_bd(%in_cons_buff_0 : memref<1024xi32> offset = 0 len = 1024 sizes = [] strides = [])
      aie.use_lock(%in_cons_cons_lock, Release, 1)
      aie.next_bd ^bb2
    ^bb2:  // pred: ^bb1
      aie.use_lock(%in_cons_prod_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%in_cons_buff_1 : memref<1024xi32> offset = 0 len = 1024 sizes = [] strides = [])
      aie.use_lock(%in_cons_cons_lock, Release, 1)
      aie.next_bd ^bb1
    ^bb3:  // pred: ^bb0
    ...
    ^bb4:  // 2 preds: ^bb3, ^bb5
      ...
      aie.dma_bd(%out_buff_0 : memref<1024xi32> offset = 0 len = 1024 sizes = [] strides = [])
      aie.use_lock(%out_prod_lock, Release, 1)
      aie.next_bd ^bb5
    ^bb5:  // pred: ^bb4
      aie.use_lock(%out_cons_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%out_buff_1 : memref<1024xi32> offset = 0 len = 1024 sizes = [] strides = [])
      aie.use_lock(%out_prod_lock, Release, 1)
      aie.next_bd ^bb4
    ^bb6:  // pred: ^bb3
      aie.end
