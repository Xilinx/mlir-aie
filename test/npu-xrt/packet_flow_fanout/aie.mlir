  ...
  aie.device(NPUDEVICE) {
    ...
    %memtile_dma_0_1 = aie.memtile_dma(%tile_0_1) {
      ...
      %6 = aie.dma(MM2S, 3) [{
        ...
        aie.dma_bd(%objFifo_out1_buff_0 : memref<64x64xi8> sizes = [] strides = []) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 6>}
        aie.use_lock(%objFifo_out1_prod_lock, Release, 1)
      }]
      %7 = aie.dma(S2MM, 3) [{
        aie.use_lock(%objFifo_out1_prod_lock, AcquireGreaterEqual, 1)
        aie.dma_bd(%objFifo_out1_buff_0 : memref<64x64xi8> sizes = [] strides = [])
    ...
    %mem_0_3 = aie.mem(%tile_0_3) {
      %0 = aie.dma(S2MM, 0) [{
        ...
        aie.dma_bd(%objFifo_core03_cons_buff_0 : memref<64x64xi8> sizes = [] strides = [])
        aie.use_lock(%objFifo_core03_cons_cons_lock, Release, 1)
      }]
      %1 = aie.dma(MM2S, 0) [{
        aie.use_lock(%objFifo_core03_cons_lock, AcquireGreaterEqual, 1)
        aie.dma_bd(%objFifo_core03_buff_0 : memref<64x64xi8> sizes = [] strides = []) {packet = #aie.packet_info<pkt_type = 0, pkt_id = 5>}
