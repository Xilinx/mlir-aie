  ...
  aie.device(NPUDEVICE) @aie_add_1 {
    ...
    aie.core(%c0) {
      ...
      scf.for %s = %lb to %ub step %st {
        %e0 = aie.objectfifo.acquire @inc0(Consume) : memref<8xi32>
        %e1 = aie.objectfifo.acquire @outc0(Produce) : memref<8xi32>
    ...
    aie.core(%c1) {
      ...
      %val = arith.constant 1 : i32
      scf.for %s = %lb to %ub step %st {
        %e0 = aie.objectfifo.acquire @inc1(Consume) : memref<8xi32>
        %e1 = aie.objectfifo.acquire @outc1(Produce) : memref<8xi32>
        scf.for %i = %lb to %ub step %st {
          %0 = memref.load %e0[%i] : memref<8xi32>
          %1 = arith.addi %0, %val : i32
          memref.store %1, %e1[%i] : memref<8xi32>
    ...
    aie.core(%c2) {
      ...
      %val = arith.constant 1 : i32
      scf.for %s = %lb to %ub step %st {
        %e0 = aie.objectfifo.acquire @inc2(Consume) : memref<8xi32>
        %e1 = aie.objectfifo.acquire @outc2(Produce) : memref<8xi32>
        scf.for %i = %lb to %ub step %st {
          %0 = memref.load %e0[%i] : memref<8xi32>
          %1 = arith.addi %0, %val : i32
          memref.store %1, %e1[%i] : memref<8xi32>
