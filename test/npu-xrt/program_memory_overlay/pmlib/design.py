...
def build(cfg):
    ...
    entry = Kernel(ENTRY, "slot.ld", [tile_ty, tile_ty])
    ...
    def core_fn(in_cons, out_prod, flag, ovl_wait, entry):
    ...
    worker = Worker(
        core_fn,
        [of_in.cons(), of_out.prod(), flag, ovl_wait, entry],
        tile=compute_tile,
        while_true=False,
    )
    ...
    def sequence(host_in, host_out, in_prod, out_cons):
        ...
        for phase, ovl in enumerate(cfg.phases):
            if cfg.payloads and phase not in cfg.skip_write:
                ...
                write_payload(words, cfg.slot.base, ovl, f"_p{phase}", delta)
...
def emit_slot_ld(geometry, path, entry=ENTRY):
    ...
    with open(path, "w") as f:
        f.write(
            f"/* Generated by pmlib/design.py. The resident calls {entry} at this\n"
            f"   fixed address; its body is written there at run time. */\n"
            f"{entry} = 0x{s.base:x};\n"
