module {
  // Use link_symbol to map to mangled name
  func.func private @kernel_add(memref<8xi32>, i32) attributes { link_with="kernel1.o", link_symbol="kernel(int*, int)" }
  
  // Use demangled name directly (no link_symbol needed if it matches)
  func.func private @"kernel(int*, int)"(memref<8xi32>, i32) attributes { link_with="kernel2.o" }
  ...
  aie.device(npu1_1col) {
    ...
    aie.core(%t02) {
      ...
      scf.for %steps = %c0 to %c8 step %c1 {
        ...
        func.call @kernel_add(%elem1, %c10) : (memref<8xi32>, i32) -> ()
        func.call @"kernel(int*, int)"(%elem1, %c5) : (memref<8xi32>, i32) -> ()
