...
// CHECK:   aie.connect<South : 0, North : {{[0-9]+}}>
// CHECK: }
// Trunk ends at the fan-out input; branch starts at the fan-out output.
// CHECK: aie.flow(%[[T02]], DMA : 0, %[[T04]], South : 0) via (%[[T02]] : DMA : 0 -> North : {{[0-9]+}}, %[[T03]] : South : {{[0-9]+}} -> North : {{[0-9]+}})
// CHECK: aie.flow(%[[T04]], North : {{[0-9]+}}, %[[T05]], DMA : 0) via (%[[T05]] : South : {{[0-9]+}} -> DMA : 0)
module {
  aie.device(xcvc1902) {
    %t02 = aie.tile(0, 2)
    %t04 = aie.tile(0, 4)
