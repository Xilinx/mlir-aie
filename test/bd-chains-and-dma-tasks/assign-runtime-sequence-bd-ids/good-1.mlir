...
module {
  aie.device(npu1) {
    ...
    aie.bd_chain @simple_chain(%arg0: memref<8xi16>, %arg1: memref<12xi16>, %arg2: memref<8xi16>) {
            ...
            aie.next_bd ^bd1
        ^bd1:
            aie.dma_bd(%arg1 : memref<12xi16> offset = 0 len = 12 sizes = [] strides = [])
            aie.next_bd ^bd2
        ^bd2:
            aie.dma_bd(%arg2 : memref<8xi16> offset = 0 len = 8 sizes = [] strides = [])
            aie.end
    }
    ...
    aie.runtime_sequence(%arg0: memref<8xi16>, %arg1: memref<12xi16>, %arg2: memref<8xi16>) {
      ...
      // CHECK:   aie.dma_bd(%arg0 : memref<8xi16> offset = 0 len = 8 sizes = [1, 2, 2, 2] strides = [0, 2, 4, 1]) {bd_id = 0 : i32}
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.dma_bd(%arg1 : memref<12xi16> offset = 0 len = 12 sizes = [] strides = []) {bd_id = 1 : i32}
      // CHECK:   aie.next_bd ^bb2
      // CHECK: ^bb2:
      // CHECK:   aie.dma_bd(%arg2 : memref<8xi16> offset = 0 len = 8 sizes = [] strides = []) {bd_id = 2 : i32}
      // CHECK:   aie.end
      // CHECK: }
      ...
      // CHECK:   aie.dma_bd(%arg2 : memref<8xi16> offset = 0 len = 8 sizes = [1, 2, 2, 2] strides = [0, 2, 4, 1]) {bd_id = 3 : i32}
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.dma_bd(%arg1 : memref<12xi16> offset = 0 len = 12 sizes = [] strides = []) {bd_id = 4 : i32}
      // CHECK:   aie.next_bd ^bb2
      // CHECK: ^bb2:
      ...
      // CHECK:   aie.dma_bd(%arg0 : memref<8xi16> offset = 0 len = 8 sizes = [1, 2, 2, 2] strides = [0, 2, 4, 1]) {bd_id = 6 : i32}
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.dma_bd(%arg1 : memref<12xi16> offset = 0 len = 12 sizes = [] strides = []) {bd_id = 7 : i32}
      // CHECK:   aie.next_bd ^bb2
      // CHECK: ^bb2:
