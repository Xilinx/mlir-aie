...
// This test ensures that unranked memref arguments with 4 dimensions
// (including repeat/iteration count as the 4th dimension) are correctly
// handled in the dma-tasks-to-npu pass.
//
// IMPORTANT: The length attribute should only include the lowest 3 dimensions,
// NOT the 4th dimension (repeat count). The 4th dimension is handled separately
// as the BD repeat/iteration count.
  ...
  aie.device(npu1) {
    ...
    aie.runtime_sequence(%arg0: memref<*xbf16>) {
      ...
      %t1 = aiex.dma_configure_task(%tile_0_0, MM2S, 0) {
        ...
        aie.dma_bd(%arg0 : memref<*xbf16> offset = 0 len = 128 sizes = [4, 8, 4, 4] strides = [128, 64, 16, 1]) {bd_id = 0 : i32}
        aie.end
      } {issue_token = true}
