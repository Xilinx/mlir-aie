...
module {
  aie.device(npu1) {
    ...
    aie.bd_chain @simple_chain(%buf: memref<8xi16>, %l0: index, %l1: index, %l2: index) {
            ...
            aie.dma_bd(%buf : memref<8xi16> offset = 0 len = 8 sizes = [] strides = [])
    ...
    aie.runtime_sequence(%buf: memref<8xi16>) {
      ...
      // CHECK:   aie.dma_bd(%buf : memref<8xi16> offset = 0 len = 8 sizes = [] strides = [])
      // CHECK:   aie.use_lock(%lock_1, "Release", 1)
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.use_lock(%lock_1, "Acquire", 1)
      // CHECK:   aie.dma_bd(%buf : memref<12xi16> offset = 0 len = 8 sizes = [] strides = [])
