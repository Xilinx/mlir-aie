...
module {
  aie.device(npu1) {
    ...
    aie.bd_chain @simple_chain(%buf: memref<8xi16>, %l0: index, %l1: index, %l2: index) {
            ...
            aie.use_lock(%l0, "Acquire", %c1_ul1)
            aie.dma_bd(%buf : memref<8xi16> offset = 0 len = 8)
            %c1_ul2 = arith.constant 1 : i32
            aie.use_lock(%l1, "Release", %c1_ul2)
            aie.next_bd ^bd1
        ^bd1:
            %c1_ul3 = arith.constant 1 : i32
            aie.use_lock(%l1, "Acquire", %c1_ul3)
            aie.dma_bd(%buf : memref<8xi16> offset = 0 len = 8)
            %c1_ul4 = arith.constant 1 : i32
            aie.use_lock(%l2, "Release", %c1_ul4)
            aie.next_bd ^bd2
        ^bd2:
            %c1_ul5 = arith.constant 1 : i32
            aie.use_lock(%l2, "Acquire", %c1_ul5)
            aie.dma_bd(%buf : memref<8xi16> offset = 0 len = 8)
            %c1_ul6 = arith.constant 1 : i32
            aie.use_lock(%l0, "Release", %c1_ul6)
            aie.end
    }
    ...
    aie.runtime_sequence(%buf: memref<8xi16>) {
      ...
      // CHECK:   aie.use_lock(%lock_1, "Release", %{{.*}})
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.use_lock(%lock_1, "Acquire", %{{.*}})
      // CHECK:   aie.dma_bd(%buf : memref<12xi16> offset = {{.*}} len = {{.*}})
      // CHECK:   aie.use_lock(%lock_2, "Release", %{{.*}})
