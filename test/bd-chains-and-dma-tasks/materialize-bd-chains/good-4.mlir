  ...
  aie.device(npu1) {
    ...
    aie.bd_chain @simple_chain(%arg0: memref<8xi16>, %arg1: memref<8xi16>) {
      ...
      %c8_i32 = arith.constant 8 : i32
            ...
            aie.next_bd ^bd1
        ^bd1:
            aie.dma_bd(%arg1 : memref<8xi16> offset = %c0_i32 len = %c8_i32 sizes = [] strides = [])
            aie.end
    }
    ...
    aie.runtime_sequence(%arg0: memref<8xi16>, %arg1: memref<12xi16>, %arg2: memref<8xi16>) {
      ...
      // CHECK:   aie.dma_bd(%[[BUF1]] : memref<8xi16> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
      // CHECK:   aie.next_bd ^bb1
      // CHECK: ^bb1:
      // CHECK:   aie.dma_bd(%[[BUF0]] : memref<8xi16> offset = {{.*}} len = {{.*}} sizes = {{.*}} strides = {{.*}})
      // CHECK:   aie.end
      // CHECK: }
