...
add_workers, of_add_out = EltwiseAdd(of_a.cons, of_b.cons, n_cores=4, ...)
mul_workers, of_c = EltwiseMul(of_add_out.cons, of_add_out.cons, n_cores=4, ...)
...
with io.build_sequence(tensor_ty, tensor_ty, tensor_ty) as (a_in, b_in, c_out):
    ...
    for t in io.tile_loop(tiler.tile_iter()):
        io.fill(of_a.prod, t, a_in)
        io.fill(of_b.prod, t, b_in)
        io.drain(of_c.cons, t, c_out)
...
add_workers, of_add_out = EltwiseAdd(of_a.cons, of_b.cons, n_cores=4, placement_resources=dev.tiles[0])
mul_workers, of_c = EltwiseMul(of_add_out.cons, of_add_out.cons, n_cores=4, placement_resources=dev.tiles[1])

io = IOCoordinator()
with io.build_sequence(tensor_ty, tensor_ty, tensor_ty) as (a_in, b_in, c_out):
    tiler = TensorTiler2D(M, N)
    for t in io.tile_loop(tiler.tile_iter()):
        io.fill(of_a.prod, t, a_in, placement=dev.tiles[0].shim)
        io.fill(of_b.prod, t, b_in, placement=dev.tiles[0].shim)
        io.drain(of_c.cons, t, c_out, placement=dev.tiles[1].shim)
