...
with mlir_mod_ctx() as ctx:
  ...
  def device_body():
    # Types, tile declarations, and AIE movement with object fifos
    matrix_ty = T.memref(MATRIX_HEIGHT, MATRIX_WIDTH, T.i32())
    tile_ty = T.memref(TILE_HEIGHT, TILE_WIDTH, T.i32())
    ShimTile = tile(0, 0)
    ...
    def core_body():
      for _ in for_(sys.maxsize): # Effective while(1)
        ...
        for i in for_(TILE_HEIGHT):
          for j in for_(TILE_WIDTH):
            v0 = memref.load(elem_in, [i, j])
            v1 = arith.addi(v0, arith.constant(1, T.i32()))
            memref.store(v1, elem_out, [i, j])
            yield_([])
          yield_([])
