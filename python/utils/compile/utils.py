...
# Opt-in: a compiler cache is a policy about the developer's machine, not a
# property of the compile.  Accepts any launcher that takes the compiler as its
# first argument (ccache, sccache).
_COMPILER_LAUNCHER = os.environ.get("AIE_KERNEL_COMPILER_LAUNCHER", "")


def _warn_launcher_pch_interaction(launcher: str) -> None:
    """Warn that ccache silently declines to cache a -include-pch compile.

    The call is counted "uncacheable" rather than missed, so setting both levers
    gets neither; ccache's own explanation only appears under CCACHE_DEBUG.
    """
