...
def _unwrap_handle(arg):
    """Unwrap a resolved handle object (e.g. iron.Buffer) to its underlying
    MLIR value via its ``.op`` property.

    Checked on the *class*, not the instance, so this never touches the
    property getter of an unresolved handle (which raises NotResolvedError)
    -- accessing a property through the class returns the descriptor itself,
    not its value. Duck-typed on the ``.op`` convention shared by every
    resolvable handle in aie.iron (Buffer, Tile, RuntimeData, ...) rather than
    importing those classes, which live in aie.iron and would reintroduce the
    aie.iron <-> aie.utils.compile import cycle this module exists to avoid.
    """
    return arg.op if hasattr(type(arg), "op") else arg
...
class BaseKernel(Resolvable):
    ...
    def __call__(self, *args, **kwargs):
        ...
        arg_ops = [_unwrap_handle(a) for a in args]
