...
class flow_runner:
    ...
    async def chesshack(self, task, llvmir):
        ...
        await self.do_call(
            task, ["sed", "-i", "s/noalias_sidechannel[^,],//", llvmir_chesshack]
        )
        ...
        await self.do_call(
            task, ["sed", "-i", "-E", "/define .*@/ s/%[0-9]*//g", llvmir_chesslinked]
        )
        await self.do_call(
            task, ["sed", "-i", "-E", "s/mustprogress//g", llvmir_chesslinked]
        )
        await self.do_call(
            task, ["sed", "-i", "-E", "s/poison/undef/g", llvmir_chesslinked]
        )
        await self.do_call(
            task, ["sed", "-i", "-E", "s/nocallback//g", llvmir_chesslinked]
        )
        await self.do_call(
            task, ["sed", "-i", "-E", "s/memory\(none\)/readnone/g", llvmir_chesslinked]
        )
        await self.do_call(
            task, ["sed", "-i", "-E", "s/memory\(read\)/readonly/g", llvmir_chesslinked]
        )
        await self.do_call(
            task,
            ["sed", "-i", "-E", "s/memory\(write\)/writeonly/g", llvmir_chesslinked],
        )
        await self.do_call(
            task,
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: readwrite\)/argmemonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: read\)/argmemonly readonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: write\)/argmemonly writeonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(inaccessiblemem: readwrite\)/inaccessiblememonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(inaccessiblemem: read\)/inaccessiblememonly readonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(inaccessiblemem: write\)/inaccessiblememonly writeonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: readwrite, inaccessiblemem: readwrite\)/inaccessiblemem_or_argmemonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: read, inaccessiblemem: read\)/inaccessiblemem_or_argmemonly readonly/g",
                llvmir_chesslinked,
            ],
        )
        await self.do_call(
            ...
            [
                ...
                "-i",
                "-E",
                "s/memory\(argmem: write, inaccessiblemem: write\)/inaccessiblemem_or_argmemonly writeonly/g",
                llvmir_chesslinked,
            ],
        )
    ...
    async def prepare_for_chesshack(self, task):
        if opts.compile and opts.xchesscc:
            ...
            chess_intrinsic_wrapper_cpp = os.path.join(
                runtime_lib_path, self.aie_target.upper(), "chess_intrinsic_wrapper.cpp"
            )

            self.chess_intrinsic_wrapper = os.path.join(
            ...
            await self.do_call(
                ...
                [
                    "xchesscc_wrapper",
                    self.aie_target.lower(),
                    "+w",
                    os.path.join(self.tmpdirname, "work"),
                    "-c",
                    "-d",
                    "-f",
                    "+f",
                    "+P",
                    "4",
                    chess_intrinsic_wrapper_cpp,
                    "-o",
                    self.chess_intrinsic_wrapper,
                ],
            )
            await self.do_call(
                task, ["sed", "-i", "s/^target.*//", self.chess_intrinsic_wrapper]
            )
            ...
            await self.do_call(
                ...
                [
                    "sed",
                    "-i",
                    "s/noalias_sidechannel[^,]*,//",
                    self.chess_intrinsic_wrapper,
                ],
            )
            await self.do_call(
                task,
                ["sed", "-i", "s/nocallback[^,]*,//", self.chess_intrinsic_wrapper],
            )
    ...
    async def process_core(self, core):
        async with self.limit:
            ...
            runtime_lib_path = os.path.join(
                install_path, "aie_runtime_lib", self.aie_target.upper()
            )
            ...
            llvmlibc_build_lib_path = os.path.join(
                ...
                "..",
                "runtimes",
                "runtimes-" + self.aie_target.lower() + "-none-unknown-elf-bins",
                "libc",
                "lib",
                "libc.a",
            )
            llvmlibc_install_lib_path = os.path.join(
                ...
                "..",
                "lib",
                self.aie_target.lower() + "-none-unknown-elf",
                "libc.a",
            )
            ...
            if not opts.unified:
                ...
                await self.do_call(
                    task, ["aie-opt", *aie_opt_passes, file_core, "-o", file_opt_core]
                )
            ...
            if opts.compile and opts.xchesscc:
                if not opts.unified:
                    file_core_llvmir_chesslinked = await self.chesshack(
                        task, file_core_llvmir
                    )
                    if self.opts.link and self.opts.xbridge:
                        ...
                        await self.do_call(
                            ...
                            [
                                "clang",
                                "-O2",
                                "--target=" + self.aie_peano_target,
                                file_core_obj,
                                *clang_link_args,
                                "-Wl,-T," + file_core_ldscript,
                                "-o",
                                file_core_elf,
                            ],
                        )
    ...
    async def gen_sim(self, task):
        ...
        runtime_simlib_path = os.path.join(
            install_path, "aie_runtime_lib", self.aie_target.upper(), "aiesim"
        )
        runtime_testlib_path = os.path.join(
            install_path,
            "runtime_lib",
    ...
    async def run_flow(self):
        ...
        with progress.Progress(
            *progress.Progress.get_default_columns(),
            ...
            if opts.unified:
                self.file_opt_with_addresses = os.path.join(
                    self.tmpdirname, "input_opt_with_addresses.mlir"
                )
                await self.do_call(
                    progress_bar.task,
                    [
                ...
                if opts.compile and opts.xchesscc:
                    file_llvmir_hacked = await self.chesshack(
                        progress_bar.task, self.file_llvmir
                    )
                    await self.do_call(
                        progress_bar.task,
                        [
