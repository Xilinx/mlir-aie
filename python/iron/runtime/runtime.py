...
class Runtime(Resolvable):
    ...
    def fill(
        ...
        Args:
            ...
            placement (Tile, optional): The Shim tile to associate the data transfer with.
        ...
        # RuntimeEndpoint must be placed on Shim tile
        placement = placement if placement else Tile()
        if placement.tile_type and placement.tile_type != Tile.SHIM:
            raise ValueError(
                f"RuntimeEndpoint requires Tile.SHIM, got tile_type='{placement.tile_type}'"
            )
        placement.tile_type = Tile.SHIM
