...
def _transform_gen(func, inputs: list, output, *params):
    """
...
def _transform_parallel_gen(func, inputs: list, output):
    """
    ...
    Args:
        func: Lambda/callable function to apply element-wise.
        inputs: List of input tensors (will be tiled automatically)
        output: Output tensor (will be tiled automatically)

    TODO: Add support for using with ExternalFunctions
    """
    # Validate all tensors have same shape and dtype
    all_tensors = inputs + [output]
    ref_shape = all_tensors[0].shape
    ...
    def core_body(*of_args):
        # of_args = [*of_inputs_cons, of_out_prod, func]
        of_ins = of_args[:num_inputs]
        of_output = of_args[num_inputs]
        func_to_apply = of_args[num_inputs + 1]
    ...
    # Runtime operations to move data to/from the AIE-array
    rt = Runtime()
    all_types = [tensor_ty] * num_inputs + [tensor_ty]
    with rt.sequence(*all_types) as seq_args:
        input_seq_args = seq_args[:num_inputs]
        output_seq_arg = seq_args[num_inputs]
...
def transform(func, input, output, *params):
    """
    ...
    return _transform_gen(func, [input], output, *params)


def transform_binary(func, first, second, output, *params):
    """
    Transform binary inputs to output using tiled processing.

    Args:
        func: Function to apply, either a lambda/callable or ExternalFunction.
              For ExternalFunction, arg_types should be [input_tile0, input_tile1, output_tile, *params]
        first: First input tensor (will be tiled automatically)
        second: Second input tensor (will be tiled automatically)
        output: Output tensor (will be tiled automatically)
        *params: Additional parameters for ExternalFunction only.
                 Scalar dtypes (np.int32, etc.) are passed as MLIR constants;
                 array types are transferred via ObjectFifos.
    ...
    return _transform_gen(func, [first, second], output, *params)


def transform_parallel(func, input, output):
    """Parallel unary transform across multiple AIE tiles."""

    return _transform_parallel_gen(func, [input], output)


def transform_parallel_binary(func, first, second, output):
    """Parallel binary transform across multiple AIE tiles."""

    return _transform_parallel_gen(func, [first, second], output)
