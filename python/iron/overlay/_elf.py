...
"""Just enough ELF32 to place and inspect program-memory overlays.
...
Ported from test/npu-xrt/program_memory_overlay/pmlib/elf.py. The only
substantive change is that a build-time failure here raises `OverlayELFError`
instead of calling `sys.exit` -- this is a library used mid-build by
[`iron.overlay.ProgramMemoryOverlayDesign`][], not a standalone CLI tool.
"""
...
class OverlayELFError(Exception):
    """An ELF this module was asked to read or place is unusable."""
...
def read_elf(path):
    ...
    if blob[:4] != b"\x7fELF" or blob[4] != 1 or blob[5] != 1:
        raise OverlayELFError(f"{path}: not a little-endian 32-bit ELF")
...
def section_file_offset(path, want):
    ...
    for nm, _, _, _, off, _, _, _, _, _ in shdrs:
        ...
        if blob[shstr + nm : end].decode() == want:
            return off
    raise OverlayELFError(f"{path}: no section named {want}")
...
def text_words(path):
    ...
    raise OverlayELFError(f"{path}: no .text")
...
def find_core_elf(path, col=0, row=2):
    ...
    if not matches:
        raise OverlayELFError(f"{path}: no core ({col},{row}) ELF found")
    if len(matches) > 1:
        raise OverlayELFError(
            f"{path}: ambiguous core ELFs for ({col},{row}): {matches}"
        )
...
def peano(tool):
    ...
    if not root:
        raise OverlayELFError("PEANO_INSTALL_DIR is not set")
