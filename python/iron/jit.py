...
class NPUKernel:
    def __init__(self, xclbin_path, insts_path, device_index = 0, kernel_name="PP_FD_PRE"):
        ...
        insts_buffer_bo_np = np.frombuffer(self.__insts_buffer_bo.map(), dtype=insts.dtype).reshape(insts.shape)
    ...
    def __call__(self, *args):
        ...
        for tensor in args:
            if not hasattr(tensor, "buffer_object"):
                raise TypeError(f"Expected Tensor with .buffer_object(), got {type(tensor)}")
            kernel_args.append(tensor.buffer_object())
        h = self.__kernel(
            opcode,
            self.__insts_buffer_bo,
            self.__n_insts,
            *kernel_args
        )
        r = h.wait()
...
def jit(*, debug=False, verify=False, use_cache=True):
    def decorator(fn):
        with mlir_mod_ctx() as ctx:
            fn()
            if verify:
                assert ctx.module.operation.verify(), f"Verification failed for '{fn.__name__}'"
        ...
        if not use_cache or not xclbin_exists or not inst_exists:
            ...
            compile_mlir_to_binary(mlir_path = mlir_path, inst_filename= inst_filename, xclbin_filename= xclbin_filename)
