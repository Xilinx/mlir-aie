...
def find_aiebu_asm():
    ...
    if shutil.which(asm_bin) is None:
        ...
        if shutil.which(asm_bin) is None:
            asm_bin = None
    if asm_bin is None:
        logger.error("aiebu-asm not found.")
        sys.exit(1)
    return asm_bin
        ...
        for runtime_seq in find_ops(
            ...
            for load_pdi_op in find_ops(
                ...
                if device_name not in device_to_id_mapping:
                    logger.warning(
                        "Device '%s' for load_pdi instruction does not have a matching device PDI.",
                        device_name,
                    )
                    sys.exit(1)
...
def handle_pass_failure(
    ...
    logger.error(
        "\n%s\nAIECC COMPILATION FAILED%s\n%s\n"
        "\nIntermediate MLIR saved to:\n  %s"
        "\n\nFor developers, the error can be reproduced with:"
        "\n  $ aie-opt --pass-pipeline='%s' %s"
        "\n\nRepeater script generated:\n  %s\n  $ bash %s\n%s\n",
        "=" * 80,
        desc_str,
        "=" * 80,
        mlir_filename,
...
def get_peano_target(aie_target):
    if not re.fullmatch("AIE.?.?", aie_target):
        logger.error("Unexpected target %s. Exiting...", aie_target)
        exit(-3)
...
class FlowRunner:
    ...
    def run_passes(
        ...
        """
        logger.debug("Running: %s", pass_pipeline)
        diags = []
    ...
    async def do_call(self, task_id, command, force=False):
        ...
        if self.opts.execute or force:
            ...
            ret = proc.returncode
            if stdout:
                logger.debug("%s", stdout.decode())
            if ret != 0 and stderr:
                logger.error("%s", stderr.decode())
        else:
            ret = 0
        end = time.time()
        logger.debug("Done in %.3f sec: %s", end - start, commandstr)
        ...
        if ret != 0:
            ...
            logger.error("Error encountered while running: %s", commandstr)
            sys.exit(ret)
    ...
    async def process_txn(self, module, device_name):
        ...
        logger.debug("copy %s to %s", file_txn, txn_dest)
    ...
    async def assemble_full_elf(
        ...
        logger.debug("Generated full ELF: %s", output_elf_path)
    ...
    async def generate_full_elf(self, devices, device_to_id_mapping, parent_task=None):
        ...
        logger.debug("Generated config.json: %s", config_json_path)
        if parent_task:
            self.progress_bar.update(
    ...
    async def gen_sim(self, task, aie_target, file_physical, device_name):
        ...
        logger.info("Simulation generated...")
        logger.info("To run simulation: %s", sim_script)
    ...
    async def run_flow(self):
        ...
        with progress.Progress(
            *progress.Progress.get_default_columns(),
            ...
            if len(devices) == 0:
                logger.error("input MLIR must contain at least one aie.device")
                sys.exit(1)
            ...
            if len(aie_targets) == 0 or not all(
                ...
                logger.error("all device targets in the file must be the same")
...
def run(mlir_module, args=None):
    ...
    try:
        ...
        logger.debug("Found Ryzen AI software version %s at %s", version, path)
    ...
    if xchesscc_path:
        ...
        logger.debug("Found xchesscc at %s", xchesscc_path)
        ...
        logger.debug("xchesscc not found.")

    if opts.aietools_path is None:
        logger.debug("Could not find aietools from Vitis or Ryzen AI Software.")
    ...
    logger.debug("Compiling %s", opts.filename)
...
def main():
    ...
    if opts.filename is None:
        logger.error("the 'file' positional argument is required.")
        sys.exit(1)
