...
class FlowRunner:
    ...
    async def process_cores(
        ...
        if opts.unified:
            ...
            self.run_passes(
                str(AIE_LOWER_TO_LLVM(device_name)),
                module,
        ...
        async with self.limit:
            ...
            if not opts.unified:
                ...
                with Context(), Location.unknown():
                    module = Module.parse(await read_file_async(file_with_addresses))
                self.run_passes(
                    str(AIE_LOWER_TO_LLVM(device_name, corecol, corerow)),
                    module,
    ...
    async def process_ctrlpkt(self, module_str, device_name):
        with Context(), Location.unknown():
            ...
            ctrlpkt_module = self.run_passes(
                ...
                description=f"Control packet payload (binary) generation for {device_name}",
            )
            await self.do_call(
                None,
                [
            ...
            # ctrlpkt_module = Module.parse(ctrlpkt_mlir_str)
            self.run_passes(
