...
def my_design(a_in: In, b_out: Out):
    ...
    rt = Runtime(
        sequence,
        [*[np.ndarray[(1024,), np.dtype[np.int32]]] * 2, of_in.prod(), of_out.cons()],
    )
...
| [**Runtime**](../docs/api/iron.md#iron.runtime.runtime.Runtime) | The host-side description of `fill` / `drain` operations, defined by the body passed to `Runtime(seq, fn_args)`. Workers are passed to `Program(workers=...)` rather than started from the body. Distinct from "host code" (the C++/Python testbench that calls the design). |
