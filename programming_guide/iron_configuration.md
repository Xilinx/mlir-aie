...
| `aie.utils.test.create_npu_kernel(opts)` | Build an `NPUKernel` (plus optional `TraceBuffer`) from a parsed `argparse.Namespace`.  See `programming_examples/basic/vector_scalar_mul/test.py` for the canonical use pattern. |
