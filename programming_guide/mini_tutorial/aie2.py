...
# Runtime sequence tasks are submitted to and executed by a dedicated command processor in the order
# they are submitted in. Tasks which are set to 'wait' will block until a token associated with their
...
# Note: The task to start the workers currently only registers the input workers as part of the
# Program, but does not launch the Workers themselves. This means that this task can be added in
# the sequence at any point and does not need to be the first one.
rt = Runtime()
...
# After the placement is complete, the resolve_program() will check that each component has
# sufficient information to be lowered to its MLIR equivalent.
