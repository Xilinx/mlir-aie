...
```
The following code snippet shows how the same example as above is written at the lower-level IRON abstraction with explicit tile endpoints.
```python
A = tile(1, 3)
B = tile(2, 4)
of_in = object_fifo("in", A, B, 3, np.ndarray[(256,), np.dtype[np.int32]])
