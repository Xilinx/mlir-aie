...
| `dma_wait(object_fifo)` | configure host-ShimDMA synchronization for accessing external memory | `metadata`: Identifies the ObjectFifo (by Python object or name string) whose half-DMA completion we are waiting on. | `None` | dma_wait(of_out) |
| `dma_ordered_wait([object_fifo])` | generate one or more `dma_wait`s, respecting order of arguments | `metadatas`: A list of object fifos or object fifo names whose half-DMA completions we will wait on. | `None` | dma_ordered_wait([of_in, of_out]) |
