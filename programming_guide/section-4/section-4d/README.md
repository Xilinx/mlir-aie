...
| [Nightly charts](https://xilinx.github.io/mlir-aie/bench/npu2/) | The benchmark's history, one chart per case and metric. |
...
1. Find the symbol the factory actually compiles, and the file it lives in. The remarks tool prints it for every build: for the `add` factory it is `eltwise_add_bf16_vector` in [add.cc](../../../aie_kernels/generic/add.cc).
...
The bf16 [add.cc](../../../aie_kernels/generic/add.cc) kernel used to run one load/add/store chain per loop iteration. It now runs four. Here is how that change looks through each tool.
...
[add.cc](../../../aie_kernels/generic/add.cc) holds three functions, one row per loop: `eltwise_add_bf16_scalar`, `eltwise_add_bf16_vector` and `eltwise_add_bf16_vector_size`. The `add` factory calls `eltwise_add_bf16_vector`, so that is the row to read. Its `value` and `range`, before and after:
...
* **Clamp in bf16, not in float.** AIE2P has no native `float` vector min/max either, and the emulated clamp held even the identity epilogue of [the fused matrix multiply](../../../aie_kernels/generic/mm_fused_mmul.h) at II 30 per 16 lanes. Rounding is monotone and leaves representable values alone, so clamping the bf16 result against bounds rounded the same way gives identical raw output. Identity/clamp epilogue per chunk: **126 → 19** cycles (combined with cursors, 32 lanes, and the unroll by 2 in lever 3).
...
1. Reproduce the [worked example](#a-worked-example-add): benchmark `add/1024x16/bfloat16` with the shipped kernel and with the one-chain loop below in a copy of [add.cc](../../../aie_kernels/generic/add.cc), and read both remarks rows.
    ```C++
...
1. In your copy of [scale.cc](../../../aie_kernels/aie2/scale.cc), change the scalar kernel's `c[i] = factor * a[i];` to `c[i] = a[i] / factor;`. Rebuild the 32-bit scalar design from [section 4c](../section-4c) and run `llvm-nm -u` on the kernel object. What do you see, and what does it cost?
    <details markdown="1"><summary>Show answer</summary>
