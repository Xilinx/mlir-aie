...
{
  auto sectionInfo = std::make_unique<SectionInfo>(AIE_PARTITION, "AIE_PARTITION", hrx::factory<SectionAIEPartition*>());
...
{
  auto iter = std::find_if(CTTypes.begin(), CTTypes.end(), [&](const auto& entry) {return hrx::iequals(entry.first, sTypeName);});
...
static void
process_PDI_uuid(const hrx::property_tree::ptree& ptPDI,
                 aie_pdi& aieParitionPDI)
{
  ...
  // Clean up the UUID
  hrx::replace_all(uuid, "0x", "");  // Remove all "0x"
  hrx::replace_all(uuid, "-", "");   // Remove all minus signs
                      ...
                      std::vector<char>& buffer)
{
  ...
  XUtil::TRACE(hrx::format("Reading in the file: '%s'") % filePath.string());
...
static void
process_PDI_file(const hrx::property_tree::ptree& ptAIEPartitionPDI,
...
static void
process_pre_cdo_groups(const hrx::property_tree::ptree& ptAIECDOGroup,
...
static void
process_PDI_cdo_groups(const hrx::property_tree::ptree& ptAIEPartitionPDI,
                       aie_pdi& aiePDI,
                       SectionHeap& heap)
{
  XUtil::TRACE("Processing CDO Groups");

  std::vector<hrx::property_tree::ptree> ptCDOs = XUtil::as_vector<hrx::property_tree::ptree>(ptAIEPartitionPDI, "cdo_groups");
...
static void
process_PDIs(const hrx::property_tree::ptree& ptAIEPartition,
             const fs::path& relativeFromDir,
             ...
             SectionHeap& heap)
{
  ...
  std::vector<hrx::property_tree::ptree> ptPDIs = XUtil::as_vector<hrx::property_tree::ptree>(ptAIEPartition, "PDIs");
...
static void
process_partition_info(const hrx::property_tree::ptree& ptAIEPartition,
                       aie_partition_info& partitionInfo,
                       SectionHeap& heap)
{
  XUtil::TRACE("Processing partition info");
  static const hrx::property_tree::ptree ptEmpty;
  ...
  // XUtil::TRACE(hrx::str(hrx::format("  start_column (0x%lx)") % partitionInfo.start_columns.offset));
                        ...
                        std::ostringstream& osBuffer)
{
  ...
  hrx::property_tree::read_json(iStream, pt);
  XUtil::TRACE_PrintTree("AIE_PARTITION", pt);

  const hrx::property_tree::ptree& ptAIEPartition = pt.get_child("aie_partition");
  ...
  // XUtil::TRACE(hrx::str(hrx::format("  kernel_commit_id (0x%lx): '%s'") % aie_partitionHdr.kernel_commit_id % sKernelCommitId));
                                    ...
                                    std::ostringstream& buffer) const
{
  ...
  if (!sSubSectionName.empty()) {
    auto errMsg = hrx::format("ERROR: Subsection '%s' not support by section '%s")
...
populate_partition_info(const char* pBase,
                        ...
                        hrx::property_tree::ptree& ptAiePartition)
{
  XUtil::TRACE("Populating Partition Info");
  hrx::property_tree::ptree ptPartitionInfo;

  // Column Width
  ptPartitionInfo.put("column_width", (hrx::format("%d") % aiePartitionInfo.column_width).str());

  // Start Columns
  hrx::property_tree::ptree ptStartColumnArray;
  ...
  for (uint32_t index = 0; index < aiePartitionInfo.start_columns.size; index++) {
    hrx::property_tree::ptree ptElement;
    ptElement.put("", (hrx::format("%d") % columnArray[index]).str());
...
populate_pre_cdo_groups(const char* pBase,
                        ...
                        hrx::property_tree::ptree& ptCDOGroup)
{
  ...
  hrx::property_tree::ptree ptPreCDOGroupArray;
  ...
  for (uint32_t index = 0; index < aieCDOGroup.pre_cdo_groups.size; index++) {
    ...
    ptElement.put("", (hrx::format("0x%x") % element));
...
populate_cdo_groups(const char* pBase,
                    ...
                    hrx::property_tree::ptree& ptAiePDI)
{
  XUtil::TRACE("Populating CDO groups");
  hrx::property_tree::ptree ptCDOGroupArray;
  ...
  for (uint32_t index = 0; index < aiePDI.cdo_groups.size; index++) {
    ...
    // PDI ID
    ptElement.put("pdi_id", (hrx::format("0x%x") % element.pdi_id).str());

    // DPU Kernel IDs
    if (element.dpu_kernel_ids.size) {
      hrx::property_tree::ptree ptDPUKernelIDs;
      ...
      for (uint32_t kernelIDindex = 0; kernelIDindex < element.dpu_kernel_ids.size; kernelIDindex++) {
        hrx::property_tree::ptree ptID;
        ptID.put("", (hrx::format("0x%x") % kernelIDsArray[kernelIDindex]).str());
                ...
                const fs::path& relativeToDir)
{
  ...
  XUtil::TRACE(hrx::format("Creating PDI Image: %s") % filePath.string());

  std::fstream oPDIFile;
  oPDIFile.open(filePath.string(), std::ifstream::out | std::ifstream::binary);
  if (!oPDIFile.is_open()) {
    const auto errMsg = hrx::format("ERROR: Unable to open the file for writing: %s") % filePath.string();
              ...
              hrx::property_tree::ptree& ptAiePartition)
{
  ...
  hrx::property_tree::ptree ptPDIArray;
                       ...
                       std::ostream& oStream)
{
  ...
  if (bufferSize < sizeof(aie_partition)) {
    auto errMsg = hrx::format("ERROR: Segment size (%d) is smaller than the size of the aie_partition structure (%d)") % bufferSize % sizeof(aie_partition);
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  // TOPs
  ptAiePartition.put("operations_per_cycle", (hrx::format("%d") % pHdr->operations_per_cycle).str());
  ptAiePartition.put("inference_fingerprint", (hrx::format("%d") % pHdr->inference_fingerprint).str());
  ptAiePartition.put("pre_post_fingerprint", (hrx::format("%d") % pHdr->pre_post_fingerprint).str());
  ...
  if (pHdr->kernel_commit_id != 0) {
    ...
    XUtil::TRACE(hrx::format("Open an existing xclbin: kernel_commit_id is 0x%lx") % pHdr->kernel_commit_id);
  }
  ...
  hrx::property_tree::write_json(oStream, ptRoot);
}
                                      ...
                                      const axlf_section_header& sectionHeader)
{
  ...
  if (m_bufferSize < sizeof(aie_partition)) {
    auto errMsg = hrx::format("ERROR: Segment size (%d) is smaller than the size of the aie_partition structure (%d)") 
