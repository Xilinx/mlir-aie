...
DTC::DTC(const hrx::property_tree::ptree& _ptDTC,
         const FDTProperty::PropertyNameFormat& _propertyNameFormat)
    : DTC()
{
                         ...
                         const FDTProperty::PropertyNameFormat& _propertyNameFormat)
{
  ...
  if (_size < FDT_HEADER_SIZE) {
    auto errMsg = hrx::format("ERROR: The given DTC buffer's header size (%d bytes) is smaller then the expected size (%d bytes).") %  _size % FDT_HEADER_SIZE;
  ...
  if (ntohl(pHdr->magic) != MAGIC) {
    auto errMsg = hrx::format("ERROR: Missing DTC magic number.  Expected: 0x%x, Found: 0x%x.") % MAGIC % ntohl(pHdr->magic);
    throw std::runtime_error(errMsg.str());
  }

  // Validate Size
  if (ntohl(pHdr->totalsize) != _size) {
    auto errMsg = hrx::format("ERROR: The expected size (%d bytes) does not match actual (%d bytes)") %  ntohl(pHdr->totalsize) % _size;
    throw std::runtime_error(errMsg.str());
  }

  // -- Get and validate the strings offset --
  if (ntohl(pHdr->off_dt_strings) > _size) {
    auto errMsg = hrx::format("ERROR: The string block offset (0x%x) exceeds then image size (0x%x)") % ntohl(pHdr->off_dt_strings) % _size;
    throw std::runtime_error(errMsg.str());
  }

  if ((ntohl(pHdr->off_dt_strings) + ntohl(pHdr->size_dt_strings)) > _size) {
    auto errMsg = hrx::format("ERROR: The string block offset and size (0x%x) exceeds then image size (0x%x)") % (ntohl(pHdr->off_dt_strings) + ntohl(pHdr->size_dt_strings)) % _size;
    throw std::runtime_error(errMsg.str());
  }
  ...
  if (ntohl(pHdr->off_dt_struct) > _size) {
    auto errMsg = hrx::format("ERROR: The structure block offset (0x%x) exceeds then image size (0x%x)") % ntohl(pHdr->off_dt_struct) % _size;
    throw std::runtime_error(errMsg.str());
  }

  if ((ntohl(pHdr->off_dt_struct) + ntohl(pHdr->size_dt_struct)) > _size) {
    auto errMsg = hrx::format("ERROR: The structure block offset and size (0x%x) exceeds then image size (0x%x)") % (ntohl(pHdr->off_dt_struct) + ntohl(pHdr->size_dt_struct)) % _size;
    throw std::runtime_error(errMsg.str());
  }
...
void
DTC::marshalToJSON(hrx::property_tree::ptree& _dtcTree,
...
void
DTC::marshalFromJSON(const hrx::property_tree::ptree& _ptDTC,
                     const FDTProperty::PropertyNameFormat& _propertyNameFormat)
{
