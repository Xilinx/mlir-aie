...
#include "hrx_util.h"
#include "hrx_util.h"
#include "hrx_util.h"                  // for uuid
#include "hrx_util.h"               // for to_string
#include <filesystem>
#include <random>
#include <sstream>
#include <stdexcept>
...
bool getVersionMajorMinorPath(const char* _pVersion, uint8_t& _major, uint8_t& _minor, uint16_t& _patch)
{
  ...
  hrx::split(tokens, sVersion, hrx::is_any_of("."));
  if (tokens.size() == 1) {
    _major = 0;
    _minor = 0;
...
XclBin::readXclBinBinarySections(std::fstream& _istream)
{
  ...
  for (unsigned int index = 0; index < numberOfSections; ++index) {
    XUtil::TRACE(hrx::format("Examining Section: %d of %d") % (index + 1) % m_xclBinHeader.m_header.m_numSections);
                         ...
                         bool _bMigrate)
{
  ...
  if (_bMigrate) {
    hrx::property_tree::ptree pt_mirrorData;
...
void
XclBin::addHeaderMirrorData(hrx::property_tree::ptree& _pt_header)
{
...
void
XclBin::writeXclBinBinaryHeader(std::ostream& _ostream, hrx::property_tree::ptree& _mirroredData)
{
  ...
  // Get mirror data
  hrx::property_tree::ptree pt_header;
...
void
XclBin::writeXclBinBinarySections(std::ostream& _ostream, hrx::property_tree::ptree& _mirroredData)
{
  // Nothing to write
  if (m_sections.empty()) {
    return;
  ...
  for (unsigned int index = 0; index < m_sections.size(); ++index) {
    XUtil::TRACE(hrx::format("Writing section: Index: %d, ID: %d") % index % sectionHeader[index].m_sectionKind);
    ...
    if (runningOffset != sectionHeader[index].m_sectionOffset) {
      auto errMsg = hrx::format("ERROR: Expected offset (0x%lx) does not match actual (0x%lx)") % sectionHeader[index].m_sectionOffset % runningOffset;
    ...
    {
      XUtil::TRACE("");
      XUtil::TRACE(hrx::format("Adding mirror properties[%d]") % index);

      hrx::property_tree::ptree pt_sectionHeader;

      XUtil::TRACE(hrx::format("Kind: %d, Name: %s, Offset: 0x%lx, Size: 0x%lx")
      ...
      pt_sectionHeader.put("Kind", (hrx::format("%d") % sectionHeader[index].m_sectionKind).str());
      pt_sectionHeader.put("Name", (hrx::format("%s") % sectionHeader[index].m_sectionName).str());
      pt_sectionHeader.put("Offset", (hrx::format("0x%lx") % sectionHeader[index].m_sectionOffset).str());
      pt_sectionHeader.put("Size", (hrx::format("0x%lx") % sectionHeader[index].m_sectionSize).str());

      hrx::property_tree::ptree pt_Payload;
...
XclBin::writeXclBinBinaryMirrorData(std::ostream& _ostream,
                                    const hrx::property_tree::ptree& _mirroredData) const
{
  _ostream << mirroDataStart;
  hrx::property_tree::write_json(_ostream, _mirroredData, false /*Pretty print*/);
...
XclBin::updateUUID()
{
  ...
  XUtil::TRACE(hrx::format("Updated xclbin UUID to: '%s'") % uuidStream.str());
}
                          ...
                          bool _bSkipUUIDInsertion)
{
  ...
  // Mirrored data
  hrx::property_tree::ptree mirroredData;
  ...
  // Update header file length
  {
    ...
    hrx::property_tree::ptree dummyData;
  ...
  XUtil::QUIET(hrx::format("Successfully wrote (%ld bytes) to the output file: %s")
                             % m_xclBinHeader.m_header.m_length % _binaryFileName);
}


void
XclBin::addPTreeSchemaVersion(hrx::property_tree::ptree& _pt, SchemaVersion const& _schemaVersion)
{

  XUtil::TRACE("");
  XUtil::TRACE("Adding Versioning Properties");

  hrx::property_tree::ptree pt_schemaVersion;

  XUtil::TRACE(hrx::format("major: %d, minor: %d, patch: %d")
                             % _schemaVersion.major
                             % _schemaVersion.minor
                             % _schemaVersion.patch);

  pt_schemaVersion.put("major", (hrx::format("%d") % _schemaVersion.major).str());
  pt_schemaVersion.put("minor", (hrx::format("%d") % _schemaVersion.minor).str());
  pt_schemaVersion.put("patch", (hrx::format("%d") % _schemaVersion.patch).str());
  _pt.add_child("schema_version", pt_schemaVersion);
}


void
XclBin::getSchemaVersion(hrx::property_tree::ptree& _pt, SchemaVersion& _schemaVersion)
{
...
void
XclBin::findAndReadMirrorData(std::fstream& _istream, hrx::property_tree::ptree& _mirrorData) const
{
  ...
  if (XUtil::findBytesInStream(_istream, mirroDataStart, startOffset) == true) {
    XUtil::TRACE(hrx::format("Found MIRROR_DATA_START at offset: 0x%lx") % startOffset);
  ...
  if (XUtil::findBytesInStream(_istream, mirrorDataEnd, bufferSize) == true) {
    XUtil::TRACE(hrx::format("Found MIRROR_DATA_END.  Buffersize: 0x%lx") % bufferSize);
  }  else {
  ...
  try {
    hrx::property_tree::read_json(ss, _mirrorData);
  } catch (const hrx::property_tree::json_parser_error& e) {
    auto errMsg = hrx::format("ERROR: Parsing mirror metadata in the xclbin archive on line %d: %s") % e.line() % e.message();
    throw std::runtime_error(errMsg.str());
  }
...
void
XclBin::readXclBinHeader(const hrx::property_tree::ptree& _ptHeader,
...
XclBin::readXclBinSection(std::fstream& _istream,
                          const hrx::property_tree::ptree& _ptSection)
{
...
XclBin::readXclBinaryMirrorImage(std::fstream& _istream,
                                 const hrx::property_tree::ptree& _mirrorData)
{
  // Iterate over each entry
  for (hrx::property_tree::ptree::const_iterator ptEntry = _mirrorData.begin();
...
static void
readJSONFile(const std::string& filename, hrx::property_tree::ptree& pt)
{
  ...
  try {
    hrx::property_tree::read_json(fs, pt);
  } catch (const hrx::property_tree::json_parser_error& e) {
    auto errMsg = hrx::format("ERROR: Parsing the file '%s' on line %d: %s") % filename % e.line() % e.message();
    throw std::runtime_error(errMsg.str());
  }
}
...
XclBin::addMergeSection(ParameterSectionData& _PSD)
{
  ...
  hrx::property_tree::ptree ptAll;
  readJSONFile(_PSD.getFile(), ptAll);

  // Find the section of interest
  const std::string jsonNodeName = Section::getJSONOfKind(eKind);
  const hrx::property_tree::ptree ptEmpty;
  const hrx::property_tree::ptree& ptMerge = ptAll.get_child(jsonNodeName, ptEmpty);

  if (ptMerge.empty()) {
    auto errMsg = hrx::format("ERROR: Nothing to add for the section '%s'\n.Either the JSON node name '%s' is missing or the contents of this node is empty.")
  ...
  hrx::property_tree::ptree ptPayload;
  ...
  try {
    ...
    hrx::property_tree::write_json(jsonBuf, ptMerge, true);
  ...
  // Report our success
  XUtil::QUIET("");
  XUtil::QUIET(hrx::format("Section: '%s'(%d) merged successfully with\nFile: '%s'")
...
XclBin::removeSection(const Section* _pSection)
{
  ...
  for (unsigned int index = 0; index < m_sections.size(); ++index) {
    if ((void*)m_sections[index] == (void*)_pSection) {
      XUtil::TRACE(hrx::format("Removing and deleting section '%s' (%d).") % _pSection->getSectionKindAsString() % (unsigned int) _pSection->getSectionKind());
  ...
  auto errMsg = hrx::format("ERROR: Section '%s' (%d) not found") % _pSection->getSectionKindAsString() % (unsigned int) _pSection->getSectionKind();
...
XclBin::removeSection(const std::string& _sSectionToRemove)
{
  ...
  if (sectionIndex != std::string::npos) {
    ...
    if (sectionIndexEndDelimiter != _sSectionToRemove.back()) {
      auto errMsg = hrx::format("Error: Expected format <section>[<section_index>] when using a section index.  Received: %s.") % _sSectionToRemove;
      throw std::runtime_error(errMsg.str());
    }
  ...
  if ((Section::supportsSectionIndex(_eKind) == true) &&
    ...
    auto errMsg = hrx::format("ERROR: Section '%s' can only be deleted with indexes.") % sectionName;
    throw std::runtime_error(errMsg.str());
  }

  if ((Section::supportsSectionIndex(_eKind) == false) &&
      (!sectionIndexName.empty())) {
    auto errMsg = hrx::format("ERROR: Section '%s' cannot be deleted with index values (not supported).") % sectionName;
    throw std::runtime_error(errMsg.str());
  }

  const Section* pSection = findSection(_eKind, sectionIndexName);
  if (pSection == nullptr) {
    auto errMsg = hrx::format("ERROR: Section '%s' is not part of the xclbin archive.") % _sSectionToRemove;
    throw XUtil::XclBinUtilException(xet_missing_section, errMsg.str());
  }
  ...
  XUtil::QUIET("");
  XUtil::QUIET(hrx::format("Section '%s%s'(%d) was successfully removed") 
...
XclBin::replaceSection(ParameterSectionData& _PSD)
{
  ...
  Section* pSection = findSection(eKind);
  if (pSection == nullptr) {
    auto errMsg = hrx::format("ERROR: Section '%s' does not exist.") % _PSD.getSectionName();
    throw XUtil::XclBinUtilException(xet_missing_section, errMsg.str());
  }
  ...
  XUtil::TRACE(hrx::format("Section '%s' (%d) successfully added.") % pSection->getSectionKindAsString() % (unsigned int)  pSection->getSectionKind());
  XUtil::QUIET("");
  XUtil::QUIET(hrx::format("Section: '%s'(%d) was successfully added.\nSize   : %ld bytes\nFormat : %s\nFile   : '%s'")
...
XclBin::updateHeaderFromSection(Section* _pSection)
{
  ...
  if (_pSection->getSectionKind() == BUILD_METADATA) {
    hrx::property_tree::ptree pt;
    _pSection->getPayload(pt);

    hrx::property_tree::ptree ptDsa;
    ptDsa = pt.get_child("build_metadata.dsa");

    auto feature_roms = XUtil::as_vector<hrx::property_tree::ptree>(ptDsa, "feature_roms");

    hrx::property_tree::ptree featureRom;
...
XclBin::addSubSection(ParameterSectionData& _PSD)
{
  ...
  if (Section::supportsSubSections(eKind) == false) {
    auto errMsg = hrx::format("ERROR: Section '%s' doesn't support sub sections.") % _PSD.getSectionName();
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  if (pSection != nullptr) {
    ...
    if (Section::supportsSubSectionName(pSection->getSectionKind(), sSubSection) == false) {
      auto errMsg = hrx::format("ERROR: Section '%s' does not support the subsection: '%s'") % pSection->getSectionKindAsString() % sSubSection;
      throw std::runtime_error(hrx::str(errMsg));
    }
    ...
    if (pSection->subSectionExists(_PSD.getSubSectionName()) == true) {
      auto errMsg = hrx::format("ERROR: Section '%s' subsection '%s' already exists") % pSection->getSectionKindAsString() % sSubSection;
      throw std::runtime_error(hrx::str(errMsg));
    }
  } else {
  ...
  XUtil::TRACE(hrx::str(hrx::format(
  ...
  if (!(pSection->getSectionIndexName().empty())) 
    optionalIndex = (hrx::format("[%s]") % pSection->getSectionIndexName()).str();

  XUtil::QUIET("");
  XUtil::QUIET(hrx::str(hrx::format(
...
XclBin::addSection(ParameterSectionData& _PSD)
{
  ...
  Section* pSection = findSection(eKind);
  if (pSection != nullptr) {
    auto errMsg = hrx::format("ERROR: Section '%s' already exists.") % _PSD.getSectionName();
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  if (Section::doesSupportAddFormatType(pSection->getSectionKind(), _PSD.getFormatType()) == false) {
    auto errMsg = hrx::format("ERROR: The %s section does not support reading the %s file type.")
                                % pSection->getSectionKindAsString()
  ...
  XUtil::TRACE(hrx::str(hrx::format("Section '%s' (%d) successfully added.") % sSectionAddedName % (unsigned int) pSection->getSectionKind()));
  XUtil::QUIET("");
  XUtil::QUIET(hrx::str(hrx::format(
...
XclBin::addSections(ParameterSectionData& _PSD)
{
  ...
  if (_PSD.getFormatType() != Section::FormatType::json) {
    auto errMsg = hrx::format("ERROR: Expecting JSON format type, got '%s'.") % _PSD.getFormatTypeAsStr();
    throw std::runtime_error(errMsg.str());
  }
  ...
  try {
    ...
    auto errMsg = hrx::format("ERROR: Parsing the file '%s' on line %d: %s") % sJSONFileName % e.line() % e.message();
    throw std::runtime_error(errMsg.str());
  }
  ...
  for (hrx::property_tree::ptree::iterator ptSection = pt.begin(); ptSection != pt.end(); ++ptSection) {
    ...
    Section* pSection = findSection(eKind);
    if (pSection != nullptr) {
      auto errMsg = hrx::format("ERROR: Section '%s' already exists.") % pSection->getSectionKindAsString();
      throw std::runtime_error(errMsg.str());
    }
    ...
    try {
      ...
      std::cerr << "       Exception Message: " << e.what() << "\n";
      std::ostringstream jsonBuf;
      hrx::property_tree::write_json(jsonBuf, pt, true);
      std::cerr << jsonBuf.str() << "\n";
      throw std::runtime_error("Aborting remaining operations");
    }
    ...
    if (pSection->getSize() == 0) {
      XUtil::QUIET("");
      XUtil::QUIET(hrx::format("Section: '%s'(%d) was empty.  No action taken.\nFormat : %s\nFile   : '%s'")
                                 % pSection->getSectionKindAsString()
                                 % (unsigned int) pSection->getSectionKind()
  ...
  for (hrx::property_tree::ptree::iterator ptSection = pt.begin(); ptSection != pt.end(); ++ptSection) {
    ...
    if (pSection == nullptr) {
      ...
      if ((eKind == PARTITION_METADATA) ||
        ...
        auto errMsg = hrx::format("ERROR: Section '%s' doesn't exists for JSON key '%s'.  Must have an existing section in order to append.") % pTempSection->getSectionKindAsString() % sectionName;
        throw std::runtime_error(errMsg.str());
      }
    }
    ...
    try {
      ...
      std::ostringstream jsonBuf;
      hrx::property_tree::write_json(jsonBuf, ptSection->second, true);
      std::cerr << jsonBuf.str() << "\n";
      throw std::runtime_error("Aborting remaining operations");
    }
    ...
    XUtil::TRACE(hrx::format("Section '%s' (%d) successfully appended to.") % pSection->getSectionKindAsString() % (unsigned int) pSection->getSectionKind());
    XUtil::QUIET("");
    XUtil::QUIET(hrx::format("Section: '%s'(%d) was successfully appended to.\nFormat : %s\nFile   : '%s'")
                               % pSection->getSectionKindAsString()
                               % (unsigned int) pSection->getSectionKind()
                               % _PSD.getFormatTypeAsStr() % sectionName);
  }
...
XclBin::dumpSubSection(ParameterSectionData& _PSD)
{
  ...
  Section* pSection = findSection(eKind, _PSD.getSectionIndexName());
  if (pSection == nullptr) {
    auto errMsg = hrx::format("ERROR: Section %s[%s] does not exist.") % _PSD.getSectionName() % _PSD.getSectionIndexName();
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  if (pSection->subSectionExists(_PSD.getSubSectionName()) == false) {
    auto errMsg = hrx::format("ERROR: Section '%s' subsection '%s' doesn't exists") % pSection->getSectionKindAsString() % sSubSection;
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  XUtil::TRACE(hrx::format("Section '%s' (%d) dumped.") % pSection->getSectionKindAsString() % (unsigned int) pSection->getSectionKind());
  XUtil::QUIET("");

  std::string optionalIndex;
  if (!(pSection->getSectionIndexName().empty())) 
    optionalIndex = hrx::str(hrx::format("[%s]") % pSection->getSectionIndexName());

  XUtil::QUIET(hrx::format("Section: '%s%s%s%s'(%d) was successfully written.\nFormat : %s\nFile   : '%s'")
                             % pSection->getSectionKindAsString()
                             % optionalIndex
                             % (sSubSection.empty() ? "" : "-")
                             % sSubSection % (unsigned int) pSection->getSectionKind()
...
XclBin::dumpSection(ParameterSectionData& _PSD)
{
  ...
  Section* pSection = findSection(eKind);
  if (pSection == nullptr) {
    auto errMsg = hrx::format("ERROR: Section '%s' does not exists.") % _PSD.getSectionName();
    throw XUtil::XclBinUtilException(xet_missing_section, hrx::str(errMsg));
  }
  ...
  if (Section::doesSupportDumpFormatType(pSection->getSectionKind(), _PSD.getFormatType()) == false) {
    auto errMsg = hrx::format("ERROR: The %s section does not support writing to a %s file type.")
                                % pSection->getSectionKindAsString()
                                % _PSD.getFormatTypeAsStr();
    throw std::runtime_error(hrx::str(errMsg));
  }
  ...
  XUtil::TRACE(hrx::format("Section '%s' (%d) dumped.") % pSection->getSectionKindAsString() % (unsigned int) pSection->getSectionKind());
  XUtil::QUIET("");
  XUtil::QUIET(hrx::format("Section: '%s'(%d) was successfully written.\nFormat: %s\nFile  : '%s'")
                             % pSection->getSectionKindAsString()
                             % (unsigned int) pSection->getSectionKind()
                             % _PSD.getFormatTypeAsStr() % sDumpFileName);
}
...
XclBin::dumpSections(ParameterSectionData& _PSD)
{
  ...
  switch (_PSD.getFormatType()) {
    case Section::FormatType::json: {
        ...
        hrx::property_tree::write_json(oDumpFile, pt, true /*Pretty print*/);
        break;
      }
  ...
  XUtil::QUIET("");
  XUtil::QUIET(hrx::format("Successfully wrote all of sections which support the format '%s' to the file: '%s'")
                             % _PSD.getFormatTypeAsStr() % sDumpFileName);
}
                              ...
                              std::string& _value)
{
  ...
  if (tokens.size() != 3) {
    auto errMsg = hrx::format("ERROR: Expected format [USER | SYS]:<key>:<value> when using adding a key value pair.  Received: %s.") % _keyValue;
    throw std::runtime_error(errMsg.str());
  }

  hrx::to_upper(tokens[0]);
  _domain = tokens[0];
  _key = tokens[1];
  _value = tokens[2];
}
...
XclBin::setKeyValue(const std::string& _keyValue)
{
  ...
  XUtil::TRACE(hrx::format("Setting key-value pair \"%s\":  domain:'%s', key:'%s', value:'%s'")
  ...
  if (sDomain == "SYS") {
    if (sKey == "mode") {
      if (sValue == "flat") {
        ...
        auto errMsg = hrx::format("ERROR: Unknown value '%s' for key '%s'. Key-value pair: '%s'.") % sValue % sKey % _keyValue;
        throw std::runtime_error(errMsg.str());
      }
    ...
    if (sKey == "action_mask") {
      if (sValue == "LOAD_AIE") {
        ...
        auto errMsg = hrx::format("ERROR: Unknown bit mask '%s' for the key '%s'. Key-value pair: '%s'.") % sValue % sKey % _keyValue;
        throw std::runtime_error(errMsg.str());
      }
      return; // Key processed
    }   
    ...
    auto errMsg = hrx::format("ERROR: Unknown key '%s' for key-value pair '%s'.") % sKey % _keyValue;
    throw std::runtime_error(errMsg.str());
  }
  ...
  if (sDomain == "USER") {
    ...
    hrx::property_tree::ptree ptKeyValueMetadata;
    pSection->getPayload(ptKeyValueMetadata);

    XUtil::TRACE_PrintTree("KEYVALUE:", ptKeyValueMetadata);
    hrx::property_tree::ptree ptKeyValues = ptKeyValueMetadata.get_child("keyvalue_metadata");
    auto keyValues = XUtil::as_vector<hrx::property_tree::ptree>(ptKeyValues, "key_values");
    ...
    if (bKeyFound == false) {
      hrx::property_tree::ptree keyValue;
    ...
    hrx::property_tree::ptree ptKeyValuesNew;
    for (const auto &keyvalue : keyValues) {
      ptKeyValuesNew.push_back({"", keyvalue});
    }

    hrx::property_tree::ptree ptKeyValueMetadataNew;
  ...
  auto errMsg = hrx::format("ERROR: Unknown key domain for key-value pair '%s'.  Expected either 'USER' or 'SYS'.") % sDomain;
  throw std::runtime_error(errMsg.str());
}
...
XclBin::removeKey(const std::string& _sKey)
{
  ...
  XUtil::TRACE(hrx::format("Removing User Key: '%s'") % _sKey);

  Section* pSection = findSection(KEYVALUE_METADATA);
  if (pSection == nullptr) {
    auto errMsg = hrx::format("ERROR: Key '%s' not found.") % _sKey;
    throw std::runtime_error(errMsg.str());
  }
...
// Note: A file name can contain a colen (e.g., C:\test)
{
  ...
  XUtil::TRACE(hrx::format("PSKernel command arguments: mem_banks='%s', symbol_name='%s'; num_instances=%d; library='%s'") % mem_banks % symbol_name % num_instances % path_to_library);
}
...
void putSectionPayload(XclBin* pXclBin,
                       axlf_section_kind kind,
                       const hrx::property_tree::ptree& ptPayLoad)
{
...
void
updateKernelSections(const std::vector<hrx::property_tree::ptree> &kernels,
                     bool isFixedPS,
                     XclBin *pXclbin)
{
  for (const auto& kernel : kernels) {
    hrx::property_tree::ptree ptEmbedded;
    hrx::property_tree::ptree ptIPLayout;
    hrx::property_tree::ptree ptConnectivity;
    hrx::property_tree::ptree ptMemTopology;
...
XclBin::addPsKernel(const std::string& encodedString)
{
  ...
  hrx::property_tree::ptree ptFunctions;
  ...
  hrx::property_tree::ptree ptPSKernels;
  ...
  const hrx::property_tree::ptree ptKernels = ptPSKernels.get_child("ps-kernels", ptEmpty);
  auto kernels = XUtil::as_vector<hrx::property_tree::ptree>(ptKernels, "kernels");
  ...
  for (const auto& ptKernel : kernels) {
    ...
    if (pSection != nullptr) {
      auto errMsg = hrx::format("ERROR: The PS Kernel (e.g SOFT_KERNEL) section with the symbolic name '%s' already exists") % kernelName;
      throw std::runtime_error(errMsg.str());
    }

    // Create the section
    pSection = Section::createSectionObjectOfKind(SOFT_KERNEL, kernelName);
    XUtil::TRACE(hrx::format("Adding PS Kernel SubSection '%s' OBJ") % kernelName);
    ...
    // -- Add the metadata
    XUtil::TRACE(hrx::format("Adding PS Kernel SubSection '%s' METADATA") % kernelName);
    hrx::property_tree::ptree ptPsKernel;
    ...
    hrx::property_tree::ptree ptRTD;
    ptRTD.add_child("soft_kernel_metadata", ptPsKernel);

    std::ostringstream buffer;
    hrx::property_tree::write_json(buffer, ptRTD);
    ...
    XUtil::QUIET("");
    XUtil::QUIET(hrx::format("Section: SOFT_KERNEL (PS KERNEL), SubName: '%s' was successfully added.") % kernelName);
  }
}
...
XclBin::addKernels(const std::string& jsonFile)
{
  ...
  hrx::property_tree::ptree ptFixKernels;
  hrx::property_tree::read_json(ifFixedKernels, ptFixKernels);
  XUtil::TRACE_PrintTree("Fixed Kernels Metadata", ptFixKernels);

  const hrx::property_tree::ptree ptEmpty;    // Empty ptree

  // Get the kernels from the property_tree
  const hrx::property_tree::ptree ptKernels = ptFixKernels.get_child("ps-kernels", ptEmpty);
  auto kernels = XUtil::as_vector<hrx::property_tree::ptree>(ptKernels, "kernels");
  if (kernels.empty()) {
...
XclBin::updateInterfaceuuid()
{
  ...
  hrx::property_tree::ptree ptRoot;
  ...
  hrx::property_tree::ptree ptPartitionMetadata = ptRoot.get_child("partition_metadata", ptEmpty);
  if (ptPartitionMetadata.empty()) {
    throw std::runtime_error("ERROR: Partition metadata node not found.");
  }

  // Look for the "interfaces" node
  auto ptInterfaces = XUtil::as_vector<hrx::property_tree::ptree>(ptPartitionMetadata, "interfaces");
  ...
  hrx::property_tree::ptree ptInterface = ptInterfaces[0];
