...
// Note: A file name can contain a colen (e.g., C:\test)
{
  ...
  if (tokens.size() != 3) {
    auto errMsg = hrx::format("Error: Expected format <section>:<format>:<file> when using adding a section.  Received: %s.") % _formattedString;
  ...
  if (!sSection.empty()) {
    ...
    if (sectionIndex != std::string::npos) {
      ...
      if (sectionIndexEndDelimiter != sSection.back()) {
        auto errMsg = hrx::format("Error: Expected format <section>[<section_index>]:<format>:<file> when using a section index.  Received: %s.") % _formattedString;
        throw std::runtime_error(errMsg.str());
      }
    ...
    if (m_section.empty()) {
      auto errMsg = hrx::format("Error: Missing section name. Expected format <section>[<section_index]:<format>:<file> when using a section index.  Received: %s.") % _formattedString;
      throw std::runtime_error(errMsg.str());
    }
    ...
    if (!m_subSection.empty() && (Section::supportsSubSections(eKind) == false)) {
      auto errMsg = hrx::format("Error: The section '%s' doesn't support subsections (e.g., '%s').") % m_section % m_subSection;
      throw std::runtime_error(errMsg.str());
    }

    // Does the section support section indexes
    if (!m_sectionIndex.empty() && (Section::supportsSectionIndex(eKind) == false)) {
      auto errMsg = hrx::format("Error: The section '%s' doesn't support section indexes (e.g., '%s').") % m_section % m_sectionIndex;
      throw std::runtime_error(errMsg.str());
    }
  }
}
