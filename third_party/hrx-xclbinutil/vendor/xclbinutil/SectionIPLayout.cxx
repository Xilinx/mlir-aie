...
{
  auto sectionInfo = std::make_unique<SectionInfo>(IP_LAYOUT, "IP_LAYOUT", hrx::factory<SectionIPLayout*>());
...
SectionIPLayout::getIPTypeStr(IP_TYPE _ipType) const
{
  ...
  return (hrx::format("UNKNOWN (%d)") % (unsigned int)_ipType).str();
}
...
SectionIPLayout::getIPControlTypeStr(IP_CONTROL _ipControlType) const
{
  ...
  return (hrx::format("UNKNOWN (%d)") % (unsigned int)_ipControlType).str();
}
...
SectionIPLayout::getFunctionalStr(PS_FUNCTIONAL eFunctional) const
{
  ...
  return (hrx::format("UNKNOWN (%d)") % static_cast<unsigned int>(eFunctional)).str();
}
...
SectionIPLayout::getFunctionalEnumStr(const std::string& sFunctional) 
{
  ...
  try {
    ...
    sFunctionalEnum = (hrx::format("%d") % static_cast<unsigned int>(eFunctional)).str();
...
SectionIPLayout::getSubTypeStr(PS_SUBTYPE eSubType) const
{
  ...
  return (hrx::format("UNKNOWN (%d)") % static_cast<unsigned int>(eSubType)).str();
}
...
SectionIPLayout::getSubTypeEnumStr(const std::string& sSubType)
{
  ...
  try {
    ...
    sSubTypeEnum = (hrx::format("%d") % static_cast<unsigned int>(eSubType)).str();
  } catch (const std::runtime_error&) {
...
SectionIPLayout::marshalToJSON(char* _pDataSection,
                               ...
                               hrx::property_tree::ptree& _ptree) const
{
  XUtil::TRACE("");
  ...
  if (_sectionSize == 0) {
    ...
    hrx::property_tree::ptree iplayout;
    iplayout.put("m_count", "0");
    hrx::property_tree::ptree ipData;
  ...
  if (_sectionSize < sizeof(ip_layout)) {
    auto errMsg = hrx::format("ERROR: Section size (%d) is smaller than the size of the ip_layout structure (%d)")
  ...
  XUtil::TRACE(hrx::format("m_count: %d") % pHdr->m_count);
  ...
  ptIPLayout.put("m_count", (hrx::format("%d") % (unsigned int)pHdr->m_count).str());

  uint64_t expectedSize = ((uint64_t)&(pHdr->m_ip_data[0]) - (uint64_t)pHdr) + (sizeof(ip_data) * pHdr->m_count);

  if (_sectionSize != expectedSize) {
    auto errMsg = hrx::format("ERROR: Section size (%d) does not match expected section size (%d).") % _sectionSize % expectedSize;
    throw std::runtime_error(errMsg.str());
  }

  hrx::property_tree::ptree ptIPData;
  for (int index = 0; index < pHdr->m_count; ++index) {
    hrx::property_tree::ptree ptIPEntry;
    ...
    if (((IP_TYPE)pHdr->m_ip_data[index].m_type == IP_MEM_DDR4) ||
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %s, m_index: %d, m_pc_index: %d, m_base_address: 0x%lx, m_name: '%s'")
                   % index
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %s, properties: 0x%x {m_ip_control: %s, m_interrupt_id: %d, m_int_enable: %d}, m_base_address: 0x%lx, m_name: '%s'")
                   % index
                   % getIPTypeStr((IP_TYPE)pHdr->m_ip_data[index].m_type)
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %s, m_subtype: %s, m_functional: %s, m_kernel_id: 0x%x, m_base_address: 0x%lx, m_name: '%s'")
                   % index
                   % getIPTypeStr((IP_TYPE)pHdr->m_ip_data[index].m_type)
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %s, properties: 0x%x, m_base_address: 0x%lx, m_name: '%s'")
                   % index
                   % getIPTypeStr((IP_TYPE)pHdr->m_ip_data[index].m_type)
                   % pHdr->m_ip_data[index].properties
                   % pHdr->m_ip_data[index].m_base_address
    ...
    switch ((IP_TYPE)pHdr->m_ip_data[index].m_type) {
      ...
      case IP_MEM_HBM_ECC:
      {
        ptIPEntry.put("m_index", (hrx::format("%d") % (unsigned int)pHdr->m_ip_data[index].indices.m_index).str());
        ptIPEntry.put("m_pc_index", (hrx::format("%d") % (unsigned int)pHdr->m_ip_data[index].indices.m_pc_index).str());
        break;
      }
      case IP_KERNEL:
      {
        ptIPEntry.put("m_int_enable", (hrx::format("%d") % ((pHdr->m_ip_data[index].properties & ((uint32_t)IP_INT_ENABLE_MASK)))).str());
        ptIPEntry.put("m_interrupt_id", (hrx::format("%d") % (((pHdr->m_ip_data[index].properties & ((uint32_t)IP_INTERRUPT_ID_MASK)) >> IP_INTERRUPT_ID_SHIFT))).str());
        std::string sIPControlType = getIPControlTypeStr((IP_CONTROL)((pHdr->m_ip_data[index].properties & ((uint32_t)IP_CONTROL_MASK)) >> IP_CONTROL_SHIFT));
      ...
      case IP_PS_KERNEL:
      {
        ...
        if ((PS_SUBTYPE)pHdr->m_ip_data[index].ps_kernel.m_subtype == ST_DPU) { 
          ...
          ptIPEntry.put("m_kernel_id", (hrx::format("0x%x") % (unsigned int)pHdr->m_ip_data[index].ps_kernel.m_kernel_id).str());
        } else {
          ptIPEntry.put("properties", (hrx::format("0x%x") % pHdr->m_ip_data[index].properties).str());
        }
        break;
      }
      default:
    ...
    if (pHdr->m_ip_data[index].m_base_address != ((uint64_t)-1)) {
      ptIPEntry.put("m_base_address", (hrx::format("0x%lx") % pHdr->m_ip_data[index].m_base_address).str());
    } else {
      ptIPEntry.put("m_base_address", "not_used");
    }
    ptIPEntry.put("m_name", (hrx::format("%s") % pHdr->m_ip_data[index].m_name).str());
...
void
SectionIPLayout::marshalFromJSON(const hrx::property_tree::ptree& _ptSection,
                                 std::ostringstream& _buf) const
{
  const hrx::property_tree::ptree& ptIPLayout = _ptSection.get_child("ip_layout");
  ...
  XUtil::TRACE(hrx::format("m_count: %d") % ipLayoutHdr.m_count);
  ...
  hrx::property_tree::ptree ipDatas = ptIPLayout.get_child("m_ip_data");
  for (const auto& kv : ipDatas) {
    ip_data ipDataHdr = ip_data{};
    hrx::property_tree::ptree ptIPData = kv.second;
    ...
    switch (ipDataHdr.m_type) {
      ...
      default:
      {
        ...
        { // m_interrupt_id
          ...
          if (!sInterruptID.empty()) {
            ...
            if (interruptID > maxValue) {
              const auto errMsg = hrx::format("ERROR: The m_interrupt_id (%d), exceeds maximum value (%d).") % interruptID % maxValue;
              throw std::runtime_error(errMsg.str());
            }
        ...
        { // m_ip_control
          hrx::optional<std::string> bIPControl;
          ...
          if (bIPControl.is_initialized()) {
            ...
            if (ipControl > maxValue) {
              const auto errMsg = hrx::format("ERROR: The m_ip_control (%d), exceeds maximum value (%d).") % (unsigned int)ipControl % maxValue;
              throw std::runtime_error(errMsg.str());
            }
    ...
    if (sm_name.length() >= sizeof(ip_data::m_name)) {
      const auto errMsg = hrx::format("ERROR: The m_name entry length (%d), exceeds the allocated space (%d).  Name: '%s'")
    ...
    if ((ipDataHdr.m_type == IP_MEM_DDR4) ||
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %d, m_index: %d, m_pc_index: %d, m_base_address: 0x%lx, m_name: '%s'")
                   % count
      ...
      XUtil::TRACE(hrx::format("[%d]: m_type: %d, properties: 0x%x, m_base_address: 0x%lx, m_name: '%s'")
                   % count
                   % (unsigned int)ipDataHdr.m_type
      ...
      if ((PS_SUBTYPE)ipDataHdr.ps_kernel.m_subtype == ST_DPU) { 
        XUtil::TRACE(hrx::format("[%d]: m_type: %d, m_subtype: %d, m_functional: %d, m_kernel_id: 0x%x, m_base_address: 0x%lx, m_name: '%s'")
                   % count
                   % (unsigned int)ipDataHdr.m_type
  ...
  if (count != (unsigned int)ipLayoutHdr.m_count) {
    const auto errMsg = hrx::format("ERROR: Number of connection sections (%d) does not match expected encoded value: %d")
  ...
  if (bufferSize > maxBufferSize) {
    const auto errMsg = hrx::format("CRITICAL WARNING: The buffer size for the IP_LAYOUT section (%d) exceed the maximum size of %d.\nThis can result in lose of data in the driver.")
...
template<typename T>
std::vector<T> as_vector(hrx::property_tree::ptree const& pt,
                         hrx::property_tree::ptree::key_type const& key)
{
  std::vector<T> r;
...
void
SectionIPLayout::appendToSectionMetadata(const hrx::property_tree::ptree& _ptAppendData,
                                         hrx::property_tree::ptree& _ptToAppendTo)
{
  XUtil::TRACE_PrintTree("To Append To", _ptToAppendTo);
  XUtil::TRACE_PrintTree("Append data", _ptAppendData);


  auto ip_datas = as_vector<hrx::property_tree::ptree>(_ptAppendData, "m_ip_data");
  unsigned int appendCount = _ptAppendData.get<unsigned int>("m_count");

  if (appendCount != ip_datas.size()) {
    auto errMsg = hrx::format("ERROR: IP layout section append's count (%d) does not match the number of ip_data entries (%d).") % appendCount % ip_datas.size();
    throw std::runtime_error(errMsg.str());
  }
  ...
  // Now copy the data
  hrx::property_tree::ptree& ptIPLayoutAppendTo = _ptToAppendTo.get_child("ip_layout");
  hrx::property_tree::ptree& ptDest_m_ip_data = ptIPLayoutAppendTo.get_child("m_ip_data");

  for (auto ip_data : ip_datas) {
    hrx::property_tree::ptree new_ip_data;
