...
FDTProperty::getWordLength(DataFormat _eDataFormat) const
{
  ...
  auto errMsg = hrx::format("ERROR: Unknown data format: %d") % (unsigned int)_eDataFormat;
    ...
    : FDTProperty()
{
  ...
  if (_size < sizeof(FDTLenOffset)) {
    auto errMsg = hrx::format("ERROR: The given property buffer's header size (%d bytes) is smaller then its header (%d bytes).") %  _size % sizeof(FDTLenOffset);
    throw std::runtime_error(errMsg.str());
  }
  ...
  XUtil::TRACE(hrx::format("Property Name: '%s', length: %d, type: %s") % m_name % m_dataLength % prettyTypeName);
...
void
FDTProperty::au8MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  XUtil::TRACE("   Type: Array of 8 bits");

  const uint8_t* uint8Array = (const uint8_t*)m_pDataBuffer;

  hrx::property_tree::ptree ptProperty;
  for (unsigned int index = 0; index < m_dataLength; ++index) {
    hrx::property_tree::ptree ptChildArrayElement;
    ptChildArrayElement.put("", (hrx::format("0x%x") % uint8Array[index]).str());
...
void
FDTProperty::au16MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  if ((m_dataLength % byteBoundary) != 0) {
    auto errMsg = hrx::format("ERROR: Data length (%d) does not end on a 2-byte boundary.") % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }
  ...
  for (unsigned int index = 0; index < numElements; ++index) {
    hrx::property_tree::ptree ptChildArrayElement;
    ptChildArrayElement.put("", (hrx::format("0x%x") % ntohs(uint16Array[index])).str());
    ptProperty.push_back({ "", ptChildArrayElement });
  }
  _ptTree.add_child(m_name.c_str(), ptProperty);
}
...
void
FDTProperty::au32MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  if ((m_dataLength % byteBoundary) != 0) {
    auto errMsg = hrx::format("ERROR: Data length (%d) does not end on a 4-byte boundary.") % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }
  ...
  for (unsigned int index = 0; index < numElements; ++index) {
    hrx::property_tree::ptree ptChildArrayElement;
    ptChildArrayElement.put("", (hrx::format("0x%x") % ntohl(uint32Array[index])).str());
    ptProperty.push_back({ "", ptChildArrayElement });
  }
  _ptTree.add_child(m_name.c_str(), ptProperty);
}
...
void
FDTProperty::u16MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  _ptTree.put(m_name.c_str(), (hrx::format("0x%x") % uint16Value).str());
}

void
FDTProperty::u32MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  XUtil::TRACE("   Type: 32 bits");

  // Check and make sure that all is good
  if (m_dataLength != sizeof(uint32_t)) {
    auto errMsg = hrx::format("ERROR: Data length for a 32-bit word is invalid: Expected: %d, Actual: %d") % sizeof(uint32_t) % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }

  const uint32_t uint32Value = ntohl(*((const uint32_t*)m_pDataBuffer));
  _ptTree.put(m_name.c_str(), (hrx::format("0x%x") % uint32Value).str());
}

void
FDTProperty::u128MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  if (m_dataLength != expectedSize) {
    auto errMsg = hrx::format("ERROR: Data length for a 128-bit word is invalid: Expected: %d, Actual: %d") % expectedSize % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }
  ...
  _ptTree.put(m_name.c_str(), (hrx::format("0x%s") % s128Hex).str());
}

void
FDTProperty::u64MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  XUtil::TRACE("   Type: 64 bits");

  // Check and make sure that all is good
  if (m_dataLength != sizeof(uint64_t)) {
    auto errMsg = hrx::format("ERROR: Data length for a 64-bit word is invalid: Expected: %d, Actual: %d") % sizeof(uint64_t) % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }

  std::string s64Hex;
  XUtil::binaryBufferToHexString((const unsigned char*)m_pDataBuffer, sizeof(uint64_t), s64Hex);
  _ptTree.put(m_name.c_str(), (hrx::format("0x%s") % s64Hex).str());
}

void
FDTProperty::au64MarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  if ((m_dataLength % byteBoundary) != 0) {
    auto errMsg = hrx::format("ERROR: Data length (%d) does not end on a 8-byte boundary.") % m_dataLength;
    throw std::runtime_error(errMsg.str());
  }
  ...
  for (unsigned int index = 0; index < m_dataLength; index += byteBoundary) {
    ...
    ptChildArrayElement.put("", (hrx::format("0x%s") % s64Hex).str());
    ptProperty.push_back({ "", ptChildArrayElement });
  }
...
void
FDTProperty::aszMarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
  ...
  for (auto sString : arrayOfStrings) {
    ...
    ptProperty.push_back({ "", ptChildArrayElement });
    XUtil::TRACE(hrx::format("String: %s") % sString);
  }
...
void
FDTProperty::szMarshalToJSON(hrx::property_tree::ptree& _ptTree) const
{
                           ...
                           const std::string& _sData)
{
  ...
  XUtil::TRACE(hrx::format("Storing property: '%s' with value: '%s'") % m_name % _sData);
  ...
  switch (_eDataFormat) {
    ...
    case DataFormat::au8: {
        ...
        if (dataWord > UINT8_MAX) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' exceeds the maximum byte storage space'.") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }
    ...
    case DataFormat::u16: {
        ...
        if (dataWord > UINT16_MAX) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' exceeds the maximum uint16_t storage space.") %  m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }
    ...
    case DataFormat::u32: {
        ...
        if (dataWord > UINT32_MAX) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' exceeds the maximum uint32_t storage space.") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }
    ...
    case DataFormat::u64: {
        ...
        uint64_t dataWord = std::strtoul(_sData.c_str(), NULL, 0);
        if (errno == ERANGE) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' exceeds the maximum uint64_t storage space.") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }
    ...
    case DataFormat::u128: {
        ...
        if ((_sData.compare(0, 2, "0x") != 0) &&
          ...
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' must be a hex value (e.g., start with '0x').") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }

        // Must be of even length
        if ((_sData.size() % 2) != 0) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' doesn't support nibble length values, must be full byte values.") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }

        // Must not be too long
        if (_sData.size() > 34) {
          auto errMsg = hrx::format("ERROR: Property '%s' data value '%s' exceeds the maximum uint128_t storage space.") % m_name % _sData;
          throw std::runtime_error(errMsg.str());
        }
    ...
    default: {
        auto errMsg = hrx::format("ERROR: Unknown data type for property '%s'") % m_name;
        throw std::runtime_error(errMsg.str());
      }
      break;
  }
...
void
FDTProperty::marshalDataFromJSON(hrx::property_tree::ptree::const_iterator& _iter,
                                 const PropertyNameFormat& _propertyNameFormat)
{
  // Get the name
  m_name = _iter->first;
  ...
  const hrx::property_tree::ptree& ptData = _iter->second;
  ...
  if ((arraySize > 1) && !isDataFormatArray(m_eDataFormat)) {
    auto errMsg = hrx::format("ERROR: Array of data found for the variable: '%s'") % m_name;
    throw std::runtime_error(errMsg.str());
  }
...
FDTProperty::FDTProperty(hrx::property_tree::ptree::const_iterator& _iter,
                         const PropertyNameFormat& _propertyNameFormat)
    : FDTProperty()
{
...
void
FDTProperty::marshalToJSON(hrx::property_tree::ptree& _ptTree,
                           const PropertyNameFormat& _propertyNameFormat) const
{
  ...
  XUtil::TRACE(hrx::format("-- Serializing Property: '%s', Type: %s") % m_name % sTypeName);
