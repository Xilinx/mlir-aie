...
Section::addSectionType(std::unique_ptr<SectionInfo> sectionInfo)
{
  ...
  if (sectionInfo->name.empty()) {
    auto errMsg = hrx::format("ERROR: CMD name for the section kind (%d) is empty. This needs to be defined.") % sectionInfo->eKind;
  ...
  if (std::any_of(sections.begin(), sections.end(), [&](const auto& entry) {return entry->eKind == sectionInfo->eKind;})) {
    auto errMsg = hrx::format("ERROR: Attempting to register (%d : %s). Constructor enum of kind (%d) already registered.")
  ...
  // Is the cmd name already registered
  {
    ...
    if (iter != sections.end()) {
      auto errMsg = hrx::format("ERROR: Attempting to register: (%d : %s). Constructor name '%s' already registered to eKind (%d).")
  ...
  if (!sectionInfo->nodeName.empty()) {
    ...
    if (iter != sections.end()) {
      auto errMsg = hrx::format("ERROR: Attempting to register: (%d : %s). JSON mapping name '%s' already registered to eKind (%d).")
          % sectionInfo->eKind % sectionInfo->name
                                       ...
                                       axlf_section_kind& eKind)
{
  ...
  if (iter == sections.end()) {
    auto errMsg = hrx::format("ERROR: Section '%s' isn't a valid section name.") % sectionName;
    throw std::runtime_error(errMsg.str());
  }
...
Section::supportsSubSections(axlf_section_kind& eKind)
{
  ...
  if (iter == sections.end()) {
    auto errMsg = hrx::format("ERROR: The section kind value '%d' does not exist.") % eKind;
    throw std::runtime_error(errMsg.str());
  }
...
Section::supportsSubSectionName(axlf_section_kind eKind, const std::string& sSubSectionName)
{
  ...
  return std::any_of(subSections.begin(), subSections.end(), [&](const auto& entry) {return hrx::iequals(entry, sSubSectionName);});
}
...
{
  auto iter = std::find_if(formatTypeTable.begin(), formatTypeTable.end(), [&](const auto& entry) {return hrx::iequals(entry.first, sFormatType);});
...
Section::getKindOfJSON(const std::string& nodeName)
{
  ...
  if (iter == sections.end()) {
    auto errMsg = hrx::format("ERROR: Node name '%s' does not map to a given section type.") % nodeName;
    throw std::runtime_error(errMsg.str());
  }
...
Section::getJSONOfKind(axlf_section_kind eKind)
{
  ...
  if (iter == sections.end()) {
    auto errMsg = hrx::format("ERROR: The given enum kind (%d) does not exist.") % eKind;
    throw std::runtime_error(errMsg.str());
  }
                                   ...
                                   const std::string sIndexName)
{
  ...
  if (iter == sections.end()) {
    auto errMsg = hrx::format("ERROR: Section constructor for the archive section ID '%d' does not exist.  This error is most likely the result of examining a newer version of an archive image than this version of software supports.") % eKind;
    throw std::runtime_error(errMsg.str());
  }
  ...
  XUtil::TRACE(hrx::format("Created segment: %s (%d), index: '%s'")
...
Section::readXclBinBinary(std::istream& _istream, const axlf_section_header& _sectionHeader)
{
  ...
  if ((axlf_section_kind)_sectionHeader.m_sectionKind != getSectionKind()) {
    auto errMsg = hrx::format("ERROR: Unexpected section kind.  Expected: %d, Read: %d") % getSectionKind() % _sectionHeader.m_sectionKind;
    throw std::runtime_error(errMsg.str());
  }
  ...
  XUtil::TRACE(hrx::format("Section: %s (%d)") % getSectionKindAsString() % (unsigned int)getSectionKind());
  XUtil::TRACE(hrx::format("  m_name: %s") % m_name);
  XUtil::TRACE(hrx::format("  m_size: %ld") % m_bufferSize);
}


void
Section::readJSONSectionImage(const hrx::property_tree::ptree& _ptSection)
{
  ...
  if (m_bufferSize == 0) {
    auto errMsg = hrx::format("WARNING: Section '%s' content is empty.  No data in the given JSON file.") % getSectionKindAsString();
                          ...
                          const hrx::property_tree::ptree& _ptSection)
{
  ...
  if (eKind != getSectionKind()) {
    auto errMsg = hrx::format("ERROR: Unexpected section kind.  Expected: %d, Read: %d") % (unsigned int)getSectionKind() % (unsigned int)eKind;
    throw std::runtime_error(errMsg.str());
  }
  ...
  hrx::optional<const hrx::property_tree::ptree&> ptPayload = _ptSection.get_child_optional("payload");

  if (ptPayload.is_initialized()) {
    XUtil::TRACE(hrx::format("Reading in the section '%s' (%d) via metadata.") % getSectionKindAsString() % (unsigned int)getSectionKind());
    readJSONSectionImage(ptPayload.get());
  } else {
    // We don't initialize the buffer via any metadata.  Just read in the section as is
    XUtil::TRACE(hrx::format("Reading in the section '%s' (%d) as a image.") % getSectionKindAsString() % (unsigned int)getSectionKind());
  ...
  XUtil::TRACE(hrx::format("Adding Section: %s (%d)") % getSectionKindAsString() % (unsigned int)getSectionKind());
  XUtil::TRACE(hrx::format("  m_name: %s") % m_name);
  XUtil::TRACE(hrx::format("  m_size: %ld") % m_bufferSize);
}
...
void
Section::getPayload(hrx::property_tree::ptree& _pt) const
{
...
void
Section::appendToSectionMetadata(const hrx::property_tree::ptree& _ptAppendData,
...
void
Section::marshalFromJSON(const hrx::property_tree::ptree& _ptSection,
                         std::ostringstream& _buf) const
{
  XUtil::TRACE_PrintTree("Payload", _ptSection);
  auto errMsg = hrx::format("ERROR: Section '%s' (%d) missing payload parser.") % getSectionKindAsString() % (unsigned int)getSectionKind();
  throw std::runtime_error(errMsg.str());
}
...
Section::readPayload(std::istream& _istream, FormatType _eFormatType)
{
  switch (_eFormatType) {
    ...
    case FormatType::json: {
        ...
        hrx::property_tree::read_json(ss, pt);
...
Section::dumpContents(std::ostream& _ostream, FormatType _eFormatType) const
{
  switch (_eFormatType) {
    ...
    case FormatType::html: {
        ...
        _ostream << hrx::format("<!DOCTYPE html><html><body><h1>Section: %s (%d)</h1><pre>\n") % getSectionKindAsString() % (unsigned int)getSectionKind();
        hrx::property_tree::write_json(_ostream, pt, true /*Pretty print*/);
...
Section::printHeader(std::ostream& _ostream) const
{
  ...
  _ostream << hrx::format("  Type    : '%s'\n") % getSectionKindAsString();
  _ostream << hrx::format("  Name    : '%s'\n") % getName();
  _ostream << hrx::format("  Size    : '%d'\n") % getSize();
}
                        ...
                        Section::FormatType _eFormatType)
{
  ...
  if (m_bufferSize == 0) {
    auto errMsg = hrx::format("WARNING: Section '%s' content is empty.") % getSectionKindAsString();
    throw std::runtime_error(errMsg.str());
  }
...
{
  auto errMsg = hrx::format("FATAL ERROR: Section '%s' virtual method readSubPayLoad() not defined.") % getSectionKindAsString();
  throw std::runtime_error(errMsg.str());
}
...
{
  auto errMsg = hrx::format("FATAL ERROR: Section '%s' virtual method writeSubPayload() not defined.") % getSectionKindAsString();
  throw std::runtime_error(errMsg.str());
}
