...
{
  return hrx::str(hrx::format("%ld") % _xclBinHeader.m_header.m_timeStamp);
}

std::string
FormattedOutput::getFeatureRomTimeStampAsString(const axlf& _xclBinHeader)
{
  return hrx::str(hrx::format("%d") % _xclBinHeader.m_header.m_featureRomTimeStamp);
}

std::string
FormattedOutput::getVersionAsString(const axlf& _xclBinHeader)
{
  return hrx::str(hrx::format("%d.%d.%d") % _xclBinHeader.m_header.m_versionMajor % _xclBinHeader.m_header.m_versionMinor % _xclBinHeader.m_header.m_versionPatch);
}
...
{
  return hrx::str(hrx::format("%s") % _xclBinHeader.m_magic);
}
...
{
  return hrx::str(hrx::format("%ld") % _xclBinHeader.m_header.m_length);
}
...
{
  return hrx::str(hrx::format("%d") %  _xclBinHeader.m_header.m_mode);
}
...
{
  return hrx::str(hrx::format("%s") % _xclBinHeader.m_header.m_platformVBNV);
}
...
{
  return hrx::str(hrx::format("%s") % _xclBinHeader.m_header.m_debug_bin);
}
...
FormattedOutput::getKernelDDRMemory(const std::string& _sKernelInstanceName,
                                    ...
                                    hrx::property_tree::ptree& _ptKernelInstance,
                                    hrx::property_tree::ptree& _ptMemoryConnections)
{
  ...
  hrx::property_tree::ptree ptSections;
  ...
  hrx::property_tree::ptree& ptMemTopology = ptSections.get_child("mem_topology");
  auto memTopology = XUtil::as_vector<hrx::property_tree::ptree>(ptMemTopology, "m_mem_data");

  hrx::property_tree::ptree& ptConnectivity = ptSections.get_child("connectivity");
  auto connectivity = XUtil::as_vector<hrx::property_tree::ptree>(ptConnectivity, "m_connection");

  hrx::property_tree::ptree& ptIPLayout = ptSections.get_child("ip_layout");
  auto ipLayout = XUtil::as_vector<hrx::property_tree::ptree>(ptIPLayout, "m_ip_data");
...
reportXclbinInfo(std::ostream& _ostream,
                 ...
                 hrx::property_tree::ptree& _ptMetaData,
                 const std::vector<Section*> _sections)
{
  ...
  if (!_sInputFile.empty()) {
    try {
      ...
      if (xclBinPKCSStats.is_PKCS_signed) {
        sSignatureState = (hrx::format("Present - Signed PKCS - Offset: 0x%lx, Size: 0x%lx") % xclBinPKCSStats.signature_offset % xclBinPKCSStats.signature_size).str();
      }
    } catch (...) {
      // Do nothing
    }
  ...
  // Generated By:
  {
    ...
    auto sGeneratedBy = (hrx::format("%s (%s) on %s") % sTool % sVersion % sTimeStamp).str();
    if (sTool.empty()) {
      sGeneratedBy = "<unknown>";
    }

    _ostream << hrx::format("   %-23s %s\n") % "Generated by:" 
                                               % sGeneratedBy;
  }

  // Version:
  _ostream << hrx::format("   %-23s %d.%d.%d\n") % "Version:" 
  ...
  // Kernels
  {
    ...
    if (!_ptMetaData.empty()) {
      hrx::property_tree::ptree& ptXclBin = _ptMetaData.get_child("xclbin");
      auto userRegions = XUtil::as_vector<hrx::property_tree::ptree>(ptXclBin, "user_regions");
      for (auto& userRegion : userRegions) {
        auto kernels = XUtil::as_vector<hrx::property_tree::ptree>(userRegion, "kernels");
  ...
  // Action Masks
  {
    ...
    if (_xclBinHeader.m_header.m_actionMask) {
      _ostream << hrx::format("   %-23s ") % "Action Mask(s):";
...

  {
    ...
    for (auto pSection : _sections) {
      ...
      hrx::property_tree::ptree ptPartitionMetadata = ptRoot.get_child("partition_metadata");
      if (ptPartitionMetadata.empty()) {
        continue;
      }

      // Look for the "interfaces" node
      hrx::property_tree::ptree ptInterfaces = ptPartitionMetadata.get_child("interfaces");
      ...
      for (const auto& kv : ptInterfaces) {
        hrx::property_tree::ptree ptInterface = kv.second;
 ...
 */
std::string
getPTreeValue(hrx::property_tree::ptree& _ptMetaData, std::string name)
{
                       ...
                       hrx::property_tree::ptree& _ptMetaData)
{
  ...
  if (!_ptMetaData.empty()) {
    ...
    // Created
    {
      ...
      _ostream << hrx::format("   %-23s %s") % "Created:\n" % sCreated;
    }
...
reportClocks(std::ostream& _ostream,
             const std::vector<Section*> _sections)
{
  hrx::property_tree::ptree ptEmpty;

  _ostream << "Scalable Clocks\n";
  _ostream << "---------------\n";

  hrx::property_tree::ptree ptClockFreqTopology;
  for (Section* pSection : _sections) {
  ...
  auto clockFreqs = XUtil::as_vector<hrx::property_tree::ptree>(ptClockFreqTopology, "m_clock_freq");
  for (unsigned int index = 0; index < clockFreqs.size(); ++index) {
    hrx::property_tree::ptree& ptClockFreq = clockFreqs[index];
    ...
    _ostream << hrx::format("   %-10s %s\n") % "Name:" % sName;
    _ostream << hrx::format("   %-10s %d\n") % "Index:" % index;
    _ostream << hrx::format("   %-10s %s\n") % "Type:" % sType;
    _ostream << hrx::format("   %-10s %s MHz\n") % "Frequency:" % sFreqMhz;
  ...
  //    },
  //    ...
  hrx::property_tree::ptree ptXsa;
  for (const auto& pSection : _sections) {
    if (pSection->getSectionKind() == SYSTEM_METADATA) {
      hrx::property_tree::ptree pt;
      pSection->getPayload(pt);
      if (!pt.empty()) {
        hrx::property_tree::ptree ptSysDiaMet = pt.get_child("system_diagram_metadata", ptEmpty);
  ...
  auto clocks = XUtil::as_vector<hrx::property_tree::ptree>(ptXsa, "clocks");
  if (clocks.empty()) {
  ...
  for (const auto& ptClock : clocks) {
    ...
    if (hrx::iequals(sType, "RESERVED"))
      continue;

    hrx::format systemClockFmt("   %-15s %s %s\n");
                          ...
                          const std::vector<Section*> _sections)
{
  ...
  auto memDatas = XUtil::as_vector<hrx::property_tree::ptree>(ptMemTopology, "m_mem_data");
  for (unsigned int index = 0; index < memDatas.size(); ++index) {
    hrx::property_tree::ptree& ptMemData = memDatas[index];
    ...
    _ostream << hrx::format("   %-13s %s\n") % "Name:" % sName;
    _ostream << hrx::format("   %-13s %d\n") % "Index:" % index;
    _ostream << hrx::format("   %-13s %s\n") % "Type:" % sType;
    _ostream << hrx::format("   %-13s %s\n") % "Base Address:" % sBaseAddress;
    _ostream << hrx::format("   %-13s 0x%lx\n") % "Address Size:" % addressSize;
    _ostream << hrx::format("   %-13s %s\n") % "Bank Used:" % sBankUsed;
              ...
              const std::vector<Section*> _sections)
{
  ...
  std::vector<hrx::property_tree::ptree> memTopology;
  std::vector<hrx::property_tree::ptree> connectivity;
  std::vector<hrx::property_tree::ptree> ipLayout;

  for (auto pSection : _sections) {
    hrx::property_tree::ptree pt;
    if (MEM_TOPOLOGY == pSection->getSectionKind()) {
      pSection->getPayload(pt);
      memTopology = XUtil::as_vector<hrx::property_tree::ptree>(pt.get_child("mem_topology"), "m_mem_data");
    } else if (CONNECTIVITY == pSection->getSectionKind()) {
      pSection->getPayload(pt);
      connectivity = XUtil::as_vector<hrx::property_tree::ptree>(pt.get_child("connectivity"), "m_connection");
    } else if (IP_LAYOUT == pSection->getSectionKind()) {
      pSection->getPayload(pt);
      ipLayout = XUtil::as_vector<hrx::property_tree::ptree>(pt.get_child("ip_layout"), "m_ip_data");
    }
  }
  ...
  for (auto& userRegion : userRegions) {
    ...
    for (auto& ptKernel : kernels) {
      ...
      _ostream << hrx::format("%s %s\n") % "Kernel:" % sKernel;

      auto ports = XUtil::as_vector<hrx::property_tree::ptree>(ptKernel, "ports");
      auto arguments = XUtil::as_vector<hrx::property_tree::ptree>(ptKernel, "arguments");
      auto instances = XUtil::as_vector<hrx::property_tree::ptree>(ptKernel, "instances");
      ...
      // Ports
      {
        ...
        for (auto& ptPort : ports) {
          ...
          _ostream << hrx::format("   %-14s %s\n") % "Port:" % sPort;
          _ostream << hrx::format("   %-14s %s\n") % "Mode:" % sMode;
          _ostream << hrx::format("   %-14s %s\n") % "Range (bytes):" % sRangeBytes;
          _ostream << hrx::format("   %-14s %s bits\n") % "Data Width:" % sDataWidthBits;
          _ostream << hrx::format("   %-14s %s\n") % "Port Type:" % sPortType;
      ...
      for (auto& ptInstance : instances) {
        ...
        for (unsigned int argumentIndex = 0; argumentIndex < arguments.size(); ++argumentIndex) {
          hrx::property_tree::ptree& ptArgument = arguments[argumentIndex];
          ...
          for (auto& ptConnection : connectivity) {
            ...
            if (ipIndex >= ipLayout.size()) {
              auto errMsg = hrx::format("ERROR: connectivity section 'm_ip_layout_index' (%d) exceeds the number of 'ip_layout' elements (%d).  This is usually an indication of curruptions in the xclbin archive.") % ipIndex % ipLayout.size();
            ...
            if (ipLayout[ipIndex].get<std::string>("m_name") == sKernelInstance) {
              if (ptConnection.get<unsigned int>("arg_index") == argumentIndex) {
                ...
                if (memIndex >= memTopology.size()) {
                  auto errMsg = hrx::format("ERROR: connectivity section 'mem_data_index' (%d) exceeds the number of 'mem_topology' elements (%d).  This is usually an indication of curruptions in the xclbin archive.") % memIndex % memTopology.size();
                  throw std::runtime_error(errMsg.str());
                }
                ...
                _ostream << hrx::format("   %-18s %s (%s)\n") % "Memory:" % sMemName.c_str() % sMemType;
              }
            }
          }
          if (!bFoundMemConnection) {
            _ostream << hrx::format("   %-18s <not applicable>\n") % "Memory:";
          }
           ...
           hrx::property_tree::ptree& _ptMetaData)
{
  ...
  // Version
  {
    ...
    _ostream << hrx::format("   %-14s %s - %s (SW BUILD: %s)\n") % "Version:" % sVersion % sTimeStamp % sCL;
  }
                    ...
                    const std::vector<Section*> _sections)
{
  ...
  std::vector<hrx::property_tree::ptree> keyValues;

  for (Section* pSection : _sections) {
    if (pSection->getSectionKind() == KEYVALUE_METADATA) {
      hrx::property_tree::ptree pt;
      pSection->getPayload(pt);
      keyValues = XUtil::as_vector<hrx::property_tree::ptree>(pt.get_child("keyvalue_metadata"), "key_values");
      break;
    }
  }
                      ...
                      const std::vector<Section*> _sections)
{
  ...
  hrx::property_tree::write_json(_ostream, pt, true /*Pretty print*/);
}
...
{
  // Get the Metadata
  hrx::property_tree::ptree ptMetaData;
