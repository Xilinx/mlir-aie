...
// boost-free: search_path() reimplemented as a $PATH scan below.
namespace {
std::filesystem::path xclbinutil_search_path(const std::string& exe) {
  const char* pathEnv = std::getenv("PATH");
  if (!pathEnv) return {};
  std::string paths(pathEnv);
  size_t start = 0, sep;
  do {
    sep = paths.find(':', start);
    std::string dir = paths.substr(start, sep == std::string::npos ? std::string::npos : sep - start);
    if (!dir.empty()) {
      std::filesystem::path cand = std::filesystem::path(dir) / exe;
      std::error_code ec;
...
auto findExecutablePath(const std::string& executable)
{
  ...
  if (executablePath.string().empty()) {
    ...
    auto path = xclbinutil_search_path(executable);
...
// 0000000000003ac4 g    DF .text  00000000000001e8  Base        kernel0_init(void*, unsigned char const*)
{
  ...
  hrx::split(entries, output, hrx::is_any_of("\n"), hrx::token_compress_on);
  ...
  for (const auto& entry : entries) {
    ...
    hrx::algorithm::trim(functionSig);
...
using AbbrevCollection = std::vector<std::pair<unsigned long, hrx::property_tree::ptree>>;
...
//    <e71>   DW_AT_type        : (ref4) <0xdec>, __shared_ptr_access<pscontext::impl, (__gnu_cxx::_Lock_policy)2, false, false>
{
  ...
  hrx::algorithm::trim(returnValue);
  return returnValue;
}
...
//    <d51>   DW_AT_byte_size   : (data1) 8
{
  ...
  hrx::algorithm::trim(byteSizeValue);
...
//    <d52>   DW_AT_type        : (ref4) <0x55f>, float
{
  ...
  hrx::property_tree::ptree ptDWTAG;
...
static const hrx::property_tree::ptree&
...
// (ref4) <0xcbd>, xrtHandles
{
  ...
  hrx::algorithm::trim(offsetValue);
  ...
  auto it = std::find_if(argTags.begin(), argTags.end(),
                         [&offset](const std::pair<unsigned long, hrx::property_tree::ptree>& element) {return element.first == offset;});

  const static hrx::property_tree::ptree ptEmpty;
  if (it == argTags.end()) {
...
evaluate_DW_TAG_type(const std::string& typeOffset,
                     const AbbrevCollection& argTags,
                     hrx::property_tree::ptree& ptArgument)
{
  ...
  const hrx::property_tree::ptree& ptTag = get_dw_type(typeOffset, argTags);
  if (ptTag.empty())
...
static std::string
create_function_signature(const hrx::property_tree::ptree& ptArgsArray)
{
...
add_DWTAG_subprogram(size_t& index,
                     ...
                     hrx::property_tree::ptree& ptFunctionArray)
...

{
  ...
  hrx::property_tree::ptree ptFunction;
  ...
  if (hrx::algorithm::ends_with(functionName, "_init"))
    functionType = "init";

  if (hrx::algorithm::ends_with(functionName, "_fini"))
    functionType = "fini";

  ptFunction.put("type", functionType);

  // -- Find and add the arguments
  hrx::property_tree::ptree ptArgsArray;
                             ...
                             hrx::property_tree::ptree& ptFunctions)
{
  ...
  for (auto& entry: argTags) {
    std::string hexEntry = hrx::str(hrx::format("Tag cache: 0x%x") % entry.first);
                                 ...
                                 std::vector<std::string>& dwarfEntries)
{
  ...
  std::string output = os_stdout.str();
  hrx::split(dwarfEntries, output, hrx::is_any_of("\n"), hrx::token_compress_on);
}
...
drcCheckExportedFunctions(const std::vector<std::string> exportedFunctions)
{
  ...
  if (!mangledFunctions.empty()) {
    ...
    auto errMsg = hrx::str(hrx::format("ERROR: C++ mangled functions are not supported, please export the function. \nOffending function(s):\n"));

    for (const auto& entry : mangledFunctions) 
      errMsg += hrx::str(hrx::format("     %s\n") % entry);
...
void
XclBinUtilities::dataMineExportedFunctionsDWARF(const std::string& elfLibrary, hrx::property_tree::ptree& ptFunctions)
{
