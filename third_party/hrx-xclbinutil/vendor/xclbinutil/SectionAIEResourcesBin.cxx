...
{
  auto sectionInfo = std::make_unique<SectionInfo>(AIE_RESOURCES_BIN, "AIE_RESOURCES_BIN", hrx::factory<SectionAIEResourcesBin*>());
...
SectionAIEResourcesBin::getSubSectionEnum(const std::string& sSubSectionName)
{
  ...
  auto iter = std::find_if(subSectionTable.begin(), subSectionTable.end(), [&](const auto& entry) {return hrx::iequals(entry.first, sSubSectionName);});
...
SectionAIEResourcesBin::subSectionExists(const std::string& _sSubSectionName) const
{
  ...
  if (eSS == SubSection::metadata) {
    ...
    try{
      ...
      (void)e;	    
      auto errMsg = hrx::format("ERROR: Unable to parse  metadata file of section '%s'") % getSectionIndexName();
      throw std::runtime_error(errMsg.str());
    }
    hrx::property_tree::ptree& ptAieResourcesBin = pt.get_child("aie_resources_bin_metadata");
                                            ...
                                            std::ostringstream& _buffer) const
{
  ...
  if (_origSectionSize < sizeof(aie_resources_bin)) {
    auto errMsg = hrx::format("ERROR: Segment size (%d) is smaller than the size of the aie_resources_bin structure (%d)") % _origSectionSize % sizeof(aie_resources_bin);
    throw std::runtime_error(errMsg.str());
  }
  ...
  XUtil::TRACE(hrx::format("Original: \n"
                             "  mpo_name (0x%lx): '%s'\n"
  ...
  hrx::property_tree::ptree& ptSK = pt.get_child("aie_resources_bin_metadata");
  ...
  // mpo_name
  {
    ...
    if (sValue.compare(getSectionIndexName()) != 0) {
      auto errMsg = hrx::format("ERROR: Metadata data name '%s' does not match expected section name '%s'") % sValue % getSectionIndexName();
      throw std::runtime_error(errMsg.str());
    }

    aieResourcesBinHdr.mpo_name = sizeof(aie_resources_bin) + stringBlock.tellp();
    stringBlock << sValue << '\0';
    XUtil::TRACE(hrx::format("  name (0x%lx): '%s'") % aieResourcesBinHdr.mpo_name % sValue);
  }
  ...
  // mpo_version
  {
    ...
    stringBlock << sValue << '\0';
    XUtil::TRACE(hrx::format("  version (0x%lx): '%s'") % aieResourcesBinHdr.mpo_version % sValue);
  }
  ...
  // m_start_column
  {
    ...
    stringBlock << sValue << '\0';
    XUtil::TRACE(hrx::format("  start_column (0x%lx): '%s'") % aieResourcesBinHdr.m_start_column % sValue);
  }
  ...
  // m_num_columns
  {
    ...
    stringBlock << sValue << '\0';
    XUtil::TRACE(hrx::format("  num_columns (0x%lx): '%s'") % aieResourcesBinHdr.m_num_columns % sValue);
  }
  ...
  // Last item to be initialized
  {
    ...
    XUtil::TRACE(hrx::format("  m_image_offset: 0x%lx") % aieResourcesBinHdr.m_image_offset);
    XUtil::TRACE(hrx::format("    m_image_size: 0x%lx") % aieResourcesBinHdr.m_image_size);
  }
                                  ...
                                  std::ostringstream& _buffer) const
{
  ...
  switch (eSubSection) {
    ...
    default: {
        auto errMsg = hrx::format("ERROR: Subsection '%s' not supported by section '%s") % _sSubSectionName % getSectionKindAsString();
        throw std::runtime_error(errMsg.str());
      }
      break;
  }
...
SectionAIEResourcesBin::writeObjImage(std::ostream& _oStream) const
{
  ...
  if (m_bufferSize < sizeof(aie_resources_bin)) {
    auto errMsg = hrx::format("ERROR: Segment size (%d) is smaller than the size of the bmc structure (%d)") % m_bufferSize % sizeof(aie_resources_bin);
    throw std::runtime_error(errMsg.str());
  }
...
SectionAIEResourcesBin::writeMetadata(std::ostream& _oStream) const
{
  ...
  // Do we have enough room to overlay the header structure
  if (m_bufferSize < sizeof(aie_resources_bin)) {
    auto errMsg = hrx::format("ERROR: Segment size (%d) is smaller than the size of the aie_resources_bin structure (%d)") % m_bufferSize % sizeof(aie_resources_bin);
    throw std::runtime_error(errMsg.str());
  }
  ...
  hrx::property_tree::ptree ptAieResourcesBin;
  ...
  hrx::property_tree::ptree root;
  root.put_child("aie_resources_bin_metadata", ptAieResourcesBin);

  hrx::property_tree::write_json(_oStream, root);
}
                                        ...
                                        std::fstream&  _oStream) const
{
  ...
  switch (eSubSection) {
    ...
    case SubSection::unknown:
    default: {
        auto errMsg = hrx::format("ERROR: Subsection '%s' not support by section '%s") % _sSubSectionName % getSectionKindAsString();
        throw std::runtime_error(errMsg.str());
      }
      break;
  }
