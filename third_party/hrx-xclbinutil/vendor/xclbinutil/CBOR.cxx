...
{
  XUtil::TRACE((hrx::format("CBOR: [Encode] %s(%d), Count: %d") % enum_to_string(majorType) % static_cast<unsigned int>(majorType) % count).str());
                                         ...
                                         MajorTypes& majorType, uint64_t& count)
{
  ...
  if ((commandByte & 0x18) == 0x18) {
    ...
    if (payloadBytes > sizeof(uint64_t))
      throw std::runtime_error((hrx::format("Error: Unsupported payload value: 0x%x") % (payloadBytes - 1)).str());
  ...
  XUtil::TRACE((hrx::format("CBOR: [Decode] %s(%d), Count: %d") % enum_to_string(majorType) % majorTypeValue % count).str());
}
