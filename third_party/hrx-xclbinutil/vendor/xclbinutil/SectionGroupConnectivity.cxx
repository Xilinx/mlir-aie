...
{
  auto sectionInfo = std::make_unique<SectionInfo>(ASK_GROUP_CONNECTIVITY, "GROUP_CONNECTIVITY", hrx::factory<SectionGroupConnectivity*>());
...
{
  XUtil::TRACE("");
  XUtil::TRACE("Extracting: GROUP CONNECTIVITY");
  ...
  if (_sectionSize < sizeof(connectivity)) {
    auto errMsg = hrx::format("ERROR: Section size (%d) is smaller than the size of the connectivity structure (%d)") % _sectionSize % sizeof(connectivity);
    throw std::runtime_error(errMsg.str());
  }

  connectivity* pHdr = (connectivity*)_pDataSection;
  hrx::property_tree::ptree connectivity;

  XUtil::TRACE(hrx::format("m_count: %d") % (unsigned int)pHdr->m_count);
  ...
  connectivity.put("m_count", (hrx::format("%d") % (unsigned int)pHdr->m_count).str());
  ...
  hrx::property_tree::ptree m_connection;
  for (int index = 0; index < pHdr->m_count; ++index) {
    hrx::property_tree::ptree connection;


    XUtil::TRACE(hrx::format("[%d]: arg_index: %u, m_ip_layout_index: %u, mem_data_index: %u")
                 % index
    ...
    connection.put("arg_index", (hrx::format("%d") % (unsigned int)pHdr->m_connection[index].arg_index).str());
    connection.put("m_ip_layout_index", (hrx::format("%d") % (unsigned int)pHdr->m_connection[index].m_ip_layout_index).str());
    connection.put("mem_data_index", (hrx::format("%d") % (unsigned int)pHdr->m_connection[index].mem_data_index).str());

    m_connection.push_back({ "", connection });   // Used to make an array of objects
  }

  connectivity.add_child("m_connection", m_connection);

  _ptree.add_child("group_connectivity", connectivity);
  XUtil::TRACE("-----------------------------");
}


void
SectionGroupConnectivity::marshalFromJSON(const hrx::property_tree::ptree& _ptSection,
                                          std::ostringstream& _buf) const
{
  const hrx::property_tree::ptree& ptConnectivity = _ptSection.get_child("group_connectivity");
  ...
  XUtil::TRACE(hrx::format("m_count: %d") % connectivityHdr.m_count);
  ...
  const hrx::property_tree::ptree connections = ptConnectivity.get_child("m_connection");
  for (const auto& kv : connections) {
    connection connectionHdr = connection{};
    hrx::property_tree::ptree ptConnection = kv.second;
    ...
    XUtil::TRACE(hrx::format("[%d]: arg_index: %d, m_ip_layout_index: %d, mem_data_index: %d")
  ...
  if (count != (unsigned int)connectivityHdr.m_count) {
    auto errMsg = hrx::format("ERROR: Number of connection sections (%d) does not match expected encoded value: %d")
  ...
  if (bufferSize > maxBufferSize) {
    auto errMsg = hrx::format("CRITICAL WARNING: The buffer size for the CONNECTIVITY section (%d) exceed the maximum size of %d.\nThis can result in lose of data in the driver.")
