...
{
  auto sectionInfo = std::make_unique<SectionInfo>(ASK_GROUP_TOPOLOGY, "GROUP_TOPOLOGY", hrx::factory<SectionGroupTopology*>());
...
SectionGroupTopology::getMemTypeStr(MEM_TYPE _memType) const
{
  ...
  return (hrx::format("UNKNOWN (%d)") % (unsigned int)_memType).str();
}
...
void
SectionGroupTopology::marshalToJSON(char* _pDataSection,
                                    unsigned int _sectionSize,
                                    hrx::property_tree::ptree& _ptree) const
{
  XUtil::TRACE("");
  XUtil::TRACE("Extracting: GROUP_TOPOLOGY");
  ...
  if (_sectionSize < sizeof(mem_topology)) {
    auto errMsg = hrx::format("ERROR: Section size (%d) is smaller than the size of the mem_topology structure (%d)")
        % _sectionSize % sizeof(mem_topology);
    throw std::runtime_error(errMsg.str());
  }

  mem_topology* pHdr = (mem_topology*)_pDataSection;
  hrx::property_tree::ptree mem_topology;
  ...
  mem_topology.put("m_count", (hrx::format("%d") % (unsigned int)pHdr->m_count).str());
  ...
  hrx::property_tree::ptree m_mem_data;
  for (int index = 0; index < pHdr->m_count; ++index) {
    hrx::property_tree::ptree mem_data;

    XUtil::TRACE(hrx::format("[%d]: m_type: %s, m_used: %d, m_sizeKB: 0x%lx, m_tag: '%s', m_base_address: 0x%lx")
                 % index
    ...
    mem_data.put("m_used", (hrx::format("%d") % (unsigned int)pHdr->m_mem_data[index].m_used).str());
    mem_data.put("m_sizeKB", (hrx::format("0x%lx") % pHdr->m_mem_data[index].m_size).str());
    mem_data.put("m_tag", (hrx::format("%s") % pHdr->m_mem_data[index].m_tag).str());
    mem_data.put("m_base_address", (hrx::format("0x%lx") % pHdr->m_mem_data[index].m_base_address).str());
  ...
  _ptree.add_child("group_topology", mem_topology);
  XUtil::TRACE("-----------------------------");
}


void
SectionGroupTopology::marshalFromJSON(const hrx::property_tree::ptree& _ptSection,
                                      std::ostringstream& _buf) const
{
  const hrx::property_tree::ptree& ptMemtopPayload = _ptSection.get_child("group_topology");
  ...
  XUtil::TRACE("GROUP_TOPOLOGY");
  XUtil::TRACE(hrx::format("m_count: %d") % memTopologyHdr.m_count);
  ...
  hrx::property_tree::ptree memDatas = ptMemtopPayload.get_child("m_mem_data");
  for (const auto& kv : memDatas) {
    mem_data memData = mem_data{};
    hrx::property_tree::ptree ptMemData = kv.second;
    ...
    if (sm_tag.length() >= sizeof(mem_data::m_tag)) {
      auto errMsg = hrx::format("ERROR: The m_tag entry length (%d), exceeds the allocated space (%d).  Name: '%s'")
    ...
    if (memData.m_type != MEM_STREAMING_CONNECTION) {
      ...
      hrx::optional<std::string> sizeBytes = ptMemData.get_optional<std::string>("m_size");
      ...
      if (sizeBytes.is_initialized()) {
        ...
        if ((memData.m_size % 1024) != 0) {
          auto errMsg = hrx::format("ERROR: The memory size (%ld) does not align to a 1K (1024 bytes) boundary.") % memData.m_size;
          throw std::runtime_error(errMsg.str());
        }
      ...
      hrx::optional<std::string> sizeKB = ptMemData.get_optional<std::string>("m_sizeKB");
      if (sizeBytes.is_initialized() && sizeKB.is_initialized()) {
        auto errMsg = hrx::format("ERROR: 'm_size' (%s) and 'm_sizeKB' (%s) are mutually exclusive.")
    ...
    XUtil::TRACE(hrx::format("[%d]: m_type: %d, m_used: %d, m_size: 0x%lx, m_tag: '%s', m_base_address: 0x%lx")
                 % count
  ...
  if (count != (unsigned int)memTopologyHdr.m_count) {
    auto errMsg = hrx::format("ERROR: Number of mem_data sections (%d) does not match expected encoded value: %d")
  ...
  if (bufferSize > maxBufferSize) {
    auto errMsg = hrx::format("CRITICAL WARNING: The buffer size for the MEM_TOPOLOGY (%d) exceed the maximum size of %d.\nThis can result in lose of data in the driver.")
