...
void XclBinUtilities::QUIET(const hrx::format& fmt) {
  QUIET(hrx::str(fmt));
}
...
void 
XclBinUtilities::TRACE(const hrx::format &fmt, bool endl) {
  TRACE(hrx::str(fmt), endl);
}
...
void printTree(const hrx::property_tree::ptree& pt, std::ostream& _buf = std::cout, int level = 0);
...
void printTree(const hrx::property_tree::ptree& pt, std::ostream& _buf, int level) {
  if (pt.empty()) {
    _buf << "\"" << pt.data() << "\"";
  } else {
    if (level)
    ...
    for (hrx::property_tree::ptree::const_iterator pos = pt.begin(); pos != pt.end();) {
...
XclBinUtilities::TRACE_PrintTree(const std::string& _msg,
                                 const hrx::property_tree::ptree& _pt) {
  if (!m_bVerbose)
    return;
  ...
  hrx::property_tree::write_json(outputBuffer, _pt, true /*Pretty print*/);
...
XclBinUtilities::hexStringToBinaryBuffer(const std::string& _inputString,
  ...
  if (_inputString.length() != _bufferSize * 2) {
    ...
    XUtil::TRACE(hrx::format("InputString: %d (%s), BufferSize: %d") %  _inputString.length() % _inputString % _bufferSize);
    throw std::runtime_error(errMsg);
  }
...
{
  static_assert (sizeof(hrx::uuids::uuid) == 16, "Error: UUID size mismatch");

  // Copy the values to the UUID structure
  hrx::uuids::uuid uuid;
  memcpy((void *) &uuid, (void *) &_uuid, sizeof(hrx::uuids::uuid));

  // Now decode it to a string we can work with
  return hrx::uuids::to_string(uuid);
}
...
static void addConnection( std::vector<hrx::property_tree::ptree> & groupConnectivity, 
                           unsigned int argIndex, unsigned int ipLayoutIndex, unsigned int memIndex)
{
  hrx::property_tree::ptree ptConnection;
  ptConnection.put("arg_index", (hrx::format("%d") % argIndex).str());
  ptConnection.put("m_ip_layout_index", (hrx::format("%d") % ipLayoutIndex).str());
  ptConnection.put("mem_data_index", (hrx::format("%d") % memIndex).str());
...
static bool
isEqual(const hrx::property_tree::ptree & first,
        const hrx::property_tree::ptree & second)
{
  // A simple way to determine if the trees are the same
  std::ostringstream outputBufferFirst;
  hrx::property_tree::write_json(outputBufferFirst, first, false /*Pretty print*/);

  std::ostringstream outputBufferSecond;
  hrx::property_tree::write_json(outputBufferSecond, second, false /*Pretty print*/);
...
createMemoryBankGroupEntries( std::vector<WorkingConnection> & workingConnections, 
                              std::vector<hrx::property_tree::ptree> & groupTopology, 
                              std::vector<hrx::property_tree::ptree> & groupConnectivity)
{
  ...
  for (unsigned int index = 0; index < workingConnections.size(); ++index) {
    ...
    hrx::property_tree::ptree ptGroupMemory = groupTopology[workingConnections[startIndex].memIndex];

    // Prepare a new entry
    {
      const hrx::optional<std::string> sSizeBytes = ptGroupMemory.get_optional<std::string>("m_size");
      if (sSizeBytes.is_initialized()) 
        ptGroupMemory.put("m_size", (hrx::format("0x%lx") % groupSize).str());
      else 
        ptGroupMemory.put("m_sizeKB", (hrx::format("0x%lx") % (groupSize / 1024)).str());
...
validateMemoryBankGroupEntries( const unsigned int startGroupMemIndex,
                                const std::vector<hrx::property_tree::ptree> & groupTopology, 
                                const std::vector<hrx::property_tree::ptree> & groupConnectivity)
{
    ...
    if (memIndex >= startGroupMemIndex) {
      for (unsigned int searchIndex = 0; searchIndex < groupConnectivity.size(); ++searchIndex) {
        ...
        if (searchMemIndex == memIndex) {
          auto errMsg = hrx::format("ERROR: Connection indexes at %d and %d in the GROUP_CONNECTIVITY section are duplicates of each other.") % index % searchIndex;
          throw std::runtime_error(errMsg.str());
        }

        // Memory connectivity is not continuous (when using grouped memories)
        auto errMsg = hrx::format("ERROR: Invalid memory grouping (not continuous).\n"
                                    "       Connection:\n"
                                    "           arg_index       : %d\n"
                                    "           ip_layout_index : %d\n"
...
static void
transformMemoryBankGroupingCollections(const std::vector<hrx::property_tree::ptree> & connectivity,
                                       std::vector<hrx::property_tree::ptree> & groupTopology, 
                                       std::vector<hrx::property_tree::ptree> & groupConnectivity)
{
  ...
  for (auto & connection : connectivity) {
    ...
    hrx::optional<std::string> sSizeBytes = groupTopology[memIndex].get_optional<std::string>("m_size");
    if (sSizeBytes.is_initialized())
      sizeBytes = XUtil::stringToUInt64(static_cast<std::string>(sSizeBytes.get()));
    else {
      hrx::optional<std::string> sSizeKBytes = groupTopology[memIndex].get_optional<std::string>("m_sizeKB");
...
XclBinUtilities::createMemoryBankGrouping(XclBin & xclbin)
{
  ...
  const auto memTopology = XUtil::as_vector<hrx::property_tree::ptree>(ptMemTopology.get_child("mem_topology"), "m_mem_data");
  ...
  // Copy the data
  std::vector<hrx::property_tree::ptree> groupTopology = memTopology;
  ...
  if (pConnectivity != nullptr) {
    ...
    const auto connectivity = XUtil::as_vector<hrx::property_tree::ptree>(ptConnectivity.get_child("connectivity"), "m_connection");
    if ( connectivity.empty() ) {
      ...
      for (unsigned int index = 0; index < connectivity.size(); ++index) {
        ...
        if (memIndex >= groupTopology.size()) {
          auto errMsg = hrx::format("ERROR: Connectivity section 'mem_data_index' (%d) at index %d exceeds the number of 'mem_topology' elements (%d).  This is usually an indication of corruption in the xclbin archive.") % memIndex % index % groupTopology.size();
          throw std::runtime_error(errMsg.str());
        }
      }
  ...
  // Re-create the property tree, create and re-populate the Group Topology section, and add it.
  {
    ...
    hrx::property_tree::ptree ptGroupTopology;
                      ...
                      std::ostringstream & os_stderr)
{
  ...
  if (pid == 0) {
    // child: redirect stdout/stderr to the write ends, then exec
