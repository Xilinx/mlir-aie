...
void buildXMLKernelEntry(const hrx::property_tree::ptree& ptKernel,
                         bool isFixedPS,
                         hrx::property_tree::ptree& ptKernelXML)
{
  ...
  hrx::property_tree::ptree ptKernelAttributes;
  ...
  hrx::property_tree::ptree ptExtendedData = ptKernel.get_child("extended-data", ptEmpty);
  if (!ptExtendedData.empty()) {
    ...
    hrx::property_tree::ptree ptEntry;
    ptEntry.add_child("<xmlattr>", ptExtendedData);
    ptKernelXML.add_child("extended-data", ptEntry);
  }

  // -- Build kernel arguments
  const hrx::property_tree::ptree& ptArguments = ptKernel.get_child("arguments", ptEmpty);

  unsigned int argID = 0;
  for (const auto& argument : ptArguments) {
    const hrx::property_tree::ptree& ptArgument = argument.second;
    hrx::property_tree::ptree ptArgAttributes;
...
void
XclBinUtilities::addKernel(const hrx::property_tree::ptree& ptKernel,
                           bool isFixedPS,
                           hrx::property_tree::ptree& ptEmbeddedData)
{
  ...
  if (ptEmbeddedData.get_child("project.platform.device.core", ptEmpty).empty()) {
    ...
    hrx::property_tree::ptree ptCore;

    hrx::property_tree::ptree ptDevice;
    ptDevice.add_child("core", ptCore);

    hrx::property_tree::ptree ptPlatform;
    ptPlatform.add_child("device", ptDevice);

    hrx::property_tree::ptree ptProject;
  ...
  hrx::property_tree::ptree& ptCore = ptEmbeddedData.get_child("project.platform.device.core", ptEmpty);
...
void addArgsToMemoryConnections(const unsigned int ipLayoutIndexID,
                                const hrx::property_tree::ptree& ptArgs,
  ...
  for (const auto& arg: ptArgs) {
    ...
    if (memoryConnection.empty()) {
      ...
      auto iter = std::find_if(memTopology.begin(), memTopology.end(),
                               [](const hrx::property_tree::ptree& pt)
      ...
      if (iter == memTopology.end()) {
        ...
        hrx::property_tree::ptree ptMemData;
      ...
      if (isNumeric(memoryConnection)) {
        ...
        for (const auto& memBankIndex : viMemBanks) {
          ...
          {
            const auto errMsg = hrx::format("Specified memory bank (%d) is invalid.  Valid ranges are from 0 to %d.") % memBankIndex % memTopology.size(); 
            throw std::runtime_error(errMsg.str());
          }
        }
      } else {
        auto iter = std::find_if(memTopology.begin(), memTopology.end(),
                               [memoryConnection](const hrx::property_tree::ptree& pt)
    ...
    for (const auto& memBank : vMemBanks) {
      ...
      hrx::property_tree::ptree& ptMemData = memTopology[index];
...
void transformVectorToPtree(const std::vector<hrx::property_tree::ptree>& vectorOfPtree,
...
void
XclBinUtilities::addKernel(const hrx::property_tree::ptree& ptKernel,
                           hrx::property_tree::ptree& ptMemTopologyRoot,
                           hrx::property_tree::ptree& ptIPLayoutRoot,
                           hrx::property_tree::ptree& ptConnectivityRoot)
{
  ...
  const hrx::property_tree::ptree& ptIPLayout = ptIPLayoutRoot.get_child("ip_layout", ptEmpty);
  auto ipData = XUtil::as_vector<hrx::property_tree::ptree>(ptIPLayout, "m_ip_data");

  const hrx::property_tree::ptree& ptMemTopology = ptMemTopologyRoot.get_child("mem_topology", ptEmpty);
  auto memTopology = XUtil::as_vector<hrx::property_tree::ptree>(ptMemTopology, "m_mem_data");

  const hrx::property_tree::ptree& ptConnectivity = ptConnectivityRoot.get_child("connectivity", ptEmpty);
  ...
  for (const auto& instance: ptKernel.get_child("instances", ptEmpty)) {
    ...
    //   "dpu_kernel_id": "0x101"
    // }   
    hrx::optional< const hrx::property_tree::ptree& > boExtendedData = ptKernel.get_child_optional("extended-data");
    if (boExtendedData) {
      const hrx::property_tree::ptree& ptExtendedData = boExtendedData.get();
      if (!ptExtendedData.empty()) {
...
void
validateSignature(std::vector<hrx::property_tree::ptree> functions,
                  ...
                  const std::string& kernelLibrary)
{
  ...
  if (functions.size() > 1) {
    ...
    throw std::runtime_error("Error: Only one " + functionType + " kernel supported in a library, multiple " + functionType + " kernels found.\n"
                             ...
                             "       Kernels: " + hrx::algorithm::join(functionsFound, ", "));
  }

  const hrx::property_tree::ptree& ptFunction = functions[0];
  ...
  // Validate arguments
  auto args = XUtil::as_vector<hrx::property_tree::ptree>(ptFunction, "args");
  if (args.size() != expectedArgs.size())
    throw std::runtime_error("Error: " + ptFunction.get<std::string>("type") + " kernel signature argument count mismatch.\n"
                             "Shared Library: '" + kernelLibrary + "'\n"
                             "      Expected: '" + name + "(" + hrx::algorithm::join(expectedArgs, ", ") + ")'\n"
...
void
XclBinUtilities::validateFunctions(const std::string& kernelLibrary, const hrx::property_tree::ptree& ptFunctions)
{
  XUtil::TRACE_PrintTree("Validate ptFunctions", ptFunctions);

  auto functions = XUtil::as_vector<hrx::property_tree::ptree>(ptFunctions, "functions");

  // Collect the functions
  std::vector<hrx::property_tree::ptree> initKernels;
  std::vector<hrx::property_tree::ptree> finiKernels;
  std::vector<hrx::property_tree::ptree> kernels;
  ...
  auto args = XUtil::as_vector<hrx::property_tree::ptree>(kernels[0], "args");
...
XclBinUtilities::createPSKernelMetadata(const std::string& memBanks,
                                        ...
                                        const hrx::property_tree::ptree& ptFunctions,
                                        const std::string& kernelLibrary,
                                        hrx::property_tree::ptree& ptPSKernels)
{
  ...
  for (const auto& ptFunction : functions) {
    ...
    // Gather arguments
    auto args = XUtil::as_vector<hrx::property_tree::ptree>(ptFunction, "args");
    hrx::property_tree::ptree ptArgArray;
    uint64_t offset = 0;
    for (const auto& entry : args) {
      ...
      ptArg.put("offset", hrx::str(hrx::format("0x%x") % offset));
      offset += byteSize;
    ...
    // Add the instances
    hrx::property_tree::ptree ptInstanceArray;
    for (unsigned long instance = 0; instance < numInstances; ++instance) {
      hrx::property_tree::ptree ptInstance;
  ...
  hrx::property_tree::ptree ptKernels;
