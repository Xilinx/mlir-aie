    ...
    : FDTNode()
{
  ...
  // Get the node name
  {
    ...
    XUtil::TRACE(hrx::format("DTC Node Name: '%s'") % m_name);
  ...
  while (true) {
    XUtil::TRACE(hrx::format("Looping Index: %d (0x%x)") % index % index);
    ...
    switch (dtcToken) {
      ...
      default: {
          auto errMsg = hrx::format("ERROR: Unknown token: 0x%x") % dtcToken;
...
FDTNode::FDTNode(const hrx::property_tree::ptree& _ptDTC,
    ...
    : FDTNode()
{
  ...
  for (hrx::property_tree::ptree::const_iterator iter = _ptDTC.begin();
       iter != _ptDTC.end();
       ++iter) {
    std::string keyName = iter->first;

    if (_propertyNameFormat.find(keyName) != _propertyNameFormat.end()) {
      XUtil::TRACE(hrx::format("Property Found: %s") % keyName);
      FDTProperty* pProperty = new FDTProperty(iter, _propertyNameFormat);
      m_properties.push_back(pProperty);
    } else {
      XUtil::TRACE(hrx::format("Node Found: %s") % keyName);
                        ...
                        const FDTProperty::PropertyNameFormat& _propertyNameFormat)
{
  ...
  if (_size < MinSizeBytes) {
    auto errMsg = hrx::format("ERROR: The size of the structure block is too small.  Minimum size: 0x%x") % MinSizeBytes;
    throw std::runtime_error(errMsg.str());
  }

  if ((_size % 4) != 0) {
    auto errMsg = hrx::format("ERROR: The size of the structure block is not word aligned. Size: 0x%x") % _size;
    throw std::runtime_error(errMsg.str());
  }
  ...
  if (startDtcToken != FDT_BEGIN_NODE) {
    auto errMsg = hrx::format("ERROR: Missing FDT_BEGIN_NODE token at the start of the structure block. Expected: 0x%x, Actual: 0x%x") % FDT_BEGIN_NODE % startDtcToken;
    throw std::runtime_error(errMsg.str());
  }
  ...
  if (lastDtcToken != FDT_END) {
    auto errMsg = hrx::format("ERROR: Missing FDT_END token at end of the structure block. Expected: 0x%x, Actual: 0x%x") % FDT_END % lastDtcToken;
    throw std::runtime_error(errMsg.str());
  }
  ...
  if (endDtcToken != FDT_END) {
    auto errMsg = hrx::format("ERROR: Missing FDT_END_NODE token at end of the structure block. Expected: 0x%x, Actual: 0x%x") % FDT_END % endDtcToken;
    throw std::runtime_error(errMsg.str());
  }

  // -- Validate that all of the bytes were examined --
  if (_size != index) {
    auto errMsg = hrx::format("ERROR: Structure Node Buffer wasn't completely examined.  Expected: 0x%x, Actual: 0x%s.") % _size % index;
    throw std::runtime_error(errMsg.str());
  }
...
FDTNode*
FDTNode::marshalFromJSON(const hrx::property_tree::ptree& _ptDTC,
                         const FDTProperty::PropertyNameFormat& _propertyNameFormat)
{
  std::string topName = "";
...
void
FDTNode::marshalSubNodeToJSON(hrx::property_tree::ptree& _ptTree,
                              const FDTProperty::PropertyNameFormat& _propertyNameFormat) const
{
  XUtil::TRACE(hrx::format("** Examining SubNode: '%s'") % m_name);

  hrx::property_tree::ptree ptSubNode;
...
void
FDTNode::marshalToJSON(hrx::property_tree::ptree& _ptTree,
                       const FDTProperty::PropertyNameFormat& _propertyNameFormat) const
{
  XUtil::TRACE(hrx::format("** Examining Node: '%s'") %  m_name);

  if (m_name.length() != 0) {
    auto errMsg = hrx::format("ERROR: The given node '%s' is not the top node of the tree.") % m_name;
    throw std::runtime_error(errMsg.str());
  }
