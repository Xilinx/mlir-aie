...
DTCStringsBlock::parseDTCStringsBlock(const char* _pBuffer, const unsigned int _size)
{
  ...
  for (index = 0; index < _size; ++index) {
    if (_pBuffer[index] == 0) {
      ...
      XUtil::TRACE(hrx::format("Adding DTCString: %s") % dtcString);
      unsigned int offset = addString(dtcString);

      if (offset != lastIndex) {
        auto errMsg = hrx::format("ERROR: DTC string offset mismatch.  Expected: 0x%x, Actual: 0x%x") % lastIndex % offset;
...
DTCStringsBlock::getString(unsigned int _offset) const
{
  ...
  if (_offset > blockString.size()) {
    auto errMsg = hrx::format("ERROR: Offset (0x%x) is greater then the string buffer (0x%x).") %  _offset % blockString.size();
    throw std::runtime_error(errMsg.str());
  }
