...
{
  auto sectionInfo = std::make_unique<SectionInfo>(MEM_TOPOLOGY, "MEM_TOPOLOGY", hrx::factory<SectionMemTopology*>());
                                  ...
                                  hrx::property_tree::ptree& _ptree) const
{
  ...
  if (_sectionSize < sizeof(mem_topology)) {
    auto errMsg = hrx::format("ERROR: Section size (%d) is smaller than the size of the mem_topology structure (%d)") % _sectionSize % sizeof(mem_topology);
...
void
SectionMemTopology::marshalFromJSON(const hrx::property_tree::ptree& _ptSection,
                                    std::ostringstream& _buf) const
{
  const hrx::property_tree::ptree& ptMemtopPayload = _ptSection.get_child("mem_topology");
  ...
  for (const auto& kv : memDatas) {
    ...
    if (sm_tag.length() > maxTagLength) {
      auto errMsg = hrx::format("ERROR: The m_tag entry length (%d), exceeds the allocated space (%d) available.  Name: '%s'")
