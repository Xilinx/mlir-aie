...
int main_(int argc, const char** argv) {
  ...
  namespace po = hrx::program_options;

  po::options_description desc("Options");
  desc.add_options()
      ("add-merge-section", hrx::program_options::value<decltype(sectionsToAddMerge)>(&sectionsToAddMerge)->multitoken(), "Section name to add or merge.  Format: <section>:<format>:<file>")
      ("add-pskernel", hrx::program_options::value<decltype(addPsKernels)>(&addPsKernels)->multitoken(), "Helper option to add PS kernels.  Format: [<mem_banks>]:[<symbol_name>]:[<instances>]:<path_to_shared_library>")
      ("add-replace-section", hrx::program_options::value<decltype(sectionsToAddReplace)>(&sectionsToAddReplace)->multitoken(), "Section name to add or replace.  Format: <section>:<format>:<file>")
      ("add-section", hrx::program_options::value<decltype(sectionsToAdd)>(&sectionsToAdd)->multitoken(), "Section name to add.  Format: <section>:<format>:<file>")
      ("add-signature", hrx::program_options::value<decltype(sSignature)>(&sSignature), "Adds a user defined signature to the given xclbin image.")
      ("certificate", hrx::program_options::value<decltype(sCertificate)>(&sCertificate), "Certificate used in signing and validating the xclbin image.")
      ("digest-algorithm", hrx::program_options::value<decltype(sDigestAlgorithm)>(&sDigestAlgorithm), "Digest algorithm. Default: sha512")
      ("dump-section", hrx::program_options::value<decltype(sectionsToDump)>(&sectionsToDump)->multitoken(), "Section to dump. Format: <section>:<format>:<file>")
      ("force", hrx::program_options::bool_switch(&bForce), "Forces a file overwrite.")
      ("get-signature", hrx::program_options::bool_switch(&bGetSignature), "Returns the user defined signature (if set) of the xclbin image.")
      ("help,h", "Print help messages")
      ("info", hrx::program_options::value<decltype(sInfoFile)>(&sInfoFile)->default_value("")->implicit_value("<console>"), "Report accelerator binary content.  Including: generation and packaging data, kernel signatures, connectivity, clocks, sections, etc.  Note: Optionally an output file can be specified.  If none is specified, then the output will go to the console.")
      ("input,i", hrx::program_options::value<std::string>(&sInputFile), "Input file name. Reads xclbin into memory.")
      ("key-value", hrx::program_options::value<decltype(keyValuePairs)>(&keyValuePairs)->multitoken(), "Key value pairs.  Format: [USER|SYS]:<key>:<value>")
      ("list-sections", hrx::program_options::bool_switch(&bListSections), "List all possible section names (Stand Alone Option)")
      ("migrate-forward", hrx::program_options::bool_switch(&bMigrateForward), "Migrate the xclbin archive forward to the new binary format.")
      ("output,o", hrx::program_options::value<std::string>(&sOutputFile), "Output file name. Writes in memory xclbin image to a file.")
      ("private-key", hrx::program_options::value<decltype(sPrivateKey)>(&sPrivateKey), "Private key used in signing the xclbin image.")
      ("quiet,q", hrx::program_options::bool_switch(&bQuiet),     "Minimize reporting information.")
      ("remove-key", hrx::program_options::value<decltype(keysToRemove)>(&keysToRemove)->multitoken(), "Removes the given user key from the xclbin archive." )
      ("remove-section", hrx::program_options::value<decltype(sectionsToRemove)>(&sectionsToRemove)->multitoken(), "Section name to remove.")
      ("remove-signature", hrx::program_options::bool_switch(&bRemoveSignature), "Removes the signature from the xclbin image.")
      ("replace-section", hrx::program_options::value<decltype(sectionsToReplace)>(&sectionsToReplace)->multitoken(), "Section to replace. ")
      ("target", hrx::program_options::value<decltype(sTarget)>(&sTarget), "Target flow for this image.  Valid values: hw, hw_emu, and sw_emu.")
      ("validate-signature", hrx::program_options::bool_switch(&bValidateSignature), "Validates the signature for the given xclbin archive.")
      ("verbose,v", hrx::program_options::bool_switch(&bVerbose), "Display verbose/debug information.")
      ("version", hrx::program_options::bool_switch(&bVersion), "Version of this executable.")
      ("file-check", hrx::program_options::bool_switch(&fileCheck), "Check for Linux file command utility compliance")
 ;
  ...
  hrx::program_options::options_description hidden("Hidden options");
  ...
  hidden.add_options()
    ("add-kernel", hrx::program_options::value<decltype(addKernels)>(&addKernels)->multitoken(), "Helper option to add fixed kernels.  Format: <path_to_json>")
    ("append-section", hrx::program_options::value<decltype(sectionsToAppend)>(&sectionsToAppend)->multitoken(), "Section to append to.")
    ("BAD-DATA", hrx::program_options::value<decltype(badOptions)>(&badOptions)->multitoken(), "Dummy Data." )
    ("dump-signature", hrx::program_options::value<decltype(sSignatureOutputFile)>(&sSignatureOutputFile), "Dumps a sign xclbin image's signature.")
    ("reset-bank-grouping", hrx::program_options::bool_switch(&bResetBankGrouping), "Resets the memory bank grouping section(s).")
    ("signature-debug", hrx::program_options::bool_switch(&bSignatureDebug), "Dump section debug data.")
    ("skip-bank-grouping", hrx::program_options::bool_switch(&bSkipBankGrouping), "Disables creating the memory bank grouping section(s).")
    ("skip-uuid-insertion", hrx::program_options::bool_switch(&bSkipUUIDInsertion), "Do not update the xclbin's UUID")
    ("trace,t", hrx::program_options::bool_switch(&bTrace), "Trace")
    ("transform-pdi", hrx::program_options::bool_switch(&bTransformPdi), "Transform the PDIs in AIE_PARTITION, this option only valid on Linux")
  ;

  hrx::program_options::options_description all("Allowed options");
