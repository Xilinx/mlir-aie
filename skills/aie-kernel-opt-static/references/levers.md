...
| S53 | ic loop II23 per two steps (LLVM's unroll by 2), NS 2 → II9 per step, NS 3 with ZOL; frame 0 → 288 B, `pm_bytes` 432 → 464 | `conv2dk1_i8` (AIE2, npu1): S43's loop under `#if AIE_TUNED_AIE2`: the block's activations through two pointers, one per half block, and the ic loop kept rolled under `AIE_LOOP_NO_UNROLL`. The accumulators were already unrolled into registers | 871 → 801 (1.09x); bit-identical on HW; AIE2P `.text` byte-identical | hit in direction; trips × II gives 576 of 801, as S43 |
...
| S80 | Exp loop II251 per 2 rows → two half-row loops at II26 per half row; max loop II72 → 58; `partial_softmax` frame 1280 → 416 B, aiecc stack 1376 → 512 B | `mha_softmax` (AIE2, npu1): `exp2_bf16.h` on m - a·s rewritten around `mac_elem_16_2`, which sums one product from each 16-lane half: [a \| m] · [-s \| 1] forms y, and the Horner steps keep their constants in the upper half. k = round(y) is not clamped: a saturating `upshift` into the exponent field and `maxdiff`, which compares before it subtracts, give +0 for every y ≥ 128, masked lanes included. y + magic as the same mac's accumulator is wrong: the mac rounds after each product, so it rounds m - round(a·s) (8 of 21 gate cases fail). Rejected (static): a whole row per trip, II34 with 16 vector spills; 16 lanes per trip, no schedule | 4096x4 9818 → 4895 (2.0x), diagonal 10349 → 5455 (1.9x); 1878 / 961 words differ, worst element 0.74% from exp2 in both arms | hit, including magnitude: 32 × II251 → 128 × II26 and 8 × (72 → 58) predicts 4816 of the 4923 saved |
| S81 | bf16 8x8 II45 per block → II16 per two blocks, NS 3; u32 8x8 II53 → II14 per block, NS 2 | `transpose` (AIE2, npu1): `aie::transpose` replaced by two-register `::shuffle` stages, each moving one bit of the element index between the register number and the lane (bf16: `T16_32x2`, `T64_8x2`, `T16_16x4`; u32: `T32_16x2`, `T32_8x4`), on concatenated 256-bit row pairs. Needs a walked pointer: indexing `in + o` gives II17 / II27 | 256x32 bf16 5761 → 1041 (5.5x), 32x32 bf16 721 → 144, 32x32 u32 852 → 239; bit-identical | hit, including magnitude: 64 × II16 = 1024 of 1041, 8 × 16 = 128 of 144, 16 × 14 = 224 of 239 |
| S82 | bf16 4x4 II25 per row of 32 → II16 per four rows, NS 2; u8 4x4 row loop not pipelined (no remark) → II6 per four rows, NS 4 | `transpose` (AIE2, npu1): S81 for 4x4 blocks, 32 columns of rows j and j + 2 per register (bf16: `T16_16x4`, `T64_8x2`, `T16_4x16`; u8: `T8_2x64`, `T64_2x8`, `T16_16x4`). The bf16 loop holds all 12 x registers, so it doesn't reach its 12 shuffles | 32x32 bf16 199 → 141 (1.4x), 32x32 u8 96 → 63 (1.5x); bit-identical | hit: 8 × II16 = 128 of 141, 8 × II6 = 48 of 63 |
| S83 | Pass 2 II19 → II15 per two chunks; f32 pass 3 II11 per chunk → II31 per four; affine pass 3 II27 → II25, frame 128 → 64 B | `layer_norm_f32`, `layer_norm_affine_cast` (AIE2, npu1): the third bf16 limb as x - [hi \| mid] · [1 \| 1] in one msc, not a second residual on x - hi; the f32 normalize at four chunks per trip; the affine products regrouped as [n0 \| n1] · [g0 \| g1] and [n1 \| n2] · g0 into beta, plus n0 · [g1 \| g2]. Two chunks per affine trip, or the affine sum as one chain, fail to pipeline (NS 1, II66 to 110) | f32 1962 → 1621 (1.21x), affine 3015 → 2757 (1.09x); bit-identical | hit, including magnitude: trips × ΔII predicts -336 and -256 |
...
- **Do:** advance a `T *__restrict` cursor, and mark non-aliasing in/out
  ...
  `aie_kernels/quant/q4nx_dequant.cc`) so the scalar ALU carries no address
