...
Both `Flow` and `PacketFlow` (and `CascadeFlow`, documented in `python_api.md`) implement the `Resolvable` protocol (`aie.iron.resolvable.Resolvable`) — a structural `Protocol` requiring `resolve(loc, ip)` and `tiles()`. You only need to know about it if you're writing a custom low-level primitive that should participate in `Program.resolve_program()`'s placement/resolution walk.
