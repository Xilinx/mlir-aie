...
//
// Compile with the Peano / AIECC toolchain or Chess. Note AIE_PREPARE_FOR_PIPELINING
// is a no-op under Peano/AIECC (only Chess honors it); AIE_LOOP_MIN_ITERATION_COUNT
// is the annotation that enables pipelining on both.
...
static inline void eltwise_add_impl(const T_in *__restrict a,
                                    ...
                                    T_out      *__restrict c) {
    constexpr int32_t VEC = 32;          // natural bf16 width; int8 uses 64 (see references/architecture.md)
