...
2. **Explicit DMA Control**: The MemTile DMA is controlled via explicit locks rather than automatic ObjectFifo management. The DMA waits for a lock release triggered by `memtile_lock.set(1)` in the runtime sequence.
...
- **MemTile** (row 1) - Stores initialized data, controlled by `memtile_lock`
