...
int main(int argc, const char *argv[]) {
  ...
  std::vector<std::int16_t> A_sf;

  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<int16_t> distrib(std::numeric_limits<int16_t>::min(), std::numeric_limits<int16_t>::max());
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    std::int16_t *bufOut = bo_out.map<std::int16_t *>();
    ...
    if (VERIFY) {
      for (int t = 0; t < NUM_TILES; t++) {
        for (int pr = 0; pr < TILE_SIZE; pr++) {
          std::int16_t sf = A_sf[t * SF_BLOCK_SIZE + pr / SF_BLOCK_SIZE];
          int val = (int)(A_private[t * TILE_SIZE + pr]);
          if (val >= 8)
            val = (val & 0x7) - 8; // Two's complement, but threes a crowd
          std::int16_t scaled = sf * val;

          std::int16_t from_AIE = bufOut[(t * TILE_SIZE) + pr];
          ...
          std::int16_t abs_diff = abs(from_AIE - scaled);
          if (abs(from_AIE) > 0){
            if ((abs_diff / abs(from_AIE)) > 0.01) {
