...
def test_objectfifo_bd_chain_scenarios_placed():
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            input_ty = np.ndarray[(input_size,), np.dtype[np.uint8]]
            ...
            task_repeat_count = 1    
            wrap_count = 4
            output_ty = np.ndarray[(chunk_size * wrap_count,), np.dtype[np.uint8]]  # 64 elements total 
...
def test_objectfifo_bd_chain_error_zero_placed():
    try:
        with mlir_mod_ctx() as ctx:
            @device(dev)
            def device_body():
                line_ty = np.ndarray[(1024,), np.dtype[np.uint8]]
                ShimTile = tile(col, 0)
                MemTile = tile(col, 1)
