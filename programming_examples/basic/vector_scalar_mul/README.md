...
1. **Runtime Sequence:** a `sequence(...)` function receives the host-side buffers plus the ObjectFifo handles and moves data via `in_h.fill(...)` / `out_h.drain(...)`; it is passed to `Runtime(sequence, [...])`, and `Program.enable_trace` activates tracing when requested. (The plumbing here is provided by `aie.iron.algorithms.transform`.)
