#  Supported Matrix Multiplication modes in the 2023.2 AIE-ML API User guide
#  Using the following parameters to denote the API size: 
#  ------ r x s x t ------
...
# Create the parser
parser = argparse.ArgumentParser(description="Script to find optimal single AIE MatMul sizes for various data types")

# Add an argument with possible choices for supported datatypes
parser.add_argument('--in_a_type', type=str, choices=['int8', 'int16', 'bf16'], help='input a data type')
parser.add_argument('--in_b_type', type=str, choices=['int8', 'int16', 'bf16'], help='input b data type')
parser.add_argument('--out_c_type', type=str, choices=['int8', 'int16', 'int32', 'bf16', 'fp32'], help='output c data type')
...
df = DataFrame(columns=['m_k_n', 'MACs', 'Data_mem_double_buff', 'A_mem', 'B_mem', 'C_mem', 'Ideal Cycles'])
...
for m in range(m_min, 200, r):
    for k in range(k_min, 256, s):
        for n in range(n_min, 200, t):

            # find the expected kernel cycles
            expected_kernel_cycles = (m*k*n / peak_MACs_per_cycle) / expected_kernel_efficiency
            ...
            if (DMA_in_a_cycles <= expected_kernel_cycles 
                ...
                if ((2*single_mem_size) <= 63*1024):
                    ...
                    # if int8, singel AIE MatMul kenrel unrolled 4 times in m dimension and 2 times in n dimension
                    if in_a_type == "int8" and in_b_type == "int8":

                        if ((m % (4*r) == 0) and (n % (2*t) == 0)):

                            # add valid solution to dataframe
                            df.loc[len(df)] = [str(m) + '_' + str(k) + '_' + str(n), m*k*n, 2*single_mem_size/1024, A_mem_size/1024, B_mem_size/1024, C_mem_size/1024, (m*k*n / peak_MACs_per_cycle)]


                    # if int16, singel AIE MatMul kenrel unrolled 2 times in each m and n dimension
                    if in_a_type == "int16" and in_b_type == "int16":
                    ...
                    # if bf16, singel AIE MatMul kenrel unrolled 4 times in each m and n dimension
                    elif in_a_type == "bf16" and in_b_type == "bf16":

                        if ((m % (4*r) == 0) and (n % (4*t) == 0)):
...
# short by
sorted_df = df.sort_values(by=['C_mem', 'MACs'], ascending=[True, False])
