...
def my_matmul(
    ...
    def device_body():
        ...
        def sequence(A, B, C):
            ...
            # 3) one out tile for C, i.e., m * n_aie_rows * n
            max_BDs_per_A_B = 5
            ...
            total_col_tiles = N // n // n_aie_cols
            ...
            for i in range(total_row_tiles):
                ...
                for j in range(total_col_tiles):
                    ...
                    for shim_col in range(n_aie_cols):

                        if shim_col < n_aie_rows:

                            # whole array offset
                            A_array_row_offset = A_row_index * (n_aie_rows * m * K)

                            # each shim tile internal offset
                            A_internal_row_offset = shim_col * (m * K)

                            A_offset = A_array_row_offset + A_internal_row_offset

                            # sizes and strides in mtk representation
                            A_sizes = [1, K // mtk, m, mtk]
                            A_strides = [0, mtk, K, 1]

                            npu_dma_memcpy_nd(
                                metadata=A_l3l2_fifos[shim_col],
                        ...
                        if b_col_maj:
                            # whole array offset
                            B_array_col_offset = B_col_index * (n_aie_cols * K * n)

                            # each shim tile internal offset
                            B_internal_col_offset = shim_col * K * n

                            # ktn representation in col-maj
                            B_sizes = [1, K // ktn, n, ktn]
                            B_strides = [0, ktn, K, 1]
                        else:
                            # whole array offset
                            B_array_col_offset = B_col_index * (n_aie_cols * n)

                            # each shim tile internal offset
                            B_internal_col_offset = shim_col * n

                            B_sizes = [1, K // k, k, n]
                            B_strides = [0, k * N, N, 1]

                        B_offset = B_array_col_offset + B_internal_col_offset

                        npu_dma_memcpy_nd(
                            metadata=B_l3l2_fifos[shim_col],
            ...
            for i in range(total_row_tiles):
                for j in range(total_col_tiles):
                    ...
                    for shim_col in range(n_aie_cols):
                        ...
                        C_array_row_offset = i * (n_aie_rows * m * N)
                        C_array_col_offset = j * (n_aie_cols * n)

                        C_internal_col_offset = shim_col * n

                        C_offset = (
                            C_array_row_offset
                            + C_array_col_offset
                            + C_internal_col_offset
                        )

                        C_sizes = [1, 1, m * n_aie_rows, n]
                        C_strides = [0, 0, N, 1]

                        npu_dma_memcpy_nd(
                            metadata=C_l2l3_fifos[shim_col],
                            bd_id=0,
                            mem=C,
                    ...
                    # syncronize for each (m * n_aie_rows * n) C tile
