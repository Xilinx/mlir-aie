...
constexpr int TOTAL_TILE_SIZE =
    (TILE_SIZE / 2) + (TILE_SIZE / SF_BLOCK_SIZE) * 4; // including min

constexpr int IN_SIZE = (TEST_SIZE / 2) + (TEST_SIZE / SF_BLOCK_SIZE) * 4; // including min
...
int main(int argc, const char *argv[]) {
  ...
  std::vector<std::bfloat16_t> A_min;
  ...
  for (int t = 0; t < NUM_TILES; t++) {
    ...
    for (int isf = 0; isf < TILE_SIZE / SF_BLOCK_SIZE; isf++) {
      std::bfloat16_t sf = std::bfloat16_t((float)rand() / (float)(RAND_MAX));
    ...
    for (int isf = 0; isf < TILE_SIZE / SF_BLOCK_SIZE; isf++) {
      ...
      std::int8_t upper = (std::int8_t)(bits >> 8);
      std::int8_t lower = (std::int8_t)(bits & 0x00ff);
      AVec[t * TOTAL_TILE_SIZE + TILE_SIZE / 2 + 2*TILE_SIZE / SF_BLOCK_SIZE + isf * 2] = lower;
      AVec[t * TOTAL_TILE_SIZE + TILE_SIZE / 2 + 2*TILE_SIZE / SF_BLOCK_SIZE + isf * 2 + 1] = upper;
      A_min.push_back(sf);
      if (verbosity >= 2) {
        if (t == 0)
          std::cout << std::hex
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (verbosity >= 2) {
      ...
      std::cout << "Reference sfs    " << A_sf[0] << ", " << A_sf[1]
                << std::endl;
      std::cout << "Reference mins   " << A_min[0] << ", " << A_min[1]
                << std::endl;
    }
    float npu_time =
    ...
    if (VERIFY) {
      for (int t = 0; t < NUM_TILES; t++) {
        for (int pr = 0; pr < TILE_SIZE; pr++) { 
          ...
          std::bfloat16_t _min = A_min[t * SF_BLOCK_SIZE + pr / SF_BLOCK_SIZE];
          uint val = (uint)(A_private[t * TILE_SIZE + pr]);
          //if (val >= 8)
          //  val = (val & 0x7) - 8; // Two's complement, but threes a crowd
          std::bfloat16_t scaled = sf * (std::bfloat16_t)val + _min;
          ...
          if ((abs_diff / fabs(from_AIE)) > 0.01) {
            std::cout << "Tile " << t << ":" << pr << " From AIE "
                      ...
                      << std::setprecision(12) << sf << "*" << val << "+" << _min
                      << std::dec << std::endl;
            std::cout << "Tile " << t << ":" << pr << " From AIE " << std::hex
                      << from_AIE_raw << " ref " << scaled_raw << " from "
                      << *reinterpret_cast<std::uint16_t *>(&sf) << "*" << val << *reinterpret_cast<std::uint16_t *>(&_min)
                      << std::dec << std::endl;
            errors++;
          }
        }
