...
def my_expand():
    ...
    NUM_SUPERBLOCKS = N // SUPERBLOCK_SIZE
    NUM_BLOCKS = SUPERBLOCK_SIZE // BLOCK_SIZE

    superblock_in_bytes = (SUPERBLOCK_SIZE // 2) + (NUM_BLOCKS * 2) + 4
    
    N_in_bytes = NUM_SUPERBLOCKS * superblock_in_bytes

    A_sz_in_i32s = N_in_bytes // 4
    # They are 4 bits per element, we need to add on the scale factors and mins later though
    B_sz_in_i32s = N // 2  # Returning 16 bits at the moment
    B_sz_in_bytes = N * 2  # Returning 16 bits at the moment

    input_buffer_size_bytes = superblock_in_bytes  # They are bfloat16 and int8 sfs and mins after the private values
    output_buffer_size_bytes = SUPERBLOCK_SIZE * 2  # The scaled values

    n_cores = 1
    tiles = NUM_SUPERBLOCKS // n_cores
    buffer_depth = 2
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            expand_uint4_to_bfloat16 = external_func(
                "expand_uint4_to_bfloat16", inputs=[memRef_i_ty, memRef_o_ty]
            )
            ...
            def core_body():
                for _ in for_(0xFFFFFFFF):
                    for _ in for_(tiles):
                        ...
                        call(expand_uint4_to_bfloat16, [elem_in, elem_out])
