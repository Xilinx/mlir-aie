...
* [Passthrough DMAs](./passthrough_dmas) - Data movement memcpy using ObjectFifos via DMAs only, without involving the AIE core.  Includes optional `--plio input` / `--plio output` modes that demonstrate PLIO-connected soft DMAs in programmable logic on the Xilinx VCK5000.
...
* [Vector Vector Modulo](./vector_vector_modulo) - Single tile performs `vector % vector` of size `256`.
* [Vector Vector Multiply](./vector_vector_mul) - Single tile performs `vector * vector` of size `256`.
