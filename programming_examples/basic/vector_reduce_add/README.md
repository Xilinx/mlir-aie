...
The design body is a single `aie.iron.algorithms.reduce(reduce_add_vector, in_ty, out_ty)` call; the algorithms library handles the ObjectFifo / Worker / Runtime plumbing for the reduce shape (whole-input single-kernel-call).
