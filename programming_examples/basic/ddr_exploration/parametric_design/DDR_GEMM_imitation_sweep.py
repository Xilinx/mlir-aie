...
def main():
    ...
    parser.add_argument('--npu_columns', type=int, choices=[1, 2, 4, 8], help='NPU columns. Phoenix: max 4, Strix: max 8')
    ...
    k_values = [2**i for i in range(4, 10)]


    for k in k_values:

        if b_col_maj:
            # Don't sweep across n, because contiguous data for B is accross k in this case
            n = 32
        else:
            # set n=k when B in row-maj to capture contiguous data BW
            n = k
