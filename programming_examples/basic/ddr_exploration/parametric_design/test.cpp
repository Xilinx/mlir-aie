...
int main(int argc, const char *argv[]) {
  ...
  auto bo_inA = xrt::bo(device, K * N * sizeof(int8_t), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(3));
  auto bo_inB = xrt::bo(device, K * N * sizeof(int8_t), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(4));
  auto bo_out = xrt::bo(device, (M * K + K * N) * sizeof(int8_t), XRT_BO_FLAGS_HOST_ONLY,
                        kernel.group_id(5));
  ...
  int total_size_Kbytes = (2*(M*K + K*N))*sizeof(int8_t)/1024;
