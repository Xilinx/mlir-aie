...
def main():
    ...
    # Create the parser
    parser = argparse.ArgumentParser(description="This script imitates A and B tiled matrix reads from DDR")

    parser.add_argument('--npu_columns', type=int, choices=[1, 2, 4, 8], help='NPU columns. Phoenix: max 4, Strix/Kracken: max 8')
    ...
    # common file name
    file_name = str(M) + 'x' + str(K) + 'x' + str(N) + "_A_B_row_maj"
    ...
    df = DataFrame(columns=['m_k_n', 'Total Size', 'Avg DDR BW (GB/s)', 'Max DDR BW (GB/s)', 'Min DDR BW (GB/s)'])

    # set makefile var both_row to 1, to run only row-maj if B in col-maj, else set to 1
    both_row = 1
    ...
    # sweep across n to capture contiguous data for B matrix in row-maj
    ...
    for k in k_values:
        for n in n_values:
            ...
            make_variables = [f'm={m}', f'k={k}', f'n={n}', f'M={M}', f'K={K}', f'N={N}', 
                                f'both_row={both_row}', f'npu_cols{npu_columns}']
            ...
            if avg_ddr_bw and max_ddr_bw and min_ddr_bw:
                ...
                df.loc[len(df)] = [str(m) + '_' + str(k) + '_' + str(n), total_size_MB.group(1), avg_ddr_bw.group(1), max_ddr_bw.group(1), min_ddr_bw.group(1)]
