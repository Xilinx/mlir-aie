...
def my_passthrough(dev, m, M, k, K, n, N, npu_cols, data_layout_DDR):
    ...
    with mlir_mod_ctx() as ctx:
        ...
        if dev == "npu":
            ...
            raise ValueError("[ERROR] Device name {} is unknown".format(device))
        ...
        def device_body():
            ...
            def sequence(A, B, C):
                ...
                for i in range(npu_cols):
                    ...
                    sizes_A_row_maj = [1, 1, 1, (M // npu_cols) * (K)]
                    strides_A_row_maj = [0, 0, 0, 1]
                    ...
                    # inputs B
                    # row-major
                    sizes_B_row_maj = [1, 1, 1, (N // npu_cols) * K]
                    strides_B_row_maj = [0, 0, 0, 1]
                    ...
                    npu_dma_memcpy_nd(
                        ...
                        bd_id=1,
                        mem=B,
                        offsets=(offsets_B_row_maj),
                        sizes=(sizes_B_row_maj),
                        strides=(strides_B_row_maj),
                        # burst_length=64,
                    )
                    ...
                    npu_dma_memcpy_nd(
                        ...
                        offsets=(
                            write_off_B_row_maj
                        ),  # A writes in first M*K and B after that
                        sizes=(sizes_B_row_maj),
                        strides=(strides_B_row_maj),
                        # burst_length=64,
                    )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "dev", help="npu => Phoenix, npu2 => Strix, Kracken", type=str, default="npu"
    )
    p.add_argument("dims", help="m, M, k, K, n, N", type=int, nargs="*")
    p.add_argument(
        "npu_cols",
        help="Number of NPU columns",
        type=int,
        choices=[1, 2, 4, 8],
        default=4,
    )
    p.add_argument(
        "data_layout_DDR",
        help="Layout of data in DDR",
        type=str,
        choices=["A_row_B_row", "A_row_B_col"],
        default="A_row_B_row",
    )
    args = p.parse_args()

    if len(args.dims) != 6:
        print("ERROR: Must provide all 6 dimensions", file=sys.stderr)
        exit(-1)
