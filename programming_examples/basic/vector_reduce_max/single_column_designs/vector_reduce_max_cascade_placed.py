# single_column_designs/vector_reduce_max_cascade_placed.py -*- Python -*-
#
    ...
    def device_body():
        ...
        for i in range(n_cores):
            ...
            if i == n_cores - 1:
                ...
                def core_body():
                    ...
                    elem_out = out_fifos[i].acquire(ObjectFifoPort.Produce, 1)
                    elem_in = out_fifos[i + 1].acquire(ObjectFifoPort.Consume, 1)
                    compute_max(elem_in, nextC_buffer, elem_out)
                    out_fifos[i + 1].release(ObjectFifoPort.Consume, 1)
                    out_fifos[i].release(ObjectFifoPort.Produce, 1)
        ...
        def sequence(A, C):
            ...
            out_task = shim_dma_single_bd_task(
                out_fifos[0], C, sizes=[1, 1, 1, O], issue_token=True
            )
