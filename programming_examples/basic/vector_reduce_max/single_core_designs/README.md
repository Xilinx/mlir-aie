...
The design body is a single `aie.iron.algorithms.reduce(reduce_max, in_ty, out_ty, trace_size=trace_size)` call; the algorithms library handles the ObjectFifo / Worker / Runtime plumbing, including the optional trace.

## Source Files Overview

1. `vector_reduce_max.py`: An `@iron.jit`-decorated design that delegates its dataflow body to `aie.iron.algorithms.reduce`. Two invocation modes:
