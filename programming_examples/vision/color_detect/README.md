...
`-d npu2` for Strix; `-W` / `-H` override the image dimensions.  Verifies the output against a per-stage numpy reference (`kernels.rgba2hue_ref` + threshold + bitwise OR/AND + gray2rgba) that mirrors `aie_kernels/vision/rgba2hue.cc` plus the other kernel sources.
