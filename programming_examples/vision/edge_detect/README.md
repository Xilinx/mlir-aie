...
The pipeline is mapped onto a single column of the NPU: one Shim tile (0, 0), one Mem tile (0, 1), and four compute tiles (0, 2)–(0, 5). The `--aie-place-tiles` compiler pass assigns these using its default sequential placer. `rgba2gray`, `filter2D`, and `threshold` each get their own compute tile; `gray2rgba` + `addWeighted` share tile (0, 5).
...
Compute results flow through one-to-one ObjectFifos between consecutive stages. The shared-tile `gray2rgba` + `addWeighted` worker uses a local `Buffer` named `rgba_line` to hand data between the two kernels. Both calls execute sequentially in the same Worker, so this scratch storage needs no ObjectFifo acquire/release synchronization. The final RGBA edge-overlaid output goes back through the Mem tile and out the Shim tile.
...
`--dev npu2` selects Strix; `-W` / `-H` override the image dimensions. This mode JIT-compiles and runs on random data, then checks the output against a NumPy reference using the C++ testbench's average-error tolerance.

Inspect the generated IR without running on an NPU:

```shell
python3 edge_detect.py --dev npu2 --emit-mlir -W 64 -H 4
```
