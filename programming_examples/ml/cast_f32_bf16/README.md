...
This design implements an element-wise narrowing cast from `float32` to `bfloat16` across an 8-core split, round-to-nearest-even. It runs on NPU1 (aie2) and NPU2 (aie2p); the underlying kernel lives under `aie_kernels/aie2p/` and builds for both.
...
1. `cast_f32_bf16.cc`: the kernel, pulled from [`aie_kernels/aie2p/`](../../../aie_kernels/aie2p/); the same source builds for aie2.
