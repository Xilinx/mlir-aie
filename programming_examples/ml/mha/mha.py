...
def batched_matmul_single_core(
    ...
    assert num_q_blocks % number_of_pipelines == 0, "Number of Q blocks must be divisible by number of pipelines (for now)"
    ...
    inQ = ObjectFifo(np.ndarray[(number_of_pipelines_join_distribute * B_q, d), np.dtype[dtype]], name="inQ")
    memQ = inQ.cons().split(offsets=[B_q * d * i for i in range(number_of_pipelines_join_distribute)],
    ...
    if number_of_pipelines > 6:
        inQ2 = ObjectFifo(np.ndarray[(number_of_pipelines_join_distribute * B_q, d), np.dtype[dtype]], name="inQ2")
        memQ += inQ2.cons().split(offsets=[B_q * d * i for i in range(number_of_pipelines_join_distribute)],
                                        obj_types=[q_ty] * number_of_pipelines_join_distribute,
    ...
    for i in range(number_of_pipelines):
        memA.append(ObjectFifo(qk_ty, 
                               depth=of_depth,
                               name=f"memA{i}"
                               )
                    )
        outA.append(memA[i].cons().forward(name=f"outA{i}", dims_to_stream=a_dims, 
                                            depth=of_depth,
    ...
    outP = []
    for i in range(number_of_pipelines):    
        memP.append(ObjectFifo(qk_ty, 
                               depth=of_depth,
                               name=f"memP{i}"))
        outP.append(memP[i].cons().forward(name=f"outP{i}", dims_to_stream=q_dims, 
                                           depth=of_depth,
                                            # placement=Tile(col=i, row=1) 
                                           )) # Local to 1 pipeline
    ...
    memO = ObjectFifo(np.ndarray[(number_of_pipelines_join_distribute * B_q, d), np.dtype[dtype]], name="memO", dims_to_stream=o_dims)
    outO = memO.prod().join(offsets=[B_q * d * i for i in range(number_of_pipelines_join_distribute)],
                            obj_types=[q_ty] * number_of_pipelines_join_distribute,
    ...
    if number_of_pipelines > 6:
        memO2 = ObjectFifo(np.ndarray[(number_of_pipelines_join_distribute * B_q, d), np.dtype[dtype]], name="memO2", dims_to_stream=o_dims)
    ...
    def softmax(of_in_a, of_out_p, of_out_scale, partial_softmax, init_scale_buffer, memcopy_kernel_scale):
        
        scale_buffer = LocalBuffer(initial_value=np.zeros(shape=(4*B_q,), dtype=dtype))
    ...
    Q_tiles = TensorTiler2D.group_tiler((heads * S_q, d), (number_of_pipelines_join_distribute * B_q, d), (1, 1))
    
    K_tiles = TensorTiler2D.group_tiler((heads* S_kv, d), (S_kv, d), (1, 1))
    
    V_tiles = TensorTiler2D.group_tiler((heads* S_kv, d), (S_kv, d), (1, 1))
    
    O_tiles = TensorTiler2D.group_tiler((heads * S_q, d), (number_of_pipelines_join_distribute * B_q, d), (1, 1))
    ...
    with rt.sequence(Q_ty, KV_ty, KV_ty, Q_ty) as (Q, K, V, O):
        ...
        for head_idx in range(heads):
            ...
            for q_block_idx in range(num_q_blocks // number_of_pipelines):
                ...
                rt.fill(inQ.prod(), Q, tap=Q_tiles[head_idx*2 + q_block_idx], 
                            placement = Tile(col = 4, row = 0)
                        )
                if number_of_pipelines > 6:
                    rt.fill(inQ2.prod(), Q, tap=Q_tiles[head_idx*2 + q_block_idx + 1], 
                                placement = Tile(col = 4, row = 0)
                            )
                
                # Thow on bd containing the full K and V in the object fifo, then does it transfer cunks of inKV size at the time?
                rt.fill(inK.prod(), K, tap=K_tiles[head_idx],
                            placement = Tile(col = 5, row = 0)
                        )
                rt.fill(inV.prod(), V, tap=V_tiles[head_idx],
                            placement = Tile(col = 6, row = 0)
                        )
                    
                rt.drain(memO.cons(), O, tap=O_tiles[head_idx*2 + q_block_idx],
                         wait=True,
                         placement = Tile(col = 7, row = 0)
                        )
                if number_of_pipelines > 6:
                    rt.drain(memO2.cons(), O, tap=O_tiles[head_idx*2 + q_block_idx + 1],
                            wait=True,
                            placement = Tile(col = 7, row = 0)
                            )
