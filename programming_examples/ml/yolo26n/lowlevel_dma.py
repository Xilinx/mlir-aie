...
class StaticWeightStream(Resolvable):
    ...
    def resolve(self, loc=None, ip=None) -> None:
        ...
        # --- MemTile side ---
        # Source-side locks are intentionally omitted. With a static-init
        # producer + no upstream link, the BD chain has no source to
        # replenish a `cons` lock — using one would cap total firings at
        # `init` and silently deadlock at N>1 dispatches (we hit exactly
        # this bug for chunked-OC weights via ObjectFifo.set_iter_count;
        # the placed-dialect path here has the same shape). Downstream
        # back-pressure via the compute tile's S2MM `comp_prod_lock`
        # naturally gates BD firings — the DMA stream pauses when the
        # consumer's recv buffer is full and resumes after `release()`.
        # This mirrors mlir-aie's `isCycledStaticInitProducer` lock-skip
        # path in AIEObjectFifoStatefulTransform.cpp.
        ...
        if self._ping_pong_buf is not None:
            ...
            def _mtdma(block):
                ...
                with block[1]:
                    dma_bd(wts_buf)
                    next_bd(block[1])  # infinite loop, gated by downstream S2MM
                with block[2]:
                    EndOp()
