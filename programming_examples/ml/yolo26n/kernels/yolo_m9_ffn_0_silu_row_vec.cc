...
//
// Vectorized streamed (chunked-OC) 1x1 i8 conv 128 -> 256 + bias + SiLU
// LUT, for the PSA ffn's first conv. Drop-in .o-level replacement for
// yolo_m9_ffn_0_silu_row.cc (same symbol + ABI).
//
// Same pattern as yolo_m9_cv1_split_vec.cc but without the top/bot split:
// per call writes its chunk_oc OC slice into a single `mid_out` buffer
// at offset `chunk_idx * chunk_oc`. Inner reduction: aie::mmul<4,8,8,int8,int8>.
//
//===----------------------------------------------------------------------===//
  ...
  for (int oc_t = 0; oc_t < chunk_oc_tiles; ++oc_t) {
    for (int x_tile = 0; x_tile < x_tiles; ++x_tile) {
      ...
      for (int p = 0; p < 4; ++p) {
        ...
        for (int j = 0; j < 8; ++j) {
          ...
          mid_out[x_out * out_c + dst_oc_offset + oc_t * 8 + j] = silu_lut[sr + 128];
        }
      }
    }
    ...
    for (int x = x_tiles * 4; x < input_width; ++x) {
      for (int j = 0; j < 8; ++j) {
        ...
        if (sr > I8_MAX) sr = I8_MAX;
        if (sr < I8_MIN) sr = I8_MIN;
        mid_out[x * out_c + dst_oc_offset + chunk_oc_full] = silu_lut[sr + 128];
      }
    }
  }
