...
#define AIE_HINT_IC
  ...
  for (int chunk_oc_t = 0; chunk_oc_t < chunk_oc_tiles; ++chunk_oc_t) {
    ...
    // SRS+saturated result (no separate vec epilogue, no helper call).
    ...
    for (int x_tile = 0; x_tile < x_tiles; ++x_tile) {
      MMUL4x8x8 acc;
      acc = bias_acc;  // bias-init instead of zero
      ...
      for (int p = 0; p < 4; ++p) {
        ...
        for (int j = 0; j < 8; ++j) {
          ...
          output[x_out * OUT_C + oc_full] = silu_lut[int(srs_v[p * 8 + j]) + 128];
        }
      }
    }
