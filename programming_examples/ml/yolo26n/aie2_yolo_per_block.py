...
def _build_c3k2_heavy(block_name: str, act_in, manifest):
    ...
    split_a = ObjectFifo(_i8((in_w, 1, cp)), depth=4, exact_depth=_exact)
    split_b = ObjectFifo(_i8((in_w, 1, cp)), depth=in_h, exact_depth=_exact)
    inner_0_out = ObjectFifo(_i8((in_w, 1, cp)), depth=4, exact_depth=_exact)
...
def per_block_iron(block_name: str) -> str:
    ...
    if block_name == "m9":
        ...
        # alias.
        stage = int(os.environ.get("M9_STAGE", os.environ.get("M9_CHAIN_STAGE", "10")))
        spec = importlib.util.spec_from_file_location(
            "m9_stage",
            pathlib.Path(__file__).parent / "scripts" / "m9_stage.py",
        )
