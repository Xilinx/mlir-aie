...
def run_one_seed(seed: int, opts, npu_kernel) -> int:
    ...
    wq_fp = random_w(QD, D)
    bq = np.zeros(QD, dtype=np.int32)
    wo_fp = random_w(D, QD)
    bo = np.zeros(D, dtype=np.int32)
    wg_fp = random_w(HD, D)
    ...
    def requant(fp, inv):
        s = fp * np.float32(inv)
        r = np.where(
            s >= 0, np.floor(s + np.float32(0.5)), np.ceil(s - np.float32(0.5))
        )
    ...
    wq_prefix = fp32_bytes(ACT_SCALE, q_inv_out, q_out_scale, 0.0) + b"\x00" * 48
    ...
    wu_prefix = (
        fp32_bytes(ACT_SCALE, up_inv_out, silu_up_scale, silu_inv_out_scale)
        + b"\x00" * 48
    )
...
def main():
    ...
    print(
        f"layer-d2048 (perchan + dynamic-scale runtime) D={D} HD={HD} "
    ...
    if n_fail == 0:
        print(
            f"BIT-EXACT PASS x {len(seeds)}  (perchan + dynamic-scale single-layer D=2048)"
        )
        return 0
