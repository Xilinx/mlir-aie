...
class LayerW:
    ...
    wq_i8:    np.ndarray; wq_sc: np.ndarray   # (Q_DIM, D),  (Q_DIM,)
    wk_i8:    np.ndarray; wk_sc: np.ndarray   # (KV_DIM, D), (KV_DIM,)
    wv_i8:    np.ndarray; wv_sc: np.ndarray   # (KV_DIM, D), (KV_DIM,)
    wo_i8:    np.ndarray; wo_sc: np.ndarray   # (D, Q_DIM),  (D,)
    wg_i8:    np.ndarray; wg_sc: np.ndarray   # (HD, D),     (HD,)
    wu_i8:    np.ndarray; wu_sc: np.ndarray   # (HD, D),     (HD,)
    wd_i8:    np.ndarray; wd_sc: np.ndarray   # (D, HD),     (D,)


@dataclass
class ModelW:
    embed_i8: np.ndarray; embed_sc: np.ndarray   # (V, D), (V,)
    final_norm: np.ndarray                       # (D,) bf16
    layers: list[LayerW]


def _load_qw(data_dir: Path, stem: str, shape: tuple[int, int]) -> tuple[np.ndarray, np.ndarray]:
    q   = np.frombuffer((data_dir / f"{stem}.i8.bin").read_bytes(), dtype=np.int8).reshape(shape)
    sc  = np.frombuffer((data_dir / f"{stem}.scales.f32.bin").read_bytes(), dtype=np.float32)
...
def _load_bf16(data_dir: Path, stem: str, shape: tuple[int, ...]) -> np.ndarray:
    return np.frombuffer((data_dir / f"{stem}.bf16.bin").read_bytes(),
                         dtype=ml_dtypes.bfloat16).reshape(shape)
...
def load_model(data_dir: Path) -> ModelW:
    ...
    for L in range(N_LAYERS):
        ...
        layers.append(LayerW(
            ...
            wq_i8=wq_i8, wq_sc=wq_sc,
            wk_i8=wk_i8, wk_sc=wk_sc,
            wv_i8=wv_i8, wv_sc=wv_sc,
            wo_i8=wo_i8, wo_sc=wo_sc,
            wg_i8=wg_i8, wg_sc=wg_sc,
            wu_i8=wu_i8, wu_sc=wu_sc,
            wd_i8=wd_i8, wd_sc=wd_sc,
        ))
    return ModelW(embed_i8=embed_i8, embed_sc=embed_sc,
...
def rmsnorm(x_f32: np.ndarray, gamma_bf16: np.ndarray, eps: float = RMS_EPS) -> np.ndarray:
...
# ---------------------------------------------------------------------------
def forward_layer_int8(x_f32: np.ndarray, W: LayerW,
...
def main():
    ...
    p.add_argument("--layer", type=int, default=0,
                   help="single layer to test (default 0)")
    ...
    if args.full:
        ...
        print(f"  out shape: {out.shape}  norm: {np.linalg.norm(out):.3f}  "
