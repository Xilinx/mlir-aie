...
def build():
    ...
    workers = [
        ...
        # flowkv_qk: drops kr
        Worker(w_binary, [of_qr.cons(), of_kr.cons(), of_rec.prod(), k_first_QD_KVD], tile=Tile(0, 4)),
        # flowkv_sv: drops vf  (rec comes first to match k_first_QD_KVD signature)
        Worker(w_binary, [of_rec.cons(), of_vf.cons(), of_af.prod(), k_first_QD_KVD],
               tile=Tile(0, 5)),
        # o_proj
        Worker(w_unary, [of_af.cons(),  of_op.prod(), k_copy_QD_D],    tile=Tile(3, 2)),
        # add1
        Worker(w_binary, [of_op.cons(), of_in.cons(), of_x1.prod(), k_add_D], tile=Tile(6, 5)),
        # rmsnorm2
        ...
        # silu_mul
        Worker(w_binary, [of_gf.cons(), of_uf.cons(), of_sf.prod(), k_add_HD], tile=Tile(4, 5)),
        # down_proj
        Worker(w_unary, [of_sf.cons(), of_df.prod(), k_copy_HD_D],     tile=Tile(6, 2)),
        # add2
        Worker(w_binary, [of_df.cons(), of_x1.cons(), of_out.prod(), k_add_D], tile=Tile(7, 5)),
    ]
