...
def build():
    ...
    k_q = Kernel(
        "llama_gemm_tiled_attn_K2048_N4_perchan_v2_up",
        KO_GEMM,
        [t_D_i8, t_WQ_slot, t_QF_i8, np.int32],
    )
    # o_proj: _perchan_v2. Slot WO_SLOT (64 B prefix). 4 args.
    k_o = Kernel(
        "llama_gemm_tiled_attn_K64_N4_perchan_v2",
        KO_GEMM,
    ...
    workers = [
        Worker(
            w_rms,
            [of_xin.cons(), of_gamma.cons(), of_h1.prod(), k_rms],
            tile=Tile(5, 4),
        ),
        Worker(
            w_q,
        ...
        Worker(
            w_add, [of_op.cons(), of_xin.cons(), of_out.prod(), k_add], tile=Tile(6, 5)
        ),
    ]
    ...
    with rt.sequence(rt_xin_ty, rt_w_ty, rt_kv_ty, rt_out_ty) as (
        ...
        rt.fill(
            of_kcache.prod(),
            kvblob,
            tap=strided_tap(KV_BYTES, OFF_K, KCACHE_PADDED, KCACHE_PADDED, 1),
        ...
        rt.fill(
            of_vcache.prod(),
            kvblob,
            tap=strided_tap(KV_BYTES, OFF_V, VCACHE_PADDED, VCACHE_PADDED, 1),
