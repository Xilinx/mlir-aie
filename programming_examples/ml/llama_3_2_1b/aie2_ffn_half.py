...
def build():
    ...
    workers = [
        ...
        Worker(
            w_add, [of_df.cons(), of_xin.cons(), of_out.prod(), k_add], tile=Tile(7, 5)
        ),
    ]
    ...
    with rt.sequence(*seq_tys) as seq_args:
        ...
        rt.fill(
            of_wu.prod(),
            wblob,
            tap=strided_tap(WEIGHTS_BYTES, OFF_WU, WU_SLOT, WU_SLOT, N_TILES_UP),
            task_group=wu_tg,
        )
        ...
        rt.fill(
            of_wd.prod(),
            wblob,
            tap=strided_tap(WEIGHTS_BYTES, OFF_WD, WD_SLOT, WD_SLOT, N_TILES_DOWN),
            task_group=wd_tg,
        )
        ...
        if debug_drain:
            ...
            tap = (
                strided_tap(HD, 0, D, D, 1)
                if debug_drain in ("gf", "uf", "sf")
                else None
            )
            if tap is not None:
                rt.drain(
                    target.cons(), debug_out, tap=tap, wait=True, task_group=debug_tg
                )
            else:
                rt.drain(target.cons(), debug_out, wait=True, task_group=debug_tg)
