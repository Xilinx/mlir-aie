...
# kvblob: per-layer block, each containing 8 KV heads' (k_header+kcache,
# v_header+vcache) combined slots = 8 * 16392 = 131136 B per layer.
PER_KV_HEAD_BYTES = KCACHE_PADDED + VCACHE_PADDED     # 16392
PER_LAYER_KV      = N_HEADS_KV * PER_KV_HEAD_BYTES    # 131136
KV_HALF_BYTES     = (N_HEADS_KV // 2) * PER_KV_HEAD_BYTES  # 65568
...
def build():
    ...
    workers = [
        # Router
        Worker(w_router, [of_seed.cons(), of_back.cons(), of_routed.prod()],
               tile=Tile(3, 4), stack_size=PSK),
    ...
    with rt.sequence(rt_xin_ty, rt_w_ty, rt_kv_ty, rt_out_ty) as (
        ...
        fill_specs = []
        def add_per_layer(prod, src, base_off, per_layer_stride,
        ...
        add_per_layer(of_gam_in.prod(),   wblob, OFF_GAMMA_IN,
                      GAMMA_BYTES, GAMMA_BYTES, 1, WEIGHTS_BYTES)
        add_per_layer(of_wq.prod(),       wblob, OFF_WQ,
                      N_TILES_Q * WQ_SLOT, WQ_SLOT, N_TILES_Q, WEIGHTS_BYTES)
        add_per_layer(of_cs.prod(),       wblob, OFF_CS,
                      CS_BYTES, CS_BYTES, 1, WEIGHTS_BYTES)
        add_per_layer(of_wo.prod(),       wblob, OFF_WO,
                      N_TILES_O * WO_SLOT, WO_SLOT, N_TILES_O, WEIGHTS_BYTES)
        add_per_layer(of_afscales.prod(), wblob, OFF_AF_SCALES,
                      AF_SCALES_BYTES, AF_SCALES_BYTES, 1, WEIGHTS_BYTES)
        add_per_layer(of_gam_post.prod(), wblob, OFF_GAMMA_POST,
                      GAMMA_BYTES, GAMMA_BYTES, 1, WEIGHTS_BYTES)
        add_per_layer(of_wg.prod(),       wblob, OFF_WG,
                      N_TILES_G * WG_SLOT, WG_SLOT, N_TILES_G, WEIGHTS_BYTES)
        add_per_layer(of_wu.prod(),       wblob, OFF_WU,
                      N_TILES_U * WU_SLOT, WU_SLOT, N_TILES_U, WEIGHTS_BYTES)
        add_per_layer(of_wd.prod(),       wblob, OFF_WD,
                      N_TILES_D * WD_SLOT, WD_SLOT, N_TILES_D, WEIGHTS_BYTES)

        # 2 KV fills per layer (lo / hi halves; each fans out to 4 attn workers).
        kv_specs = [
            (of_kv_lo.prod(), kvblob, 0,             PER_LAYER_KV,
             KV_HALF_BYTES, 1, KV_BYTES),
            (of_kv_hi.prod(), kvblob, KV_HALF_BYTES, PER_LAYER_KV,
             KV_HALF_BYTES, 1, KV_BYTES),
        ]
