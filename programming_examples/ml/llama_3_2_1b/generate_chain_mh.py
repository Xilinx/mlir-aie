"""Phase 7b end-to-end: real-weight host-decode loop around the N=16
multi-head GQA chain xclbin. Per token: tokenize -> embed -> numpy
forward through all N layers (for per-layer dynamic-scale calibration)
-> pack wblob/kvblob -> NPU dispatch (16 mh layers) -> final_norm +
lm_head -> argmax -> next token.

Output won't be coherent until on-device KV append lands (Phase 8) --
KV cache is still random per seed. But this exercises the entire N=16
mh production pipeline through every dispatch.
...
from aie2_chain_dynscale_mh import (
    D, N_LAYERS, ACT_SCALE, INV_ACT_SCALE, SILU_GATE_SCALE,
)
...
def main():
    ...
    for L in range(N_LAYERS):
        ...
        layers.append(layer)
    print(f"loaded {N_LAYERS} mh layers", flush=True)
    ...
    for step in range(opts.new_tokens):
        ...
        # Numpy forward through all N_LAYERS layers for dynamic-scale
        # calibration. Same pattern as generate.py / chain_dynscale.
        x_after = x_in.copy()
        for L in range(N_LAYERS):
            x_after, scales = numpy_layer_mh_forward(x_after, layers[L])
