...
def numpy_gemm_perchan(act_i8: np.ndarray, x_scale: float,
                       w_i8: np.ndarray, w_scales: np.ndarray,
                       bias_i32: np.ndarray, inv_out_scale: float) -> np.ndarray:
    ...
    fp  = (acc.astype(np.float32) * np.float32(x_scale)
                                  * w_scales.astype(np.float32))       # (N,)
...
def pack_tiled_perchan(w_i8: np.ndarray, w_scales: np.ndarray,
                       bias_i32: np.ndarray, n_tile: int) -> np.ndarray:
    ...
    for t in range(n_tiles):
        ...
        off = base
        blob[off:off + rows.size] = rows; off += rows.size
        blob[off:off + bs.size]   = bs;   off += bs.size
...
def main():
    ...
    print(f"per-channel scales: min={w_scales.min():.4f}  max={w_scales.max():.4f}  "
    ...
    fp_out = (acc.astype(np.float32) * np.float32(act_scale)
                                     * w_scales.astype(np.float32))
    ...
    # --- numpy reference ---
    expected = numpy_gemm_perchan(act_i8, act_scale, w_i8, w_scales,
                                  bias_i32, inv_out_scale)
    print(f"numpy expected: range=[{int(expected.min())}, {int(expected.max())}]  "
    ...
    rebuild = (not stamp_path.exists()
               or stamp_path.read_text().strip() != stamp)
    if rebuild:
        build_dir.mkdir(parents=True, exist_ok=True)
        print(f"rebuilding xclbin with act_scale={act_scale} inv_out_scale={inv_out_scale} ...")
        # Force re-MLIR by removing the previous MLIR and xclbin.
        for f in ["aie_gemm_tiled_perchan_K{K}_N{N}_t{nt}.mlir",
        ...
        rc = subprocess.run(
            ["make", "tiled_gemm_perchan",
             f"TILED_PC_K={K}", f"TILED_PC_N={N}", f"TILED_PC_N_TILE={n_tile}",
    ...
    fake_opts = Namespace(xclbin=str(xclbin), instr=str(insts),
                          kernel="MLIR_AIE", verbosity=opts.verbosity,
                          verify=False, iters=1, warmup_iters=0,
                          trace_size=0, trace_file=None,
                          in1_size=None, in2_size=None, out_size=None,
                          ddr_id=None, enable_ctrl_pkts=False)
    npu = test_utils.create_npu_kernel(fake_opts)

    rc = DefaultNPURuntime.run_test(npu.npu_kernel,
                                    [a_t, w_t, o_t], {},
    ...
    print(f"tiled_gemm_perchan NPU vs numpy: K={K} N={N} N_TILE={n_tile}  "
