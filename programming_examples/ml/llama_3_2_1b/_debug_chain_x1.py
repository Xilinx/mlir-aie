...
def numpy_per_layer_x1(x_in, layers):
    ...
    for L in range(N_LAYERS):
        ...
        sv_fp = compute_sv_fp(
            qr, kcache, vcache, HEAD_D, T, q_out_scale, k_scale, v_scale, lut_exp
        )
...
def main():
    ...
    rc = DefaultNPURuntime.run_test(
        npu_kernel, args, {}, verify=False, verbosity=opts.verbosity
    )
    if rc != 0:
        return rc
    if trace_x1:
    ...
    if trace_af:
        ...
        for L in range(N_LAYERS):
            ...
            fp_q = (
                (
                    layer["wq_i8"].astype(np.int32) @ h1.astype(np.int32) + layer["bq"]
                ).astype(np.float32)
                * np.float32(ACT_SCALE)
                * layer["wq_sc"].astype(np.float32)
            )
            qf = requant(fp_q, sc["q_inv_out"])
            qr = numpy_rope_dyn(qf, layer["cos"], layer["sin"], N_HEADS, HEAD_D)
            af_ref = numpy_attention(
                qr,
                layer["kcache"],
                layer["vcache"],
                HEAD_D,
    ...
    if trace_qf:
        ...
        for L in range(N_LAYERS):
            ...
            tail = np.frombuffer(
                qf_trace[L * QF_BYTES + QD : L * QF_BYTES + QD + 8].tobytes(),
                dtype=np.float32,
            )
            packed_scale = np.float32(sc["q_out_scale"])
            tail_ok = (
                "✓"
                if tail[0] == packed_scale
                else f"✗ tail={tail[0]} expect={packed_scale}"
            )
            print(
                f"      tail bytes: q_scale={tail[0]:.10f}  spare={tail[1]:.6f}  ({tail_ok})"
            )
            x, _ = numpy_layer_forward(x, layer)
    ...
    if trace_cs:
        ...
        for L in range(N_LAYERS):
            ...
            if diff and L == 0:
                ...
                print(
                    f"      bf16 mismatches: {bdiff}/{len(actual_bf)}  first={first.tolist()}"
                )
                for i in first:
                    print(
                        f"      bf16 i={i}: NPU={float(actual_bf[i])} ref={float(ref_bf[i])}"
                    )
        ...
        for L in range(N_LAYERS):
            ...
            if n_diff and L == 0:
                for i in np.argwhere(diff != 0).flatten()[:5]:
                    print(
                        f"      i={i}: NPU={qr_actual[i]} ref={qr_ref[i]} qf[i]={qf[i]}"
                    )
            x, _ = numpy_layer_forward(x, layer)
