...
def build():
    ...
    of_routed = ObjectFifo(
        t_D_i8, depth=1, name="routed"
    )  # broadcast to rmsnorm1 + add1
    ...
    k_q = Kernel(
        "llama_gemm_tiled_layer_K2048_N4_perchan_v2_up_q",
        KO_GEMM,
        [t_D_i8, t_WQ_slot, t_QF_i8, np.int32],
    )
    k_o = Kernel(
        "llama_gemm_tiled_layer_K64_N4_perchan_v2_o",
        KO_GEMM,
    ...
    k_rope = Kernel("llama_rope_int8_dyn", KO_ROPE, [t_QF_i8, t_CS_bf16, t_QR_i8])
    k_qk = Kernel("llama_flowkv_qk_dyn", KO_FKV, [t_QR_i8, t_KCACHE_i8, t_PROBS_fp32])
    k_sv = Kernel("llama_flowkv_sv_dyn", KO_FKV, [t_VCACHE_i8, t_PROBS_fp32, t_AF_i8])
    ...
    workers = [
        ...
        Worker(
            w_rope, [of_qf.cons(), of_cs.cons(), of_qr.prod(), k_rope], tile=Tile(4, 4)
        ),
        Worker(
            w_qk,
    ...
    if trace_qf:
        seq_tys = seq_tys + (rt_trace_qf_ty,)
    if trace_qr:
        seq_tys = seq_tys + (rt_trace_qr_ty,)
    if trace_cs:
        seq_tys = seq_tys + (rt_trace_cs_ty,)
    with rt.sequence(*seq_tys) as seq_args:
        ...
        kv_specs = [
            (of_kcache.prod(), kvblob, OFF_K, PER_LAYER_KV, KCACHE_PADDED, 1, KV_BYTES),
            (of_vcache.prod(), kvblob, OFF_V, PER_LAYER_KV, VCACHE_PADDED, 1, KV_BYTES),
        ]
