...
from aie2_chain_dynscale import (
    D, HD, HEAD_D, QD, KVD, T, N_TILE, N_LAYERS,
    ...
    KV_HEADER, PER_LAYER_KV, OFF_K, OFF_V, KV_BYTES, WEIGHTS_BYTES,
    ACT_SCALE, INV_ACT_SCALE,
)
from test_chain_dynscale import (
    gen_real_layer, numpy_layer_forward, requant,
)
...
def load_tokenizer(tokenizer_path: Path):
    ...
    PAT = (r"(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,3}"
           r"| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+")
    SPECIAL = {"<|begin_of_text|>": 128000, "<|end_of_text|>": 128001,
               "<|eot_id|>": 128009}
    return tiktoken.Encoding(name="llama3", pat_str=PAT,
...
def pack_blobs(layers):
    ...
    def pack_slot(off, w_i8, w_sc, bias, n_tiles_per_layer, prefix_bytes):
        slot_bytes = (len(prefix_bytes) + N_TILE * w_i8.shape[1]
                      + N_TILE * 4 + N_TILE * 4)
