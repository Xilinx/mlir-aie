...
from aie2_ffn_half import (
    D, HD, N_TILE,
    N_TILES_GATE, N_TILES_UP, N_TILES_DOWN,
    OFF_GAMMA, OFF_WG, OFF_WU, OFF_WD,
    GAMMA_BYTES, ACT_SCALE, SILU_GATE_SCALE, GATE_INV_OUT_SCALE,
    WG_SLOT, WU_SLOT, WD_SLOT, WEIGHTS_BYTES,
)
...
def numpy_gemm_perchan(act_i8, x_scale, w_i8, w_scales, bias_i32,
                       inv_out_scale):
...
def pack_perchan_slots(w_i8, w_scales, bias_i32, n_tile,
...
def run_one_seed(seed: int, opts, lut_bf, npu_kernel) -> int:
    ...
    up_prefix = (fp32_bytes(ACT_SCALE, up_inv_out_scale,
                            silu_up_scale, silu_inv_out_scale)
                 + b"\x00" * 48)
    wu_packed = pack_perchan_slots(wu_i8, wu_sc, bu, N_TILE,
                                   prefix_bytes=up_prefix)
    # Down: 64 B prefix = (down_act_scale, down_inv_out_scale) + 56 B pad.
    down_prefix = (fp32_bytes(down_act_scale, down_inv_out_scale)
                   + b"\x00" * 56)
    wd_packed = pack_perchan_slots(wd_i8, wd_sc, bd, N_TILE,
                                   prefix_bytes=down_prefix)

    assert wg_packed.size == N_TILES_GATE * WG_SLOT, (wg_packed.size, N_TILES_GATE * WG_SLOT)
    assert wu_packed.size == N_TILES_UP   * WU_SLOT, (wu_packed.size, N_TILES_UP   * WU_SLOT)
    assert wd_packed.size == N_TILES_DOWN * WD_SLOT, (wd_packed.size, N_TILES_DOWN * WD_SLOT)
...
def main():
    ...
    print(f"FFN-half (perchan + dynamic-scale runtime) D={D} HD={HD} "
