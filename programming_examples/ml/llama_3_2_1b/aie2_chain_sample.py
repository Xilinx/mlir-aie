...
def build():
    ...
    k_gemm_DD  = Kernel("llama_gemm_int8_srs_D_to_D",   KO_GEMM,
                        [t_D_i8,  t_WQ_i8, t_D_i8,  np.int32])
    k_gemm_DHD = Kernel("llama_gemm_int8_srs_D_to_HD",  KO_GEMM,
                        [t_D_i8,  t_WG_i8, t_HD_i8, np.int32])
    k_gemm_HDD = Kernel("llama_gemm_int8_srs_HD_to_D",  KO_GEMM,
                        [t_HD_i8, t_WD_i8, t_D_i8,  np.int32])
    k_gemm_DV  = Kernel("llama_gemm_int8_srs_D_to_V",   KO_GEMM,
                        [t_D_i8,  t_LMW_i8, t_V_i8, np.int32])
    ...
    def w_gemm(c_act, c_w, c_out, k):
        for _ in range_(N_LAYERS):
            a = c_act.acquire(1); w = c_w.acquire(1); o = c_out.acquire(1)
            k(a, w, o, RIGHT_SHIFT)
            c_act.release(1); c_w.release(1); c_out.release(1)
    ...
    def w_add2_tail(c_a, c_b, c_back, c_xfinal, k):
        ...
        a = c_a.acquire(1); b = c_b.acquire(1); o = c_xfinal.acquire(1)
        k(a, b, o)
        c_a.release(1); c_b.release(1); c_xfinal.release(1)

    # --- lm_head + sample workers (fire once, no range_) ---
    def w_lm_head(c_x, c_w, c_logits, k):
        x = c_x.acquire(1); w = c_w.acquire(1); o = c_logits.acquire(1)
        k(x, w, o, RIGHT_SHIFT)
        c_x.release(1); c_w.release(1); c_logits.release(1)

    def w_sample(c_logits, c_params, c_token, k):
        l = c_logits.acquire(1); p = c_params.acquire(1); t = c_token.acquire(1)
        k(l, t, p)
        c_logits.release(1); c_params.release(1); c_token.release(1)
    ...
    workers = [
        ...
        Worker(w_rms, [of_routed.cons(), of_gam_in.cons(), of_h1.prod(), k_rms],
               tile=Tile(5, 4)),
        Worker(w_gemm, [of_h1.cons(), of_wq.cons(), of_qf.prod(), k_gemm_DD],
               tile=Tile(0, 2)),
        Worker(w_rope_fn, [of_qf.cons(), of_cs.cons(), of_qr.prod(), k_rope],
               tile=Tile(4, 4)),
        Worker(w_qk_fn, [of_qr.cons(), of_kcache.cons(), of_probs.prod(), k_qk],
               tile=Tile(0, 4)),
        Worker(w_sv_fn, [of_vcache.cons(), of_probs.cons(), of_af.prod(), k_sv],
               tile=Tile(0, 5)),
        Worker(w_gemm, [of_af.cons(), of_wo.cons(), of_op.prod(), k_gemm_DD],
               tile=Tile(3, 2)),
        Worker(w_add_fn, [of_op.cons(), of_routed.cons(), of_x1.prod(), k_add],
               tile=Tile(6, 5)),
        Worker(w_rms, [of_x1.cons(), of_gam_post.cons(), of_h2.prod(), k_rms],
               tile=Tile(6, 4)),
        Worker(w_gemm, [of_h2.cons(), of_wg.cons(), of_gf.prod(), k_gemm_DHD],
               tile=Tile(4, 2)),
        Worker(w_gemm, [of_h2.cons(), of_wu.cons(), of_uf.prod(), k_gemm_DHD],
               tile=Tile(5, 2)),
        Worker(w_silu_fn, [of_gf.cons(), of_uf.cons(), of_sf.prod(), k_silu],
               tile=Tile(4, 5)),
        Worker(w_gemm, [of_sf.cons(), of_wd.cons(), of_df.prod(), k_gemm_HDD],
               tile=Tile(6, 2)),
        Worker(w_add2_tail, [of_df.cons(), of_x1.cons(),
                             of_back.prod(), of_x_final.prod(), k_add],
               tile=Tile(7, 5)),
        ...
        Worker(w_sample,
               ...
               tile=Tile(5, 5),
               stack_size=16384),  # Bug 5: kV-sized stack arrays
    ]

    rt = Runtime()
    with rt.sequence(rt_xin_ty, rt_w_ty, rt_kv_ty, rt_aux_ty, rt_token_ty) as (
        xin, wblob, kvblob, aux, token
    ):
        rt.start(*workers)
        ...
        def per_fifo(fifo_prod, src, off_per_layer, off_offset, nbytes, total):
            ...
            rt.fill(fifo_prod, src,
                    ...
                    task_group=tg)
        per_fifo(of_gam_in.prod(),   wblob,  PER_LAYER_W,  OFF_GAMMA_IN,   GAMMA_BYTES, TOTAL_W)
        per_fifo(of_gam_post.prod(), wblob,  PER_LAYER_W,  OFF_GAMMA_POST, GAMMA_BYTES, TOTAL_W)
