...
from aie2_ffn_half import (
    D, HD, N_TILE,
    N_TILES_GATE, N_TILES_UP,
    OFF_GAMMA, OFF_WG, OFF_WU,
    GAMMA_BYTES, ACT_SCALE, INV_ACT_SCALE,
    GATE_INV_OUT_SCALE, UP_INV_OUT_SCALE,
    SILU_IN_SCALE, SILU_INV_OUT_SCALE,
    WG_SLOT, WG_TOTAL,
    factor, strided_tap,
    _i8, _bf16,
)
...
def build():
    ...
    k_gemm = Kernel("llama_gemm_tiled_K2048_N4_perchan", KO_GEMM,
                    [t_D_i8, t_WG_slot, t_HD_i8, np.int32,
                     np.float32, np.float32])
    k_silu = Kernel("llama_silu_mul_int8", KO_SILU,
                    [t_HD_i8, t_HD_i8, t_HD_i8, np.float32, np.float32]) if _INCLUDE_SILU else None

    def w_rms(c_in, c_g, c_out, k):
        x = c_in.acquire(1); g = c_g.acquire(1); o = c_out.acquire(1)
        k(x, g, o, ACT_SCALE, INV_ACT_SCALE)
        c_in.release(1); c_g.release(1); c_out.release(1)
    ...
    workers = [
        Worker(w_rms,  [of_xin.cons(), of_gamma.cons(), of_h2.prod(), k_rms],
               tile=Tile(5, 4)),
        Worker(w_gate, [of_h2.cons(), of_wg.cons(), of_gf.prod(), k_gemm],
               tile=Tile(0, 2)),
        Worker(w_up,   [of_h2.cons(), of_wu.cons(), of_uf.prod(), k_gemm],
               tile=Tile(1, 2)),
    ]
    if _INCLUDE_SILU:
    ...
    with rt.sequence(rt_xin_ty, rt_w_ty, rt_gf_ty, rt_uf_ty) as (xin, wblob, gf_out, uf_out):
        rt.start(*workers)
        ...
        rt.fill(of_wu.prod(), wblob,
                tap=strided_tap(WEIGHTS_BYTES, OFF_WU, WG_SLOT,
                                WG_SLOT, N_TILES_UP),
                task_group=wu_tg)
