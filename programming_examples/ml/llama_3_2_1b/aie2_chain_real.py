...
def build():
    ...
    of_back   = ObjectFifo(t_D_i8, depth=N_LAYERS, name="back")
    of_routed = ObjectFifo(t_D_i8, depth=N_LAYERS, name="routed")  # fans out to rmsnorm1 + add1
    ...
    of_x1     = ObjectFifo(t_D_i8,   depth=N_LAYERS, name="x1")  # residual hold #2
    ...
    workers = [
        ...
        Worker(w_add2_tail, [of_df.cons(), of_x1.cons(),
                             of_back.prod(), of_out.prod(), k_add],
               tile=Tile(7, 5)),
    ]

    rt = Runtime()
    with rt.sequence(rt_xin_ty, rt_w_ty, rt_kv_ty, rt_out_ty) as (xin, wblob, kvblob, out):
        rt.start(*workers)

        seed_tg = rt.task_group()
        rt.fill(of_seed.prod(), xin, task_group=seed_tg)

        def tap(total, offset, nbytes):
