...
def numpy_attention(
    q_i8, k_i8, v_i8, head_dim, t, q_scale, k_scale, v_scale, inv_out_scale, lut
):
    ...
    vs = np.float32(v_scale)
    ios = np.float32(inv_out_scale)
...
def main():
    ...
    expected = numpy_attention(
        q, k, v, head_dim, t, Q_SCALE, K_SCALE, V_SCALE, INV_OUT_SCALE, lut
    )
