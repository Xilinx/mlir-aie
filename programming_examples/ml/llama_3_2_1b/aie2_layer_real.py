...
def build():
    ...
    k_rms_D    = Kernel("llama_rmsnorm_int8",  KO_RMS,
    ...
    k_rope_i8 = Kernel("llama_rope_int8", KO_ROPE,
                       [t_QD_i8, t_CS_bf16, t_QD_i8, np.float32])

    k_silu_HD = Kernel("llama_silu_mul_int8", KO_SILU,
                       [t_HD_i8, t_HD_i8, t_HD_i8, np.float32, np.float32])

    k_fkv_qk = Kernel("llama_flowkv_qk", KO_FKV,
                      [t_QD_i8, t_KCACHE_i8, t_PROBS_fp32, np.float32, np.float32])
    k_fkv_sv = Kernel("llama_flowkv_sv", KO_FKV,
    ...
    k_add_D = Kernel("llama_pt_add_D", "llama_layer_pt.cc.o",
                     [t_D_i8, t_D_i8, t_D_i8])
    ...
    workers = [
        # rmsnorm1
        Worker(w_rms, [of_xin.cons(), of_gam_in.cons(), of_h1.prod(), k_rms_D],
               tile=Tile(5, 4)),
        # q_proj
        Worker(w_gemm, [of_h1.cons(), of_wq.cons(), of_qf.prod(), k_gemm_DD],
               tile=Tile(0, 2)),
        # rope_q
        Worker(w_rope, [of_qf.cons(), of_cs.cons(), of_qr.prod(), k_rope_i8],
               tile=Tile(4, 4)),
        # flowkv_qk
        Worker(w_qk, [of_qr.cons(), of_kcache.cons(), of_probs.prod(), k_fkv_qk],
               tile=Tile(0, 4)),
        # flowkv_sv
        Worker(w_sv, [of_vcache.cons(), of_probs.cons(), of_af.prod(), k_fkv_sv],
               tile=Tile(0, 5)),
        # o_proj
        Worker(w_gemm, [of_af.cons(), of_wo.cons(), of_op.prod(), k_gemm_DD],
               tile=Tile(3, 2)),
        # add1
        Worker(w_add, [of_op.cons(), of_xin.cons(), of_x1.prod(), k_add_D],
               tile=Tile(6, 5)),
        # rmsnorm2
        Worker(w_rms, [of_x1.cons(), of_gam_post.cons(), of_h2.prod(), k_rms_D],
               tile=Tile(6, 4)),
        ...
        Worker(w_gemm, [of_h2.cons(), of_wu.cons(), of_uf.prod(), k_gemm_DHD],
               tile=Tile(5, 2)),
        Worker(w_silu, [of_gf.cons(), of_uf.cons(), of_sf.prod(), k_silu_HD],
               tile=Tile(4, 5)),
        Worker(w_gemm, [of_sf.cons(), of_wd.cons(), of_df.prod(), k_gemm_HDD],
               tile=Tile(6, 2)),
        # add2
        Worker(w_add, [of_df.cons(), of_x1.cons(), of_out.prod(), k_add_D],
               tile=Tile(7, 5)),
    ]
