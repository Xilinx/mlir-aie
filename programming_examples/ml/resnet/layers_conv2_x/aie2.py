...
def resnet_conv_x():
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def deviceBody():
            ...
            weightsLayer1_ty_init= MemRefType.get((tensorInCInit * tensorInCInit,), int8_ty)
            weightsLayer1_ty_rest = MemRefType.get((tensorInCRest * tensorInCInit,), int8_ty)
            ...
            weightsLayer3_ty_init = MemRefType.get((2*tensorInCInit* tensorInCRest,), int8_ty)
            weightsLayer3_ty_rest = MemRefType.get((tensorInCRest//4 * tensorInCRest,), int8_ty)
            ...
            allWeights_ty_init = MemRefType.get(
                (
                    ...
                    + tensorInCInit * tensorInCRest + tensorInCInit * tensorInCRest,
                ),
                int8_ty,
            )
            ...
            layer1_wts_sizes = [weightsLayer1_ty_init, weightsLayer1_ty_rest, weightsLayer1_ty_rest]
            laye1_act_sizes=[tensorLayer1In_ty_init,tensorLayer1In_ty_rest,tensorLayer1In_ty_rest]
            layer3_wts_sizes = [weightsLayer3_ty_init, weightsLayer3_ty_rest, weightsLayer3_ty_rest]
            ...
            rtp = [
                [rtpComputeTile02, rtpComputeTile03, rtpComputeTile04, rtpComputeTile05],
                [rtpComputeTile15, rtpComputeTile14, rtpComputeTile13, rtpComputeTile12],
                [rtpComputeTile22, rtpComputeTile23, rtpComputeTile24, rtpComputeTile25],
             
            ]
            rtp_name = [
                ["rtpComputeTile02", "rtpComputeTile03", "rtpComputeTile04", "rtpComputeTile05"],
                ["rtpComputeTile12","rtpComputeTile13", "rtpComputeTile14", "rtpComputeTile15"],
                ["rtpComputeTile22", "rtpComputeTile23", "rtpComputeTile24", "rtpComputeTile25"],
            ...
            conv3_kernels=["conv2dk1_skip_init.o","conv2dk1_skip.o","conv2dk1_skip.o"]
            conv3_kernels_call=[conv2dk1_skip_init_i8,conv2dk1_skip_ui8,conv2dk1_skip_ui8]
            ...
            for i in range(n_cols):
                ...
                object_fifo_link(wts_fifo_names[i], [wts_sub_fifo_names[i][0], wts_sub_fifo_names[i][1], wts_sub_fifo_names[i][2]])
            ...
            conv3_out_fifo=[act1_fifos[act1_fifo_names[1]],act1_fifos[act1_fifo_names[2]],outOFL2L3]
            ...
            for i in range(n_cols):
                ...
                def core_body():
                    for _ in for_(sys.maxsize):
                        ...
                        element0Weights = wts_sub_fifos[wts_sub_fifo_names[i][0]].acquire(ObjectFifoPort.Consume, 1)
                        scale = memref.load(rtp[i][0], [0])
                        for _ in for_(tensorInH):
                            element0ActivactionsIn = act1_fifos[act1_fifo_names[i]].acquire(
                                ObjectFifoPort.Consume, 1
                            )
                            element0ActivactionsOut = act2_fifos[act2_fifo_names[i]].acquire(
                            ...
                            objectfifo_release(ObjectFifoPort.Consume, act1_fifo_names[i], 1)

                            objectfifo_release(ObjectFifoPort.Produce, act2_fifo_names[i], 1)
                            yield_([])
                        objectfifo_release(ObjectFifoPort.Consume, wts_sub_fifo_names[i][0], 1)
                        yield_([])
            ...
            for i in range(n_cols):
                ...
                def core_body():
                    ...
                    for _ in for_(sys.maxsize):
                        ...
                        element0Weights = wts_sub_fifos[wts_sub_fifo_names[i][1]].acquire(ObjectFifoPort.Consume, 1)
                        ...
                        element0ActivactionsOut = act3_fifo_1[act3_fifo_names_1[i]].acquire(ObjectFifoPort.Produce, 1)
                        res = call(
                            conv2dk3,
                            [
                        ...
                        objectfifo_release(ObjectFifoPort.Produce, act3_fifo_names_1[i], 1)
                        ...
                        for _ in for_(tensorInH - 2):
                            ...
                            element0ActivactionsOut =  act3_fifo_1[act3_fifo_names_1[i]].acquire(
                                ObjectFifoPort.Produce, 1
                            )
                            res = call(
                                conv2dk3,
                            ...
                            objectfifo_release(ObjectFifoPort.Consume, act2_fifo_names[i], 1)
                            objectfifo_release(ObjectFifoPort.Produce, act3_fifo_names_1[i], 1)
                            yield_([])
                        ...
                        objectfifo_release(ObjectFifoPort.Consume, act2_fifo_names[i], 2)
                        objectfifo_release(ObjectFifoPort.Produce, act3_fifo_names_1[i], 1)

                        objectfifo_release(ObjectFifoPort.Consume, wts_sub_fifo_names[i][1], 1)
                        yield_([])
            ...
            for i in range(n_cols):
                ...
                def core_body():
                    ...
                    for _ in for_(sys.maxsize):
                        ...
                        element0ActivactionsOut = act3_fifo_2[act3_fifo_names_2[i]].acquire(ObjectFifoPort.Produce, 1)
                        res = call(
                            conv2dk3,
                            [
                                elementActivactionsIn[0],
                        ...
                        objectfifo_release(ObjectFifoPort.Produce, act3_fifo_names_2[i], 1)
                        ...
                        for _ in for_(tensorInH - 2):
                            ...
                            element0ActivactionsOut = act3_fifo_2[act3_fifo_names_2[i]].acquire(
                                ObjectFifoPort.Produce, 1
                            )
                            res = call(
                                conv2dk3,
            ...
            for i in range(n_cols):
                ...
                def core_body():
                    for _ in for_(sys.maxsize):
                        ...
                        # acquire weights and rtps once
                        element0Weights =  wts_sub_fifos[wts_sub_fifo_names[i][2]].acquire(ObjectFifoPort.Consume, 1)
                        scale = memref.load(rtp[i][2], [0])
                        skipScale = memref.load(rtp[i][2], [1])

                        for _ in for_(tensorInH):
                            element0ActivactionsIn = act3_fifo_1[act3_fifo_names_1[i]].acquire(
                                ObjectFifoPort.Consume, 1
                            )
                            element1ActivactionsIn = act3_fifo_2[act3_fifo_names_2[i]].acquire(
                                ObjectFifoPort.Consume, 1
                            )
                            ...
                            elementSkipsIn = skip_fifos[skip_fifo_names[i]].acquire(ObjectFifoPort.Consume, 1)
                            call(
                            ...
                            objectfifo_release(ObjectFifoPort.Consume, act3_fifo_names_1[i], 1)
                            objectfifo_release(ObjectFifoPort.Consume, act3_fifo_names_2[i], 1)
                            objectfifo_release(ObjectFifoPort.Produce, conv3_out_fifo_names[i], 1)
                            
                            
                            objectfifo_release(ObjectFifoPort.Consume, skip_fifo_names[i], 1)
                            yield_([])
                        objectfifo_release(ObjectFifoPort.Consume, wts_sub_fifo_names[i][2], 1)
                        yield_([])
            ...
            totalWeightsSize32b_complete=totalWeightsSize32b_init+repeat*totalWeightsSize32b_rest
            ...
            weightsInL3_ty_complete = MemRefType.get((totalWeightsSize32b_complete,), int32_ty)


            @FuncOp.from_py_func(activationsInL3_ty, weightsInL3_ty_complete, activationsOutL3_ty)
            def sequence(inputFromL3, weightsFromL3, outputToL3):
            
                for c,col in enumerate(rtp_name):
                    for r,row in enumerate(col):
                        IpuWriteRTPOp(
                            row, col=c, row=r+2, index=0, value=1
                        )  # scale
                
                IpuWriteRTPOp(
                    "rtpComputeTile04", col=0, row=4, index=0, value=0
                ) 
                IpuWriteRTPOp(
                ...
                IpuWriteRTPOp(
                    "rtpComputeTile13", col=1, row=3, index=0, value=0
                )

                IpuWriteRTPOp(
                    "rtpComputeTile24", col=2, row=4, index=0, value=0
                )
            ...
            # #     # write RTP parameters
                ...
                ipu_dma_memcpy_nd(
                    ...
                    bd_id=1,
                    mem=weightsFromL3,
                    offsets=[0, 0, 0, totalWeightsSize32b_init+totalWeightsSize32b_rest],
                    sizes=[1, 1, 1, totalWeightsSize32b_rest],
                )
