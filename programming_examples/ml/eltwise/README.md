...
1. `add.cc` / `mul.cc`: Vectorized AIE kernels for vector add / multiply, pulled from the IRON kernel library. Sources live under [`aie_kernels/eltwise/add.cc`](../../../aie_kernels/eltwise/add.cc) and [`mul.cc`](../../../aie_kernels/eltwise/mul.cc), one file per op with the vector width selected from `__AIE_ARCH__`.
