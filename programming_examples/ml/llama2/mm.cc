...
//
void zero(dtype *__restrict C) {
  for (unsigned i = 0; i < m * n; i += MMUL::size_C) {
    aie::store_v(C + i, aie::zeros<dtype, MMUL::size_C>());
  }
}
...
void matrix_multiplication(const unsigned op, const dtype *__restrict A,
                           const dtype *__restrict B, dtype *__restrict C) {
  if (op == 0) {
    zero(C);
    return;
  }
  for (unsigned row = 0; row < m / r; row += 2) {
    for (unsigned col = 0; col < n / t; col += 2) {
      ...
      const aie::vector<dtype, MMUL::size_C> C00_in = aie::load_v<MMUL::size_C>(
          C + ((row + 0) * (n / t) + (col + 0)) * MMUL::size_C);
      const aie::vector<dtype, MMUL::size_C> C01_in = aie::load_v<MMUL::size_C>(
          C + ((row + 0) * (n / t) + (col + 1)) * MMUL::size_C);
      const aie::vector<dtype, MMUL::size_C> C10_in = aie::load_v<MMUL::size_C>(
          C + ((row + 1) * (n / t) + (col + 0)) * MMUL::size_C);
      const aie::vector<dtype, MMUL::size_C> C11_in = aie::load_v<MMUL::size_C>(
...
void matrix_multiplication_scalar(const dtype *__restrict A,
  ...
  for (unsigned tile_row = 0; tile_row < m / r; tile_row += 1) {
    for (unsigned tile_col = 0; tile_col < n / t; tile_col += 1) {
      for (unsigned tile_i = 0; tile_i < k / s; tile_i += 1) {
        const dtype *A_base = A + (tile_row * (k / s) + tile_i) * r * s;
        const dtype *B_base = B + (tile_i * (n / t) + tile_col) * s * t;
        dtype *C_base = C + (tile_row * (n / t) + tile_col) * r * t;
