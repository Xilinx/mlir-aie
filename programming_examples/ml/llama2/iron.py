    ...
    class Graph:
        ...
        def execute(self):
            ...
            for i, op in enumerate(graph_operations):
                ...
                if func == matmul:
                    ...
                    a, b, operation, device, tensor_id = op[1], op[2], op[3], op[4], op[5]
                    device = "cpu"  # Always use CPU for now
        ...
        def to_dot(self, filename=None):
            ...
            for node_id, symbolic_name in input_map.items():
                dot_lines.append(f'  "{symbolic_name}" [label="{symbolic_name}", fillcolor="lightblue"];')
            ...
            for i, op in enumerate(graph_operations):
                ...
                if func == matmul:
                    a, b, device, tensor_id = op[1], op[2], op[3], op[4]
                    dot_lines.append(f'  "{op_id}" [label="MatMul\\n{tensor_id}", fillcolor="lightgreen"];')
                    ...
                    x, device, tensor_id = op[1], op[2], op[3]
                    dot_lines.append(f'  "{op_id}" [label="SiLU\\n{tensor_id}", fillcolor="lightyellow"];')
                    ...
                    a, b, operation, device, tensor_id = op[1], op[2], op[3], op[4], op[5]
                    op_name = "Multiply" if operation.__name__ == "<lambda>" else operation.__name__.title()
                    dot_lines.append(f'  "{op_id}" [label="{op_name}\\n{tensor_id}", fillcolor="lightcoral"];')
            ...
            for tensor_id, producer_op in tensor_nodes.items():
                dot_lines.append(f'  "{tensor_id}" [label="{tensor_id}", fillcolor="lightgray"];')
