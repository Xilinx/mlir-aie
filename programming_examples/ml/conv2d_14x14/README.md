...
Definitions
* P/P2 - Total pixels divided by 2 or the remaining pixels. In this design, we have a kernel size of 14x14 or 196 pixels. Since the lowest dimension is every 2 pixels, on the outer dimension, we iterate of 196/2 or 98
...
* `num_act` controls how many copies of the input image the host ships to the AIE array: the single-core design has no on-chip repeat and needs one copy per output-channel group (`co_group` = 72 for the default 1152 channels), while the 32-core design uses shim repeat and needs only 1. The `Makefile` now defaults `num_act` correctly for both variants based on `multi`; only override it if you change `--out_channels`/`-oc` and need a different `co_group`.
