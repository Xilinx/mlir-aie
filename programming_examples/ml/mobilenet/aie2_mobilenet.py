...
class initConv:
    def __init__(self,_computeTile,_act_buf_in, _wts_buf, _act_buf_out, _init_func, _in_W, _in_H, in_C, _out_W, _out_H,_out_C,_init_scaleFactor):
        ...
        def core_body():
                ...
                for _ in for_(sys.maxsize):
                    ...
                    # pre-amble: top row
                    elementActivactionsIn = self.act_buf_in.acquire(ObjectFifoPort.Consume, 2)
                    element0ActivactionsOut = self.act_buf_out.acquire(ObjectFifoPort.Produce, 1)
                    res = call(self.init_func, [elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], self.wts_buf, element0ActivactionsOut, self.in_W, self.in_C, self.out_C, 3, 3, 0, scale, 0, 0, ],
                    )
                    ...
                    for _ in for_(self.out_H - 1):
                        elementActivactionsIn = self.act_buf_in.acquire(ObjectFifoPort.Consume, 3)
                        element0ActivactionsOut = self.act_buf_out.acquire(ObjectFifoPort.Produce, 1)
                        res = call(self.init_func, [elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], self.wts_buf, element0ActivactionsOut, self.in_W, self.in_C, self.out_C, 3, 3, 1, scale, 0, 0, ], )
...
class postBlock:
    def __init__(self,_computeTile,_act_buf_in, 
                    #_wts_buf,
                    post_L1_tile_prod_lock, post_L1_tile_cons_lock, post_L1_tile_buff,
                    _act_buf_out, _post_func, _in_W, _in_H, in_C, _out_W, _out_H,_out_C,_out_C_padd,_post_scaleFactor):
        self.computeTile=_computeTile
        self.act_buf_in=_act_buf_in
        self.act_buf_out=_act_buf_out
        #self.wts_buf=_wts_buf
                ...
                for _ in for_(0xFFFFFFFE):
                    ...
                    for yIndex in for_(0,self.in_H):
                        ...
                        for WeightIndex in for_(0,PostOutputSplit): 
                            ...
                            call(self.post_func, [elemIn, post_L1_tile_buff, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C), arith.constant(self.out_C), arith.constant(self.out_C_padd),scale,\
                                 yIndex_cast,PostOutputSplit, WeightIndex_cast], )
...
class postBlockL2:
    def __init__(self,_computeTile1,_computeTile2,_computeTile3,_computeTile4,_act_buf_in, _wts_buf1,_wts_buf2, _wts_buf3,_wts_buf4,
                    _act_buf_out1, _act_buf_out2,_act_buf_out3,_act_buf_out4,
                    _post_func, _in_W, _in_H, in_C,in_C_pad, _out_W, _out_H,_out_C,\
                    _post_scaleFactorFC1,_post_scaleFactorFC2,
                    PostL2Tile_1_cons_prod_lock,PostL2Tile_2_cons_prod_lock,PostL2Tile_3_cons_prod_lock,PostL2Tile_4_cons_prod_lock,
                    PostL2Tile_1_cons_cons_lock,PostL2Tile_2_cons_cons_lock,PostL2Tile_3_cons_cons_lock,PostL2Tile_4_cons_cons_lock
        ):
        ...
        def core_body():
                for _ in for_(0xFFFFFFFF):
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            use_lock(PostL2Tile_1_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf1, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            # elemWts = self.wts_buf1.acquire(ObjectFifoPort.Consume, 1)
                            use_lock(PostL2Tile_1_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf1, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C_pad), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                            use_lock(PostL2Tile_1_cons_prod_lock, LockAction.Release)
                            # self.wts_buf1.release(ObjectFifoPort.Consume,1)
                            self.act_buf_out1.release(ObjectFifoPort.Produce,1)
                            yield_([])
        ...
        def core_body():
                for _ in for_(0xFFFFFFFF):
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            use_lock(PostL2Tile_2_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf2, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            # elemWts = self.wts_buf2.acquire(ObjectFifoPort.Consume, 1)
                            use_lock(PostL2Tile_2_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf2, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C_pad), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                            # self.wts_buf2.release(ObjectFifoPort.Consume,1)
                            use_lock(PostL2Tile_2_cons_prod_lock, LockAction.Release)
                            self.act_buf_out2.release(ObjectFifoPort.Produce,1)
                            yield_([])
        ...
        def core_body():
                for _ in for_(0xFFFFFFFF):
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            use_lock(PostL2Tile_3_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf3, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            # elemWts = self.wts_buf3.acquire(ObjectFifoPort.Consume, 1)
                            use_lock(PostL2Tile_3_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf3, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C_pad), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                            # self.wts_buf3.release(ObjectFifoPort.Consume,1)
                            use_lock(PostL2Tile_3_cons_prod_lock, LockAction.Release)
                            self.act_buf_out3.release(ObjectFifoPort.Produce,1)
                            yield_([])
        ...
        def core_body():
                for _ in for_(0xFFFFFFFF):
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            use_lock(PostL2Tile_4_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf4, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                            ...
                            # elemWts = self.wts_buf4.acquire(ObjectFifoPort.Consume, 1)
                            use_lock(PostL2Tile_4_cons_cons_lock, LockAction.AcquireGreaterEqual)
                            call(self.post_func, [elemIn, self.wts_buf4, elemOut0, arith.constant(self.in_W), arith.constant(self.in_C_pad), arith.constant(self.in_C_pad),arith.constant(co), scale, ], )
                            # self.wts_buf4.release(ObjectFifoPort.Consume,1)
                            use_lock(PostL2Tile_4_cons_prod_lock, LockAction.Release)
                            self.act_buf_out4.release(ObjectFifoPort.Produce,1)
                            yield_([])
...
class BottleneckBCore:
    def __init__(self,_bottleneckName,_computeTileBN10_1,_computeTileBN10_2,_computeTileBN10_3,_computeTileBN11_1,_computeTileBN11_2,_computeTileBN11_3,_computeTileBN12_1,_computeTileBN12_2, 
                 _weightsInBN10_1,_weightsInBN10_2,_weightsInBN10_3, _weightsInBN11_1,_weightsInBN11_2,_weightsInBN11_3, _weightsInBN12_1,_weightsInBN12_2_3,
                 _tensorLayer12_2Out_ty,
                 _rtpBN10_1,_rtpBN10_2,_rtpBN10_3,_rtpBN11_1,_rtpBN11_2,_rtpBN11_3,_rtpBN12_1,_rtpBN12_2,
                 _skipMemTile,
        ...
        OF_b10_act_layer1_layer2 = object_fifo("OF_b10_act_layer1_layer2", self.computeTileBN10_1, [self.computeTileBN10_2], 4,b10_layer2_in) 
        ...
        OF_b11_act_layer1_layer2 = object_fifo("OF_b11_act_layer1_layer2", self.computeTileBN11_1, [self.computeTileBN11_2], 4,b11_layer2_in)
        ...
        self.of_act_bn12_2_3 = object_fifo("act_bn12_2_3", self.computeTileBN12_2, self.computeTileBN12_2, 1, self.tensorLayer12_2Out_ty)
    ...
    # ************************ bneck10 ************************
        ...
        def core_body():
            for _ in for_(sys.maxsize):
                ...
                for _ in for_(b10_InH1):
                    ...
                    call( bn10_conv2dk1_fused_relu, [ element0ActivactionsIn, self.weightsInBN10_layer1, element0ActivactionsOut, b10_InW1, b10_InC1, b10_OutC1, scale, ], )
        ...
        def core_body():
            ...
            for _ in for_(sys.maxsize):
                ...
                res = call( bn10_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], self.weightsInBN10_layer2, element0ActivactionsOut, b10_InW2, 1, b10_OutC2, 3, 3, 0, scale, 0, ], )
                ...
                for _ in for_(b10_InH2 - 2):
                    ...
                    res = call( bn10_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], self.weightsInBN10_layer2, element0ActivactionsOut, b10_InW2, 1, b10_OutC2, 3, 3, 1, scale, 0, ], )
                ...
                element0ActivactionsOut = OF_b10_act_layer2_layer3.acquire(ObjectFifoPort.Produce, 1)
                res = call( bn10_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[1], self.weightsInBN10_layer2, element0ActivactionsOut, b10_InW2, 1, b10_OutC2, 3, 3, 2, scale, 0, ], )
        ...
        def core_body():
            for _ in for_(0xFFFFFFFF):
                ...
                for _ in for_(b10_InH3):
                    ...
                    call( bn10_conv2dk1_ui8, [ elemIn, self.weightsInBN10_layer3, elemOut0, b10_InW3, b10_OutC2, b10_OutC3, scale, ], )
    ...
    # #     #     # 1x1 conv2d
        ...
        def core_body():
            for _ in for_(sys.maxsize):
                ...
                for _ in for_(b10_InH1):
                    ...
                    res = call( bn11_conv2dk1_fused_relu, [ element0ActivactionsIn, self.weightsInBN11_layer1, element0ActivactionsOut, b10_InW1, b10_OutC3, b11_OutC1, scale, ], )
        ...
        def core_body():
            ...
            for _ in for_(sys.maxsize):
                ...
                res = call( bn11_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], self.weightsInBN11_layer2 , element0ActivactionsOut, b10_InW2, 1, b11_OutC2, 3, 3, 0, scale, 0, ], )
                ...
                for _ in for_(b10_InH2 - 2):
                    ...
                    res = call( bn11_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], self.weightsInBN11_layer2, element0ActivactionsOut, b10_InW2, 1, b11_OutC2, 3, 3, 1, scale, 0, ], )
                ...
                element0ActivactionsOut = OF_b11_act_layer2_layer3.acquire(ObjectFifoPort.Produce, 1)
                res = call( bn11_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[1], self.weightsInBN11_layer2, element0ActivactionsOut, b10_InW2, 1, b11_OutC2, 3, 3, 2, scale, 0, ], )
        ...
        def core_body():
            ...
            for _ in for_(0xFFFFFFFF):
                ...
                for _ in for_(b10_InH3):
                    ...
                    call( bn11_conv2dk1_skip, [ elemIn, self.weightsInBN11_layer3, elemOut0, elementSkipsIn, b10_InW3, b11_OutC2, b11_OutC3, scale, skipScale, ], )
    ...
    #     # 1x1 conv2d
        ...
        def core_body():
            ...
            for _ in for_(sys.maxsize):
                weightsInBN12_layer2 = memref_view(self.weightsInBN12_layer2_3, [3 * 3 * b12_OutC2 * 1], shift=0)
                weightsInBN12_layer3 = memref_view(self.weightsInBN12_layer2_3, [b12_OutC2 * b12_OutC3], shift=3 * 3 * b12_OutC2 * 1)
                # acquire weights and rtps once
                # element0Weights = self.weightsInBN12_layer2.acquire(ObjectFifoPort.Consume, 1)
                # scale = memref.load(rtpComputeTile3, 0)

                # pre-amble: top row
                elementActivactionsIn = OF_b12_act_layer1_layer2.acquire( ObjectFifoPort.Consume, 2 )
                element0ActivactionsOut = self.of_act_bn12_2_3.acquire(ObjectFifoPort.Produce, 1)
                res = call( bn12_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], weightsInBN12_layer2, element0ActivactionsOut, b10_InW3, 1, b12_OutC2, 3, 3, 0, scale2, 0, ], )
                ...
                call( bn12_conv2dk1_ui8, [ elemIn, weightsInBN12_layer3, elemOut0, b12_InW2, b12_OutC2, b12_OutC3, scale3, ], )
                ...
                for _ in for_(b12_InH2 - 2):
                    elementActivactionsIn = OF_b12_act_layer1_layer2.acquire( ObjectFifoPort.Consume, 3 )
                    element0ActivactionsOut = self.of_act_bn12_2_3.acquire( ObjectFifoPort.Produce, 1 )
                    res = call( bn12_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], weightsInBN12_layer2, element0ActivactionsOut, b10_InW3, 1, b12_OutC2, 3, 3, 1, scale2, 0, ], )
...
class bottleneckCCore:
    def __init__(self,_computeTileBN13_1,_computeTileBN13_2,_computeTileBN13_3,_computeTileBN13_4,_computeTileBN13_5,_computeTileBN14_1,_computeTileBN14_2,_computeTileBN14_3,_computeTileBN14_4,_computeTileBN14_5,
        ...
        bneck_13_OutC3 = 80
        OutputSplit2=2 #calculate 8 OCs at a time, should bneck_13_InC1rease to more --> 4
    ...
    # ************************ bneck13 ************************
        ...
        # define wts
        #layer1
        ty_bneck_13_layer1_wts_split = MemRefType.get(((bneck_13_InC1//InputSplit)*(bneck_13_OutC1//OutputSplit),), int8_ty)
        ...
        # layer3
        ty_bneck_13_layer3_wts_split = MemRefType.get(((bneck_13_OutC2 // InputSplit)*(bneck_13_OutC3//OutputSplit2),), int8_ty)
    ...
    # ************************ bneck14 ************************
        ...
        # define wts
        # layer1
        ty_bneck_14_layer1_wts_split = MemRefType.get(((bneck_14_InC1 // InputSplit) * (bneck_14_OutC1//OutputSplit),), int8_ty)
        ...
        # layer3
        ty_bneck_14_layer3_wts_split = MemRefType.get(((bneck_14_OutC2 // InputSplit) * (bneck_14_OutC3//OutputSplit2),), int8_ty)
        ...
        bn13_conv2dk1_fused_relu_get = external_func( "bn13_1_conv2dk1_i8_ui8_partial_width_get_new", inputs=[ ty_bneck_13_layer1_in, ty_bneck_13_layer1_wts_split, ty_bneck_13_layer1_out, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn13_conv2dk1_fused_relu_put = external_func( "bn13_1_conv2dk1_i8_ui8_partial_width_put_new", inputs=[ ty_bneck_13_layer1_in, ty_bneck_13_layer1_wts_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn13_conv2dk3_dw = external_func( "bn13_conv2dk3_ui8_out_split", inputs=[ ty_bneck_13_layer2_in, ty_bneck_13_layer2_in, ty_bneck_13_layer2_in, ty_bneck_13_layer2_wts, ty_bneck_13_layer2_out_split, ty_bneck_13_layer2_out_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )

        bn13_layer3_conv2dk1_put = external_func( "bn13_1_conv2dk1_ui8_ui8_input_split_partial_width_put_new", inputs=[ ty_bneck_13_layer2_out_split, ty_bneck_13_layer3_wts_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )

        bn13_layer3_conv2dk1_skip_get = external_func( "bn_13_2_conv2dk1_ui8_i8_i8_scalar_input_split_partial_width_get_new", inputs=[ ty_bneck_13_layer2_out_split, ty_bneck_13_layer3_wts_split, ty_bneck_13_layer3_out, ty_bneck_13_layer1_in, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn14_conv2dk1_fused_relu_get = external_func( "bn14_1_conv2dk1_i8_ui8_partial_width_get_new", inputs=[ ty_bneck_14_layer1_in, ty_bneck_14_layer1_wts_split, ty_bneck_14_layer1_out, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn14_conv2dk1_fused_relu_put = external_func( "bn14_1_conv2dk1_i8_ui8_partial_width_put_new", inputs=[ ty_bneck_14_layer1_in, ty_bneck_14_layer1_wts_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn14_conv2dk3_dw = external_func( "bn14_conv2dk3_ui8_out_split", inputs=[ ty_bneck_14_layer2_in, ty_bneck_14_layer2_in, ty_bneck_14_layer2_in, ty_bneck_14_layer2_wts, ty_bneck_14_layer2_out_split, ty_bneck_14_layer2_out_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        bn14_layer3_conv2dk1_put = external_func( "bn14_1_conv2dk1_ui8_ui8_input_split_partial_width_put_new", inputs=[ ty_bneck_14_layer2_out_split, ty_bneck_14_layer3_wts_split, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )

        bn14_layer3_conv2dk1_skip_get = external_func( "bn_14_2_conv2dk1_ui8_i8_i8_scalar_input_split_partial_width_get_new", inputs=[ ty_bneck_14_layer2_out_split, ty_bneck_14_layer3_wts_split, ty_bneck_14_layer3_out, ty_bneck_14_layer1_in, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        ...
        def core_body():
            for _ in for_(0xFFFFFFFF):
                for _ in for_(bneck_13_InH1):
                    ...
                    for WeightIndex in for_(0,OutputSplit): #how many input channel splits, 1 in case InputSplit is 2
                        WeightIndex_cast= arith.IndexCastOp(T.i32(), WeightIndex)
                        ...
                        for oc in for_(0,OC8):  
                                ...
                                x_start=0  
                                call( bn13_conv2dk1_fused_relu_put, [ elemIn, elemWts, arith.constant(bneck_13_InW1), arith.constant(bneck_13_InC1), arith.constant(bneck_13_OutC1), InputSplit, WeightIndex_cast, x_start, oc_cast ], )
                                yield_([])
                ...
                for _ in for_(bneck_13_InH1):
                    ...
                    for WeightIndex in for_(0,OutputSplit):
                        ...
                        for oc in for_(0,OC8):         
                                oc_cast= arith.IndexCastOp(T.i32(), oc)
                                x_start=0    
                                call( bn13_conv2dk1_fused_relu_get, [ elemIn, elemWts, elemOut0, arith.constant(bneck_13_InW1), arith.constant(bneck_13_InC1), arith.constant(bneck_13_OutC1), scale, InputSplit, OutputSplit, WeightIndex_cast, x_start, oc_cast ], )
                                yield_([])
        ...
        def core_body():
            ...
            for _ in for_(sys.maxsize):
                ...
                res = call( bn13_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], self.weightsInBN13_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_13_InW2, 1, bneck_13_OutC2, 3, 3, 0, scale, 0, ], )
                ...
                for _ in for_(bneck_13_InH2 - 2):
                    ...
                    element0ActivactionsOut = bn13_act_layer2_layer3_first.acquire(ObjectFifoPort.Produce, 1)
                    element1ActivactionsOut = bn13_act_layer2_layer3_second.acquire(ObjectFifoPort.Produce, 1)
                    res = call( bn13_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], self.weightsInBN13_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_13_InW2, 1, bneck_13_OutC2, 3, 3, 1, scale, 0, ], )
                ...
                element0ActivactionsOut = bn13_act_layer2_layer3_first.acquire(ObjectFifoPort.Produce, 1)
                element1ActivactionsOut = bn13_act_layer2_layer3_second.acquire(ObjectFifoPort.Produce, 1)
                res = call( bn13_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[1], self.weightsInBN13_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_13_InW2, 1, bneck_13_OutC2, 3, 3, 2, scale, 0, ], )
        ...
        def core_body():
            for _ in for_(0xFFFFFFFF):
                ...
                for _ in for_(bneck_13_InH3):
                    ...
                    for WeightIndex in for_(0,OutputSplit2): #how many input channel splits, 1 in case InputSplit is 2
                        WeightIndex_cast= arith.IndexCastOp(T.i32(), WeightIndex)
                        elemWts = self.weightsInBN13_layer3_put.acquire(ObjectFifoPort.Consume, 1)
                        for oc in for_(0,OC8_out):  
                            oc_cast= arith.IndexCastOp(T.i32(), oc)  
                            x_start=0  
                            call( bn13_layer3_conv2dk1_put, [ elemIn, elemWts, arith.constant(bneck_13_InW3), arith.constant(bneck_13_OutC2), arith.constant(bneck_13_OutC3), InputSplit, WeightIndex_cast, x_start, oc_cast ], )
                            yield_([])
                ...
                for _ in for_(bneck_13_InH3):
                    ...
                    for WeightIndex in for_(0,OutputSplit2):
                        ...
                        for oc in for_(0,OC8_out):         
                            ...
                            call( bn13_layer3_conv2dk1_skip_get, [ elemIn, elemWts, elemOut0, elementSkipsIn, arith.constant(bneck_13_InW3), arith.constant(bneck_13_OutC2), arith.constant(bneck_13_OutC3), scale, scale_skip, InputSplit, OutputSplit2, WeightIndex_cast, x_start, oc_cast ], )
                ...
                for _ in for_(bneck_13_InH1):
                    ...
                    for WeightIndex in for_(0,OutputSplit): 
                        ...
                        for oc in for_(0,OC8):  
                                oc_cast= arith.IndexCastOp(T.i32(), oc)  
                                x_start=0  
                                call( bn14_conv2dk1_fused_relu_put, [ elemIn, elemWts, arith.constant(bneck_14_InW1), arith.constant(bneck_14_InC1), arith.constant(bneck_14_OutC1), InputSplit, WeightIndex_cast, x_start, oc_cast ], )
                                yield_([])
                ...
                for _ in for_(bneck_13_InH1):
                    ...
                    for WeightIndex in for_(0,OutputSplit):
                        ...
                        for oc in for_(0,OC8):         
                                oc_cast= arith.IndexCastOp(T.i32(), oc)
                                x_start=0 
                                call( bn14_conv2dk1_fused_relu_get, [ elemIn, elemWts, elemOut0, arith.constant(bneck_14_InW1), arith.constant(bneck_14_InC1), arith.constant(bneck_14_OutC1), scale, InputSplit, OutputSplit, WeightIndex_cast, x_start, oc_cast ], )
                                yield_([])
        ...
        def core_body():
            ...
            for _ in for_(sys.maxsize):
                ...
                res = call( bn14_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1], self.weightsInBN14_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_14_InW2, 1, bneck_14_OutC2, 3, 3, 0, scale, 0, ], )
                ...
                for _ in for_(bneck_14_InH2 - 2):
                    ...
                    element0ActivactionsOut = bn14_act_layer2_layer3_first.acquire(ObjectFifoPort.Produce, 1)
                    element1ActivactionsOut = bn14_act_layer2_layer3_second.acquire(ObjectFifoPort.Produce, 1)
                    res = call( bn14_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2], self.weightsInBN14_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_14_InW2, 1, bneck_14_OutC2, 3, 3, 1, scale, 0, ], )
                ...
                element0ActivactionsOut = bn14_act_layer2_layer3_first.acquire(ObjectFifoPort.Produce, 1)
                element1ActivactionsOut = bn14_act_layer2_layer3_second.acquire(ObjectFifoPort.Produce, 1)
                res = call( bn14_conv2dk3_dw, [ elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[1], self.weightsInBN14_layer2, element0ActivactionsOut, element1ActivactionsOut, bneck_14_InW2, 1, bneck_14_OutC2, 3, 3, 2, scale, 0, ], )
                ...
                for _ in for_(bneck_14_InH3):
                    ...
                    for WeightIndex in for_(0,OutputSplit2):  #how many input channel splits, 1 in case InputSplit is 2
                        ...
                        for oc in for_(0,OC8_out):  
                            oc_cast= arith.IndexCastOp(T.i32(), oc)  
                            x_start=0 
                            call( bn14_layer3_conv2dk1_put, [ elemIn, elemWts, arith.constant(bneck_14_InW3), arith.constant(bneck_14_OutC2), arith.constant(bneck_14_OutC3), InputSplit, WeightIndex_cast, x_start, oc_cast ], )
                            yield_([])
                ...
                for _ in for_(bneck_14_InH3):
                    ...
                    for WeightIndex in for_(0,OutputSplit2): 
                        ...
                        for oc in for_(0,OC8_out):  
                            ...
                            call( bn14_layer3_conv2dk1_skip_get, [ elemIn, elemWts, elemOut0, elementSkipsIn, arith.constant(bneck_14_InW3), arith.constant(bneck_14_OutC2), arith.constant(bneck_14_OutC3), scale, scale_skip, InputSplit, OutputSplit2, WeightIndex_cast, x_start, oc_cast ], )
                            yield_([])
...
def mobilenetV3_A_B(tileColIndex = 0, b_start_col=0, b_start_row=2, 
                    tensorInW = 224, tensorInH = 224, tensorInC = 8, init_tensorOutC=16,
                            init_scaleFactor=8,
                      bn0_scaleFactor2 = 9, bn0_scaleFactor3 = 8,  bn0_scaleFactorAdd = 2, bn0_depthWiseStride = 1, bn0_withSkip=True,
                       ...
                       bn10_scaleFactor1=10,bn10_scaleFactor2=7,bn10_scaleFactor3=9,
                        ...
                        post_scaleFactor=8,post_FC1_scaleFactor=9,post_FC2_scaleFactor=9):
...
# 
    ...
    def device_body():
        ...
        b12_layer2_wts = MemRefType.get((3 * 3 * b12_OutC2 * 1,), int8_ty ) 
        b12_layer3_wts = MemRefType.get((b12_OutC2 * b12_OutC3,), int8_ty ) 
        b12_all_wts= MemRefType.get(( b12_OutC2 * b12_OutC3, ), int8_ty, ) 
        # output 
        b12_layer1_out = MemRefType.get((b10_InW3, 1, b12_OutC1, ), uint8_ty, ) 
        b12_layer2_out = MemRefType.get((b12_InW2, 1, b12_OutC2, ), uint8_ty, ) 
        b12_layer3_out = MemRefType.get((b12_InW2, 1, b12_OutC3, ), int8_ty, )
        ...
        ty_post_act_Layer1_L2L1 =  MemRefType.get((post_L1_InW ,1, post_L1_InC  ,), int8_ty)
        ...
        post_act_out_Layer2_split = 1 * post_L2_OutC  // (PostOutputSplitL2*post_L2_n_core)
        ...
        post_wts_Layer2_split_L1= post_wts_Layer2_all // (PostOutputSplitL2*post_L2_n_core)
        ...
        ty_post_wts_Layer2_split_L1 =  MemRefType.get((post_wts_Layer2_split_L1,), int8_ty)
        ty_post_Layer2_out_all =  MemRefType.get((post_act_out_Layer2,), uint16_ty)
        ty_post_Layer2_out_split =  MemRefType.get((post_act_out_Layer2_split,), uint16_ty)
        
        # ******************************************************************* WTS B block *******************************************************************

        bn10_1_wts_ary=np.fromfile(weights_path+"bn10_1_chain.txt", sep=",", dtype=np.int8)
        bn10_2_wts_ary=np.fromfile(weights_path+"bn10_2_chain.txt", sep=",", dtype=np.int8)
        bn10_3_wts_ary=np.fromfile(weights_path+"bn10_3_chain.txt", sep=",", dtype=np.int8)

        bn11_1_wts_ary=np.fromfile(weights_path+"bn11_1_chain.txt", sep=",", dtype=np.int8)
        bn11_2_wts_ary=np.fromfile(weights_path+"bn11_2_chain.txt", sep=",", dtype=np.int8)
        bn11_3_wts_ary=np.fromfile(weights_path+"bn11_3_chain.txt", sep=",", dtype=np.int8)

        bn12_1_wts_ary=np.fromfile(weights_path+"bn12_1_chain.txt", sep=",", dtype=np.int8)
        ...
        bn12_2_3_wts_ary=np.fromfile(weights_path+"bn12_2_3_chain.txt", sep=",", dtype=np.int8)
        
        bn10_1_wts_static = buffer(bn10_tile_1, np.ndarray[(b10_layer1_wts_size,), np.dtype[np.int8]], "bn10_1_wts_static", initial_value=bn10_1_wts_ary)
        bn10_2_wts_static = buffer(bn10_tile_2, np.ndarray[(b10_layer2_wts_size,), np.dtype[np.int8]], "bn10_2_wts_static", initial_value=bn10_2_wts_ary)
        bn10_3_wts_static = buffer(bn10_tile_3, np.ndarray[(b10_layer3_wts_size,), np.dtype[np.int8]], "bn10_3_wts_static", initial_value=bn10_3_wts_ary)

        bn11_1_wts_static = buffer(bn11_tile_1, np.ndarray[(b11_layer1_wts_size,), np.dtype[np.int8]], "bn11_1_wts_static", initial_value=bn11_1_wts_ary)
        bn11_2_wts_static = buffer(bn11_tile_2, np.ndarray[(b11_layer2_wts_size,), np.dtype[np.int8]], "bn11_2_wts_static", initial_value=bn11_2_wts_ary)
        bn11_3_wts_static = buffer(bn11_tile_3, np.ndarray[(b11_layer3_wts_size,), np.dtype[np.int8]], "bn11_3_wts_static", initial_value=bn11_3_wts_ary)

        bn12_1_wts_static = buffer(bn12_tile_1, np.ndarray[(b12_layer1_wts_size,), np.dtype[np.int8]], "bn12_1_wts_static", initial_value=bn12_1_wts_ary)
        bn12_2_3_wts_static = buffer(bn12_tile_2, np.ndarray[(b12_layer2_3_wts_size,), np.dtype[np.int8]], "bn12_2_3_wts_static", initial_value=bn12_2_3_wts_ary)
        ...
        bn10_1_rtp = buffer(bn10_tile_1, np.ndarray[(16,), np.dtype[np.int32]], "bn10_1_rtp")
        bn10_2_rtp = buffer(bn10_tile_2, np.ndarray[(16,), np.dtype[np.int32]], "bn10_2_rtp")
        bn10_3_rtp = buffer(bn10_tile_3, np.ndarray[(16,), np.dtype[np.int32]], "bn10_3_rtp")

        bn11_1_rtp = buffer(bn11_tile_1, np.ndarray[(16,), np.dtype[np.int32]], "bn11_1_rtp")
        bn11_2_rtp = buffer(bn11_tile_2, np.ndarray[(16,), np.dtype[np.int32]], "bn11_2_rtp")
        bn11_3_rtp = buffer(bn11_tile_3, np.ndarray[(16,), np.dtype[np.int32]], "bn11_3_rtp")


        bn12_1_rtp = buffer(bn12_tile_1 , np.ndarray[(16,), np.dtype[np.int32]], "bn12_1_rtp")
        bn12_2_rtp = buffer(bn12_tile_2, np.ndarray[(16,), np.dtype[np.int32]], "bn12_2_rtp")
...
# ************************ bneck14 ************************
        ...
        # ************ wts block 13 ************
                        #LAYER1 
        bn13_wts_L3L2_layer1 = object_fifo("bn13_wts_L3L2_layer1", ShimTile40, MemTile01, 1, ty_bneck_13_layer1_wts_full)
        ...
        bn13_wts_memtile_layer1_get.set_repeat_count(RepeatChannels)
            # LAYER2
        bn13_2_wts_ary=np.fromfile(weights_path+"bn13_2_chain.txt", sep=",", dtype=np.int8)
        bn13_2_wts_static = buffer(bn13_tile_layer2, np.ndarray[(b13_layer2_wts_size,), np.dtype[np.int8]], "bn13_2_wts_static", initial_value=bn13_2_wts_ary)

        #LAYER3
        bn13_wts_L3L2_layer3 = object_fifo("bn13_wts_L3L2_layer3", ShimTile50, MemTile11, 1, ty_bneck_13_layer3_wts_full)
        bn13_wts_memtile_layer3_put = object_fifo("bn13_wts_memtile_layer3_put", MemTile11, bn13_tile_layer3_put, 1, ty_bneck_13_layer3_wts_split)
        bn13_wts_memtile_layer3_get = object_fifo("bn13_wts_memtile_layer3_get",MemTile11,bn13_tile_layer3_get,1,ty_bneck_13_layer3_wts_split)
        ...
        # ************ wts block 14 ************
        # wts for new block
        bn14_wts_L3L2_layer1 = object_fifo("bn14_wts_L3L2_layer1", ShimTile60, MemTile21, 1, ty_bneck_14_layer1_wts_full)
        ...
        # LAYER2
        bn14_2_wts_ary=np.fromfile(weights_path+"bn14_2_chain.txt", sep=",", dtype=np.int8)
        bn14_2_wts_static = buffer(bn14_tile_layer2, np.ndarray[(b14_layer2_wts_size,), np.dtype[np.int8]], "bn14_2_wts_static", initial_value=bn14_2_wts_ary)
        # LAYER3
        bn14_wts_L3L2_layer3 = object_fifo("bn14_wts_L3L2_layer3", ShimTile70, MemTile31, 1, ty_bneck_14_layer3_wts_full)
        bn14_wts_memtile_layer3_put = object_fifo("bn14_wts_memtile_layer3_put", MemTile31, bn14_tile_layer3_put, [1,1], ty_bneck_14_layer3_wts_split)
        bn14_wts_memtile_layer3_get = object_fifo("bn14_wts_memtile_layer3_get", MemTile31, bn14_tile_layer3_get, [1,1], ty_bneck_14_layer3_wts_split,)
        ...
        bn14_wts_memtile_layer3_get.set_repeat_count(RepeatChannels)
         ...
         # Set up compute tiles
        rtp_bn13_tile_layer1_get = buffer(bn13_tile_layer1_get, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn13_tile_layer1_get")
        rtp_bn13_tile_layer2 = buffer(bn13_tile_layer2, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn13_tile_layer2")
        rtp_bn13_tile_layer3_get = buffer(bn13_tile_layer3_get, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn13_tile_layer3_get")

        rtp_bn14_tile_layer1_get = buffer(bn14_tile_layer1_get, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn14_tile_layer1_get")
        rtp_bn14_tile_layer2 = buffer(bn14_tile_layer2, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn14_tile_layer2")
        rtp_bn14_tile_layer3_get = buffer(bn14_tile_layer3_get, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn14_tile_layer3_get")
        ...
        post_L1_wts_ary=np.fromfile(weights_path+"post_conv_chain.txt", sep=",", dtype=np.int8)
        ...
        post_L1_wts_buff = buffer( MemTile41, np.ndarray[(post_wts_Layer1_all,), np.dtype[np.int8]], "post_L1_wts_buff", initial_value=post_L1_wts_ary,)

        post_L1_tile_prod_lock = lock(PostL1Tile, lock_id=0, init=1)
        post_L1_tile_cons_lock = lock(PostL1Tile, lock_id=1, init=0)
        post_L1_tile_buff = buffer(PostL1Tile, np.ndarray[(post_wts_Layer1_L2L1,), np.dtype[np.int8]], "post_L1_tile_buff")


        rtp_post_L1 = buffer(PostL1Tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_post_L1")      
        rtp_post_L2_C1 = buffer(PostL2Tile_1, np.ndarray[(16,), np.dtype[np.int32]], "rtp_post_L2_C1")     
        rtp_post_L2_C2 = buffer(PostL2Tile_2, np.ndarray[(16,), np.dtype[np.int32]], "rtp_post_L2_C2")   

        post_L2_wts_L2L1_ty = MemRefType.get(( post_wts_Layer2_split,), int8_ty )
        post_L2_in_ty = MemRefType.get((1, 1, post_L1_OutC, ), int8_ty, )
        post_L2_out_ty = MemRefType.get((1, 1, post_L2_OutC, ), uint8_ty, ) 


        wts_ary_01=np.fromfile(weights_path+"FC1_0_chain.txt", sep=",", dtype=np.int8)
        wts_ary_21=np.fromfile(weights_path+"FC1_1_chain.txt", sep=",", dtype=np.int8)
        wts_ary_41=np.fromfile(weights_path+"FC1_2_chain.txt", sep=",", dtype=np.int8)
        wts_ary_61=np.fromfile(weights_path+"FC1_3_chain.txt", sep=",", dtype=np.int8)

        wts_ary_11=np.fromfile(weights_path+"FC2_0_chain.txt", sep=",", dtype=np.int8)
        wts_ary_31=np.fromfile(weights_path+"FC2_1_chain.txt", sep=",", dtype=np.int8)
        wts_ary_51=np.fromfile(weights_path+"FC2_2_chain.txt", sep=",", dtype=np.int8)
        wts_ary_71=np.fromfile(weights_path+"FC2_3_chain.txt", sep=",", dtype=np.int8)
        ...
        mem_L2_wts_core1 = buffer(PostL2Tile_1, np.ndarray[(post_wts_Layer2_split_L1,), np.dtype[np.int8]], name="mem_L2_wts_core1")
        ...
        mem_L2_wts_core2 = buffer(PostL2Tile_2, np.ndarray[(post_wts_Layer2_split_L1,), np.dtype[np.int8]], name="mem_L2_wts_core2")
        ...
        mem_L2_wts_core3 = buffer(PostL2Tile_3, np.ndarray[(post_wts_Layer2_split_L1,), np.dtype[np.int8]], name="mem_L2_wts_core3")
        ...
        mem_L2_wts_core4 = buffer(PostL2Tile_4, np.ndarray[(post_wts_Layer2_split_L1,), np.dtype[np.int8]], name="mem_L2_wts_core4")
    
        mem_01_prod_lock = lock(MemTile01, lock_id=0, init=0)
        mem_01_cons_lock = lock(MemTile01, lock_id=1, init=1)
        mem_01_buff = buffer( MemTile01, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_01_buff", initial_value=wts_ary_01, )
        mem_11_prod_lock = lock(MemTile11, lock_id=0, init=0)
        mem_11_cons_lock = lock(MemTile11, lock_id=1, init=1)
        mem_11_buff = buffer( MemTile11, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_11_buff", initial_value=wts_ary_11, )
        mem_21_prod_lock = lock(MemTile21, lock_id=0, init=0)
        mem_21_cons_lock = lock(MemTile21, lock_id=1, init=1)
        mem_21_buff = buffer( MemTile21, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_21_buff", initial_value=wts_ary_21, )
        mem_31_prod_lock = lock(MemTile31, lock_id=0, init=0)
        mem_31_cons_lock = lock(MemTile31, lock_id=1, init=1)
        mem_31_buff = buffer( MemTile31, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_31_buff", initial_value=wts_ary_31, )
        mem_41_prod_lock = lock(MemTile41, lock_id=0, init=0)
        mem_41_cons_lock = lock(MemTile41, lock_id=1, init=1)
        mem_41_buff = buffer( MemTile41, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_41_buff", initial_value=wts_ary_41, )
        mem_51_prod_lock = lock(MemTile51, lock_id=0, init=0)
        mem_51_cons_lock = lock(MemTile51, lock_id=1, init=1)
        mem_51_buff = buffer( MemTile51, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_51_buff", initial_value=wts_ary_51, )
        mem_61_prod_lock = lock(MemTile61, lock_id=0, init=0)
        mem_61_cons_lock = lock(MemTile61, lock_id=1, init=1)
        mem_61_buff = buffer( MemTile61, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_61_buff", initial_value=wts_ary_61, )
        mem_71_prod_lock = lock(MemTile71, lock_id=0, init=0)
        mem_71_cons_lock = lock(MemTile71, lock_id=1, init=1)
        mem_71_buff = buffer( MemTile71, np.ndarray[(post_wts_Layer2_split,), np.dtype[np.int8]], "mem_71_buff", initial_value=wts_ary_71, )
        ...
        ##End
         # *************************************************************************************************************************************
        # Set up compute tiles
        rtpinit_tile = buffer(init_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_init") #init
        rtpbn0_tile = buffer(bn0_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn0") #bn0
        rtpbn1_tile = buffer(bn1_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn1") #bn1
        rtpbn2_tile = buffer(bn2_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn2") #bn2
        rtpbn3_tile = buffer(bn3_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn3") #bn3
        rtp_bn4_5_tile = buffer(bn4_5_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn4_5_tile") #bn0
        ...
        rtpbn6_tile = buffer(bn6_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn6") #bn6
        rtpbn7_tile = buffer(bn7_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn7") #bn7
        rtpbn8_bn9_tile = buffer(bn8_bn9_tile, np.ndarray[(16,), np.dtype[np.int32]], "rtp_bn8_bn9") #bn8+bn9
        ...
        init_wts_ary=np.fromfile(weights_path+"init_chain.txt", sep=",", dtype=np.int8)
        bn0_wts_ary=np.fromfile(weights_path+"bn0_chain.txt", sep=",", dtype=np.int8)
        bn1_wts_ary=np.fromfile(weights_path+"bn1_chain.txt", sep=",", dtype=np.int8)
        # bn0_1_wts_ary=np.fromfile(weights_path+"bn0_1_chain.txt", sep=",", dtype=np.int8)
        bn2_wts_ary=np.fromfile(weights_path+"bn2_chain.txt", sep=",", dtype=np.int8)
        bn3_wts_ary=np.fromfile(weights_path+"bn3_chain.txt", sep=",", dtype=np.int8)
        ...
        bn4_5_wts_ary=np.fromfile(weights_path+"bn4_5_chain.txt", sep=",", dtype=np.int8)
        bn6_wts_ary=np.fromfile(weights_path+"bn6_chain.txt", sep=",", dtype=np.int8)
        bn7_wts_ary=np.fromfile(weights_path+"bn7_chain.txt", sep=",", dtype=np.int8)
        bn8_9_wts_ary=np.fromfile(weights_path+"bn8_9_chain.txt", sep=",", dtype=np.int8)

        init_wts_static = buffer(init_tile, np.ndarray[(init_weights_size,), np.dtype[np.int8]], "init_wts_static", initial_value=init_wts_ary)
        bn0_wts_static = buffer(bn0_tile, np.ndarray[(bn0_weights_size,), np.dtype[np.int8]], "bn0_wts_static", initial_value=bn0_wts_ary)
        bn1_wts_static = buffer(bn1_tile, np.ndarray[(bn1_weights_size,), np.dtype[np.int8]], "bn1_wts_static", initial_value=bn1_wts_ary)
        # bn0_1_wts_static = buffer(bn0_tile, (bn0_1_weights_size,), int8_ty, "bn0_1_wts_static", initial_value=bn0_1_wts_ary)

        bn2_wts_static = buffer(bn2_tile, np.ndarray[(bn2_weights_size,), np.dtype[np.int8]], "bn2_wts_static", initial_value=bn2_wts_ary)
        bn3_wts_static = buffer(bn3_tile, np.ndarray[(bn3_weights_size,), np.dtype[np.int8]], "bn3_wts_static", initial_value=bn3_wts_ary)
        bn4_5_wts_static = buffer(bn4_5_tile, np.ndarray[(bn4_5_weights_size,), np.dtype[np.int8]], "bn4_5_wts_static", initial_value=bn4_5_wts_ary)
        ...
        bn6_wts_static = buffer(bn6_tile, np.ndarray[(bn6_weights_size,), np.dtype[np.int8]], "bn6_wts_static", initial_value=bn6_wts_ary)
        bn7_wts_static = buffer(bn7_tile, np.ndarray[(bn7_weights_size,), np.dtype[np.int8]], "bn7_wts_static", initial_value=bn7_wts_ary)
        bn8_9_wts_static = buffer(bn8_bn9_tile, np.ndarray[(bn8_9_weights_size,), np.dtype[np.int8]], "bn8_9_wts_static", initial_value=bn8_9_wts_ary)
        ...
        # AIE Core Function declarations
        init_conv2dk3_stride2 = external_func("conv2dk3_stride2_i8", inputs=[tensorLayerIn_ty, tensorLayerIn_ty, tensorLayerIn_ty, init_weights_ty, bn0_tensorLayer2In_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, ], )
        act_init_bn0 = object_fifo("act_init_bn0", init_tile,bn0_tile, [5, 3], bn0_tensorLayer2In_ty)
        # act_init_bn01 = object_fifo("act_out", init_tile,ShimTile30, [3, 3], bn0_tensorLayer2In_ty)

        
        initConv(init_tile,act_in, init_wts_static, act_init_bn0, init_conv2dk3_stride2, tensor_init_InW, tensor_init_InH, tensor_init_InC, tensor_init_OutW, tensor_init_OutH,tensor_init_OutC, init_scaleFactor)
        ...
        act_bn0_bn1 = object_fifo("act_bn0_bn1", bn0_tile, bn1_tile, 2 , bn0_tensorLayer3Out_ty)
        # act_bn0_bn1 = object_fifo("act_out", bn0_tile, ShimTile10, 2, bn0_tensorLayer3Out_ty)

        bottleneckBN0Static("bn0", bn0_tile, act_init_bn0, bn0_wts_static, act_bn0_bn1, rtpbn0_tile, \
                        bn0_objectArchiveName, bn0_conv2dk3_dw_stride1_relu_ui8_ui8, bn0_conv2dk1_skip_ui8_ui8_i8,
                         bn0_tensorLayer2Out_ty, \
                        tensorL0_2InW, tensorL0_2InH, tensorL0_2InC,  bn0_depthWiseStride, tensorL0_3OutC,\
                        bn0_withSkip, bn0_scaleFactor2, bn0_scaleFactor3,  bn0_scaleFactorAdd)
        ...
        bn1_conv2dk3_dw_stride1_relu_ui8_ui8 = external_func("bn1_conv2dk3_dw_stride1_relu_ui8_ui8",inputs=[bn1_tensorLayer2In_ty,bn1_tensorLayer2In_ty,bn1_tensorLayer2In_ty, bn1_weightsLayer2_ty, bn1_tensorLayer2Out_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty])
        bn1_conv2dk1_skip_ui8_i8_i8 = external_func("bn1_conv2dk1_skip_ui8_i8_i8",inputs=[bn1_tensorLayer3In_ty, bn1_weightsLayer3_ty, bn1_tensorLayer3Out_ty, bn1_tensorLayer3Out_ty, int32_ty, int32_ty, int32_ty, int32_ty, int32_ty])
        ...
        # Compute tile 
        bn1_objectArchiveName = "bn1_combined_con2dk1fusedrelu_conv2dk3dwstride%s_conv2dk1%s.a" % (bn1_depthWiseStride, "skip" if (bn1_withSkip) else "")
        bn1_tensorLayer1Out_ty = MemRefType.get((tensorL1_2InW, 1, tensorL1_2InC),uint8_ty)
        bn1_tensorLayer2Out_ty = MemRefType.get((tensorL1_3InW, 1, tensorL1_3InC),uint8_ty)
        bn1_tensorLayer3Out_ty = MemRefType.get((tensorL1_3InW, 1, tensorL1_3OutC),int8_ty)        
        ...
        # between compute tiles
        act_bn1_bn2 = object_fifo("act_bn1_bn2", bn1_tile, bn2_tile, 2, bn1_tensorLayer3Out_ty)
        # act_bn1_bn2 = object_fifo("act_out", bn1_tile, ShimTile10, 2, bn1_tensorLayer3Out_ty)

        bottleneckACoreStatic("bn1", bn1_tile, act_bn0_bn1, bn1_wts_static, act_bn1_bn2, rtpbn1_tile, bn1_objectArchiveName,
                        bn1_conv2dk1_relu_i8_ui8, bn1_conv2dk3_dw_stride1_relu_ui8_ui8, bn1_conv2dk3_dw_stride2_relu_ui8_ui8, bn1_conv2dk1_ui8_i8, bn1_conv2dk1_skip_ui8_i8_i8,
                        bn1_tensorLayer1Out_ty, bn1_tensorLayer2Out_ty, tensorL1_1InW, tensorL1_1InH, tensorL1_1InC,  bn1_depthWiseStride, bn1_depthWiseChannels, tensorL1_3OutC,\
                        bn1_withSkip,bn1_scaleFactor1, bn1_scaleFactor2, bn1_scaleFactor3,  bn1_scaleFactorAdd)
    ...
    #                      bn0_scaleFactor2, bn0_scaleFactor3,  bn0_scaleFactorAdd,bn1_scaleFactor1, bn1_scaleFactor2, bn1_scaleFactor3)
        ...
        # between compute tiles
        act_bn2_bn3 = object_fifo("act_bn2_bn3", bn2_tile, bn3_tile, 2, bn2_tensorLayer3Out_ty)
        # act_out = object_fifo("act_out", bn2_tile, [ShimTile10], 1, bn2_tensorLayer3Out_ty)

        bottleneckACoreStatic("bn2", bn2_tile, act_bn1_bn2, bn2_wts_static, act_bn2_bn3, rtpbn2_tile, bn2_objectArchiveName,
                        ...
                        bn2_scaleFactor1, bn2_scaleFactor2, bn2_scaleFactor3,  bn2_scaleFactorAdd)
        ...
        # # # between compute tiles
        act_bn3_bn4 = object_fifo("act_bn3_bn4", bn3_tile, bn4_5_tile, 2, bn3_tensorLayer3Out_ty)
        # act_out = object_fifo("act_out", bn3_tile, [ShimTile10], 1, bn3_tensorLayer3Out_ty)
        bottleneckACoreStatic("bn3", bn3_tile, act_bn2_bn3, bn3_wts_static, act_bn3_bn4, rtpbn3_tile, bn3_objectArchiveName,
                         bn3_conv2dk1_relu_i8_ui8, bn3_conv2dk3_dw_stride1_relu_ui8_ui8, bn3_conv2dk3_dw_stride2_relu_ui8_ui8, bn3_conv2dk1_ui8_i8, bn3_conv2dk1_skip_ui8_i8_i8,
                           ...
                           bn3_scaleFactor1, bn3_scaleFactor2, bn3_scaleFactor3,  bn3_scaleFactorAdd)
        ...
        act_bn5_bn6 = object_fifo("act_bn5_bn6", bn4_5_tile, bn6_tile, 2, bn5_tensorLayer3Out_ty)
        ...
        bottleneckAFused_8and9("bn4_bn5", bn4_5_tile, init_tile, act_bn3_bn4, bn4_5_wts_static, act_bn5_bn6, rtp_bn4_5_tile, 
                               "combined_bn_4_5.a", bn4_conv2dk1_relu_i8_ui8, bn4_conv2dk3_dw_stride1_relu_ui8_ui8, bn4_conv2dk1_skip_ui8_i8_i8, 
                               bn5_conv2dk1_relu_i8_ui8, bn5_conv2dk3_dw_stride1_relu_ui8_ui8, bn5_conv2dk1_skip_ui8_i8_i8, 
                               bn4_tensorLayer1Out_ty, bn4_tensorLayer2Out_ty, bn4_tensorLayer3Out_ty, 
                               bn5_tensorLayer1Out_ty, bn5_tensorLayer2Out_ty,
                               tensorL4_1InW, tensorL4_1InH, tensorL4_1InC, bn4_depthWiseStride, bn4_depthWiseChannels, 
                               bn5_depthWiseStride, bn5_depthWiseChannels, tensorL5_3OutC,
                               bn4_scaleFactor1, bn4_scaleFactor2 , bn4_scaleFactor3 , bn4_scaleFactorAdd, bn5_scaleFactor1, bn5_scaleFactor2 , bn5_scaleFactor3 , bn5_scaleFactorAdd  )
        ...
        # between compute tiles
        act_bn6_bn7 = object_fifo("act_bn6_bn7", bn6_tile, bn7_tile, 2, bn6_tensorLayer3Out_ty)

        # act_out = object_fifo("act_out", bn6_tile, [ShimTile10], 1, bn6_tensorLayer3Out_ty)
        bottleneckACoreStatic("bn6", bn6_tile, act_bn5_bn6, bn6_wts_static, act_bn6_bn7, rtpbn6_tile, bn6_objectArchiveName,
                         bn6_conv2dk1_relu_i8_ui8, bn6_conv2dk3_dw_stride1_relu_ui8_ui8, bn6_conv2dk3_dw_stride2_relu_ui8_ui8, bn6_conv2dk1_ui8_i8, bn6_conv2dk1_skip_ui8_i8_i8,
                           ...
                           bn6_scaleFactor1, bn6_scaleFactor2, bn6_scaleFactor3,  bn6_scaleFactorAdd)
        ...
        # between compute tiles
        act_bn7_bn8 = object_fifo("act_bn7_bn8", bn7_tile, bn8_bn9_tile, 2, bn7_tensorLayer3Out_ty)
        # act_out = object_fifo("act_out", bn7_tile, [ShimTile10], 1, bn7_tensorLayer3Out_ty)
        bottleneckACoreStatic("bn7", bn7_tile, act_bn6_bn7, bn7_wts_static, act_bn7_bn8, rtpbn7_tile, bn7_objectArchiveName, 
                        ...
                        bn7_scaleFactor1, bn7_scaleFactor2, bn7_scaleFactor3,  bn7_scaleFactorAdd)
        ...
        act_bn9_bn10 = object_fifo("act_bn9_bn10", bn8_bn9_tile, bn10_tile_1, [1,2], bn8_tensorLayer3Out_ty)  #  GAGAN: make it double buffered $
        ...
        bottleneckAFused_8and9("bn8_bn9", bn8_bn9_tile, bn11_tile_2, act_bn7_bn8, bn8_9_wts_static, act_bn9_bn10, rtpbn8_bn9_tile, 
                               "combined_bn_8_9.a", bn8_conv2dk1_relu_i8_ui8, bn8_conv2dk3_dw_stride1_relu_ui8_ui8, bn8_conv2dk1_skip_ui8_i8_i8, 
                               bn9_conv2dk1_relu_i8_ui8, bn9_conv2dk3_dw_stride1_relu_ui8_ui8, bn9_conv2dk1_skip_ui8_i8_i8, 
                               bn8_tensorLayer1Out_ty, bn8_tensorLayer2Out_ty, bn8_tensorLayer3Out_ty, 
                               bn9_tensorLayer1Out_ty, bn9_tensorLayer2Out_ty,
                               tensorL8_1InW, tensorL8_1InH, tensorL8_1InC, bn8_depthWiseStride, bn8_depthWiseChannels, bn9_depthWiseStride, bn9_depthWiseChannels, tensorL9_3OutC,
                               bn8_scaleFactor1, bn8_scaleFactor2 , bn8_scaleFactor3 , bn8_scaleFactorAdd, bn9_scaleFactor1, bn9_scaleFactor2 , bn9_scaleFactor3 , bn9_scaleFactorAdd  )

        # # ##### ******************************************************************************************************************************        
        act_B_C = object_fifo("act_B_C", bn12_tile_2, [bn13_tile_layer1_put,bn13_tile_layer1_get,MemTile51], [2, 2, 2, 6], ty_bneck_13_layer1_in,)
        bn13_skip = object_fifo("bn13_skip", MemTile51, bn13_tile_layer3_get, 2, ty_bneck_13_layer1_in)
        object_fifo_link(act_B_C, bn13_skip)

        BottleneckBCore("B",bn10_tile_1,bn10_tile_2,bn10_tile_3,bn11_tile_1,bn11_tile_2,bn11_tile_3,bn12_tile_1,bn12_tile_2,
                        bn10_1_wts_static,bn10_2_wts_static,bn10_3_wts_static,
                        bn11_1_wts_static,bn11_2_wts_static,bn11_3_wts_static,
                        bn12_1_wts_static,bn12_2_3_wts_static,
                        b12_layer2_out,
                        bn10_1_rtp,bn10_2_rtp,bn10_3_rtp,bn11_1_rtp,bn11_2_rtp,bn11_3_rtp,bn12_1_rtp,bn12_2_rtp,
                        MemTile21,act_bn9_bn10,act_B_C, bn10_scaleFactor1,bn10_scaleFactor2,bn10_scaleFactor3,
                           bn11_scaleFactor1,bn11_scaleFactor2,bn11_scaleFactor3,bn11_scaleFactorAdd,
                           bn12_scaleFactor1,bn12_scaleFactor2,bn12_scaleFactor3 )
        ...
        act_in_C_post = object_fifo("act_in_C_post", bn14_tile_layer3_get, PostL1Tile, 2, ty_bneck_14_layer3_out)
        ...
        bottleneckCCore(bn13_tile_layer1_put,bn13_tile_layer1_get,bn13_tile_layer2,bn13_tile_layer3_put,bn13_tile_layer3_get,
                                    ...
                                    bn13_wts_memtile_layer1_put,bn13_wts_memtile_layer1_get,bn13_2_wts_static,bn13_wts_memtile_layer3_put,bn13_wts_memtile_layer3_get,
                                    bn14_wts_memtile_layer1_put,bn14_wts_memtile_layer1_get,bn14_2_wts_static,bn14_wts_memtile_layer3_put,bn14_wts_memtile_layer3_get,
                                    ...
                                    bn14_scaleFactor1,bn14_scaleFactor2,bn14_scaleFactor3,bn14_scaleFactorAdd,MemTile71,act_B_C,act_in_C_post,bn13_skip )

        # # AIE Core Function declarations
        post_fused_conv2dk1_i8_avg_pool = external_func("conv2dk1_xy_pool_fused_relu_large_padded_i8_ui8", inputs=[ty_post_act_Layer1_L2L1, ty_post_wts_Layer1_L2L1, ty_post_Layer1_out, \
                                                                                        int32_ty,int32_ty,int32_ty,int32_ty,int32_ty,int32_ty,int32_ty,int32_ty], )
        ...
        act_out_post_avgpool_shim = object_fifo("act_out_post_avgpool_shim", PostL1Tile, ShimTile30, 2, ty_post_Layer1_out)
        postBlock(PostL1Tile,act_in_C_post,
                    #post_L1_wts_L2_L1,
                    post_L1_tile_prod_lock, post_L1_tile_cons_lock, post_L1_tile_buff,
                    act_out_post_avgpool_shim, post_fused_conv2dk1_i8_avg_pool, post_L1_InW, post_L1_InH, post_L1_InC, post_L1_OutW, post_L1_OutH,post_L1_OutC,post_L1_OutC_padd, post_scaleFactor)
        

        act_out_post_shim_FC = object_fifo("act_out_post_shim_FC", ShimTile40, [PostL2Tile_1,PostL2Tile_2,PostL2Tile_3,PostL2Tile_4], [2,2,2,2,2], ty_post_Layer1_out)
        # AIE Core Function declarations
        post_L2_fused_conv2dk1 = external_func("post_L2_conv2dk1_relu_i16_ui16_pad", inputs=[ty_post_act_Layer2_all, ty_post_wts_Layer2_split_L1, ty_post_Layer2_out_split, \
                                                                                       int32_ty, int32_ty,int32_ty,int32_ty,int32_ty], )
        
        
        
        post_L2_out_core1 = object_fifo("post_L2_out_core1", PostL2Tile_1, [MemTile61], 2, ty_post_Layer2_out_split)
        post_L2_out_core2 = object_fifo("post_L2_out_core2", PostL2Tile_2, [MemTile61], 2, ty_post_Layer2_out_split)
        post_L2_out_core3 = object_fifo("post_L2_out_core3", PostL2Tile_3, [MemTile61], 2, ty_post_Layer2_out_split)
        post_L2_out_core4 = object_fifo("post_L2_out_core4", PostL2Tile_4, [MemTile61], 2, ty_post_Layer2_out_split)
        act_out = object_fifo("act_out", MemTile61, ShimTile70, 2, ty_post_Layer2_out_all)
        object_fifo_link([post_L2_out_core1, post_L2_out_core2,post_L2_out_core3,post_L2_out_core4],[act_out],[0,post_act_out_Layer2//4,post_act_out_Layer2//2,3*post_act_out_Layer2//4]) 

        # act_out = object_fifo("act_out", PostL2Tile, ShimTile70, 2, ty_post_Layer2_out_split)

        postBlockL2(PostL2Tile_1,PostL2Tile_2,PostL2Tile_3,PostL2Tile_4,
                                ...
                                post_L2_OutW, post_L2_OutH, post_L1_OutC, post_L1_OutC_padd, 
                                post_L2_OutW, post_L2_OutH,post_L2_OutC, post_FC1_scaleFactor,post_FC2_scaleFactor,
                                PostL2Tile_1_cons_prod_lock,PostL2Tile_2_cons_prod_lock,PostL2Tile_3_cons_prod_lock,PostL2Tile_4_cons_prod_lock,
                                PostL2Tile_1_cons_cons_lock,PostL2Tile_2_cons_cons_lock,PostL2Tile_3_cons_cons_lock,PostL2Tile_4_cons_cons_lock)
        ...
        C_block_13_wts_size32b=memtile_51_wts32b_layer1+memtile_61_wts32b_layer2+memtile_51_wts32b_layer3_2 # bn13
        ...
        post_layer2_memtile_71_offset_4= post_layer2_memtile_71_offset_3 + post_wts_Layer2_split_32b
        post_layer2_memtile_71_offset_5= post_layer2_memtile_71_offset_4 + post_wts_Layer2_split_32b
        post_layer2_memtile_71_offset_6= post_layer2_memtile_71_offset_5 + post_wts_Layer2_split_32b
        
        post_layer3_memtile_71_offset_1= post_layer2_memtile_71_offset_6 + post_wts_Layer2_split_32b
        post_layer3_memtile_71_offset_2= post_layer3_memtile_71_offset_1 + post_wts_Layer2_split_32b
        post_layer3_memtile_71_offset_3= post_layer3_memtile_71_offset_2 + post_wts_Layer2_split_32b
        post_layer3_memtile_71_offset_4= post_layer3_memtile_71_offset_3 + post_wts_Layer2_split_32b

        weightsInL3_ty = MemRefType.get((total_weights//4+
                                        B_block_totalWeightsSize32b+
                                        C_total_wts_size32b+
                                        post_L1_wts_size32b,), int32_ty)
        ...
        tiles_to_trace = [  init_tile, #initconv,     tile(1,2)
        ...
        def sequence(inputFromL3, weightsFromL3, outputToL3):
            ...
            NpuWriteRTPOp("rtp_bn13_tile_layer3_get",  index=1, value=bn13_scaleFactorAdd)
            ...
            NpuWriteRTPOp("rtp_bn14_tile_layer3_get",  index=1, value=bn14_scaleFactorAdd)
            ...
            N_in_bytes = tensorOutW * tensorOutH * tensorOutC * 2 # x2 since output is uin16
            # N_in_bytes = tensorOutW * tensorOutH * tensorOutC

            if opts.trace_size > 0:
                trace_utils.configure_packet_tracing_aie2(tiles_to_trace, ShimTile00, opts.trace_size, N_in_bytes)
...
with mlir_mod_ctx() as ctx:
    ...
    mobilenetV3_A_B(tileColIndex = 0,b_start_col=2, b_start_row=4,
                    tensorInW = 224, tensorInH = 224, tensorInC = 8, init_tensorOutC=16,
                    ...
                    bn8_depthWiseStride = 1, bn8_depthWiseChannels = 184, bn8_withSkip = True, bn8_tensorOutC = 80, bn8_scaleFactor1 = scale_factors["BN8"]["conv1x1_1"], bn8_scaleFactor2 =scale_factors["BN8"]["conv3x3"], bn8_scaleFactor3 = scale_factors["BN8"]["conv1x1_2"],  bn8_scaleFactorAdd = scale_factors["BN8"]["skip_add"],
                    ...
                    enableTrace = False, trace_size = 16384, traceSizeInInt32s = 4096,
                    bn10_scaleFactor1=scale_factors["BN10"]["conv1x1_1"],bn10_scaleFactor2=scale_factors["BN10"]["conv3x3"],bn10_scaleFactor3=scale_factors["BN10"]["conv1x1_2"],
                    ...
                    bn13_scaleFactor1=scale_factors["BN13"]["conv1x1_1"],bn13_scaleFactor2=scale_factors["BN13"]["conv3x3"],bn13_scaleFactor3=scale_factors["BN13"]["conv1x1_2"],bn13_scaleFactorAdd=scale_factors["BN13"]["skip_add"],
                    ...
                    post_scaleFactor=scale_factors["POST"]["conv1x1_1"],post_FC1_scaleFactor=scale_factors["POST"]["FC1"],post_FC2_scaleFactor=scale_factors["POST"]["FC2"])    
