...
class bottleneckBN0:
    def __init__(
        ...
        self.tensorL2InC = self.tensorInC
        ...
        def core_body():
            ...
            for _ in for_(1):  # for _ in for_(sys.maxsize):
                ...
                # JL
                # weightsLayer2 = memref_view(weightsAllLayers.output, [3 * 3 * self.tensorL2OutC * 1], shift=0)
                # weightsLayer3 = memref_view(weightsAllLayers.output, [1 * 1 * self.tensorL3OutC * self.tensorL3InC], shift=3 * 3 * self.tensorL2OutC * 1)
                weightsLayer2 = memref_view(
                    weightsAllLayers, [3 * 3 * self.tensorL2OutC * 1], shift=0
                )
                weightsLayer3 = memref_view(
                    weightsAllLayers,
                    [1 * 1 * self.tensorL3OutC * self.tensorL3InC],
                    shift=3 * 3 * self.tensorL2OutC * 1,
                )
                scaleLayer2 = memref.load(self.rtpsIn, [0])  # scaleFactor2
                scaleLayer3 = memref.load(self.rtpsIn, [1])  # scaleFactor3
                skipScaleLayer3 = memref.load(self.rtpsIn, [2])  # scaleFactorAdd

                # # pre-amble 0: row 0 in layer 2 3x3 dw; row 0 in layer 3 1x1 conv
                actInLayer2Rows = self.actIn.acquire(ObjectFifoPort.Consume, 2)
                ...
                # call(self.f3x3dwRelu, [actInLayer2Rows[0], actInLayer2Rows[0], actInLayer2Rows[1], weightsLayer2, self.buf_act_2_3, self.tensorInW, 1, self.tensorL2OutC, 3, 3, 0, scaleLayer2, 0]) # where do we plug in stride
                ...
                # call(self.f1x1Skip, [self.buf_act_2_3, weightsLayer3, actOutLayer3Row, actInLayer2Rows[0], self.tensorOutW, self.tensorL3InC, self.tensorL3OutC, scaleLayer3, skipScaleLayer3])
                ...
                for _ in for_(self.tensorOutH - 2):
                    actInLayer2Rows = self.actIn.acquire(ObjectFifoPort.Consume, 3)
                    actOutLayer2Row = self.of_act_2_3.acquire(ObjectFifoPort.Produce, 1)
                    call(
                        self.f3x3dwRelu,
                        [
                    ...
                    # call(self.f3x3dwRelu, [actInLayer2Rows[0], actInLayer2Rows[1], actInLayer2Rows[2], weightsLayer2, self.buf_act_2_3, self.tensorInW, 1, self.tensorL2OutC, 3, 3, 1, scaleLayer2, 0]) # where do we plug in
                    self.of_act_2_3.release(ObjectFifoPort.Produce, 1)
                    ...
                    # call(self.f1x1Skip, [self.buf_act_2_3, weightsLayer3, actOutLayer3Row, actInLayer2Rows[1], self.tensorOutW, self.tensorL3InC, self.tensorL3OutC, scaleLayer3, skipScaleLayer3])
                ...
                # call(self.f3x3dwRelu, [actInLayer2Rows[0], actInLayer2Rows[1], actInLayer2Rows[1], weightsLayer2, self.buf_act_2_3, self.tensorInW, 1, self.tensorL2OutC, 3, 3, 2, scaleLayer2, 0]) # where do we plug in stride
                self.of_act_2_3.release(ObjectFifoPort.Produce, 1)
...
def mobilenetV3Bottleneck0(
    ...
    def device_body():
        ...
        weightsLayer3_ty = MemRefType.get(
            (1 * 1 * tensorL3OutC * tensorL3InC,), int8_ty
        )
        tensorLayer3Out_ty = MemRefType.get((tensorOutW, 1, tensorL3OutC), int8_ty)

        weightsAllLayers_ty = MemRefType.get(
            (3 * 3 * tensorL2OutC * 1 + 1 * 1 * tensorL3OutC * tensorL3InC,), int8_ty
        )

        # AIE Core Function declarations
        conv2dk3_dw_relu_ui8_ui8 = external_func(
        ...
        # Intermediate
        of_act_2_3 = object_fifo(
            "act_2_3", ComputeTile, ComputeTile, 1, tensorLayer2Out_ty
        )

        # Set up compute tiles
        # rtpComputeTile = buffer(ComputeTile, np.ndarray[(16,), np.dtype[np.int32]], "rtp")
        ...
        def core_body():
            ...
            for _ in for_(1):  # for _ in for_(sys.maxsize):
                ...
                weightsLayer2 = memref_view(
                    weightsAllLayers, [3 * 3 * tensorL2OutC * 1], shift=0
                )
                weightsLayer3 = memref_view(
                    weightsAllLayers,
                    [1 * 1 * tensorL3OutC * tensorL3InC],
                    shift=3 * 3 * tensorL2OutC * 1,
                )
        ...
        totalWeightsSize32b = (
            3 * 3 * tensorL2OutC * 1 + 1 * 1 * tensorL3InC * tensorL3OutC
        ) // 4
