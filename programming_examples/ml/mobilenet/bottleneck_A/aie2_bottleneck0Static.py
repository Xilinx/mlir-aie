...
def mobilenetV3Bottleneck0(
    ...
    def device_body():
        ...
        weightsLayer2_size = 3 * 3 * tensorL2OutC * 1
        weightsLayer2_ty = MemRefType.get((weightsLayer2_size,), int8_ty)
        ...
        weightsLayer3_size = 1 * 1 * tensorL3OutC * tensorL3InC
        weightsLayer3_ty = MemRefType.get((weightsLayer3_size,), int8_ty)
        tensorLayer3Out_ty = MemRefType.get((tensorOutW, 1, tensorL3OutC), int8_ty)
        weightsAllLayers_size = (
            3 * 3 * tensorL2OutC * 1 + 1 * 1 * tensorL3OutC * tensorL3InC
        )
        ...
        rtplock = lock(ComputeTile, lock_id=15, init=0)
        ...
        #         reader = csv.reader(f)
        #         wts = list(reader)
        # wts_ary2 = numpy.array(wts, dtype=np.int8)

        # with open(file_path+"bn0_layer3_after_weights_mem_fmt_final.txt", newline="") as f:
        #         reader = csv.reader(f)
        #         wts = list(reader)
        # wts_ary3 = numpy.array(wts, dtype=np.int8)

        # with open(file_path+"bn0_after_weights_mem_fmt_final.txt", newline="") as f:
        #         reader = csv.reader(f)
        #         wts = list(reader)
        ...
        # file_path = "weights/bn0_layer3_after_weights_mem_fmt_final.txt"
        # wts_ary3 = np.fromfile(file_path, sep=",", dtype=np.int8)
        # Output
        ...
        # weightsAllLayers = buffer(ComputeTile, (weightsAllLayers_size,), int8_ty, "weights", initial_value=wts_ary)
        ...
        # weightsLayer2 = buffer(ComputeTile, (weightsLayer2_size,), int8_ty, "weights2", initial_value=wts_ary2)
        # weightsLayer3 = buffer(ComputeTile, (weightsLayer3_size,), int8_ty, "weights3", initial_value=wts_ary3)
        ...
        def core_body():
            ...
            for _ in for_(1):  # for _ in for_(sys.maxsize):
                ...
                use_lock(rtplock, LockAction.AcquireGreaterEqual, value=1)
                ...
                # scaleLayer2 = 9# scaleFactor2
                # scaleLayer3 = 8 # scaleFactor3
                # skipScaleLayer3 = 2 # scaleFactorAdd
        ...
        def sequence(inputFromL3, weightsFromL3, outputToL3):
            ...
            npu_write32(column=0, row=2, address=0x1F0F0, value=1)
