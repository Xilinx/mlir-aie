...
# When MOBILENET_NO_PLACEMENT=1 is set in the environment, the builders receive
# no placement (placement=None / tiles=None) and the design is emitted unplaced
# for the SA placer to assign tiles at compile time.  @iron.jit fixes the
# decorated signature to (inp, cascade_wts, out), so the toggle is read from the
# environment rather than passed as a function argument.
USE_PLACEMENT = os.environ.get("MOBILENET_NO_PLACEMENT", "0") != "1"
...
def mobilenet_iron(inp: In, cascade_wts: In, out: Out):
    ...
    When USE_PLACEMENT is False (MOBILENET_NO_PLACEMENT=1), ``P`` is empty so
    every builder receives ``None`` and emits unplaced logical tiles for the
    SA placer.
    """
    # Empty dict in no-placement mode: P.get(...) returns None for every block.
...
def _make_argparser():
    ...
    p.add_argument(
        ...
        "placement.py hints; equivalent to setting MOBILENET_NO_PLACEMENT=1",
    )
    return p
...
def main():
    ...
    if opts.no_placement:
        # The design body reads USE_PLACEMENT (module global) when it runs
        # inside run_design_cli; flip it so every builder gets None.
        global USE_PLACEMENT
        USE_PLACEMENT = False
    run_design_cli(
        mobilenet_iron,
        opts,
