...
def regular_bottlenecks(
    ...
    *,
    ...
    def _make_3layer_block(
        ...
        wts_buf = Buffer(
            _i8((wts_sz,)), initial_value=_load_wts(f"{name}_chain.txt", wts_sz)
        )
        ...
        if has_skip:
            ...
            def worker_fn(
                act_in_fifo, wts, out_f, p12, c12, p23, c23, k_pw, k_dw, k_l3
            ):
                wts_l1 = memref_view(wts.op, [l1_sz], shift=0)
                wts_l2 = memref_view(wts.op, [l2_sz], shift=l1_sz)
                wts_l3 = memref_view(wts.op, [l3_sz], shift=l1_sz + l2_sz)
    ...
    def _make_fused_pair_block(
        ...
        wts_buf = Buffer(
            _i8((wts_sz,)), initial_value=_load_wts(chain_filename, wts_sz)
        )
        ...
        ka_pw, ka_dw, ka_skip = _kernels(
            name_a, a_dw_ch, in_c, a_out_c, a_l1, a_l2, a_l3
        )
        ...
        def worker_fn(
            ...
            def a_dw(top, mid, bot, border, c12_release):
                ...
                ka_dw(
                    top, mid, bot, wa2, row_l2, in_w, 1, a_dw_ch, 3, 3, border, sa2, 0
                )
                if c12_release:
            ...
            def b_dw(top, mid, bot, border, c12_release):
                ...
                kb_dw(
                    top, mid, bot, wb2, row_l2, in_w, 1, b_dw_ch, 3, 3, border, sb2, 0
                )
                if c12_release:
            ...
            def b_skip(skip_row):
                ...
                row_out = out_f.acquire(1)
                kb_skip(
                    row_l2, wb3, row_out, skip_row, in_w, b_dw_ch, b_out_c, sb3, sb_add
                )
                b23c.release(1)
                abc.release(1)
                out_f.release(1)
    ...
    act_bn0_bn1, w = _make_2layer_skip_block(
        ...
        _BN0_DW_CH,
        _BN0_OUT_C,
        scales=s(0, "conv3x3", "conv1x1_2"),
        scale_add=sf["BN0"]["skip_add"],
        tile=placement["bn0"],
    )
    workers.append(w)
    ...
    act_bn1_bn2, w = _make_3layer_block(
        ...
        _BN1_OUT_C,
        stride=2,
        scales=s(1, "conv1x1_1", "conv3x3", "conv1x1_2"),
        scale_add=None,
        tile=placement["bn1"],
    )
    workers.append(w)
    ...
    act_bn2_bn3, w = _make_3layer_block(
        ...
        _BN2_OUT_C,
        stride=1,
        scales=s(2, "conv1x1_1", "conv3x3", "conv1x1_2"),
        scale_add=sf["BN2"]["skip_add"],
        tile=placement["bn2"],
    )
    workers.append(w)
    ...
    act_bn3_bn4, w = _make_3layer_block(
        ...
        _BN3_OUT_C,
        stride=2,
        scales=s(3, "conv1x1_1", "conv3x3", "conv1x1_2"),
        scale_add=None,
        tile=placement["bn3"],
    )
    workers.append(w)
    ...
    act_bn6_bn7, w = _make_3layer_block(
        ...
        _BN6_OUT_C,
        stride=2,
        scales=s(6, "conv1x1_1", "conv3x3", "conv1x1_2"),
        scale_add=None,
        tile=placement["bn6"],
    )
    workers.append(w)
    ...
    act_bn7_bn8, w = _make_3layer_block(
        ...
        _BN7_OUT_C,
        stride=1,
        scales=s(7, "conv1x1_1", "conv3x3", "conv1x1_2"),
        scale_add=sf["BN7"]["skip_add"],
        tile=placement["bn7"],
    )
    workers.append(w)
