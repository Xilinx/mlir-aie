...
def post_l1(act_in, sf, *, data_dir):
    ...
    # a MemTile and stream 9600 B chunks via ObjectFifo.
    PostOutputSplit = 8
    ...
    # Put the producer (source of weight data) on a MemTile. Normally a
    # Worker sets its fifo endpoint implicitly, but this fifo has no
    # producing Worker.
    post_l1_wts_of.prod().endpoint = ObjectFifoEndpoint(AnyMemTile.copy())
    ...
    # stream-switch routing capacity from the post_l1 tile. Element type is uint16:
