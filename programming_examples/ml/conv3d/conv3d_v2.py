...
def conv3dk3(
    ...
    actIn = width * in_channels  # One line
    ...
    tensorInSize = depth * width * height * in_channels  # Added depth
    tensorOutSize = depth * width * height * out_channels  # Added depth
    ...
    def core_fn(of_wts, of_act, of_out, my_rtp, conv3dk3_func, barrier):
        d_dim = depth
        x_dim = width
        ci = in_channels
        co = out_channels
        ...
        for _ in range_(d_dim):
            elemIn = of_act.acquire(1)
            elemOut0 = of_out.acquire(1)

            # Pass same plane 3 times for now (simplified 3D)
            conv3dk3_func(elemIn, elemIn, elemIn, elemWts, elemOut0,
                         x_dim, x_dim, ci, co, 1, 1, 1, 1, scale, 0)
            of_act.release(1)
            of_out.release(1)
        of_wts.release(1)
