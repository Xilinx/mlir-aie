...
"""Row-wise bf16 LayerNorm — Iron API design with ``@iron.jit`` compilation.

NPU2-only: the underlying ``layer_norm.cc`` kernel lives under
``aie_kernels/aie2p/`` and has no aie2 counterpart.

Eight cores process ``sequence_length // 8`` rows each.  Per row:
    out = (x - mean(x)) / sqrt(var(x) + eps) * gamma + beta
with gamma=1, beta=0, eps=1e-5.
"""
...
_KERNEL_SRC = Path(__file__).resolve().parents[3] / "aie_kernels/aie2p/layer_norm.cc"


def _layer_norm_extern(chunk_type):
...
def layernorm(
    ...
    *,
    ...
    layer_norm_fn = _layer_norm_extern(chunk_ty)
    ...
    workers = [
        Worker(core_fn, [of_ins[i].cons(), of_outs[i].prod(), layer_norm_fn])
...
def _make_argparser():
    p = argparse.ArgumentParser(prog="AIE LayerNorm")
    add_compile_args(p)
...
def _layer_norm_reference(x_np: np.ndarray) -> np.ndarray:
    eps = 1e-5
    gamma = 1.0
    beta = 0.0
    ...
    out = (x32 - mean) * inv_std * gamma + beta
...
def _run_and_verify(opts):
    ...
    layernorm(a_t, c_t, **_compile_kwargs(opts))

    expected = _layer_norm_reference(a_np)
    assert_pass(
        c_t.numpy().reshape(rows, cols),
        expected,
        atol=0.1,
        fail_msg="layernorm output mismatch",
    )
