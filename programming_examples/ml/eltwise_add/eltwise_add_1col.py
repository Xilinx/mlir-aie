...
def my_eltwise_add(dev, in1_size, in2_size, out_size):
    in1_dtype = bfloat16
    in2_dtype = bfloat16
    out_dtype = bfloat16
    ...
    def device_body():
        ...
        C_memTile_ty = np.ndarray[(tensor_per_shimtile,), np.dtype[out_dtype]]
            ...
            for i in range(n_cores):
                in_tasks.append(
                    shim_dma_single_bd_task(
                        inA_fifos[i],
                        A,
                        offset=tensor_per_shimtile * i,
                        sizes=[1, 1, 1, tensor_per_shimtile],
                    )
                )
                in_tasks.append(
                    shim_dma_single_bd_task(
...
with mlir_mod_ctx() as ctx:
    my_eltwise_add(dev, in1_size, in2_size, out_size)
