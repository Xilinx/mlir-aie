# eltwise_add/eltwise_add.py -*- Python -*-
#
...
"""Element-wise bf16 add — Iron API design with ``@iron.jit`` compilation.

Saturates DDR bandwidth by using every column's shim DMA in/out pairs.
The per-tile kernel comes from ``aie.iron.kernels.add`` (auto-built by
``compile_mlir_module(device=...)`` into the JIT work_dir).

Two invocation modes:

  * standalone:   ``python3 eltwise_add.py``
...
def eltwise_add(
    ...
    *,
    ...
    add_fn = kernels.add(tile_size=line_size)
    ...
    workers = Worker.grid(
        ...
        lambda i, j: Worker(
            core_fn,
            [of_in1s[i][j].cons(), of_in2s[i][j].cons(), of_outs[i][j].prod(), add_fn],
        ),
    )
...
def _make_argparser():
    p = argparse.ArgumentParser(prog="AIE Eltwise Add")
    add_compile_args(p)
...
def _run_and_verify(opts):
    ...
    eltwise_add(a_t, b_t, c_t, **_compile_kwargs(opts))
    ...
    assert_pass(
        ...
        expected,
        atol=0.00390625,
        fail_msg="eltwise_add output mismatch",
    )
