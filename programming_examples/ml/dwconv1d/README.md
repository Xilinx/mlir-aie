...
1. `dwconv1d.cc`: AIE2P kernel, pulled from [`aie_kernels/aie2p/`](../../../aie_kernels/aie2p/). Vectorized 16-outputs/iteration: two aligned loads build a 32-lane window, and `aie::shuffle_down_fill` slides a 16-wide tap window across it in-register (no unaligned loads); each tap's product is `aie::mac`'d into one `accfloat` accumulator. `aie::sliding_mul_ops` is the general, dedicated primitive for this shape; this kernel does not use it, for reasons the kernel's own header comment records in full (a standalone repro later found the primitive itself correct, so this is not a workaround for a library defect). No performance comparison between the two has been made.

1. `test.cpp`: C++ testbench. Loads the compiled XCLBIN + `insts.bin` via `setup_and_run_aie`, recomputes the reference directly from the already-padded input buffer (the same convolution the kernel itself performs, so no separate host-side padding step), and checks the output against it with a tolerance (bf16 accumulation is not bit-exact against a double-precision host reference, though on this target it measures bit-exact in practice for both the `bias=True` and `bias=False` paths).
