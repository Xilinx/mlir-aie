...
1. `dwconv1d_channels_first.cc`: AIE2P kernel, pulled from [`aie_kernels/aie2p/`](../../../aie_kernels/aie2p/). Vectorized 16-outputs/iteration: two aligned 256-bit loads build a 32-lane window and `aie::sliding_mul_ops<16, K, 1, 1, 1, bfloat16, bfloat16>` runs the K-tap correlation over it into one `accfloat` accumulator.
