...
void max8(int* actsPixels, int* reduced) {
    for(unsigned int i = 0; i < 8; i++) {
        reduced[i] = -128;
        for(unsigned int j = 0; j < MP_W * MP_W; j++) {
            reduced[i] = max(actsPixels[j*8 + i], reduced[i]);
        }
    }
}
...
void computeOutputPixels(int* acts, int* res) {
    ...
    for(unsigned int c = 0; c < C/8; c++) {
        for(unsigned int y = 0; y < (N-(MP_W-1)); y += MP_S) {
            for(unsigned int x = 0; x < (M - (MP_W-1)); x += MP_S) {
                memset(actsPixels, '\0', 8 * MP_W * MP_W);
                getActPixelsAt(x, y, c, acts, actsPixels);

                int reduced[8];
                memset(reduced, '\0', 8 * sizeof(int));
                max8(actsPixels, reduced);
...
int main(int argc, char* argv[]) {
    ...
    if(argc == 1 || (argc == 2 && argv[1][0] == '0')) {
        ...
        readActsFromFile(AIn, M * N * C, FNAME_ACTS);
    }
    ...
    writeActsToFile(AOutRef, outTileSize, AOutRef_FILENAME);
}
