...
void matrix_multiplication(const int16 *__restrict A, const int16 *__restrict B, int16 *__restrict C) {
    for(unsigned row = 0; row < m / r; row += 2) {
        for(unsigned col = 0; col < n / t; col += 2) {
            ...
            const int16 *__restrict A0_ptr = A + ((row + 0) * (k / s) +           0) * MMUL::size_A;
            const int16 *__restrict A1_ptr = A + ((row + 1) * (k / s) +           0) * MMUL::size_A;
            const int16 *__restrict B0_ptr = B + (        0 * (n / t) +   (col + 0)) * MMUL::size_B;
            const int16 *__restrict B1_ptr = B + (        0 * (n / t) +   (col + 1)) * MMUL::size_B;

            const aie::vector<int16, MMUL::size_C> C00_in = aie::load_v<MMUL::size_C>(C + ((row + 0) * (n / t) +   (col + 0)) * MMUL::size_C);
            const aie::vector<int16, MMUL::size_C> C01_in = aie::load_v<MMUL::size_C>(C + ((row + 0) * (n / t) +   (col + 1)) * MMUL::size_C);
            const aie::vector<int16, MMUL::size_C> C10_in = aie::load_v<MMUL::size_C>(C + ((row + 1) * (n / t) +   (col + 0)) * MMUL::size_C);
            const aie::vector<int16, MMUL::size_C> C11_in = aie::load_v<MMUL::size_C>(C + ((row + 1) * (n / t) +   (col + 1)) * MMUL::size_C);
            MMUL C00(C00_in);
            MMUL C01(C01_in);
            MMUL C10(C10_in);
            MMUL C11(C11_in);
            ...
            {
                const aie::vector<int16, MMUL::size_A> A0 = aie::load_v<MMUL::size_A>(A0_ptr);
                const aie::vector<int16, MMUL::size_A> A1 = aie::load_v<MMUL::size_A>(A1_ptr);
                const aie::vector<int16, MMUL::size_B> B0 = aie::load_v<MMUL::size_B>(B0_ptr);
                const aie::vector<int16, MMUL::size_B> B1 = aie::load_v<MMUL::size_B>(B1_ptr);
