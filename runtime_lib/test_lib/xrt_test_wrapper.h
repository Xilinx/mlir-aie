...
int xrt_test_run(
    int IN1_VOLUME, int IN2_VOLUME, int OUT_VOLUME, 
    struct args myargs)
{
    ...
    std::vector<uint32_t> instr_v =
        test_utils::load_instr_sequence(myargs.instr);
    ...
    auto bo_in1 = xrt::bo(device, 
                IN1_VOLUME*sizeof(T1), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
    auto bo_in2 = xrt::bo(device, 
                IN2_VOLUME*sizeof(T2), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
    auto bo_out = xrt::bo(device, 
                OUT_VOLUME*sizeof(T3)+myargs.trace_size, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(5));
    ...
    for (unsigned iter = 0; iter < num_iter; iter++) {
        ...
        if (myargs.do_verify) {
            ...
            errors += verify_results(bufIn1, bufIn2, bufOut, IN1_VOLUME, myargs.verbosity);
        ...
        if (myargs.trace_size > 0 && iter == myargs.n_warmup_iterations) {
            test_utils::write_out_trace(((char *)bufOut) + OUT_VOLUME*sizeof(T3), myargs.trace_size,
                                        myargs.trace_file);
        }
    ...
    if (macs > 0)
        std::cout << "Avg NPU gflops: "
                << macs / (1000 * npu_time_total / myargs.n_iterations) << std::endl;
