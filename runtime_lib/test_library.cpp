...
void ACDC_print_dma_status(XAie_DevInst *DevInst, XAie_LocType loc) {
  ...
  u32 dma_mm2s_status;  XAie_Read32(DevInst,tileAddr + 0x0001DF10, &dma_mm2s_status);
  u32 dma_s2mm_status;  XAie_Read32(DevInst,tileAddr + 0x0001DF00, &dma_s2mm_status);
  u32 dma_mm2s0_control; XAie_Read32(DevInst,tileAddr + 0x0001DE10, &dma_mm2s0_control);
  u32 dma_mm2s1_control; XAie_Read32(DevInst,tileAddr + 0x0001DE18, &dma_mm2s1_control);
  u32 dma_s2mm0_control; XAie_Read32(DevInst,tileAddr + 0x0001DE00, &dma_s2mm0_control);
  u32 dma_s2mm1_control; XAie_Read32(DevInst,tileAddr + 0x0001DE08, &dma_s2mm1_control);
  u32 dma_bd0_a;        XAie_Read32(DevInst,tileAddr + 0x0001D000, &dma_bd0_a); 
  u32 dma_bd0_control;  XAie_Read32(DevInst,tileAddr + 0x0001D018, &dma_bd0_control);
  ...
  printf("DMA [%d, %d] mm2s_status/0ctrl/1ctrl is %08X %02X %02X, s2mm_status/0ctrl/1ctrl is %08X %02X %02X, BD0_Addr_A is %08X, BD0_control is %08X\n",col, row, dma_mm2s_status, dma_mm2s0_control, dma_mm2s1_control, dma_s2mm_status, dma_s2mm0_control, dma_s2mm1_control, dma_bd0_a, dma_bd0_control);
  for (int bd=0;bd<8;bd++) {
      u32 dma_bd_addr_a;  XAie_Read32(DevInst,tileAddr + 0x0001D000 + (0x20*bd), &dma_bd_addr_a);
      u32 dma_bd_control; XAie_Read32(DevInst,tileAddr + 0x0001D018 + (0x20*bd), &dma_bd_control);
    if (dma_bd_control & 0x80000000) {
      ...
      if (dma_bd_control & 0x08000000) {
        u32 dma_packet; XAie_Read32(DevInst,tileAddr + 0x0001D010 + (0x20*bd), &dma_packet);
      ...
      printf("   ");
      for (int w=0;w<7; w++) {
        u32 tmpd; XAie_DataMemRdWord(DevInst, loc, (base_address+w) * 4, &tmpd);
        printf("%08X ",tmpd);
      }
      printf("\n");
      if (dma_bd_addr_a & 0x40000) {
        ...
        printf("currently ");
        u32 locks; XAie_Read32(DevInst,tileAddr + 0x0001EF00, &locks);
      ...
      if (dma_bd_control & 0x30000000) { // FIFO MODE
        int FIFO = (dma_bd_control >> 28) & 0x3;
          u32 dma_fifo_counter; XAie_Read32(DevInst,tileAddr + 0x0001DF20, &dma_fifo_counter);				
...
void ACDC_print_tile_status(XAie_DevInst *DevInst, XAie_LocType loc) {
    ...
    u32 trace_status; XAie_Read32(DevInst,tileAddr + 0x000140D8, &trace_status);
...
static void clear_range(XAie_DevInst *DevInst, u64 tileAddr, u64 low, u64 high) {
