...
void
mlir_aie_deinit_libxaie(aie_libxaie_ctx_t *ctx)
{
...
int mlir_aie_acquire_lock(aie_libxaie_ctx_t* ctx, int col, int row, int lockid, int lockval, int timeout) {
  return (XAie_LockAcquire(&(ctx->DevInst), XAie_TileLoc(col,row), XAie_LockInit(lockid, lockval), timeout) == XAIE_OK);
}

int mlir_aie_release_lock(aie_libxaie_ctx_t* ctx, int col, int row, int lockid, int lockval, int timeout) {
  return (XAie_LockRelease(&(ctx->DevInst), XAie_TileLoc(col,row), XAie_LockInit(lockid, lockval), timeout) == XAIE_OK);
}
...
void mlir_aie_dump_tile_memory(aie_libxaie_ctx_t* ctx, int col, int row) {
  ...
  for (int i = 0; i < 0x2000; i++) {
    uint32_t d;
    AieRC rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(col,row), (i * 4), &d);
    if (rc != XAIE_OK)
...
void mlir_aie_print_dma_status(aie_libxaie_ctx_t* ctx, int col, int row) {
  ...
  for (int bd = 0; bd < 8; bd++) {
    u32 dma_bd_addr_a;
    XAie_Read32(&(ctx->DevInst), tileAddr + 0x0001D000 + (0x20 * bd), &dma_bd_addr_a);
    u32 dma_bd_control;
    XAie_Read32(&(ctx->DevInst), tileAddr + 0x0001D018 + (0x20 * bd), &dma_bd_control);
    if (dma_bd_control & 0x80000000) {
      ...
      if (dma_bd_control & 0x08000000) {
        u32 dma_packet;
        XAie_Read32(&(ctx->DevInst), tileAddr + 0x0001D010 + (0x20 * bd), &dma_packet);
      ...
      for (int w = 0; w < 7; w++) {
        u32 tmpd;
        XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(col,row), (base_address + w) * 4, &tmpd);
        printf("%08X ", tmpd);
      }
      printf("\n");
...
int mlir_aie_init_device(aie_libxaie_ctx_t *ctx) {
    XAIEGBL_HWCFG_SET_CONFIG((&(ctx->AieConfig)),
                              XAIE_NUM_ROWS, XAIE_NUM_COLS, 0x800);
    XAieGbl_HwInit(&(ctx->AieConfig));
    ctx->AieConfigPtr = XAieGbl_LookupConfig(XPAR_AIE_DEVICE_ID);
    XAieGbl_CfgInitialize(&(ctx->AieInst),
                          &(ctx->TileInst[0][0]), ctx->AieConfigPtr);
    return 0;
}

int mlir_aie_acquire_lock(aie_libxaie_ctx_t* ctx, int col, int row, int lockid, int lockval, int timeout) {
  return XAieTile_LockAcquire(&(ctx->TileInst[col][row]), lockid, lockval, timeout);
}

int mlir_aie_release_lock(aie_libxaie_ctx_t* ctx, int col, int row, int lockid, int lockval, int timeout) {
  return XAieTile_LockRelease(&(ctx->TileInst[col][row]), lockid, lockval, timeout);
}
