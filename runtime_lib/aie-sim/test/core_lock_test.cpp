...
///     done
///
/// 48 is this tile's OWN lock 0. A core names a lock module by band, and the
/// bands run south/west/north/east at getNumLocks() apart, so own (east) lock 0
/// is 3*16 -- the same id a shipped kernel issues. Naming #0 here would name the
/// south neighbour's module, which is not what the counter checks below read.
///
...
const uint8_t ReleaseTwo[] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x02, 0x16, 0x18, 0x00, 0x08, 0x10,
};
