...
// !ty_22aie3A3Atile_t3C12C_43E22 = !cir.struct<struct "aie::tile_t<1, 4>"
// {!cir.int<u, 8>}>
template <int X, int Y> struct tile_t {
  static constexpr auto x() { return X; }
  static constexpr auto y() { return Y; }
  template <int X2, int Y2>
  friend constexpr auto operator==(const tile_t &, const tile_t<X2, Y2> &) {
    return X == X2 && Y == Y2;
  };
  ...
  /*
  friend constexpr auto operator<=>(const tile_t &a, const tile_t &b) {
...
*/
  ...
  template <typename T, std::size_t Size> buffer_t<T, Size> buffer() __attribute__((annotate("aie.tile.buffer"))) {
    return {};
  }

  // Typically compiled as:
  // cir.call @_ZN3aie6tile_tILi1ELi4EE7programIZ4mainE3$_0EEvOT_(%2, %7) :
  // (!cir.ptr<!ty_22aie3A3Atile_t3C12C_43E22>, !cir.ptr<!ty_22anon2E122>) -> ()
  // loc(#loc63)
...
template <int X, int Y> inline constexpr tile_t<X, Y> tile;
