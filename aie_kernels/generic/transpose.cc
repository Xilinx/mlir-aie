...
/* Individually transposes 4x4-sized subtiles in in_ptr (a matrix of size
   ...
   (i.e. in_ptr == out_ptr). */
void transpose_4x4(bfloat16 *__restrict__ in_ptr,
                   bfloat16 *__restrict__ out_ptr) {
  for (unsigned col = 0; col < DIM_m; col += VECTOR_SIZE) {
    for (unsigned row = 0; row < DIM_n; row += 4) {
      aie::vector<bfloat16, VECTOR_SIZE> row_0 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + row * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_1 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 1) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_2 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 2) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_3 =
      ...
      // ...
      std::pair<aie::vector<bfloat16, VECTOR_SIZE>,
                aie::vector<bfloat16, VECTOR_SIZE>>
...
void transpose_8x8(bfloat16 *__restrict__ in_ptr,
                   bfloat16 *__restrict__ out_ptr) {
  for (unsigned col = 0; col < DIM_m; col += VECTOR_SIZE) {
    for (unsigned row = 0; row < DIM_n; row += 8) {
      ...
      aie::vector<bfloat16, VECTOR_SIZE> row_3 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 3) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_4 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 4) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_5 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 5) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_6 =
          aie::load_v<VECTOR_SIZE>(in_ptr + col + (row + 6) * DIM_m);
      aie::vector<bfloat16, VECTOR_SIZE> row_7 =
