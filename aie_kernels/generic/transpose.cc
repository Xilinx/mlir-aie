...
// Element type. The transpose only moves bytes, so the unsigned integer of
// the element's width stands in for any dtype of that width; without a
// -DDTYPE_* flag the kernel is the library's bf16 transpose.
#if defined(DTYPE_i8)
#define DTYPE uint8_t
#elif defined(DTYPE_i16)
...
void transpose_8x8(DTYPE *__restrict__ in_ptr, DTYPE *__restrict__ out_ptr) {
  ...
  // Gated on DIM_s == 8 since the same .cc is built for DIM_s == 4 with
  // smaller m,n that never call transpose_8x8.
#if defined(DIM_s) && DIM_s == 8
