...
// #define X_C64_D8     0x00000000
// #define Z_C8_D8_I_L  0x00000000
// #define Z_C8_D8_I_U  0x00000000
// #define Z_C8_D8_I_UU 0x00000000

inline __attribute__((always_inline)) v32int8 load_w(int8_t *weightsIn) {
  v32int8 v = *(v32int8 *)weightsIn;
  return v;
}

inline __attribute__((always_inline)) v32uint8
load_a(uint8_t *actsIn, v32uint8 v, const unsigned int i) {
  v = upd_v(v, i, *(v16uint8 *)actsIn);
  return v;
}

inline __attribute__((always_inline)) uint8_t *
wb_0(uint8_t *actsOut, v16acc48 acc, const unsigned int keep_acc1) {
  v16uint8 out = ubsrs(acc, 0);
...
inline __attribute__((always_inline)) uint8_t *
wb_1(uint8_t *actsOut, v16acc48 acc, const unsigned int keep_acc1) {
  v16uint8 out = ubsrs(acc, 0);
...
applyKernel(uint8_t *&actsIn, uint8_t *actsIn_head, int8_t *&weightsIn,
            int8_t *weightsIn_head, uint8_t *&actsOut, const unsigned int M,
            const unsigned int N, const unsigned int Cin,
            const unsigned int Cout, const unsigned int F, const unsigned int S,
            const unsigned int P, const unsigned int CASC_IN_EN,
            const unsigned int CASC_OUT_EN, const unsigned int store_acc1) {
    ...
    for (unsigned int y = 0; y < F; y++) {
      for (unsigned int x = 0; x < F; x++) {
        ...
        if ((x == 0) || (pat == 1)) {
          ...
          const unsigned int nextChannels =
              (c < (CinUp / 8 - 1)) && (y == (F - 1)) && (x == (F - 2));
          const unsigned int enRewindA =
              (x == (F - 2)) && (y == (F - 1)) && (c == (CinUp / 8 - 1));
          ...
          if ((N_WRITES % 2) == 0) {
            ...
            if (regWrite == 0) {
              ...
              if (nextRow)
                actsIn -= ((1 + F / 2 - (MR / 2)) * nextRow) * 16;
              if (nextChannels)
                actsIn -=
                    ((1 + F / 2 - (MR / 2) * (N - (F - 1))) * nextChannels) *
                    16;
              if (enRewindA)
                actsIn -= (1 + F / 2 + (CinUp / 8 - 1) * MR / 2 * N +
                           (F - 1) * MR / 2 - 2) *
                          enRewindA * 16;
            } else {
        ...
        unsigned int regUp = ((N_WRITES % 2) == 0)
                                 ? ((x / 2) % 2)
                                 : ((x / 2 + y % 2 + c % 2) % 2);
        ...
        if ((x == 0 && y == 0 && c == 0) && CASC_IN_EN == 0) {
          acc0 = mul16(weightBuff0, 0, X_C64_D8, 4, 0x3210, actBuff, 0, zoff, 2,
                       0x3210);
        } else {
          acc0 = mac16(acc0, weightBuff0, 0, X_C64_D8, 4, 0x3210, actBuff, 0,
                       zoff, 2, 0x3210);
        }

        unsigned int regLow = ((N_WRITES % 2) == 0)
                                  ? ((x + 1) / 2) % 2
                                  : ((x + 1) / 2 + y % 2 + c % 2) % 2;
        actBuff = (regLow == 0) ? v0 : v1;
        zoff = (pat == 0) ? Z_C8_D8_I_UU : Z_C8_D8_I_U;
        if ((x == 0 && y == 0 && c == 0) && CASC_IN_EN == 0) {
          acc1 = mul16(weightBuff0, 0, X_C64_D8, 4, 0x3210, actBuff, 0, zoff, 2,
                       0x3210);
        } else {
          acc1 = mac16(acc1, weightBuff0, 0, X_C64_D8, 4, 0x3210, actBuff, 0,
                       zoff, 2, 0x3210);
        }
      }
    }
...
inline __attribute__((always_inline)) void
conv2d_int8_S1(uint8_t *restrict actsIn, int8_t *restrict weightsIn,
               uint8_t *restrict actsOut, const unsigned int M,
               const unsigned int N, const unsigned int Cin,
               const unsigned int Cout, const unsigned int F,
               const unsigned int S, const unsigned int P,
               const unsigned int CASC_IN_EN, const unsigned int CASC_OUT_EN) {

  uint8_t *actsIn_head = actsIn;
  int8_t *weightsIn_head = weightsIn;

  for (unsigned int f = 0; f < CoutUp / 8; f++) {
    for (unsigned int h = 0; h < outHeight; h += 1)
      chess_prepare_for_pipelining {
        for (unsigned int w = 0; w < outWidth; w += 4)
          chess_prepare_for_pipelining {
            const unsigned int cond =
                ((outWidth % 4) == 0) || !((w + 4) >= (outWidth));
            ...
            applyKernel(actsIn_copy, actsIn_head, weightsIn_copy,
                        weightsIn_head, actsOut_copy, M, N, Cin, Cout, F, S, P,
                        CASC_IN_EN, CASC_OUT_EN, cond);

            actsIn = actsIn_copy;       // Update the original pointer with the
                                        // modified copy
            weightsIn = weightsIn_copy; // Update the original pointer with the
                                        // modified copy
            actsOut = actsOut_copy;     // Update the original pointer with the
                                        // modified copy
          }
...
void conv2d_int8(uint8_t *restrict actsIn, int8_t *restrict weightsIn,
                 uint8_t *restrict actsOut, const unsigned int M,
                 const unsigned int N, const unsigned int Cin,
                 const unsigned int Cout, const unsigned int F,
  ...
  S1::conv2d_int8_S1(actsIn, weightsIn, actsOut, M, N, Cin, Cout, F, S,
