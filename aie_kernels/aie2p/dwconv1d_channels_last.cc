...
#endif
// Distance in elements between one tap's weight plane and the next.
#ifndef DWCONV1D_CL_TAP_STRIDE
#define DWCONV1D_CL_TAP_STRIDE 1024
#endif
...
// argument count: the taps are separate ObjectFifo elements, so a different K
// is a different signature and wants its own entry point.
//
// w is the weight base for all taps; this call reads plane t of channel row
// `row`. lo/hi arrive as float buffers because they are runtime parameters the
// host writes.
void dwconv1d_channels_last_k5_bf16(bf16 *__restrict w, bf16 *__restrict x_0,
                                    bf16 *__restrict x_1, bf16 *__restrict x_2,
                                    bf16 *__restrict x_3, bf16 *__restrict x_4,
                                    bf16 *__restrict y, float *lo_buffer,
                                    float *hi_buffer, const int row) {
  constexpr int C = DWCONV1D_CL_C;
  constexpr int S = DWCONV1D_CL_TAP_STRIDE;

  const bf16 *const wp[5] = {w + row * C, w + row * C + S, w + row * C + 2 * S,
                             w + row * C + 3 * S, w + row * C + 4 * S};
