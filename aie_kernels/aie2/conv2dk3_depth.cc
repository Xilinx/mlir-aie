...
void conv2dk3_i8_scalar(int8_t *line0, int8_t *line1, int8_t *line2,
  ...
  // oc++) {
  int input_depth_vectors=output_channels / 8;
  int VECTOR_SIZE=8;
  // for (oc = 0; oc < input_depth_vectors; oc++) {
  ...
  //     // output[oc * (input_width) +  input_width-1] = sum;
    for (c_rem = 0; oc < input_depth_vectors; oc++) {
        for (c8 = 0; c8 < VECTOR_SIZE; c8++) {
            ...
            for (ki = 0; ki < kernel_width; ki++) {
              ...
              // int in_indx_0 = (x - 1 + ki) * VECTOR_SIZE + (ic * input_width * VECTOR_SIZE) + c8;
              int in_indx_0=
               VECTOR_SIZE*input_width*c_rem
               +(x - 1 + ki) * VECTOR_SIZE
               + c8;
              if (check != top)
                sum += line0[in_indx_0] * wts[wts_indx_0];
          ...
          sum_srs = (sum + (1 << (scale - 1))) >> scale;
          sum_srs = (sum_srs > MAX) ? MAX : (sum_srs < 0) ? 0 : sum_srs;
          output[(c_rem * input_width * VECTOR_SIZE) + x * VECTOR_SIZE + c8] = sum_srs;
...
void conv2dk3_ui8_scalar(uint8_t *line0, uint8_t *line1, uint8_t *line2,
  ...
  int input_depth_vectors=output_channels / 8;
  int VECTOR_SIZE=8;
   for (int c_rem = 0; oc < input_depth_vectors; oc++) {
        for (int c8 = 0; c8 < VECTOR_SIZE; c8++) {
            for (x = 1; x < input_width - 1; x++) { // col of output image
