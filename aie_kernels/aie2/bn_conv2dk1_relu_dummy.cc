...
void bn10_conv2dk1_relu_i8_ui8(int8_t *input, int8_t *kernels, uint8_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale) property(disable_optimizer) {
  
  // conv2dk1_i8_scalar(input, kernels, output, input_width, input_channels,
  //                    output_channels, scale);
  event0();
  // #define CLOCKS_BN10_1 ((1080*MFAC)+AFAC) // *1.2+20
  #define CLOCKS_BN10_1 1316 
  for(int i=0; i<(CLOCKS_BN10_1+4)/8; i++) {
    asm volatile("NOPX");
  }
  event1();
                 }
...
//                      output_channels, scale);
  event0();
  // #define CLOCKS_BN11_1 2058*MFAC+AFAC // *1.2+20
  #define CLOCKS_BN11_1 2490 
  for(int i=0; i<(CLOCKS_BN11_1+4)/8; i++) {
    asm volatile("NOPX");
  }
  event1();
...
//                      output_channels, scale);
  event0();
  // #define CLOCKS_BN12_1 1050*MFAC+AFAC // *1.2+20
  #define CLOCKS_BN12_1 1280
  for(int i=0; i<(CLOCKS_BN12_1+4)/8; i++) {
    asm volatile("NOPX");
  }
  event1();
