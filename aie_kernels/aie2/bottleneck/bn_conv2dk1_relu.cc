...
void conv2dk1_ui8_ui8_scalar_input_split_partial_width_get_new(uint8_t *input, int8_t *kernels, uint8_t *output,
                                              const int32_t input_width, const int32_t input_channels,
                                              const int32_t output_channels, const int scale,
                                              const int32_t input_split, int32_t output_split, const int32_t weight_index, const int32_t x_start, const int32_t oc) {
    event0();
    int ic, ic8, oc8;
        ...
        // Loop over output channels (oc8)
            for (oc8 = 0; oc8 < 8; oc8++) {
                ...
                for (oc8 = 0; oc8 < 8; oc8++) {
                    ...
                    sum_srs = (((cascade_sum+sum) + (1 << (scale - 1)) - 1 + (((cascade_sum+sum) >> scale) & 1)) >> scale);
                    sum_srs= (sum_srs > UMAX) ? UMAX : (sum_srs< 0) ? 0 : sum_srs;
                    
                    oc_offset=oc+oc8_iter*(weight_index);  // works fine when oc8_iter is 4
...
#if defined (PARTIAL_GET_I8_CAS_WIDTH_NEW) || (BN13_2_PARTIAL_GET_I8_CAS_WIDTH_NEW) || (BN13_1_PARTIAL_GET_I8_CAS_WIDTH_NEW) || (BN14_1_PARTIAL_GET_I8_CAS_WIDTH_NEW)
// 8 Pixels Width Processing Approach: Processes 8 spatial pixels (x_start to x_start + 8) simultaneously within each output channel (oc8 iteration).
...
static void conv2dk1_ui16_partial_scalar(uint16_t *input, int8_t *kernels, int32_t *output,
                        const int32_t input_width, const int32_t input_channels,
                        const int32_t output_channels, const int scale,
  ...
  for (oc = 0; oc < output_channels / 8; oc++) {
    for (x = 0; x < input_width; x++) { // col of output image
      for (oc8 = 0; oc8 < 8; oc8++) {
        int32_t sum = 0;
        int32_t sum_srs = 0;
        int32_t accumulator = (ifm_index != 0) ? output[(oc * input_width * 8) + (x * 8) + oc8] : 0;
        ...
        {
          sum_srs = ((accumulator + (1 << (scale - 1)) - 1 + ((accumulator >> scale) & 1)) >> scale);
          sum_srs = (sum_srs > UMAX) ? UMAX : (sum_srs < 0) ? 0 : sum_srs;
...
static void conv2dk1_ui16_partial_acc_scalar(uint16_t *input, int8_t *kernels, uint8_t *output,
                        const int32_t input_width, const int32_t input_channels,
                        const int32_t output_channels, const int scale,const int32_t ifm_index,const int32_t total_ifm) {
  event0();
  ...
  for (oc = 0; oc < output_channels / 8; oc++) {
    for (x = 0; x < input_width; x++) { // col of output image
      for (oc8 = 0; oc8 < 8; oc8++) {
        ...
        if(ifm_index>0)
        { 
          v16vec_partial[accumulator_index] = lsrs(*accumulators[accumulator_index],0,0); 
          last_sum = ext_elem(v16vec_partial[accumulator_index], element_index);
        }
        // Store the result in the appropriate accumulator
        v16vec_partial[accumulator_index]=upd_elem(v16vec_partial[accumulator_index], element_index, last_sum+sum); 
        if(element_index==15)
          *accumulators[accumulator_index] = lups(v16vec_partial[accumulator_index],0);
      }
      // output[(oc * input_width * 8) + (x * 8) + oc8] = sum_srs;
    }
  }
...
#endif
#endif
// was trying to do accumulation alive in vector register below. Updated the code above to keep results in buffer.
// static void conv2dk1_ui16_partial_scalar(uint16_t *input, int8_t *kernels, uint8_t *output,
//                         const int32_t input_width, const int32_t input_channels,
//                         const int32_t output_channels, const int scale,const int32_t ifm_index,const int32_t total_ifm) {
//   event0();
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
static void conv2dk1_ui16_scalar_pad(uint16_t *input, int8_t *kernels, uint16_t *output,
                        const int32_t input_width, const int32_t input_channels, const int32_t input_channels_pad,
                        const int32_t output_channels, const int scale) {
  event0();
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
static void conv2dk1_ui16_scalar(uint16_t *input, int8_t *kernels, uint8_t *output,
                        const int32_t input_width, const int32_t input_channels,
                        const int32_t output_channels, const int scale) {
  event0();
...
#if defined (BN1)||(BN2) ||(BN3) ||(BN4) || (BN5) || (BN6) || (BN7) || (BN8) || (BN9) || (BN10) || (BN11)|| (BN12) || (BN13) || (BN14) ||  (REGULAR) ||  (POSTL1)
#ifdef INT8_ACT
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
void fused_conv2dk1_x_pool_i8_scalar(int8_t *input, int8_t *kernels, uint16_t *output,
                        const int32_t input_width, const int32_t input_channels,
                        const int32_t output_channels, const int scale) {
  int x, ic, oc, ic8, oc8;
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
void fused_conv2dk1_xy_pool_i8_scalar(int8_t *input, int8_t *kernels, uint16_t *output,
                                      const int32_t input_width, const int32_t input_channels,
                                      const int32_t output_channels, const int scale, const int y_index) {
  int x, ic, oc, ic8, oc8;
  ...
  for (oc = 0; oc < output_channels / 8; oc++) {
    for (oc8 = 0; oc8 < 8; oc8++) {
      ...
      for (x = 0; x < input_width; x++) { // Iterate over input columns (width)
        ...
        int32_t sum_srs = ((sum + (1 << (scale - 1)) - 1 + ((sum >> scale) & 1)) >> scale);
        sum_srs = (sum_srs > UMAX) ? UMAX : (sum_srs < 0) ? 0 : sum_srs;
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
void fused_conv2dk1_xy_pool_i8_large_output_channel_split_scalar(int8_t *input, int8_t *kernels, uint16_t *output,
                                      const int32_t input_width, const int32_t input_channels,
                                      const int32_t output_channels, const int scale, const int avgpool_scale, 
                                      const int y_index, int32_t output_split, int32_t weight_index) {
  int x, ic, oc, ic8, oc8;
  ...
  for (oc = 0; oc < output_channels / 8; oc++) {
    for (oc8 = 0; oc8 < 8; oc8++) {
      ...
      if (y_index == input_width - 1) {
        ...
        // int32_t rounded_avg_srs = ((rounded_avg + (1 << (avgpool_scale - 1)) - 1 + ((rounded_avg >> avgpool_scale) & 1)) >> avgpool_scale);
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
void fused_conv2dk1_xy_pool_i8_large_padded_scalar(int8_t *input, int8_t *kernels, uint16_t *output, 
                                      const int32_t input_width, const int32_t input_channels,
                                      const int32_t output_channels, const int32_t output_channels_padd, const int scale, 
                                      const int y_index, int32_t output_split, int32_t weight_index) {
  int x, ic, oc, ic8, oc8;
  int oc_tile = output_channels / output_split;
  int oc_offset = oc_tile / 8 * weight_index;
  int padded_channels = (output_channels_padd > output_channels) ? output_channels_padd - output_channels : 0;

  for (oc = 0; oc < oc_tile / 8; oc++) {
    for (oc8 = 0; oc8 < 8; oc8++) {
      // Initialize the accumulator
      uint32_t accumulator = (y_index != 0) ? output[(oc_offset + oc) * 8 + oc8] : 0;
...
// act: int8, wts: int8, out: uint8
//*****************************************************************************
void fused_conv2dk1_xy_pool_i8_large_scalar(int8_t *input, int8_t *kernels, uint16_t *output,
                                      const int32_t input_width, const int32_t input_channels,
                                      const int32_t output_channels, const int scale, 
                                      const int y_index, int32_t output_split, int32_t weight_index) {
  int x, ic, oc, ic8, oc8;
  ...
  for (oc = 0; oc < oc_tile / 8; oc++) {
    for (oc8 = 0; oc8 < 8; oc8++) {
      // Initialize the accumulator
      // uint16_t accumulator = (y_index != 0) ? output[(oc_offset + oc) * 8 + oc8] : 0;
      uint32_t accumulator = (y_index != 0) ? output[(oc_offset + oc) * 8 + oc8] : 0;
...
void bn13_1_conv2dk1_ui8_ui8_input_split_partial_width_get_new(uint8_t *input, int8_t *kernels, uint8_t *output,
                                              const int32_t input_width, const int32_t input_channels,
                                              const int32_t output_channels, const int scale,
                                              const int32_t input_split,int32_t output_split,  const int32_t weight_index, const int32_t x_start,const int32_t oc) 
                                              {

    conv2dk1_ui8_ui8_scalar_input_split_partial_width_get_new(input, kernels, output, input_width, input_channels,
...
void bn14_1_conv2dk1_i8_ui8_partial_width_get_new(int8_t *input, int8_t *kernels, uint8_t *output,
                                              const int32_t input_width, const int32_t input_channels,
                                              const int32_t output_channels, const int scale,
                                              int32_t input_split,int32_t output_split, int32_t weight_index, int32_t x_start, int32_t oc) 
                                              {
...
void bn13_1_conv2dk1_i8_ui8_partial_width_get_new(int8_t *input, int8_t *kernels, uint8_t *output,
                                              const int32_t input_width, const int32_t input_channels,
                                              const int32_t output_channels, const int scale,
                                              int32_t input_split,int32_t output_split, int32_t weight_index, int32_t x_start, int32_t oc) 
                                              {
...
void bn13_2_conv2dk1_i8_ui8_partial_width_get_new(int8_t *input, int8_t *kernels, uint8_t *output,
                                              const int32_t input_width, const int32_t input_channels,
                                              const int32_t output_channels, const int scale,
                                              int32_t input_split,int32_t output_split, int32_t weight_index, int32_t x_start, int32_t oc) 
                                              {
...
#ifdef POSTL1
void post_L1_conv2dk1_relu_i8_ui8(int8_t *input, int8_t *kernels, uint8_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale) {
...
void post_L2_conv2dk1_relu_i16_ui8(uint16_t *input, int8_t *kernels, uint8_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale, const int ifm_index,const int32_t total_ifm) {
  
  conv2dk1_ui16_partial_acc_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale,ifm_index,total_ifm);
...
void post_L2_conv2dk1_relu_i16_ui8(uint16_t *input, int8_t *kernels, int32_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale, const int ifm_index,const int32_t total_ifm) {
  
  conv2dk1_ui16_partial_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale,ifm_index,total_ifm);
...
#ifdef POSTL2_PAD
void post_L2_conv2dk1_relu_i16_ui16_pad(uint16_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels, const int32_t input_channels_pad,
                 const int32_t output_channels, const int scale) {
  
  conv2dk1_ui16_scalar_pad(input, kernels, output, input_width, input_channels, input_channels_pad,
                     output_channels, scale);
...
void conv2dk1_x_pool_fused_relu_i8_ui8(int8_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale) {
  
  fused_conv2dk1_x_pool_i8_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale);
...
void conv2dk1_xy_pool_fused_relu_i8_ui8(int8_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale, const int y_index) {
  
  fused_conv2dk1_xy_pool_i8_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale,y_index);
...
void conv2dk1_xy_pool_fused_relu_large_padded_i8_ui8(int8_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int32_t output_channels_padd,  const int scale, 
                 const int y_index,int32_t output_split,int32_t weight_index) {
  
  fused_conv2dk1_xy_pool_i8_large_padded_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, output_channels_padd, scale,y_index,output_split,weight_index);
...
void conv2dk1_xy_pool_fused_relu_large_i8_ui8(int8_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale, 
                 const int y_index,int32_t output_split,int32_t weight_index) {
  
  fused_conv2dk1_xy_pool_i8_large_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale,y_index,output_split,weight_index);
...
void conv2dk1_xy_pool_fused_relu_large_output_channel_split_i8_ui8(int8_t *input, int8_t *kernels, uint16_t *output,
                 const int32_t input_width, const int32_t input_channels,
                 const int32_t output_channels, const int scale, const int avgpool_scale, 
                 const int y_index,int32_t output_split,int32_t weight_index) {
  
  fused_conv2dk1_xy_pool_i8_large_output_channel_split_scalar(input, kernels, output, input_width, input_channels,
                     output_channels, scale,avgpool_scale,y_index,output_split,weight_index);
