...
// #endif // BN6
    ...
    void bn10_conv2dk1_ui8_i8(uint8_t *input, int8_t *kernels, int8_t *output,
      ...
      //                     output_channels, scale);
      event0();
      // #define CLOCKS_BN10_3 1470*MFAC+AFAC // *1.2+20
      #define CLOCKS_BN10_3 1784
      for(int i=0; i<(CLOCKS_BN10_3+4)/8; i++) {
        asm volatile("NOPX");
      }
      event1();
    }
    ...
    void bn12_conv2dk1_ui8_i8(uint8_t *input, int8_t *kernels, int8_t *output,
      ...
      //                     output_channels, scale);
      event0();
      // #define CLOCKS_BN12_3 1470*MFAC+AFAC // *1.2+20
      #define CLOCKS_BN12_3 1784
      for(int i=0; i<(CLOCKS_BN12_3+4)/8; i++) {
        asm volatile("NOPX");
      }
      event1();
    }
