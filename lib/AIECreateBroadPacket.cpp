...
struct AIEBroadPacketPass : public AIEBroadPacketBase<AIEBroadPacketPass> {
  void runOnOperation() override {

    ModuleOp m = getOperation();
    OpBuilder builder = OpBuilder::atBlockEnd(m.getBody());

    for (auto broadpacket : m.getOps<BroadPacketOp>()) {
      Region &r = broadpacket.ports();
      Block &b = r.front();
      Port sourcePort = broadpacket.port();
      TileOp srcTile = dyn_cast<TileOp>(broadpacket.tile().getDefiningOp());
      ...
      for (Operation &Op : b.getOperations()) {
        if (BPIDOp bpid = dyn_cast<BPIDOp>(Op)) {
          ...
          builder.setInsertionPointAfter(broadpacket);
          PacketFlowOp pkFlow =
    ...
    patterns.add<AIEOpRemoval<BroadPacketOp>>(m.getContext(), m);
...
xilinx::AIE::createAIEBroadPacketPass() {
  return std::make_unique<AIEBroadPacketPass>();
}