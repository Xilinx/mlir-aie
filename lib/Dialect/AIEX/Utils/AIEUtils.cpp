...
AIEX::traceSubviewToBlockArgument(Value value) {
  ...
  while (current) {
    ...
    if (auto reinterpretOp = dyn_cast<memref::ReinterpretCastOp>(defOp)) {
      auto sourceType = dyn_cast<MemRefType>(reinterpretOp.getSource().getType());
      if (!sourceType) {
        return std::nullopt;
      }
      ...
      if (auto strided = dyn_cast<StridedLayoutAttr>(sourceType.getLayout())) {
        for (int64_t stride : strided.getStrides()) {
          if (stride != 1) {
            return std::nullopt;  // Non-contiguous memory, cannot safely reinterpret
          }
        }
      }
    ...
    if (auto subviewOp = dyn_cast<memref::SubViewOp>(defOp)) {
      ...
      unsigned elemSizeInBits = sourceType.getElementType().getIntOrFloatBitWidth();
