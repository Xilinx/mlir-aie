...
// do not see each other. The static allocator (aie-assign-bd-ids) runs first
// and cannot walk AIEX ops, so its auto-assignment would reuse a bd_id an
// aiex.dma_configure_task pinned on the same tile. This prepass records those
// compile-time-pinned runtime ids per tile as an aiex.reserved_bd_ids attribute
// on each aie.tile; the static allocator reads it and reserves them, so its
// auto-assignment steps over ids the runtime sequence already claimed.
//
//===----------------------------------------------------------------------===//
