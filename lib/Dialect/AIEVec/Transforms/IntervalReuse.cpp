...
bool IntervalReuse::sameEnclosingLoops(Operation *op, 
  ...
  assert(blockToEnclosingLoops.count(refBlock) && 
         "block to enclosing loop mapping not computed");
...
bool IntervalReuse::potentialReuse(vector::TransferReadOp readOp,
    AffineExpr invariantBase, 
    DenseMap<Block*, SmallVector<Operation*, 8>> &blockToEnclosingLoops) {
  return sameMemRef(readOp.source()) && 
...
computeAccessExtent(vector::TransferReadOp readOp, 
                       int32_t offset, 
  ...
  int32_t ub = (isSplat ? (offset & ~(vecSize-1)) + vecSize : 
               offset+loopStepSize*vecSize) * elementSizeInBits;
...
void IntervalReuse::insertInterval(vector::TransferReadOp readOp, 
  ...
  std::pair<int32_t,int32_t> bound = computeAccessExtent(readOp, offset, 
                                    loopStepSize, isSplat, minVecSize);
...
void IntervalReuse::coalesceIntervals() {
  ...
  for (size_t i=0, e=intervals.size(); i<e; ++i) {
    canCoalesce |= vecIsLHSOperand[i] &&
                   intervals[i].second-intervals[i].first <= 256; 
  }
  if (!canCoalesce) 
    return;
  ...
  for (size_t i=0, e=intervals.size(); i<e;) {
    ...
    if (vecIsLHSOperand[i] && i<intervals.size()-1 && vecIsLHSOperand[i+1]) {
      // Get vector sizes
      ...
      if (v1size <= 256 && v2size <= 256) {
        coalescedIntervals.push_back(std::make_pair(intervals[i].first, 
