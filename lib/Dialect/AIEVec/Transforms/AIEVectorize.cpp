...
struct AIEVecAttributes {
  ...
  // Constructors
  AIEVecAttributes
    (unsigned l, unsigned vs, Type et, int32_t es) : 
    lanes(l), vecSizeInBits(vs), elementType(et), 
    elementSizeInBits(es), loadFromMemory(false), isSplat(false) {}
  AIEVecAttributes
    (unsigned l, unsigned vs, Type et, int32_t es, bool lm, bool sp) : 
    lanes(l), vecSizeInBits(vs), elementType(et), 
    elementSizeInBits(es), loadFromMemory(lm), isSplat(sp) {}
};
...
static inline AIEVecAttributes getVectorStats(VectorType type) {
  return AIEVecAttributes(getVectorLaneSize(type),
                          getVectorSizeInBits(type),
                          type.getElementType(),
                          getElementSizeInBits(type));
}
...
static inline AIEVecAttributes getOperandVecStats(Operation *op, 
                                           VectState *state, 
                                           unsigned idx=0) {
...
static inline void fuseAccessExtent(Operation *Op1, 
                                    Operation *Op2,
                                    VectState *state) {
  // Assert that the input operations are of expected type
  bool expectedTypes = isa<vector::FMAOp>(Op2) && 
            isa<MulIOp, MulFOp, vector::FMAOp>(Op1);
...
static bool isSimpleVectIntrinsic(Operation *Op,
                                  VectState *state) {
...
static AffineExpr constructLinearizedAffineExpr(TransferReadOp readOp,
  ...
  auto ret = makeFlattenedStridedExpr(memRefType.getShape(), 
                                      exprVec, memRefType.getContext());
...
static aievec::SRSOp generateSRSOp(Value source,
                                Type scalarType,
                                VectState *state,
                                Location loc) {
  ...
  // Create the SRS op 
  aievec::SRSOp srsOp = state->builder.create<aievec::SRSOp>(loc,
                                srsType, source, state->shift);
...
static aievec::UPSOp generateUPSOp(Value source,
                                VectState *state,
                                Location loc) {
  Type sourceType = source.getType();
  assert(!isAccType(sourceType) && 
        "ups source should not be accumulator");

  AccType accType = getAccType(sourceType);

  // Create a new UPS instruction
  aievec::UPSOp upsOp = state->builder.create<aievec::UPSOp>(
                        loc, accType, source, state->shift);
...
// are in opAttr.
static aievec::SelectOp generateSelectOp(Value xbuff,
  ...
  aievec::SelectOp selectOp = state->builder.create<aievec::SelectOp>(
       loc, resultType, xbuff, opAttr.select,
       opAttr.start[0], opAttr.offset[0], 
       opAttr.offset_hi[0], opAttr.square[0],
       opAttr.start[1], opAttr.offset[1], 
...
static aievec::ExtOp generateExtOp(Value source,
                                unsigned lanes,
                                int8_t idx,
                                VectState *state,
                                Location loc) {
  ...
  aievec::ExtOp extOp = state->builder.create<aievec::ExtOp>(loc,
                        resultType, source, idx);
...
static aievec::PackOp generatePackOp(Value source,
                                  VectState *state,
                                  Location loc) {
  // Create the result type
  VectorType stype = getVectorType(source);
  ...
  aievec::PackOp packOp = state->builder.create<aievec::PackOp>(loc,
                                resultType, source);
...
static aievec::AddOp generateAddOp(Operation *Op,
                                AIEOpAttributes &opAttr,
                                VectState *state) {
  ...
  aievec::AddOp addOp = state->builder.create<aievec::AddOp>(
                ...
                opAttr.start[0], 
                opAttr.offset[0], opAttr.offset_hi[0], 
                opAttr.square[0],
                opAttr.start[1], 
                opAttr.offset[1], opAttr.offset_hi[1], 
                opAttr.square[1]);
  return addOp;
}

// Generate and return a Sub op.
static aievec::SubOp generateSubOp(Operation *Op,
                                AIEOpAttributes &opAttr,
                                VectState *state) {
  // Assert that we computed the attributes for both the operands
  assert(opAttr.start.size() == opAttr.offset.size() && 
...
static Operation* generateFMAOp(vector::FMAOp fmaOp, 
                             AIEOpAttributes &opAttr,
                             VectState *state, 
  ...
  if (!isAccType(acc.getType())) { 
    ...
    LLVM_DEBUG(llvm::dbgs() << "\n\nCreated UPS op " << acc << 
              " to move the output of " << fmaOp << " into accumulator");
  }
  ...
  if (!isSimpleVectIntrinsic(fmaOp, state)) {
    ...
    if (lstat.vecSizeInBits == 256) {
      VectorType concatType = createVectorType(512/lstat.elementSizeInBits, 
                                               lstat.elementType);
  ...
  Operation* xfmaOp = state->builder.create<aievec::FMAOp>(
                fmaOp->getLoc(), lhs, fmaOp.rhs(), acc, 
                ...
                opAttr.step[1], opAttr.square[1], isSub);
...
template <typename T>
static Operation* generateMulOp(T mulOp,
                             AIEOpAttributes &opAttr,
                             VectState *state) {
  // Assert that we have computed the attributes (start, offset, etc.) for both
...
// otherwise.
static aievec::UPDOp generateUPDOp(TransferReadOp readOp, 
  ...
  // loaded.
  int32_t lb = intervalWidth <= 256 || extent.first < mid ? 
               interval.first : mid;
  int32_t ub = intervalWidth <= 256 || extent.second > mid ? 
               interval.second : mid;
  ...
  int8_t idx=1;
  for (int32_t start=interval.first; start<interval.second; start+=incr,++idx) {
    ...
    if (lb <= start && ub >= end && (updIndices & idx) == 0) {
      ...
      updOp = state->builder.create<aievec::UPDOp>(readOp.getLoc(), updVecType, 
                          readOp.source(), indices, start-offset, idx-1,
                          updOp ? updOp.result() : nullptr);
...
static int32_t computeVecorizedLoopStepSize(Operation *op,
                                            VectState *state) {
...
// wide.
int32_t computeStartInAIEVec(Operation *op,
                             VectState *state) {
...
static Operation* concatAndInterleave_i8xi8(Operation* source1, 
  ...
  SmallVector<Value> sources = {srsOp1->getResult(0), 
                                srsOp2->getResult(0)};
  ...
  auto selectOp = generateSelectOp(concatOp->getResult(0), opAttr,
                                    32, state, loc);
...
static bool canFuseMulAndAddOrSubIntoFMAOp(Operation *Op) {
   ...
   assert((isa<AddIOp>(Op) || isa<AddFOp>(Op) ||
           isa<SubIOp>(Op) || isa<SubFOp>(Op)) && 
   ...
   if (lhsVecSize != rhsVecSize || 
       rhsVecSize != accVecSize)
     return false;
...
static void reassociateMulOpBasedOnVecSize(Operation *Op, 
                                           VectState *state) {
  ...
  bool is8x8 = lstat.elementSizeInBits == 8 && 
               rstat.elementSizeInBits == 8; 
...
static void reassociateMulOpWithSplat(Operation *Op,
                                      VectState *state) {
...
static void fuseMulAndAddOrSubIntoFMAOp(Operation *Op,
                                        VectState *state) {
  Value acc = Op->getOperand(0);
  Operation *mulOp = Op->getOperand(1).getDefiningOp();
  Value lhs = mulOp->getOperand(0);
...
static void generateMulOrFMAOp(Operation *Op,
                               AIEOpAttributes &opAttr,
                               VectState *state,
  ...
  if (!nextStart.empty()) {
    ...
    if (llvm::any_of(Op->getUsers(), notMulOrFMAOp))
      repOp = concatAndInterleave_i8xi8(repOp, pairedOp, state, 
                                        Op->getLoc());
  }
...
static void computeBuffAttr_i32xi32(unsigned vecSize, // #lanes
...
static void computeXbuffAttr_i16xi16(unsigned vecSize, // #lanes
...
static void computeZbuffAttr_i16xi16(unsigned vecSize, // #lanes
              int32_t start, // computed start in AIE vec
              int32_t accIncr, // access change with each loop increment
...
// {0,0,1,1,2,2,3,3}.
static void computeXbuffAttr_i8xi8(unsigned vecSize, // #lanes
              int32_t start, // computed start in AIE vec
              int32_t colOffset, // xbuff access distance between vector cols
              AIEOpAttributes &opAttr) {
  ...
  int32_t rstep = offsetWithoutDup >= 2 ? 2 : 
                  colOffset == -1 ? 1 : offsetWithoutDup;
...
static void computeZbuffAttr_i8xi8(unsigned vecSize, // #lanes
              int32_t start, // computed start in AIE vec
              int32_t accIncr, // access change with each loop increment
              int32_t colOffset, // zbuff access distance between vector cols
              AIEOpAttributes &opAttr,
              std::string &nextStart) {
  // The colOffset must be either <=1, or a multiple of 2
  assert((colOffset <= 1 || colOffset % 2 == 0) &&
...
static void fuseFMAOps(Operation *refOp,
  ...
  if (cols <= 1 ||
      !isa<MulIOp, MulFOp, vector::FMAOp>(refOp) || 
  ...
  for (auto len=0; len<cols-1; ++len) {
    ...
    if (!isa<vector::FMAOp>(usrOp) ||
        curOp->getBlock() != usrOp->getBlock() ||
    ...
    for (size_t idx=0; idx<2; ++idx) {
      ...
      if (!related && cstat.loadFromMemory && ustat.loadFromMemory) {
        ...
        related = civ == uiv && 
                  civ->getInterval(cdefOp) == uiv->getInterval(udefOp);
      }
      if (!related) 
        break;
...
static void computeXbuffAttributes(Scheme &scheme, // vect scheme info
               int32_t start, // computed start in AIE vec
               int32_t colOffset, // xbuff access distance between vector cols
  ...
  // Case 1: 32x32 real
  if (scheme.lanes == 8  && scheme.cols == 1  && 
      scheme.xbits == 32 && scheme.zbits == 32)
    computeBuffAttr_i32xi32(scheme.lanes, start, accIncr, opAttr);
  // Case 2: 16x16 real
  else if (scheme.lanes == 16 && scheme.cols == 2  && 
           scheme.xbits == 16 && scheme.zbits == 16) {
  ...
  // Case 3: 8x8 real
  else if (scheme.lanes == 16 && scheme.cols == 8 && 
           scheme.xbits == 8  && scheme.zbits == 8) {
    // We only support a loop increment of <= 1
...
static void computeZbuffAttributes(Scheme &scheme, // vect scheme info
             int32_t start, // computed start in AIE vec
             int32_t colOffset, // zbuff access distance between vector cols
  ...
  // Case 2: 16x16 real
    ...
    computeZbuffAttr_i16xi16(scheme.lanes, start, accIncr, 
                        zeroOffset, colOffset, opAttr);
  }
  // Case 3: 8x8 real
  else if (scheme.lanes == 16 && scheme.cols == 8 && 
           scheme.xbits == 8  && scheme.zbits == 8) {
    // We only support a loop increment of <= 1
    assert(accIncr <= 1 && "loop step size greater than 1 not supported");
    computeZbuffAttr_i8xi8(scheme.lanes, start, accIncr, colOffset, 
                              opAttr, nextStart);
  }
  else
    llvm_unreachable("Unsupported vectorization scheme");
...
static void generateSchemeBasedMulOrFMAOp(Operation *Op,
                                          VectState *state) {
  ...
  auto colOffset = state->opToColOffsets.count(Op) ? 
                   state->opToColOffsets[Op] : std::make_pair(-1, -1);
  ...
  for (size_t idx=0; idx<2; ++idx) {
    ...
    if (stat.loadFromMemory) {
      ...
      accIncr = stat.isSplat ? 0 : 
                      computeVecorizedLoopStepSize(readOp, state);
...
static void fuseFMAOpsForColumnTopology(FuncOp func, 
                                        VectState *state) {
...
// dialect. 
static void generateAIEMulOrFMAOpsInFunc(FuncOp func, 
                                         VectState *state) {
...
static void generateAddOrSubOp(Operation *Op,
                               AIEOpAttributes &opAttr,
                               VectState *state) {
...
// different vector schemes. 
static void generateSchemeBasedAddOrSubOp(Operation *Op,
                                          VectState *state) {
...
static void generateAIEAddOrSubOpsInFunc(FuncOp func, 
                                          VectState *state) {
  func.walk([&](mlir::Operation *op) {
...
static void insertUPDOpsInLoop(AffineForOp forOp,
                               VectState *state) {
...
static void insertUPDOpsInFunc(FuncOp func, 
                               VectState *state) {
...
static void insertSRSOp(Operation *Op,
                        VectState *state) {
    ...
    for (auto operand : user->getOperands()) {
      if (operand.getDefiningOp() == Op) {
        ...
        if (!typeToSRSOpMap.count(scalarType)) {
          srsOp = generateSRSOp(Op->getResult(0), scalarType, 
                            state, Op->getLoc());
...
// vector.
static void insertSRSOpsInFunc(FuncOp func, 
                               VectState *state) {
  func.walk([&](mlir::Operation *op) {
...
static void computeEnclosingLoopsPerBlock(AffineForOp forOp,
                       VectState *state,
...
static void reassociateMulOpInFunc(FuncOp func, 
                                   VectState *state) {
  func.walk([&](mlir::Operation *op) {
...
static void reassociateAddOpInFunc(FuncOp func, 
                                   VectState *state) {
  func.walk([&](mlir::Operation *op) {
...
static void coalesceLHSOpVectorsInFunc(FuncOp func, 
                                       VectState *state) {
...
// can exploit for it.
static void computeReuse(TransferReadOp readOp, 
                         VectState *state) {
  ...
  for (auto interval : state->reuseIntervals) {
    ...
    if (interval->potentialReuse(readOp, base, state->blockToEnclosingLoops)) {
      ...
      interval->insertInterval(readOp, state->opToIntervalMap, 
                               offset, step, isSplat, minVecSize);
      found = true;
      break;
    }
  }
  ...
  if (!found) {
     ...
     iv->insertInterval(readOp, state->opToIntervalMap, 
                        offset, step, isSplat, minVecSize);
...
static void computeReuseInFunc(FuncOp func, 
                               VectState *state) {
...
static void rewriteFMAOpsInFunc(FuncOp func,
                                VectState *state) {
  // Find a root add op that is well formed, and start from there 
  func.walk([&](Operation *Op) {
    if (isa<AddIOp, AddFOp, SubIOp, SubFOp>(Op) && 
        isWellFormedVectorOp(Op)) {
...
static void reassociateOpsInFunc(FuncOp func,
                                 VectState *state) {
...
void AIEVectorize::runOnOperation() {
  ...
  for (FuncOp func : module.getOps<FuncOp>()) {
    ...
    VectState *state = new VectState(func.getContext(), 
                           shiftParam, zeroOffset, dupFactor);
