  ...
  LogicalResult dynamicObjectFifos(DeviceOp &device, OpBuilder &builder,
    ...
    for (auto coreOp : device.getOps<CoreOp>()) {
      if (objectFifoTiles.count(coreOp.getTileOp()) > 0) {
        ...
        std::map<ObjectFifoCreateOp, int> iterationMap;
          ...
          if (isa<scf::ForOp>(op)) {
            ...
            if (found) {
              ...
              newLoopBody->walk([&](ObjectFifoSubviewAccessOp accessOp) {
                ...
                auto bufferIndex = (accessOp.getIndex() + iterationMap[createOp]) % createOp.size();
                ...
                for (int i = 0; i < fifoSizes[createOp]; ++i) {
                  ...
                  int bufferToBeAccesed =
                      (accessOp.getIndex() + i + iterationMap[createOp]) % fifoSizes[createOp];
                  builder.create<scf::YieldOp>(
              ...
              for (auto relOp : newLoopBody->getOps<ObjectFifoReleaseOp>()) {
                if (relOp.getOperation()->getParentOp() == forLoopWithIterArgs) {
                  ObjectFifoCreateOp op = relOp.getObjectFifo();
                  // NEED FIXING: Need to convert these bounds into integers
                  iterationMap[op] += ((forLoopWithIterArgs.getUpperBound() - forLoopWithIterArgs.getLowerBound())/forLoopWithIterArgs.getStep())*relOp.getSize();
                  iterationMap[op] %= op.size();
                }
              }
            }
          }
          else if (isa<ObjectFifoReleaseOp>(op)) {
            auto funcOp = op->getParentOp();
            // Updating the count of releases into iterationMap
            // to keep track of the buffer for each subview access
            if (!isa<scf::ForOp>(funcOp)) {
              auto countOp = dyn_cast<ObjectFifoReleaseOp>(op);
              iterationMap[countOp.getObjectFifo()] += countOp.getSize();
            }
          }
          return WalkResult::advance();
        });
