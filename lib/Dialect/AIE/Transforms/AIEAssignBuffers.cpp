...
struct SimpleBankAwareAllocationPattern : public OpRewritePattern<TileOp> {
  ...
  LogicalResult matchAndRewrite(TileOp tile,
    ...
    std::vector<BankLimits> bankLimits; // the entries contain pairs of start
                                        // and end addresses for each bank
    ...
    for (auto buffer : device.getOps<BufferOp>()) {
      if (buffer.getTileOp() == tile) {
        bool has_addr = checkAndAddBufferWithAddress(
            buffer, numBanks, nextAddrInBanks, bankLimits);
        bool has_bank = checkAndAddBufferWithMemBank(
            buffer, numBanks, nextAddrInBanks, bankLimits);
